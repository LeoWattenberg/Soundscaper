...
import {
	PROJECT_SCHEMA_VERSION, SOUNDSCAPER_PROJECT_SCHEMA_FAMILY,
...
export function snapshotSoundscaperProjectFoundationCommand(command: AudioEditorCommand): AudioEditorCommand {
...
/**
 * Apply one baseline transaction. Shared editorial commands use the common
 ...
 */
export function applySoundscaperProjectFoundationCommand(
...
function applyCommandTree(
	...
	if (command.type.startsWith('mixer/')) {
		const draft = structuredClone(project) as unknown as Record<string, unknown>;
		draft.mixer = applySoundscaperMixerSurfaceCommand(project, command as Extract<AudioEditorCommand, {
...
export function soundscaperProjectForFoundationCommandConsumers(
	projectValue: SoundscaperProject | unknown,
...
function assertNoLegacyEnvelopeCommand(command: AudioEditorCommand): void {
	if ((command.type === 'track/update' || command.type === 'master/update')
		...
		throw new RangeError('Legacy strip envelopes are unavailable under Soundscaper baseline automation-lane authority.');
	}
}
...
function elevateInheritedCommandResult(
	...
	commanded.schemaFamily = SOUNDSCAPER_PROJECT_SCHEMA_FAMILY;
	commanded.schemaVersion = PROJECT_SCHEMA_VERSION;
	...
	commanded.automationLanes = preserveAutomationLanesAfterInheritedCommand(
		previous,
		commanded,
		command,
...
function retiredFolderIds(
	previousProject: SoundscaperProject,
...
function duplicateIdentity(kind: 'lane' | 'point', sourceId: string, targetTrackId: string): string {
	return `${kind}-${bytesToHex(sha256(DUPLICATE_ID_ENCODER.encode(
		`soundscaper-v1-track-duplicate\u0000${sourceId}\u0000${targetTrackId}`,
	)))}`;
}
...
function finalizeIntermediate(
	...
	value.featureRequirements = reconcileSoundscaperProjectFeatureRequirements(
		value,
		value.featureRequirements as never,
	);
	if (validateResult) validateSoundscaperProject(value);
