...
import { importSoundscaperAudacityProjectV21 } from './editor-audacity-project-import-v21.ts';
import {
...
import {
	assertSoundscaperEditorProjectEnvironmentV21,
	type SoundscaperEditorProjectEnvironmentV21,
} from './editor-project-environment-v21.ts';
import { createSoundscaperScapeNativeRuntimeV21 } from './editor-scape-native-v21.ts';

const PRESENTATION_FIELDS = ['locale', 'copy', 'fileService'] as const;

export interface SoundscaperAudioEditorControllerPresentationV21 {
...
export type SoundscaperAudioEditorControllerV21 = CommonAudioEditorController & Readonly<{
...
/** Bind the common editor implementation to exact V21 product authority. */
export function createSoundscaperAudioEditorControllerV21(
	environmentValue: SoundscaperEditorProjectEnvironmentV21 | unknown,
	presentationValue: SoundscaperAudioEditorControllerPresentationV21 | unknown = {},
): SoundscaperAudioEditorControllerV21 {
	const environment = assertSoundscaperEditorProjectEnvironmentV21(environmentValue);
	const presentation = snapshotPresentation(presentationValue);
	const scapeProjectRuntime = createSoundscaperScapeNativeRuntimeV21();
	const delegate = createAudioEditorController(null, {
		...
		adaptAudacityProject: importSoundscaperAudacityProjectV21,
		scapeProjectRuntime,
		...presentation,
	});
...
function createControllerFacade(
	...
	const actions = decorateActions(delegate.actions, automation, freeze);
	...
	return Object.freeze(Object.create(
		...
		descriptors,
	)) as SoundscaperAudioEditorControllerV21;
}
...
function decorateActions(
	...
	automation: Readonly<SoundscaperAutomationControllerBindingV21>,
	freeze: Readonly<SoundscaperAudioFreezeActionBindingV21>,
): SoundscaperAudioEditorControllerV21['actions'] {
	...
	return Object.freeze(Object.create(
		...
		descriptors,
	)) as SoundscaperAudioEditorControllerV21['actions'];
}
...
async function disposeControllerBindings(
	...
	if (failures.length > 1) {
		throw new AggregateError(failures, 'Soundscaper V21 controller disposal failed.', {
			cause: failures[0],
		});
	}
}
...
function snapshotPresentation(value: unknown): SoundscaperAudioEditorControllerPresentationV21 {
	if (value === null || typeof value !== 'object' || Array.isArray(value)) {
		throw new TypeError('Soundscaper V21 controller presentation must be a plain record.');
	}
	...
	if (keys.some((key) => typeof key !== 'string' || !PRESENTATION_FIELDS.includes(
		...
		throw new TypeError('Soundscaper V21 controller presentation contains an unsupported authority option.');
	}
	...
	for (const key of PRESENTATION_FIELDS) {
		...
		if (!descriptor.enumerable || !Object.hasOwn(descriptor, 'value')) {
			throw new TypeError(`Soundscaper V21 controller presentation ${key} must be an own data property.`);
		}
		output[key] = descriptor.value;
	}
	if (output.locale !== undefined && typeof output.locale !== 'string') {
		throw new TypeError('Soundscaper V21 controller locale must be a string.');
	}
	if (output.copy !== undefined && (
		!output.copy || typeof output.copy !== 'object' || Array.isArray(output.copy)
	)) {
		throw new TypeError('Soundscaper V21 controller copy must be an object.');
	}
	return output as SoundscaperAudioEditorControllerPresentationV21;
}
