...
/** Validate and detach every document crossing the selected V29 persistence boundary. */
export class SoundscaperProjectRepositoryV29 implements ProjectRepositoryPort {
	...
	#snapshot(project: ProjectDocument | unknown): SoundscaperProjectV29 & ProjectDocument {
		return cloneSoundscaperProjectV29(project) as SoundscaperProjectV29 & ProjectDocument
	}
}

function assertDelegate(value: unknown): asserts value is ProjectRepositoryPort {
	if (!value || typeof value !== 'object') {
		throw new TypeError('A Soundscaper V29 project repository delegate is required.')
	}
	...
	for (const method of REQUIRED_DELEGATE_METHODS) {
		if (typeof delegate[method] !== 'function') {
			throw new TypeError(`The Soundscaper V29 project repository delegate requires ${method}.`)
		}
	}
}
