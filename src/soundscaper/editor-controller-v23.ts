...
import { importSoundscaperAudacityProjectV23 } from './editor-audacity-project-import-v23.ts';
import {
...
export interface SoundscaperAudioEditorControllerPresentationV23 {
...
export type SoundscaperAudioEditorControllerV23 = CommonAudioEditorController & Readonly<{
...
/** Bind the common editor implementation to exact V23 product authority. */
export function createSoundscaperAudioEditorControllerV23(
	environmentValue: SoundscaperEditorProjectEnvironmentV23 | unknown,
	presentationValue: SoundscaperAudioEditorControllerPresentationV23 | unknown = {},
): SoundscaperAudioEditorControllerV23 {
	const environment = assertSoundscaperEditorProjectEnvironmentV23(environmentValue);
	const presentation = snapshotPresentation(presentationValue);
	const scapeProjectRuntime = createSoundscaperScapeNativeRuntimeV23();
	const delegate = createAudioEditorController(null, {
		...
		adaptAudacityProject: importSoundscaperAudacityProjectV23,
		scapeProjectRuntime,
		...presentation,
	});
...
function createControllerFacade(
	...
	return Object.freeze(Object.create(
		...
		descriptors,
	)) as SoundscaperAudioEditorControllerV23;
}
...
function decorateActions(
	...
	automation: Readonly<SoundscaperAutomationControllerBindingV21>,
	freeze: Readonly<SoundscaperAudioFreezeActionBindingV21>,
): SoundscaperAudioEditorControllerV23['actions'] {
	...
	return Object.freeze(Object.create(
		...
		descriptors,
	)) as SoundscaperAudioEditorControllerV23['actions'];
}
...
async function disposeControllerBindings(
	...
	if (failures.length > 1) {
		throw new AggregateError(failures, 'Soundscaper V23 controller disposal failed.', {
			cause: failures[0],
		});
	}
}
...
function snapshotPresentation(value: unknown): SoundscaperAudioEditorControllerPresentationV23 {
	if (value === null || typeof value !== 'object' || Array.isArray(value)) {
		throw new TypeError('Soundscaper V23 controller presentation must be a plain record.');
	}
	...
	if (keys.some((key) => typeof key !== 'string' || !PRESENTATION_FIELDS.includes(
		...
		throw new TypeError('Soundscaper V23 controller presentation contains an unsupported authority option.');
	}
	...
	for (const key of PRESENTATION_FIELDS) {
		...
		if (!descriptor.enumerable || !Object.hasOwn(descriptor, 'value')) {
			throw new TypeError(`Soundscaper V23 controller presentation ${key} must be an own data property.`);
		}
		output[key] = descriptor.value;
	}
	if (output.locale !== undefined && typeof output.locale !== 'string') {
		throw new TypeError('Soundscaper V23 controller locale must be a string.');
	}
	if (output.copy !== undefined && (
		!output.copy || typeof output.copy !== 'object' || Array.isArray(output.copy)
	)) {
		throw new TypeError('Soundscaper V23 controller copy must be an object.');
	}
	return output as SoundscaperAudioEditorControllerPresentationV23;
}
