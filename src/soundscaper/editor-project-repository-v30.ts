...
/** Validate and detach every document crossing the selected V30 persistence boundary. */
export class SoundscaperProjectRepositoryV30 implements ProjectRepositoryPort {
	...
	#snapshot(project: ProjectDocument | unknown): SoundscaperProjectV30 & ProjectDocument {
		return cloneSoundscaperProjectV30(project) as SoundscaperProjectV30 & ProjectDocument
	}
}
function assertDelegate(value: unknown): asserts value is ProjectRepositoryPort {
	if (!value || typeof value !== 'object') {
		throw new TypeError('A Soundscaper V30 project repository delegate is required.')
	}
	...
	for (const method of REQUIRED_DELEGATE_METHODS) {
		if (typeof delegate[method] !== 'function') {
			throw new TypeError(`The Soundscaper V30 project repository delegate requires ${method}.`)
		}
	}
}
