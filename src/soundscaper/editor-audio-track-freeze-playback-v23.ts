...
export interface VerifiedSoundscaperAudioTrackFreezeV23 {
...
export type SoundscaperAudioTrackFreezeStatusV23 = 'none' | 'fresh' | 'stale' | 'unknown';

export interface SoundscaperAudioTrackFreezePlaybackServiceV23 extends PlaybackProjectService {
	readonly hashSourceContent: (
		projectId: string,
		source: DataRecord,
		signal?: AbortSignal,
	) => Promise<string>;
	readonly admitVerifiedFreeze: (request: VerifiedSoundscaperAudioTrackFreezeV23) => void;
...
export function createSoundscaperAudioTrackFreezePlaybackServiceV23(
	base: PlaybackProjectService,
	store: TransientAnalysisPcmStore,
): Readonly<SoundscaperAudioTrackFreezePlaybackServiceV23> {
	...
	function admitVerifiedFreeze(request: VerifiedSoundscaperAudioTrackFreezeV23): void {
...
function freezePlaybackClipId(trackId: string): string {
	return `${soundscaperAudioTrackFreezeRequirementIdV23(trackId)}clip`;
}
