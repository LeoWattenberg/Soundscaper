...
import type {
	SoundscaperNativePluginFormat,
	SoundscaperNativePluginState,
...
export function soundscaperNativePluginRequirementId(instanceId: string): string {
	return `${SOUNDSCAPER_NATIVE_PLUGIN_REQUIREMENT_PREFIX}${instanceDigest(instanceId)}`
}

export function soundscaperNativePluginFeatureId(
	instanceId: string,
	format: SoundscaperNativePluginFormat,
): string {
...
export function reconcileSoundscaperNativePluginRequirements(
	project: DataRecord,
	...
	for (const requirement of manifest.requirements) {
		...
		if (!exact || !sameRequirement(requirement, exact)) {
			throw new TypeError('A reserved native plug-in requirement conflicts with baseline project state.')
		}
	}
...
export function nativePluginCapabilitySets(
	project: DataRecord,
): Readonly<{ known: ReadonlySet<string>; available: ReadonlySet<string> }> {
	const known = new Set<string>()
	const available = new Set<string>()
	for (const state of nativeStates(project)) {
		const featureId = soundscaperNativePluginFeatureId(state.instanceId, state.format)
		known.add(featureId)
 ...
 */
export function projectNativePluginPlayback<Project extends object>(
	project: Project,
	...
	const affected = new Set(states.filter((state) => (
		unavailable.has(soundscaperNativePluginRequirementId(state.instanceId))
...
function nativeRequirement(state: SoundscaperNativePluginState): ProjectFeatureRequirement {
	return Object.freeze({
		id: soundscaperNativePluginRequirementId(state.instanceId),
		featureId: soundscaperNativePluginFeatureId(state.instanceId, state.format),
...
function nativeStates(project: DataRecord): readonly SoundscaperNativePluginState[] {
	return Array.isArray(project.nativePluginStates)
		? project.nativePluginStates as readonly SoundscaperNativePluginState[]
		: Object.freeze([])
}
