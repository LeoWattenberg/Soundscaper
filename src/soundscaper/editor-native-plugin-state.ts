...
export type SoundscaperNativePluginFormat =
	(typeof SOUNDSCAPER_NATIVE_PLUGIN_FORMATS)[number]

export const SOUNDSCAPER_NATIVE_PLUGIN_CONTINUITIES = Object.freeze([
	'live', 'bypass', 'frozen',
] as const)

export type SoundscaperNativePluginContinuity =
	(typeof SOUNDSCAPER_NATIVE_PLUGIN_CONTINUITIES)[number]

export const SOUNDSCAPER_NATIVE_PLUGIN_STATE_LIMITS = Object.freeze({
...
export interface SoundscaperNativePluginStateBody {
...
export interface SoundscaperNativePluginState {
	readonly instanceId: string
	readonly format: SoundscaperNativePluginFormat
	readonly stablePluginId: string
	readonly binarySha256: string
	readonly stateBody: Readonly<SoundscaperNativePluginStateBody>
	readonly enabled: boolean
	readonly bypassed: boolean
	readonly continuity: SoundscaperNativePluginContinuity
...
export function createSoundscaperNativePluginState(
	value: unknown,
): Readonly<SoundscaperNativePluginState> {
	const record = closedRecord(value, STATE_FIELDS, 'Soundscaper native plug-in state')
	const format = enumValue(record.format, SOUNDSCAPER_NATIVE_PLUGIN_FORMATS, 'plug-in format')
	const continuity = enumValue(
		record.continuity,
		SOUNDSCAPER_NATIVE_PLUGIN_CONTINUITIES,
		'plug-in continuity',
	)
	...
	return Object.freeze({
		...
		latencySamples: boundedInteger(
			...
			0,
			SOUNDSCAPER_NATIVE_PLUGIN_STATE_LIMITS.maximumLatencySamples,
			'plug-in latency',
		),
	})
}

export function normalizeSoundscaperNativePluginStates(
	value: unknown,
): readonly Readonly<SoundscaperNativePluginState>[] {
	if (!Array.isArray(value) || Object.getPrototypeOf(value) !== Array.prototype
		|| value.length > SOUNDSCAPER_NATIVE_PLUGIN_STATE_LIMITS.maximumEntries) {
		throw new TypeError('project.nativePluginStates must be a bounded plain array.')
	}
	const result: Readonly<SoundscaperNativePluginState>[] = []
	const instanceIds = new Set<string>()
	for (let index = 0; index < value.length; index += 1) {
		if (!Object.hasOwn(value, index)) throw new TypeError('project.nativePluginStates must be dense.')
		const state = createSoundscaperNativePluginState(value[index])
...
function normalizeStateBody(value: unknown): Readonly<SoundscaperNativePluginStateBody> {
	...
	return Object.freeze({
		...
		byteLength: boundedInteger(
			record.byteLength,
			0,
			SOUNDSCAPER_NATIVE_PLUGIN_STATE_LIMITS.maximumStateBytes,
...
function boundedText(value: unknown, label: string): string {
	if (typeof value !== 'string' || !value || value !== value.trim()
		|| new TextEncoder().encode(value).byteLength
			> SOUNDSCAPER_NATIVE_PLUGIN_STATE_LIMITS.maximumIdentifierBytes) {
