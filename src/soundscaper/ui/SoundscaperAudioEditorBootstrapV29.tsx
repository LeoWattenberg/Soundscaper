...
import { createSoundscaperAudioEditorControllerV29 } from '../editor-controller-v29.ts';
import {
	createSoundscaperEditorProjectEnvironmentV29,
	type SoundscaperEditorProjectEnvironmentV29,
} from '../editor-project-environment-v29.ts';

type SoundscaperWebControllerV29 = ReturnType<typeof createSoundscaperAudioEditorControllerV29>;
type SoundscaperWebFileServiceV29 = ReturnType<typeof createAudioEditorFileService>;
type SoundscaperProjectRuntimeProjectionV29 =
	SoundscaperEditorProjectEnvironmentV29['runtime']['projectForRuntimeConsumers'];

const RUNTIME_PROJECTORS = new WeakMap<object, SoundscaperProjectRuntimeProjectionV29>();
const PRESENTATION_FIELDS = ['locale', 'copy'] as const;

export interface SoundscaperWebEditorRuntimePresentationV29 {
	readonly locale: string;
	readonly copy: Readonly<Record<string, unknown>>;
}

export interface SoundscaperWebEditorRuntimeV29 {
	readonly controller: SoundscaperWebControllerV29;
	readonly fileService: SoundscaperWebFileServiceV29;
	readonly dispose: () => Promise<void>;
}

export interface SoundscaperAudioEditorBootstrapV29Props {
	readonly locale: string;
	readonly fallbackCopy: Readonly<Record<string, unknown>>;
}

/** Construct the exact-V29 browser runtime from presentation-only input. */
export async function createSoundscaperWebEditorRuntimeV29(
	presentationValue: SoundscaperWebEditorRuntimePresentationV29 | unknown,
): Promise<Readonly<SoundscaperWebEditorRuntimeV29>> {
	const presentation = snapshotPresentation(presentationValue);
	const fileService = createAudioEditorFileService();
	const environment = await createSoundscaperEditorProjectEnvironmentV29({
		storeOptions: {
	...
	try {
		const controller = createSoundscaperAudioEditorControllerV29(environment, {
		...
		try {
			...
			throw new AggregateError(
				[error, cleanupError],
				'Soundscaper V29 web runtime construction and cleanup both failed.',
				{ cause: error },
			);
		}
		throw error;
...
export default function SoundscaperAudioEditorBootstrapV29({
	locale,
	fallbackCopy: fallbackCopyValue,
}: SoundscaperAudioEditorBootstrapV29Props) {
	...
	const [runtime, setRuntime] = useState<Readonly<SoundscaperWebEditorRuntimeV29> | null>(null);
	...
	useEffect(() => {
		if (!copy) return undefined;
		let active = true;
		let ownedRuntime: Readonly<SoundscaperWebEditorRuntimeV29> | null = null;
		setFailure(null);
		void createSoundscaperWebEditorRuntimeV29({ locale, copy }).then(
			(candidate) => {
				if (!active) {
...
function runtimeProjector(
	runtime: Readonly<SoundscaperWebEditorRuntimeV29>,
): SoundscaperProjectRuntimeProjectionV29 {
	const projector = RUNTIME_PROJECTORS.get(runtime);
	if (!projector) throw new TypeError('An exact Soundscaper V29 web runtime is required.');
	return projector;
}

async function disposeRuntime(
	controller: SoundscaperWebControllerV29,
	environment: Readonly<SoundscaperEditorProjectEnvironmentV29>,
): Promise<void> {
	let failure: unknown;
	try {
	...
	try {
		...
		if (failure) {
			throw new AggregateError(
				[failure, error],
				'Soundscaper V29 controller and environment disposal both failed.',
				{ cause: failure },
			);
		}
		throw error;
...
function snapshotPresentation(value: unknown): SoundscaperWebEditorRuntimePresentationV29 {
	const record = closedRecord(value, PRESENTATION_FIELDS, 'Soundscaper V29 web presentation');
	const locale = record.locale;
	if (typeof locale !== 'string' || !locale.trim() || locale.length > 128) {
		throw new TypeError('The Soundscaper V29 web locale must be a bounded string.');
	}
	return Object.freeze({
		locale,
		copy: snapshotCopy(record.copy, 'Soundscaper V29 web copy'),
	});
}
