...
import { soundscaperAudioTrackFreezeRequirementIdV30 } from './editor-project-feature-requirements-v30.ts';

type DataRecord = Readonly<Record<string, unknown>>;

export interface VerifiedSoundscaperAudioTrackFreezeV30 {
...
export type SoundscaperAudioTrackFreezeStatusV30 = 'none' | 'fresh' | 'stale' | 'unknown';

export interface SoundscaperAudioTrackFreezePlaybackServiceV30 extends PlaybackProjectService {
	readonly hashSourceContent: (
		projectId: string,
		source: DataRecord,
		signal?: AbortSignal,
	) => Promise<string>;
	readonly admitVerifiedFreeze: (request: VerifiedSoundscaperAudioTrackFreezeV30) => void;
	readonly getFreezeStatus: (
		project: object,
		trackId: string,
	) => SoundscaperAudioTrackFreezeStatusV30;
...
export function createSoundscaperAudioTrackFreezePlaybackServiceV30(
	base: PlaybackProjectService,
	store: TransientAnalysisPcmStore,
): Readonly<SoundscaperAudioTrackFreezePlaybackServiceV30> {
	...
	async function prepareProjectForActivation<Project extends object>(
		...
		if (candidate.schemaVersion !== SOUNDSCAPER_PROJECT_V30_SCHEMA_VERSION) return;
		const projectId = stableId(candidate.id, 'Soundscaper freeze activation project');
		const prepared: VerifiedSoundscaperAudioTrackFreezeV30[] = [];
	...
	function admitVerifiedFreeze(request: VerifiedSoundscaperAudioTrackFreezeV30): void {
	...
	function selectFreshAdmissions(projectValue: object): readonly FreezeAdmission[] {
		...
		if (project.schemaVersion !== SOUNDSCAPER_PROJECT_V30_SCHEMA_VERSION) return Object.freeze([]);
	...
	function getFreezeStatus(
		...
		try {
			...
			if (project.schemaVersion !== SOUNDSCAPER_PROJECT_V30_SCHEMA_VERSION) return 'unknown';
...
function freezePlaybackClipId(trackId: string): string {
	return `${soundscaperAudioTrackFreezeRequirementIdV30(trackId)}clip`;
}
