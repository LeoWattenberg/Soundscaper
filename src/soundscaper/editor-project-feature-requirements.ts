...
import { normalizeAssistanceAssetReferencesV1 } from
	'../common/editor/assistance/assistance-asset-reference-v1.ts';
...
import { reconcileSoundscaperNativePluginRequirements } from './editor-native-plugin-playback.ts';
...
export function soundscaperAudioTrackFreezeRequirementId(trackIdValue: unknown): string {
...
/** Reconcile every Soundscaper track relationship to one product-owned rendered fallback. */
export function reconcileSoundscaperFreezeRequirements(
	...
	if (manifest.schemaVersion !== 2 || !Array.isArray(manifest.requirements)) {
		throw new TypeError('Soundscaper feature requirements must be an exact schema-2 manifest.');
	}
...
export function rebindSoundscaperFreezeSourceIdentities(
	...
	const project = dataRecord(projectValue, 'Soundscaper project');
	...
	for (const track of tracks) {
		...
		const requirementId = soundscaperAudioTrackFreezeRequirementId(track.id);
...
/** Reconcile the complete baseline-owned freeze and native plug-in requirements. */
export function reconcileSoundscaperProjectFeatureRequirements(
	project: DataRecord,
	manifest: ProjectFeatureRequirementsManifest,
): ProjectFeatureRequirementsManifest {
	return reconcileSoundscaperNativePluginRequirements(
		project,
		reconcileSoundscaperFreezeRequirements(project, manifest),
	);
}

export function validateSoundscaperProjectFeatureRequirements(project: DataRecord): true {
	validateSoundscaperFreezeRequirements(project);
	const reconciled = reconcileSoundscaperProjectFeatureRequirements(
		project,
	...
	if (JSON.stringify(project.featureRequirements) !== JSON.stringify(reconciled)) {
		throw new RangeError('Soundscaper native plug-in requirements are not in exact reconciled form.');
	}
	return true;
}

/** Rebind source-owned freeze state and assistance references together. */
export function rebindSoundscaperProjectSourceIdentities(
	project: Record<string, unknown>,
	sourceIdMap: ReadonlyMap<string, string>,
): void {
	rebindSoundscaperFreezeSourceIdentities(project, sourceIdMap);
...
function expectedFreezeRequirements(project: DataRecord): readonly ProjectFeatureRequirement[] {
	...
	for (const [index, candidate] of tracks.entries()) {
		...
		const base = Object.freeze({
			id: soundscaperAudioTrackFreezeRequirementId(trackId),
