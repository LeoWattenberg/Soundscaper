...
import type {
	SoundscaperDesktopProjectLibraryV11Renderer,
} from './desktop-project-library-v11-renderer.ts';
import type {
	SoundscaperNativePluginStateBodyV29,
} from './editor-native-plugin-state-v29.ts';
import {
	assertSoundscaperDesktopProjectLibraryV11RendererComposition,
	SoundscaperDesktopProjectLibraryV11CommittedError,
	SoundscaperDesktopProjectLibraryV11IndeterminateError,
} from './desktop-project-library-v11-renderer.ts';

export interface SoundscaperDesktopProjectStoreV11Composition {
	readonly localStore: SoundscaperDesktopProjectStoreV11Local;
	readonly desktopProjectLibrary: SoundscaperDesktopProjectLibraryV11Renderer | null;
}

export interface SoundscaperDesktopProjectStoreV11Local {
...
export type SoundscaperDesktopProjectStoreV11Adapter<Store> = Store & Readonly<{
	...
	): Promise<Readonly<SoundscaperNativePluginStateBodyV29>>;
}>;
...
export function createSoundscaperDesktopProjectStoreV11Adapter<Store extends SoundscaperDesktopProjectStoreV11Local>(
...
export function createSoundscaperDesktopProjectStoreV11Adapter<Store extends SoundscaperDesktopProjectStoreV11Local>(
	...
	compositionValue: Readonly<{
		...
		readonly desktopProjectLibrary: SoundscaperDesktopProjectLibraryV11Renderer;
	}>,
): SoundscaperDesktopProjectStoreV11Adapter<Store>;
export function createSoundscaperDesktopProjectStoreV11Adapter<Store extends SoundscaperDesktopProjectStoreV11Local>(
	profileValue: EditorProjectRuntimeProfile | unknown,
	compositionValue: Readonly<{
		readonly localStore: Store;
...
): Store | SoundscaperDesktopProjectStoreV11Adapter<Store> {
	assertSoundscaperProductionProfile(profileValue);
	const composition = exactRecord(
		compositionValue, COMPOSITION_FIELDS, 'Soundscaper desktop V11 store composition',
	);
	...
	assertSoundscaperDesktopProjectLibraryV11RendererComposition(profileValue, localStore, renderer);
	...
	return new Proxy(localStore, proxyHandler(
		...
		lifecycle as SoundscaperDesktopProjectStoreV11Local['linkedOriginalStoreService'],
	)) as
		SoundscaperDesktopProjectStoreV11Adapter<Store>;
}

function proxyHandler<Store extends SoundscaperDesktopProjectStoreV11Local>(
	profile: EditorProjectRuntimeProfile,
	localStore: Store,
	renderer: SoundscaperDesktopProjectLibraryV11Renderer,
	lifecycle: SoundscaperDesktopProjectStoreV11Local['linkedOriginalStoreService'],
): ProxyHandler<Store> {
	...
	const overrides = Object.freeze({
		...
		loadProject: async (projectId: string, optionsValue: unknown = {}) => {
			...
			await lifecycle.deleteProject(projectId, async () => {
				if (!await renderer.cleanupDeletedProject(projectId)) {
					throw new Error('The definite desktop V11 delete cleanup tombstone is unavailable.');
				}
			});
			if (!await renderer.settleDeletedProject(projectId)) {
				throw new Error('The definite desktop V11 delete cleanup could not be settled.');
			}
		...
		createProjectIfAbsent: async (projectValue: unknown) => {
			...
			if (Number(project.revision) !== 0) {
				throw new Error('Soundscaper desktop V11 create requires fresh revision zero.');
			}
		...
		persistNativePluginStateBody: async (
			...
			if (persisted.bodyId !== expected.bodyId || persisted.byteLength !== expected.byteLength
				...
				throw new Error('Desktop V11 native plug-in-state persistence changed its expected identity.');
			}
		...
		deleteProject: async (projectId: string) => {
			try {
				...
				try {
					...
					if (error instanceof SoundscaperDesktopProjectLibraryV11CommittedError) throw error;
					throw new SoundscaperDesktopProjectLibraryV11CommittedError('delete', projectId, error);
				}
			} catch (error) {
				if (error instanceof SoundscaperDesktopProjectLibraryV11CommittedError
			...
			return lifecycle.duplicateProject({
				...
				createProjectIfAbsent: async (copy: ProjectDocument) => {
					try {
						...
						if ((error instanceof SoundscaperDesktopProjectLibraryV11CommittedError
							|| error instanceof SoundscaperDesktopProjectLibraryV11IndeterminateError)
...
async function assertLocalDuplicateDestinationAbsent(
	store: SoundscaperDesktopProjectStoreV11Local,
	projectId: string,
): Promise<void> {
	if (await store.loadProject(projectId) !== null) {
		throw new Error('Soundscaper desktop V11 duplicate destination has an occupied local shadow.');
	}
}

function assertLocalStore(value: unknown): asserts value is SoundscaperDesktopProjectStoreV11Local {
...
function loadOptions(value: unknown): Readonly<{ revision?: number; signal?: AbortSignal }> {
	const raw = allowedRecord(value, LOAD_FIELDS, 'Soundscaper desktop V11 load options');
...
function saveOptions(value: unknown): Record<string, unknown> {
	return allowedRecord(value, SAVE_FIELDS, 'Soundscaper desktop V11 save options');
}

function duplicateOptions(value: unknown): Readonly<{ readonly id?: string; readonly title?: unknown }> {
	const raw = allowedRecord(value, DUPLICATE_FIELDS, 'Soundscaper desktop V11 duplicate options');
	if (raw.id !== undefined && (typeof raw.id !== 'string' || !raw.id)) {
		throw new TypeError('The Soundscaper desktop V11 duplicate project id is invalid.');
	}
	return Object.freeze({
