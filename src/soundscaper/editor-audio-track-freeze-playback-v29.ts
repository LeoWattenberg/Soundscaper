...
import { soundscaperAudioTrackFreezeRequirementIdV29 } from './editor-project-feature-requirements-v29.ts';

type DataRecord = Readonly<Record<string, unknown>>;

export interface VerifiedSoundscaperAudioTrackFreezeV29 {
...
export type SoundscaperAudioTrackFreezeStatusV29 = 'none' | 'fresh' | 'stale' | 'unknown';

export interface SoundscaperAudioTrackFreezePlaybackServiceV29 extends PlaybackProjectService {
	readonly hashSourceContent: (
		projectId: string,
		source: DataRecord,
		signal?: AbortSignal,
	) => Promise<string>;
	readonly admitVerifiedFreeze: (request: VerifiedSoundscaperAudioTrackFreezeV29) => void;
	readonly getFreezeStatus: (
		project: object,
		trackId: string,
	) => SoundscaperAudioTrackFreezeStatusV29;
...
export function createSoundscaperAudioTrackFreezePlaybackServiceV29(
	base: PlaybackProjectService,
	store: TransientAnalysisPcmStore,
): Readonly<SoundscaperAudioTrackFreezePlaybackServiceV29> {
	...
	async function prepareProjectForActivation<Project extends object>(
		...
		if (candidate.schemaVersion !== SOUNDSCAPER_PROJECT_V29_SCHEMA_VERSION) return;
		const projectId = stableId(candidate.id, 'Soundscaper freeze activation project');
		const prepared: VerifiedSoundscaperAudioTrackFreezeV29[] = [];
	...
	function admitVerifiedFreeze(request: VerifiedSoundscaperAudioTrackFreezeV29): void {
	...
	function selectFreshAdmissions(projectValue: object): readonly FreezeAdmission[] {
		...
		if (project.schemaVersion !== SOUNDSCAPER_PROJECT_V29_SCHEMA_VERSION) return Object.freeze([]);
	...
	function getFreezeStatus(
		...
		try {
			...
			if (project.schemaVersion !== SOUNDSCAPER_PROJECT_V29_SCHEMA_VERSION) return 'unknown';
...
function freezePlaybackClipId(trackId: string): string {
	return `${soundscaperAudioTrackFreezeRequirementIdV29(trackId)}clip`;
}
