...
/** Validate exact Soundscaper V21 authority without a lossy legacy mixer projection. */
export function validateSoundscaperProjectV21(project: unknown): project is SoundscaperProjectV21 {
	const candidate = readClosedDomainRecord(
		project,
		'Soundscaper V21 project',
		PROJECT_FIELDS,
		PROJECT_FIELDS,
	) as Record<string, unknown>
	const graph = normalizeMixerGraphV21(readClosedDomainField(candidate, 'mixer', 'Soundscaper V21 project'))
	...
	validateFolderMixerGraphV21(candidate as unknown as SoundscaperProjectV21, graph)
	...
	validateTakeCompDocumentGroupsV17(
		readClosedDomainField(candidate, 'takeGroups', 'Soundscaper V21 project'),
		candidate,
	)
	validateTrackFieldInventories(candidate)
	validateAutomationLanes(candidate, graph)
	validateAudioFreezeRelationships(candidate)
	validateSoundscaperProjectFeatureRequirementsV21(candidate)
	return true
}
...
function validateAutomationLanes(project: Record<string, unknown>, graph: MixerGraphV21): void {
	const laneValues = readClosedDomainArray(
		readClosedDomainField(project, 'automationLanes', 'Soundscaper V21 project'),
