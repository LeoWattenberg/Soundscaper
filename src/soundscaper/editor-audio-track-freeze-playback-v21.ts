...
export interface VerifiedSoundscaperAudioTrackFreezeV21 {
...
export type SoundscaperAudioTrackFreezeStatusV21 = 'none' | 'fresh' | 'stale' | 'unknown';

export interface SoundscaperAudioTrackFreezePlaybackServiceV21 extends PlaybackProjectService {
	readonly hashSourceContent: (
		projectId: string,
		source: DataRecord,
		signal?: AbortSignal,
	) => Promise<string>;
	readonly admitVerifiedFreeze: (request: VerifiedSoundscaperAudioTrackFreezeV21) => void;
...
export function createSoundscaperAudioTrackFreezePlaybackServiceV21(
	base: PlaybackProjectService,
	store: TransientAnalysisPcmStore,
): Readonly<SoundscaperAudioTrackFreezePlaybackServiceV21> {
	...
	async function prepareProjectForActivation<Project extends object>(
		...
		const prepared: VerifiedSoundscaperAudioTrackFreezeV21[] = [];
	...
	function admitVerifiedFreeze(request: VerifiedSoundscaperAudioTrackFreezeV21): void {
...
function freezePlaybackClipId(trackId: string): string {
	return `${soundscaperAudioTrackFreezeRequirementIdV21(trackId)}clip`;
}
