...
import {
	...
	type SoundscaperDesktopV11CatalogSnapshot,
...
export function createSoundscaperDesktopV11RendererOperationId(): string {
	if (typeof globalThis.crypto?.randomUUID !== 'function') {
		throw new Error('A cryptographic renderer operation identity is required for document reconciliation.');
	}
	return `desktop-v11:${globalThis.crypto.randomUUID()}`;
}

export function createSoundscaperDesktopV11PublicationId(): string {
...
export interface SoundscaperDesktopV11DuplicateOptions {
	readonly id: string;
	readonly title: string;
	readonly timestamp: string;
}

export type SoundscaperDesktopV11Witness = Readonly<{
	readonly kind: 'absent';
	readonly expectedMetadataRevision: number;
}> | SoundscaperDesktopV11CurrentWitness;

export type SoundscaperDesktopV11CurrentWitness = Readonly<{
...
export class SoundscaperDesktopV11WitnessLedger {
	readonly #profile: EditorProjectRuntimeProfile;
	readonly #witnesses = new Map<string, SoundscaperDesktopV11Witness>();
	...
	rememberCurrent(snapshot: Readonly<SoundscaperDesktopV11BundleSnapshot>): void {
		this.#witnesses.set(snapshot.bundle.project.projectId, currentWitness(this.#profile, snapshot));
	}

	take(projectId: string): SoundscaperDesktopV11Witness {
		const witness = this.#witnesses.get(projectId);
		if (!witness) throw new Error('An authoritative desktop V11 load witness is required before mutation.');
		this.#witnesses.delete(projectId);
		return witness;
	}

	takeCurrent(projectId: string): SoundscaperDesktopV11CurrentWitness {
		const witness = this.take(projectId);
		if (witness.kind !== 'current') throw new Error('A current desktop V11 project witness is required.');
		return witness;
	}
	...
	restoreCurrent(witness: SoundscaperDesktopV11CurrentWitness, metadataRevision: number): void {
	...
	observeCatalog(snapshot: Readonly<SoundscaperDesktopV11CatalogSnapshot>): void {
...
export function validateSoundscaperDesktopV11DuplicateOptions(
	value: unknown,
): Readonly<SoundscaperDesktopV11DuplicateOptions> {
	const raw = closedRecord(value, DUPLICATE_FIELDS, 'Soundscaper desktop V11 duplicate options');
	if (typeof raw.title !== 'string' || !raw.title.trim()
		|| new TextEncoder().encode(raw.title).byteLength > MAXIMUM_TITLE_BYTES) {
		throw new TypeError('The Soundscaper desktop V11 duplicate title is invalid.');
	}
	...
	return Object.freeze({
		id: validateSoundscaperDesktopV11ProjectId(raw.id),
		title: raw.title,
		timestamp: raw.timestamp,
	});
}

export function createSoundscaperDesktopV11DuplicateProject(
...
export function sameSoundscaperDesktopV11Project(
