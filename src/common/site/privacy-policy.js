...
const EFFECTIVE_DATE = Object.freeze({ en: '21 September 2026', de: '21. September 2026' });
		...
		sections: Object.freeze([
			...
			section('network', '5. When network connections occur', `
				<ul>
					...
					<li><strong>Freesound search, details, and previews:</strong> when you search in this optional Soundscaper feature, request sound details, play a preview, or import a sound, your device sends the search terms, result page, license filter, and sort choice, or the selected numeric sound ID and any requested preview byte range, to Soundscaper's proxy hosted by Cloudflare. Cloudflare receives the technical access data described below together with these request values and returns the response to your device. The proxy sends the necessary search values or sound ID to the Freesound API using Soundscaper's server-side credential. For preview and import, it also retrieves and streams the selected Ogg preview from Freesound's content-delivery network. Freesound receives the request from the proxy rather than a direct request from your device. Your projects, recordings, and other local media are not sent with these requests.</li>
			...
			section('recipients', '8. Recipients and third-party services', `
				<p>Technical data may be processed by Cloudflare as our hosting, content-delivery, asset-storage, security, and Freesound-proxy provider; by Migadu when you email us; and by GitHub during desktop update checks or when you use repository links. For the optional Freesound feature, Cloudflare processes the search, detail, preview, and import request and response data described above. Freesound, a project operated by the Music Technology Group at Universitat Pompeu Fabra (UPF), receives the proxy's technical connection data and the necessary search values, sound ID, selected preview, and any requested byte range, and returns metadata and audio to the proxy. UPF provides Freesound as an independent third-party service under Freesound's <a href="https://freesound.org/help/privacy/">own privacy policy</a>. User-selected model hosts, package managers, external-tool sources, and Web VCR sites receive data only when you initiate those connections.</p>
		...
		sections: Object.freeze([
			...
			section('network', '5. Netzwerkverbindungen', `
				<ul>
					...
					<li><strong>Freesound-Suche, -Details und -Vorschauen:</strong> Wenn Sie in dieser optionalen Soundscaper-Funktion suchen, Sounddetails abrufen, eine Vorschau abspielen oder einen Sound importieren, sendet Ihr Gerät die Suchbegriffe, Ergebnisseite, den Lizenzfilter und die Sortierauswahl oder die gewählte numerische Sound-ID sowie einen angeforderten Byte-Bereich der Vorschau an den von Cloudflare bereitgestellten Soundscaper-Proxy. Cloudflare erhält dabei die unten beschriebenen technischen Zugriffsdaten zusammen mit diesen Anfragewerten und liefert die Antwort an Ihr Gerät. Der Proxy übermittelt die erforderlichen Suchwerte oder die Sound-ID mit einem serverseitigen Zugangsschlüssel von Soundscaper an die Freesound-API. Für Vorschau und Import ruft er außerdem die gewählte Ogg-Vorschau aus dem Freesound-Auslieferungsnetzwerk ab und streamt sie. Freesound erhält die Anfrage vom Proxy statt als direkte Verbindung Ihres Geräts. Projekte, Aufnahmen und andere lokale Medien werden mit diesen Anfragen nicht übermittelt.</li>
			...
			section('recipients', '8. Empfänger und Drittanbieter', `
				<p>Technische Daten können durch Cloudflare als Anbieter für Hosting, Inhaltsauslieferung, Dateispeicherung, Sicherheit und den Freesound-Proxy, durch Migadu bei E-Mails an uns und durch GitHub bei Desktop-Updateprüfungen oder Repositorylinks verarbeitet werden. Für die optionale Freesound-Funktion verarbeitet Cloudflare die oben beschriebenen Anfrage- und Antwortdaten von Suche, Detailabruf, Vorschau und Import. Freesound, ein von der Music Technology Group der Universitat Pompeu Fabra (UPF) betriebenes Projekt, erhält technische Verbindungsdaten des Proxys sowie die erforderlichen Suchwerte, die Sound-ID, die gewählte Vorschau und einen angeforderten Byte-Bereich und liefert Metadaten und Audio an den Proxy zurück. UPF stellt Freesound als unabhängigen Drittanbieterdienst unter der <a href="https://freesound.org/help/privacy/">eigenen Datenschutzerklärung</a> von Freesound bereit. Von Ihnen gewählte Modellhosts, Paketmanager, Quellen externer Werkzeuge und Web-VCR-Seiten erhalten Daten nur, wenn Sie diese Verbindungen auslösen.</p>
