...
const bandDynamicsWorklet = createAudioWorkletLoadOnce(async (context) => {
...
export const isBandDynamicsWorkletLoaded = (context: BaseAudioContext): boolean => bandDynamicsWorklet.isLoaded(context);

export async function ensureBandDynamicsWorklet(context: BaseAudioContext): Promise<void> {
	await bandDynamicsWorklet.ensure(context);
}
...
export function createBandDynamicsNode(context: BaseAudioContext, WorkletNode: WorkletNodeConstructor | null,
	...
	if (!bandDynamicsWorklet.isLoaded(context) || !WorkletNode) throw new Error('The band dynamics processor was not loaded.');
