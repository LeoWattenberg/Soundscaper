...
export interface ScapeVideoProxyArchiveReferenceV2 {
...
export interface ScapeVideoProxyArchiveAssetDescriptorV2 {
...
/**
 * Builds only the detached manifest metadata for a future format-2 archive.
 * It neither reads bodies nor changes the maintained format-1 archive owner.
 */
export function planScapeVideoProxyArchiveAssetsV2(
	value: unknown,
): Readonly<{
	...
	const assets: Readonly<ScapeVideoProxyArchiveAssetDescriptorV2>[] = [];
	const assetBySourceId = new Map<string, Readonly<ScapeVideoProxyArchiveAssetDescriptorV2>>();
	const assetByEntry = new Map<string, Readonly<ScapeVideoProxyArchiveAssetDescriptorV2>>();
	...
	return Object.freeze({
		formatVersion: assets.length ? 2 : 1,
		assets: Object.freeze(assets),
	});
}

function normalizeReference(value: unknown): Readonly<ScapeVideoProxyArchiveReferenceV2> {
...
function proxyDescriptor(
	reference: Readonly<ScapeVideoProxyArchiveReferenceV2>,
): Readonly<ScapeVideoProxyArchiveAssetDescriptorV2> {
	return Object.freeze({
...
function addAsset(
	asset: Readonly<ScapeVideoProxyArchiveAssetDescriptorV2>,
	assets: Readonly<ScapeVideoProxyArchiveAssetDescriptorV2>[],
	assetBySourceId: Map<string, Readonly<ScapeVideoProxyArchiveAssetDescriptorV2>>,
	assetByEntry: Map<string, Readonly<ScapeVideoProxyArchiveAssetDescriptorV2>>,
): void {
	...
	if (priorSource || priorEntry) {
		...
		throw new Error(`Scape format-2 asset ${asset.sourceId} has a conflicting identity.`);
	}
	if (assets.length >= MAXIMUM_ASSETS) {
		throw new RangeError('The Scape format-2 archive contains too many assets.');
	}
	assets.push(asset);
...
function sameAsset(
	...
	right: Readonly<ScapeVideoProxyArchiveAssetDescriptorV2>,
): boolean {
