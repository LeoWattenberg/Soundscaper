...
import { FRAMESCAPER_FINISHING_SURFACE_COPY } from '../../i18n/editor-framescaper-finishing-surface-copy.ts';
...
export interface FramescaperFinishingDialogModel {
...
export type FramescaperFinishingDialogCommand = FramescaperOwnedFinishingCommand
	| Readonly<{
		readonly type: 'batch';
		readonly commands: readonly FramescaperOwnedFinishingCommand[];
	}>;

interface CollectionSpec {
	readonly field: string;
	readonly identity: string;
	readonly type: FramescaperOwnedFinishingCommand['type'];
...
export function createFramescaperFinishingDialogModel(input: Readonly<{
	readonly copy?: Readonly<Record<string, string | undefined>>;
	readonly surface: FramescaperFinishingSurface;
	readonly project: unknown;
}>): FramescaperFinishingDialogModel {
	const project = projectRecord(input.project);
	const copy = resolveEditorCopyScope(`framescaperFinishing.surfaces.${input.surface}`,
		FRAMESCAPER_FINISHING_SURFACE_COPY[input.surface], input.copy);
...
export function createFramescaperFinishingCommand(
	surface: FramescaperFinishingSurface,
	projectValue: unknown,
	documentText: string,
): FramescaperFinishingDialogCommand {
...
export function importFramescaperCaptionSidecar(input: Readonly<{
	readonly project: unknown;
	...
	readonly command: FramescaperOwnedFinishingCommand;
}> {
	const project = projectRecord(input.project);
	...
	const command = snapshotFramescaperOwnedFinishingCommand({
...
export function exportFramescaperCaptionSidecar(input: Readonly<{
	readonly project: unknown;
	readonly trackId: string;
	readonly format: VideoCaptionInterchangeFormatV1;
...
function surfaceCommands(
	surface: Exclude<FramescaperFinishingSurface, 'dialogue-chain' | 'visual-inspector'>,
	project: Record<string, unknown>,
	draft: unknown,
): FramescaperOwnedFinishingCommand[] {
...
function collection(
	field: string,
	identity: string,
	type: FramescaperOwnedFinishingCommand['type'],
	idField: string,
...
function projectRecord(value: unknown): Record<string, unknown> {
	const project = record(value, 'Framescaper project');
	...
	if (identity.schemaFamily !== FRAMESCAPER_PROJECT_SCHEMA_FAMILY
		...
		throw new RangeError('The current Framescaper project is required.');
	}
	return project;
}
