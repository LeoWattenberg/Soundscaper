...
export interface FramescaperVisualInspectorPreset {
	readonly id: string;
...
export interface FramescaperVisualInspectorDraft {
	readonly generator: VideoGeneratorDocumentV1 | null;
	readonly opacity: number;
	readonly blendMode: VideoVisualPresentationV1['blendMode'];
	readonly maskId: string | null;
...
export function createFramescaperVisualInspectorModel(input: Readonly<{
	readonly project: unknown;
	readonly selectedClipId?: unknown;
}>): FramescaperVisualInspectorModel {
...
export function createFramescaperVisualInspectorCommand(
	projectValue: unknown,
	clipIdValue: unknown,
	draft: FramescaperVisualInspectorDraft,
): unknown {
...
function projectRecord(value: unknown): Data {
	const project = record(value, 'Framescaper visual inspector project');
	...
	if (identity.schemaFamily !== FRAMESCAPER_PROJECT_SCHEMA_FAMILY
		...
		throw new RangeError('The visual inspector requires a current Framescaper project.');
	}
	return project;
}

function boundGeneratorPresets(project: Data): readonly FramescaperVisualInspectorPreset[] {
