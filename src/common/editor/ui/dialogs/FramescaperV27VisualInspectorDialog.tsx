...
import {
	...
	type FramescaperV27VisualInspectorDraft,
} from '../framescaper-v27-visual-inspector-model.ts';
...
export default function FramescaperV27VisualInspectorDialog({
	controller, project, selectedClipId, editingBlocked, readOnly, copy = {}, run, onClose,
}: Props) {
	const model = useMemo(() => createFramescaperV27VisualInspectorModel({
		project, selectedClipId,
	}), [project, selectedClipId]);
	const [draft, setDraft] = useState<FramescaperV27VisualInspectorDraft>(() => draftFor(model));
	...
	const apply = (event: FormEvent): void => {
		...
		let command: unknown;
		try {
			command = createFramescaperV27VisualInspectorCommand(project, model.clipId, draft);
		} catch (cause) {
	...
	return <AudioEditorDialogShell
		...
		dataAttributes={{ 'data-framescaper-v27-visual-inspector': 'true' }}
	>
					...
					<select data-visual-inspector-blend value={draft.blendMode} disabled={blocked}
						onChange={(event) => {
							const blendMode = event.currentTarget.value as
								FramescaperV27VisualInspectorDraft['blendMode'];
...
function GeneratorFields(props: Readonly<{
		...
		return <p role="alert">{label(props.copy, 'externalGeneratorUnavailable',
			'External generators are unavailable in selected V27.')}</p>;
}

function draftFor(model: ReturnType<typeof createFramescaperV27VisualInspectorModel>): FramescaperV27VisualInspectorDraft {
	return {
