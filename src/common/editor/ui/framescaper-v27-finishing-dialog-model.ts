...
function projectRecord(value: unknown): Record<string, unknown> {
	let input = record(value, 'Framescaper V27, V28, V32, or F31 project');
	...
	if (input.schemaVersion !== 27 && input.schemaVersion !== 28 && input.schemaVersion !== 32) {
		throw new RangeError('The selected Framescaper V27, V28, V32, or F31 project is required.');
	}
	return input.schemaVersion === 32
		? record(framescaperProjectV27FoundationShapeV32(input), 'Framescaper V32 finishing foundation')
