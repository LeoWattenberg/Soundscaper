...
- `audio-editor-design-system.css` is an ordered import manifest. Add rules to
  the matching feature module without reordering imports. Editor component
  selectors are global; website classes and tokens own the `website-` prefix. Keep root-only resets
  and workspace state selectors attached to the editor mount.
  Every maintained CSS module must stay within the 600-line limit.
