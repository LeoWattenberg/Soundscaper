...
- A file directly under a domain is public and must be listed in
  `config/controller-domain-policy.json`. Put implementation used only by
...
- Cross-domain dependency directions are ratcheted as runtime or type-only in the
  same policy. Remove an obsolete direction with
  `npm run check:controller-domains:tighten`; do not add or promote one to runtime.
