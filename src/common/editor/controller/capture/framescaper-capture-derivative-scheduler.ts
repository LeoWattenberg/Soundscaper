		...
		if (project) {
			for (const sourceId of sourceIds) {
				...
				if (source.kind === 'audio') await generateAudioDerivative(options, source, failures);
					...
					await generateVideoDerivatives(options, source, failures);
					await scheduleVideoProxy(options, request, project, source, failures);
				}
			}
		}
		if (failures.length) {
...
async function scheduleVideoProxy(
	options: FramescaperCaptureDerivativeSchedulerOptions,
	request: FramescaperCaptureDerivativeRequest,
	...
	if (!options.scheduleProxy) return;
	try {
		await options.scheduleProxy(Object.freeze({
			projectId: request.projectId,
			sessionId: request.sessionId,
			sourceId: source.id,
			expectedProjectRevision: nonNegativeSafeInteger(
				project.revision,
				'Captured project revision',
			),
			expectedContentSha256: sha256(source.contentSha256, `${source.id} content digest`),
		}));
	} catch (error) {
		failures.push(derivativeError(`${source.id} proxy scheduling`, error));
	}
}
