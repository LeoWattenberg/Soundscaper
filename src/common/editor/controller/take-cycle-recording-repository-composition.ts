...
import {
	abortTakeCycleWriter,
	canonicalTakeCycleChunkBytes,
	writeExactTakeCyclePcm,
} from './take-cycle-recording-pcm-staging.ts';
...
export function createTakeCycleRecordingRepositoryComposition(
	...
	async function stageMedia(operation: TakeCyclePassOperation): Promise<void> {
		...
		try {
			...
			await writeExactTakeCyclePcm(writer, iterable, source, operation.ownership.signal);
		} catch (error) {
			stagedWriters.delete(entry.stageReceipt.sourceToken);
			await abortTakeCycleWriter(writer, error);
		}
	}
	...
	async function storedSourceMatches(
		...
		for await (const chunk of dependencies.sources.chunks(binding.mediaId, { signal, expectedSource: metadata })) {
			...
			const bytes = canonicalTakeCycleChunkBytes(chunk.channels, expectedFrames);
