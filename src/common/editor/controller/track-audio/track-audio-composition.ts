...
import { abortError, normalizeProjectSampleRate, throwIfAborted } from '../shared/app-helpers.ts';
import { createAudioWarpControllerComposition } from './internal/audio-warp/audio-warp-composition.ts';
import { createDeferredEditorExportService } from '../export/deferred-export-service.ts';
import { createDocumentCommandPreview } from '../document/document-command-preview.ts';
import { createDerivedAudioComposition } from './derived-audio-composition.ts';
import { createMixRenderService } from './internal/mix-render/mix-render-service.ts';
import { createSelectionViewService } from './internal/selection-view-service.ts';
import {
...
import { createStreamingStemArchive } from '../export/stem-archive.ts';
import { createTakeCompControllerComposition } from './internal/take-comp/take-comp-composition.ts';
import { createTemporaryFileSink, stemProject } from '../export/temporary-export.ts';
import type { TrackAudioCompositionDependencies, TrackAudioCompositionProject } from './track-audio-composition-types.ts';
import { createTrackActionAdapter } from './internal/track-action-adapter.ts';
import { createEditorTrackService } from './internal/track-service.ts';
...
export function createTrackAudioComposition(dependencies: TrackAudioCompositionDependencies) {
	...
	const track = createEditorTrackService({
		...
		createId: createStableId,
		commit: dependencies.commit,
		getPositionFrames: () => state.transportState === 'recording' ? state.positionFrame : engine.getPositionFrames(),
		getTransportState: () => state.transportState,
