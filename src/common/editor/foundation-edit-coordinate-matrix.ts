...
> = deepFreeze({
	...
	'rate-stretch': {
		primitive: 'rate-stretch',
		audioPlacement: 'Anchor the untouched edge; linked audio takes the absolute endpoints of its participating video companion.',
		audioExtent: 'Point-scale the immutable program-sample extent once from the operation duration scale.',
		audioSourceRange: 'Unchanged.',
		videoPlacement: 'Anchor the untouched integer sequence edge.',
		videoExtent: 'Point-scale the immutable canonical frame count once from the operation duration scale.',
		videoSourceRange: 'Unchanged in source frames and exact source time.',
		operationConformance: 'One video authority conforms the requested edge and supplies one reduced rational duration scale to the complete relation set.',
		implementation: ['frame-canonical-rate-stretch-planner.ts', 'commands/clip-transform-runtime.js'],
	},
	split: {
		primitive: 'split',
