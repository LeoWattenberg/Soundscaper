 ...
 * and refuses the second — measured, not assumed. Nothing here is loaded by a
 * browser: these bytes are staged into the encoder and nowhere else, so the
 * bundle's WOFF2-only font rule is about a different set of files and a sweep
 * that converts every font in the tree breaks caption burn-in silently. The
 * semibold weight is the one that stays legible over arbitrary picture at small
 * sizes.
 *
 ...
 * coverage never pull a font file into their graph.
 */
...
/** Where each retained subset's encoder WOFF lives, by the id the plan states. */
