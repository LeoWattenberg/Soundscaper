...
export function createAddLabelTrackCommand(options: CommandFactoryValue = {}): CommandFor<'track/add'> {
	return { type: 'track/add', track: createLabelTrack(options) as CommandObject };
}
...
export function createAddLabelCommand(
	...
	return { type: 'label/add', trackId, label: createLabel(options) as CommandObject };
}
...
function normalizeClipValue(value: CommandFactoryValue): CommandObject {
	if (value.anchor != null || value.sequenceStartFrame != null || value.kind === 'video') {
		return structuredClone(value) as CommandObject;
	}
	return createAudioClip(value) as CommandObject;
}
