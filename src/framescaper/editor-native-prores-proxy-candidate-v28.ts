...
/** Selected-V28 native ProRes Proxy candidate routed through queue V3. */
...
export interface FramescaperNativeProResProxyCandidateV28Options {
...
async function generate(context: Readonly<{
	...
	const projection = queueProjection(await context.bridge.enqueue!(Object.freeze({
		...
		reservations: FRAMESCAPER_V28_RENDER_QUEUE_RESERVATIONS,
...
function videoSource(project: FramescaperProjectV28, sourceId: string): Readonly<Record<string, unknown>> {
