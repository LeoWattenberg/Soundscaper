...
export function normalizeCleanupOperation(value: unknown): Readonly<CleanupOperationIdentity> {
	const raw = cleanupClosedRecord(value, OPERATION_FIELDS, 'Framescaper V18 claim cleanup operation');
...
export function boundedCleanupInventory(value: unknown): number {
	...
	if (!Number.isSafeInteger(value) || Number(value) < 1) {
		throw new RangeError('The V18 cleanup inventory limit must be a positive safe integer.');
	}
	return Math.min(Number(value), MAX_VIDEO_PROXY_CLAIMS);
}

export function safeCleanupNow(value: unknown): number {
	if (!Number.isSafeInteger(value) || Number(value) < 0) throw new RangeError('The V18 cleanup clock is invalid.');
	return Number(value);
}
...
export class CleanupInventoryError extends Error {
	constructor(options?: unknown) {
		super('The bounded V18 cleanup inventory is invalid', options === undefined ? undefined : { cause: options });
	}
}
