...
export function createFramescaperNativeRenderPlanAuthorityNativeMedia(
	...
	const profile = framescaperNativeMediaNativeDeliveryProfile(delivery);
	...
	if (!sequence) throw new ReferenceError('The selected nativeMedia primary sequence is unavailable.');
	...
	if (endFrame < 1) throw new Error('The selected nativeMedia sequence has no renderable picture duration.');
	...
	const sequenceDescriptor = delivery.kind === 'image-sequence'
		? framescaperNativeMediaImageSequenceDeliveryDescriptor(delivery.format) : null;
...
function sequenceHasAudio(project: FramescaperProjectNativeMedia, sequence: Record<string, unknown>): boolean {
...
function safeAdd(left: number, right: number): number {
	...
	if (!Number.isSafeInteger(result)) throw new RangeError('The selected nativeMedia render range overflows.');
	return result;
}
