...
/** Adapt selected V27 directly from V24 visual lineage into exact V13 authority. */
export function createFramescaperProjectUnifiedExactRenderPlanV27(
	profile: unknown,
...
function createFinishingNode(
	...
	if (colorContexts.length !== 1) throw new ReferenceError('V27 render requires exactly one sequence color context.');
	const audioTracks = records(project.tracks, 'V27 render tracks').filter((track) => track.type === 'audio')
		.map((track) => ({
			id: String(track.id),
			effectIds: effectIds(track.effects),
		})).sort(compareIds);
	const master = record(project.master, 'V27 render master');
	return {
		kind: 'finishing',
		...
		audioContext: {
			...
			masterChannels: positiveInteger(project.masterChannels, 'V27 render master channels'),
...
function effectIds(value: unknown): string[] {
	if (!Array.isArray(value)) throw new TypeError('V27 render audio effects must be an array.');
	return value.map((effect, index) => {
		if (!effect || typeof effect !== 'object' || Array.isArray(effect)
			|| typeof (effect as Readonly<Record<string, unknown>>).id !== 'string') {
			throw new TypeError(`V27 render audio effect ${String(index)} requires an identity.`);
		}
