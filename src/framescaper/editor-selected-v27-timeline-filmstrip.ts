...
import { createFramescaperSelectedVisualPreviewSessionV27 } from './editor-selected-v27-visual-preview.ts';

type Data = Readonly<Record<string, unknown>>;

interface DecodedFilmstripSourceV27 {
...
type DecodeFilmstripSourceV27 = (
	url: string,
	width: number,
	height: number,
	signal: AbortSignal,
) => Promise<DecodedFilmstripSourceV27>;

export interface FramescaperSelectedTimelineFilmstripOptionsV27
	...
	readonly decodeSource?: DecodeFilmstripSourceV27;
}

/** Render existing derivative cells through the selected exact V13 presentation session. */
export async function createFramescaperSelectedTimelineFilmstripV27(
	options: FramescaperSelectedTimelineFilmstripOptionsV27,
	...
	const session = await createFramescaperSelectedVisualPreviewSessionV27({
	...
	if (typeof session.renderExact !== 'function') {
		session.dispose();
		throw new Error('Selected V27 timeline thumbnails require exact preview execution.');
	}
...
function mediaLayer(
	...
	decoded: DecodedFilmstripSourceV27,
): Data {
...
async function decodeFilmstripSource(
	...
	if (typeof globalThis.fetch !== 'function' || typeof globalThis.createImageBitmap !== 'function'
		...
		throw new Error('Selected V27 timeline thumbnail decode requires a browser image runtime.');
	}
	...
	try {
		...
		if (!context) throw new Error('Selected V27 timeline thumbnail decode has no 2D context.');
