...
import { snapshotFramescaperOwnedFinishingCommandV27 } from './editor-project-v27-finishing-command.ts';

export type FramescaperCubeLutTargetKindV27 = 'presentation' | 'preset';

export interface FramescaperCubeLutTargetV27 {
	readonly kind: FramescaperCubeLutTargetKindV27;
	readonly id: string;
	readonly label: string;
}

export interface FramescaperCubeLutActionsV27 {
	targets(): readonly FramescaperCubeLutTargetV27[];
	importCubeLut(request: Readonly<{
		readonly target: Readonly<{ readonly kind: FramescaperCubeLutTargetKindV27; readonly id: string }>;
		readonly file: Blob;
		readonly signal?: AbortSignal;
	}>): Promise<VideoCubeLutReferenceV1>;
}

interface CubeLutOwnerV27 {
...
const RUNTIMES = new WeakMap<object, FramescaperCubeLutActionsV27>();
const UTF8 = new TextDecoder('utf-8', { fatal: true });

export function createFramescaperCubeLutActionsV27(options: Readonly<{
	readonly owner: CubeLutOwnerV27;
	readonly store: CubeLutStoreV27;
}>): FramescaperCubeLutActionsV27 {
	assertOptions(options);
	let active = false;
	return Object.freeze({
		targets: () => targets(options.owner.project),
		importCubeLut: async (request: Parameters<FramescaperCubeLutActionsV27['importCubeLut']>[0]) => {
			if (active) throw new Error('A selected V27 cube LUT import is already running.');
			active = true;
...
export function bindFramescaperCubeLutActionsV27(
	owner: object,
	runtime: FramescaperCubeLutActionsV27,
): void {
	if (!owner || typeof owner !== 'object') throw new TypeError('A V27 cube LUT owner is required.');
	RUNTIMES.set(owner, runtime);
}

export function framescaperCubeLutActionsV27For(owner: unknown): FramescaperCubeLutActionsV27 | null {
	return owner && (typeof owner === 'object' || typeof owner === 'function')
		? RUNTIMES.get(owner as object) ?? null : null;
}

async function importCubeLut(
	options: Readonly<{ readonly owner: CubeLutOwnerV27; readonly store: CubeLutStoreV27 }>,
	request: Parameters<FramescaperCubeLutActionsV27['importCubeLut']>[0],
): Promise<VideoCubeLutReferenceV1> {
...
function targetSnapshot(
	...
	target: Readonly<{ readonly kind: FramescaperCubeLutTargetKindV27; readonly id: string }>,
): TargetSnapshot {
	const project = projectRecord(projectValue);
	const id = stableId(target?.id, 'cube LUT target ID');
	if (target?.kind === 'presentation') {
		const value = records(project.videoVisualPresentations, 'V27 visual presentations')
			.find((item) => item.id === id);
		if (!value) throw new ReferenceError(`V27 visual presentation ${id} is unavailable.`);
		return Object.freeze({ kind: 'presentation', value: normalizeVideoVisualPresentationV1(value) });
	}
	if (target?.kind === 'preset') {
		const value = records(project.videoFinishingPresets, 'V27 finishing presets')
			.find((item) => item.id === id);
		if (!value) throw new ReferenceError(`V27 finishing preset ${id} is unavailable.`);
		return Object.freeze({ kind: 'preset', value: normalizeVideoFinishingPresetV1(value) });
	}
	throw new RangeError('The V27 cube LUT target kind is unsupported.');
}

function targets(projectValue: unknown): readonly FramescaperCubeLutTargetV27[] {
	const project = projectRecord(projectValue);
	return Object.freeze([
		...records(project.videoVisualPresentations, 'V27 visual presentations').map((value) => {
		...
		...records(project.videoFinishingPresets, 'V27 finishing presets').map((value) => {
...
function assertCurrent(project: unknown, expected: TargetSnapshot): void {
	...
	if (!same(current.value, expected.value)) {
		throw new Error('The selected V27 cube LUT target changed before publication.');
	}
}
...
function assertReplacement(project: unknown, replacement: TargetSnapshot): void {
	...
	if (!same(current.value, replacement.value)) {
		throw new Error('The selected V27 cube LUT history command did not publish its exact target.');
	}
}
...
async function rollbackBody(store: CubeLutStoreV27, key: string, error: unknown): Promise<void> {
	try {
...
function projectRecord(value: unknown): Readonly<Record<string, unknown>> {
	...
	if (project.schemaVersion !== 27 && project.schemaVersion !== 28 && project.schemaVersion !== 30) {
		throw new RangeError('Cube LUT import requires selected schema V27, V28, or V30.');
	}
	return project;
}
...
function assertOptions(options: Readonly<{ readonly owner: CubeLutOwnerV27; readonly store: CubeLutStoreV27 }>): void {
	if (!options.owner || typeof options.owner !== 'object'
		|| typeof options.owner.actions?.edit?.commit !== 'function') {
		throw new TypeError('Selected V27 cube LUT import requires a controller owner.');
	}
	if (!options.store || typeof options.store.getMediaAssetMetadata !== 'function'
		...
		throw new TypeError('Selected V27 cube LUT import requires an exact asset store.');
	}
}
