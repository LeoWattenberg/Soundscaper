...
/** Bind the exact V18 command spellings without exposing a generic product command port. */
export function createFramescaperSequenceActionsV18(
	executeValue: ((command: FramescaperProjectCommandV18) => unknown) | unknown,
): Readonly<FramescaperSequenceActionsV18> {
...
function subsequence(value: unknown): FramescaperSubsequenceV18 {
	...
	return result as unknown as FramescaperSubsequenceV18;
}

function changes(value: unknown): FramescaperSubsequenceChangesV18 {
	const result = exactFields(value, [
	...
	return result as FramescaperSubsequenceChangesV18;
}
