...
export const FRAMESCAPER_DESKTOP_V12_MAXIMUM_BODY_CHUNK_BYTES = 4 * 1024 * 1024;
...
export interface FramescaperDesktopV12BodyDescriptor {
...
export interface FramescaperDesktopV12BodyStore {
...
export interface FramescaperDesktopV12BodyBridge {
	readBodyChunk(request: Readonly<{
		body: Readonly<FramescaperDesktopV12BodyDescriptor>; offset: number; length: number;
	}>): Promise<unknown>;
...
export interface FramescaperDesktopV12PreparedBody {
	readonly descriptor: Readonly<FramescaperDesktopV12BodyDescriptor>;
	readonly blob: Blob;
	readonly metadataIdentity: string;
}

interface BodyReference {
	readonly kind: FramescaperDesktopV12BodyDescriptor['kind'];
	readonly encoding: FramescaperDesktopV12BodyDescriptor['encoding'];
	readonly storageKey: string;
	readonly mimeType: string;
...
/** Preflight every retained body and produce the only descriptors admitted to V12 main. */
export async function prepareFramescaperDesktopV12PublicationBodies(
	project: FramescaperProjectV20,
	projectSha256: string,
	store: FramescaperDesktopV12BodyStore,
	signal?: AbortSignal,
): Promise<readonly Readonly<FramescaperDesktopV12PreparedBody>[]> {
	const prepared: FramescaperDesktopV12PreparedBody[] = [];
	...
	for (const reference of bodyReferences(project, projectSha256)) {
		...
		if (metadataValue == null) throw new Error(`Managed V12 ${reference.kind} body ${reference.storageKey} is missing.`);
		...
		if (metadataIdentity(current) !== metadataIdentity(metadata)) {
			throw new Error(`Managed V12 ${reference.kind} body ${reference.storageKey} changed during preflight.`);
		}
...
export async function uploadFramescaperDesktopV12PublicationBodies(
	publicationId: string,
	prepared: readonly Readonly<FramescaperDesktopV12PreparedBody>[],
	bridge: Pick<FramescaperDesktopV12BodyBridge, 'writePublicationChunk'>,
	...
	for (const [bodyIndex, body] of prepared.entries()) {
		...
		for (let offset = 0; offset < body.descriptor.byteLength;) {
			...
			const nextOffset = Math.min(
				...
				offset + FRAMESCAPER_DESKTOP_V12_MAXIMUM_BODY_CHUNK_BYTES,
			);
			...
			if (!(buffer instanceof ArrayBuffer) || buffer.byteLength !== nextOffset - offset) {
				throw new Error(`Managed V12 ${body.descriptor.kind} body emitted an inexact chunk.`);
			}
			...
			const acknowledgement = closedRecord(
				...
				'Framescaper V12 body acknowledgement',
			);
			if (acknowledgement.bodyIndex !== bodyIndex || acknowledgement.nextOffset !== nextOffset
				...
				throw new Error('Framescaper V12 body acknowledgement changed its sequential write.');
			}
			offset = nextOffset;
		}
		if (bytesToHex(digest.digest()) !== body.descriptor.sha256) {
			throw new Error(`Managed V12 ${body.descriptor.kind} body changed during upload.`);
		}
		...
		if (metadataIdentity(trustedMetadata(descriptorReference(body.descriptor), current))
			...
			throw new Error(`Managed V12 ${body.descriptor.kind} metadata changed during upload.`);
		}
	}
}

/** Validate one bundle inventory solely against its exact V20 document and project digest. */
export function validateFramescaperDesktopV12Bodies(
	project: FramescaperProjectV20,
	projectSha256: string,
	value: unknown,
): readonly Readonly<FramescaperDesktopV12BodyDescriptor>[] {
	const supplied = denseArray(value, 'Framescaper desktop V12 bodies').map(bodyDescriptor);
	const byKey = new Map<string, Readonly<FramescaperDesktopV12BodyDescriptor>>();
	for (const body of supplied) {
		const key = bodyKey(body);
		if (byKey.has(key)) throw new Error(`Framescaper desktop V12 body ${body.storageKey} is duplicated.`);
		byKey.set(key, body);
	}
	const expected: Readonly<FramescaperDesktopV12BodyDescriptor>[] = [];
	for (const reference of bodyReferences(project, projectSha256)) {
		const body = byKey.get(bodyKey(reference));
		if (!body) {
			if (reference.required) throw new Error(`Framescaper desktop V12 ${reference.kind} body is missing.`);
			continue;
		}
	...
	if (byKey.size) throw new Error('Framescaper desktop V12 body inventory contains an unbound body.');
	if (JSON.stringify(expected) !== JSON.stringify(supplied)) {
		throw new Error('Framescaper desktop V12 body inventory order changed.');
	}
	return Object.freeze(expected);
}

/** Verify every desktop body and atomically retain only missing local media. */
export async function acquireFramescaperDesktopV12Bodies(
	project: FramescaperProjectV20,
	projectSha256: string,
	bodiesValue: unknown,
	bridge: Pick<FramescaperDesktopV12BodyBridge, 'readBodyChunk'>,
	store: FramescaperDesktopV12BodyStore,
	signal?: AbortSignal,
): Promise<void> {
	const bodies = validateFramescaperDesktopV12Bodies(project, projectSha256, bodiesValue);
	...
	try {
		for (const body of bodies) {
			...
			for (let offset = 0; offset < body.byteLength;) {
				throwIfScapeAborted(signal);
				const length = Math.min(FRAMESCAPER_DESKTOP_V12_MAXIMUM_BODY_CHUNK_BYTES, body.byteLength - offset);
				const value = await bridge.readBodyChunk({ body, offset, length });
				if (!(value instanceof Uint8Array) || value.byteLength !== length) {
					throw new Error('Framescaper desktop V12 body read returned an inexact chunk.');
				}
			...
			if (bytesToHex(digest.digest()) !== body.sha256) {
				throw new Error(`Framescaper desktop V12 ${body.kind} body failed its SHA-256 binding.`);
			}
...
function bodyReferences(project: FramescaperProjectV20, projectSha256: string): readonly BodyReference[] {
	digest(projectSha256, 'project');
	const references: BodyReference[] = [];
	const identityByKey = new Map<string, string>();
	for (const raw of project.sources) {
...
function appendReference(
	...
	if (prior) return;
	if (references.length >= MAXIMUM_BODIES) throw new RangeError('Framescaper desktop V12 body limit exceeded.');
...
function trustedMetadata(reference: BodyReference, value: unknown, exactBytes = reference.byteLength): TrustedMetadata {
	if (!value || typeof value !== 'object' || Array.isArray(value)) {
		throw new Error(`Managed V12 ${reference.kind} metadata is missing.`);
	}
	...
	if (metadata.sourceId !== reference.storageKey || metadata.mimeType !== reference.mimeType
		...
		throw new Error(`Managed V12 ${reference.kind} metadata conflicts with its project binding.`);
	}
	return metadata;
}
...
async function loadVerifiedBlob(
	...
	if (blob.size !== metadata.size || await digestMediaContent(blob, { signal }) !== metadata.sha256) {
		throw new Error(`Managed V12 ${reference.kind} body failed immutable verification.`);
	}
	if (reference.timing) {
...
function descriptorFor(reference: BodyReference, byteLength: number): Readonly<FramescaperDesktopV12BodyDescriptor> {
	if (reference.sha256 === null) throw new Error('A project-bound V12 body digest is required.');
	return Object.freeze({
...
function bodyDescriptor(value: unknown): Readonly<FramescaperDesktopV12BodyDescriptor> {
	...
	if (kind !== 'video-original' && kind !== 'video-proxy' && kind !== 'video-timing') {
		throw new TypeError('Framescaper desktop V12 body kind is unsupported.');
	}
	...
	if ((kind === 'video-original' && encoding !== 'framescaper-video-original-v1')
		...
		|| (kind === 'video-timing' && encoding !== VIDEO_TIMING_ASSET_ENCODING)) {
		throw new TypeError('Framescaper desktop V12 body encoding is unsupported.');
	}
	const result = {
		kind, encoding,
	...
	} as FramescaperDesktopV12BodyDescriptor;
	if (result.sourceId !== result.storageKey || (kind === 'video-proxy' && !PROXY_BINDING.test(result.bindingId!))) {
		throw new TypeError('Framescaper desktop V12 body identity is invalid.');
	}
	return Object.freeze(result);
}

function assertDescriptorMatches(reference: BodyReference, body: FramescaperDesktopV12BodyDescriptor): void {
	if (reference.sha256 === null || body.encoding !== reference.encoding || body.mimeType !== reference.mimeType
		...
		throw new Error(`Framescaper desktop V12 ${reference.kind} descriptor changed its project binding.`);
	}
}

function descriptorReference(body: FramescaperDesktopV12BodyDescriptor): BodyReference {
	return {
		...body, name: body.storageKey, required: true, timing: null,
	};
}

function assertPublication(publication: OwnedMediaAssetPublication, body: FramescaperDesktopV12BodyDescriptor): void {
	if (!publication || typeof publication !== 'object' || typeof publication.discardIfCurrent !== 'function'
		...
		throw new Error(`Managed V12 ${body.kind} publication changed its descriptor.`);
	}
}
...
function concatenate(chunks: readonly Uint8Array[], byteLength: number): Uint8Array {
	...
	if (offset !== byteLength) throw new Error('Framescaper desktop V12 timing body ended early.');
	return output;
}
...
function addBodyBytes(total: number, value: number): number {
	if (value > SCAPE_ARCHIVE_LIMITS.maximumExpandedBytes - total) {
		throw new RangeError('Framescaper desktop V12 managed bodies exceed their aggregate byte limit.');
	}
	return total + value;
}
