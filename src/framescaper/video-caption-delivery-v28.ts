...
/** Product-owned V28 adapter from caption tracks to exact delivery artifacts. */
...
export type FramescaperCaptionSidecarFormatV28 = 'srt' | 'vtt' | 'imsc1';

export interface FramescaperCaptionDeliveryRequestV28 {
	readonly trackId: string;
	readonly mux: boolean;
	readonly burnIn: boolean;
	readonly sidecar: FramescaperCaptionSidecarFormatV28 | null;
}

export interface FramescaperCaptionDeliveryContextV28 {
...
export interface FramescaperCaptionDeliveryAvailabilityV28 {
	readonly muxCodec: 'mov_text' | 'webvtt' | null;
	readonly burnIn: boolean;
	readonly burnInRefusal: string | null;
}

export interface FramescaperCaptionDeliveryDocumentV28
	extends VideoCaptionExportResultV1 {
	readonly sha256: string;
}

export interface FramescaperCaptionBurnPlanV28 {
...
export interface FramescaperCaptionDeliveryAdapterV28 {
	readonly track: VideoCaptionTrackV1;
	readonly delivery: UnifiedExactRenderCaptionDeliveryV15;
	readonly muxDocument: FramescaperCaptionDeliveryDocumentV28 | null;
	readonly sidecarDocument: FramescaperCaptionDeliveryDocumentV28 | null;
	readonly burnInStage: VideoBurnInStage | null;
	readonly burnInPlan: FramescaperCaptionBurnPlanV28 | null;
...
export function framescaperCaptionDeliveryAvailabilityV28(
...
/**
 * Adapt one exact V28 caption track into the staged documents and V15 authority.
 *
 ...
 */
export function createFramescaperCaptionDeliveryAdapterV28(
	trackValue: unknown,
	requestValue: unknown,
	contextValue: FramescaperCaptionDeliveryContextV28,
): FramescaperCaptionDeliveryAdapterV28 {
	const sourceTrack = normalizeVideoCaptionTrackV1(trackValue);
	const request = snapshotFramescaperCaptionDeliveryRequestV28(requestValue);
	const context = snapshotContext(contextValue);
	if (request.trackId !== sourceTrack.id) {
		throw new ReferenceError('The selected V28 caption request track does not match its exact track authority.');
	}
	if (!request.mux && !request.burnIn && request.sidecar === null) {
		throw new RangeError('A V28 caption delivery must select mux, burn-in, or a sidecar.');
	}
	...
	const availability = framescaperCaptionDeliveryAvailabilityV28(context.profileId);
	...
	if (request.burnIn && burnInStage === null) {
		throw new RangeError('The selected V28 caption track has no drawable cues in the delivered range.');
	}
...
function deliveredTrack(
	...
	range: FramescaperCaptionDeliveryContextV28['range'],
): VideoCaptionTrackV1 {
...
function document(
	...
	sampleRate: number,
): FramescaperCaptionDeliveryDocumentV28 {
	const result = exportVideoCaptionTrackV1(track, { format, sampleRate });
	return Object.freeze({ ...result, sha256: digestText(result.text) });
}

export function snapshotFramescaperCaptionDeliveryRequestV28(
	value: unknown,
): FramescaperCaptionDeliveryRequestV28 {
	const row = closedRecord(value, ['trackId', 'mux', 'burnIn', 'sidecar'], 'V28 caption delivery request');
	if (typeof row.trackId !== 'string' || row.trackId.length === 0) {
		throw new TypeError('V28 caption delivery request.trackId must be a non-empty string.');
	}
	if (typeof row.mux !== 'boolean' || typeof row.burnIn !== 'boolean') {
		throw new TypeError('V28 caption delivery request mux and burnIn must be booleans.');
	}
	if (row.sidecar !== null && row.sidecar !== 'srt'
		&& row.sidecar !== 'vtt' && row.sidecar !== 'imsc1') {
		throw new RangeError('V28 caption sidecar format is unsupported.');
	}
	return Object.freeze({
		trackId: row.trackId,
		mux: row.mux,
		burnIn: row.burnIn,
		sidecar: row.sidecar,
	}) as FramescaperCaptionDeliveryRequestV28;
}

function snapshotContext(
	value: FramescaperCaptionDeliveryContextV28,
): FramescaperCaptionDeliveryContextV28 {
	const row = closedRecord(
		value, ['profileId', 'sampleRate', 'range', 'canvas'], 'V28 caption delivery context',
	);
	...
	if (!Number.isSafeInteger(row.sampleRate) || Number(row.sampleRate) < 1) {
		throw new RangeError('V28 caption delivery sampleRate must be positive.');
	}
	const range = closedRecord(row.range, ['startFrame', 'endFrame'], 'V28 caption delivery range');
	if (!Number.isSafeInteger(range.startFrame) || Number(range.startFrame) < 0
		...
		throw new RangeError('V28 caption delivery range must be a non-empty safe frame range.');
	}
	const canvas = closedRecord(row.canvas, ['width', 'height'], 'V28 caption delivery canvas');
	if (!Number.isSafeInteger(canvas.width) || Number(canvas.width) < 1
		|| !Number.isSafeInteger(canvas.height) || Number(canvas.height) < 1) {
		throw new RangeError('V28 caption delivery canvas must have positive safe extents.');
	}
	return Object.freeze({
