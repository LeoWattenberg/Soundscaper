...
/** Closed menu-authoring model for every OpenFX Image Effect context in selected nativeMedia. */
...
export interface FramescaperOpenFxAuthoringTargetNativeMedia {
...
export interface FramescaperOpenFxAuthoringModelNativeMedia {
	readonly plugins: readonly FramescaperOpenFxPluginProjectionV1[];
	readonly targets: readonly FramescaperOpenFxAuthoringTargetNativeMedia[];
}

export interface FramescaperOpenFxAuthoringRequestNativeMedia {
...
export type FramescaperOpenFxAuthoringDraftNativeMedia = Omit<
	OfxEffectStateV26,
	'freshness' | 'frozenFallback'
>;

export function createFramescaperOpenFxAuthoringModelNativeMedia(
	...
	const push = (target: FramescaperOpenFxAuthoringTargetNativeMedia): void => {
		...
		if (targets.some(({ context, targetId }) => context === target.context && targetId === target.targetId)) {
			throw new RangeError('nativeMedia OpenFX authoring targets are ambiguous.');
		}
		targets.push(deepFreeze(structuredClone(target)));
	};
	for (const source of sources) {
		if (source.kind !== 'generator') continue;
		const generator = record(source.generator, 'nativeMedia external generator');
	...
	for (const layer of records(project.videoAdjustmentLayers, 'nativeMedia OpenFX adjustment layers')) {
	...
	for (const track of records(project.tracks, 'nativeMedia OpenFX tracks')) {
		for (const transition of recordsOrEmpty(track.videoTransitions, 'nativeMedia OpenFX transitions')) {
...
export function createFramescaperOpenFxAuthoringDraftNativeMedia(
	...
	mintId: () => string,
): FramescaperOpenFxAuthoringDraftNativeMedia {
	...
	const draft: FramescaperOpenFxAuthoringDraftNativeMedia = {
		schemaVersion: 1,
		instanceId,
...
export function defaultFramescaperOpenFxParameterStateNativeMedia(
