...
export interface FramescaperImageClipSetCommandTimelineImage {
	readonly type: 'image-clip/set';
	readonly clipId: string;
	readonly expectedClip: FramescaperImageClipV1 | null;
	readonly expectedPlacement: FramescaperImageClipPlacementTimelineImage | null;
	readonly clip: FramescaperImageClipV1 | null;
	readonly placement: FramescaperImageClipPlacementTimelineImage | null;
}

export type FramescaperImageCommandTimelineImage =
	| FramescaperImageSourceSetCommandTimelineImage
	| FramescaperImageClipSetCommandTimelineImage;

const ID = /^[A-Za-z0-9][A-Za-z0-9._:-]{0,127}$/u;

export function snapshotFramescaperImageCommandTimelineImage(value: unknown): FramescaperImageCommandTimelineImage {
	...
	throw new RangeError('Framescaper timelineImage image command type is unsupported.');
}

export function applyFramescaperImageCommandTimelineImage(
	project: Record<string, unknown>,
	command: FramescaperImageCommandTimelineImage,
): void {
	if (command.type === 'image-source/set') applySource(project, command);
	else applyClip(project, command);
}

function sourceCommand(value: unknown): FramescaperImageSourceSetCommandTimelineImage {
	const command = readClosedDomainRecord(value, 'Framescaper timelineImage image source command', [
...
function clipCommand(value: unknown): FramescaperImageClipSetCommandTimelineImage {
	const command = readClosedDomainRecord(value, 'Framescaper timelineImage image clip command', [
	...
	if ((expectedClip === null) !== (expectedPlacement === null)
		...
		throw new RangeError('A timelineImage image clip and its placement are present or absent together.');
	}
...
function applySource(project: Record<string, unknown>, command: FramescaperImageSourceSetCommandTimelineImage): void {
...
function applyClip(project: Record<string, unknown>, command: FramescaperImageClipSetCommandTimelineImage): void {
	...
	if (!same(current, command.expectedClip) || !same(currentPlacement, command.expectedPlacement)) {
		throw new Error('The expected timelineImage image clip or placement is stale.');
	}
...
function videoTrack(tracks: readonly Record<string, unknown>[], trackId: string): Record<string, unknown> {
	...
	if (!target || target.type !== 'video') {
		throw new ReferenceError('A timelineImage image clip requires a video track.');
	}
	assertUnlocked(target, 'target');
	if (!Array.isArray(target.clipIds)) {
		throw new TypeError('A timelineImage image clip target track needs a clip list.');
	}
	return target;
}
...
function assertMutation(before: unknown, after: unknown, name: string): void {
	if (before === null && after === null) throw new RangeError(`A timelineImage ${name} command must mutate state.`);
}
...
function assertIdentity(id: string, before: unknown, after: unknown, name: string): void {
	for (const value of [before, after]) {
		if (value !== null && (value as Readonly<{ id: string }>).id !== id) {
			throw new RangeError(`A timelineImage ${name} command cannot change identity.`);
		}
	}
}

function commandType(value: unknown): string {
	const command = record(value, 'Framescaper timelineImage image command');
	const type = readClosedDomainField(command, 'type', 'Framescaper timelineImage image command');
	if (typeof type !== 'string') throw new TypeError('Framescaper timelineImage image command.type must be a string.');
	return type;
}

function field(recordValue: Readonly<Record<string, unknown>>, name: string): unknown {
	return readClosedDomainField(recordValue, name, 'Framescaper timelineImage image command');
}

function stableId(value: unknown, name: string): string {
	if (typeof value !== 'string' || !ID.test(value)) throw new TypeError(`Framescaper timelineImage ${name} must be a stable ID.`);
	return value;
}

function assertUnlocked(track: Record<string, unknown> | undefined, name: string): void {
	if (track?.locked === true) throw new Error(`The timelineImage image clip ${name} track is locked.`);
}
