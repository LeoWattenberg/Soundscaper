...
import {
	validateFramescaperProjectFeatureRequirementsV22,
	framescaperProjectFeatureRequirementsForV20FoundationV22,
...
export interface FramescaperVideoTrackV22 extends Readonly<Record<string, unknown>> {
	readonly id: string;
	readonly type: 'video';
...
export interface FramescaperNonVideoTrackV22 extends Readonly<Record<string, unknown>> {
	readonly id: string;
	readonly type: 'audio' | 'label';
	readonly videoTransitions?: never;
}

export interface FramescaperProjectV22 extends Record<string, unknown> {
	readonly id: string;
	...
	readonly tracks: readonly (FramescaperVideoTrackV22 | FramescaperNonVideoTrackV22)[];
...
export function validateFramescaperProjectV22(
	profile: unknown,
	project: unknown,
): project is FramescaperProjectV22 {
	...
	if (data(candidate, 'schemaVersion') !== FRAMESCAPER_PROJECT_V22_SCHEMA_VERSION) {
	...
	validateFramescaperProjectV20(
		...
		framescaperProjectV20FoundationV22(profile, candidate),
	);
	validateTransitionOwnership(candidate);
	validateFramescaperProjectFeatureRequirementsV22(profile, candidate);
	return true;
}

/** Build a detached transient exact-V20 document with no transition backdoor. */
export function framescaperProjectV20FoundationV22(
	profile: unknown,
	project: unknown,
	...
	result.featureRequirements = framescaperProjectFeatureRequirementsForV20FoundationV22(
		profile,
		candidate,
	);
	for (const track of records(result.tracks, 'tracks')) delete track.videoTransitions;
	return result as unknown as FramescaperProjectV20;
}

/** Return every canonical proper overlap, independent of persisted transition objects. */
export function framescaperProjectProperOverlapsV22(project: unknown): readonly ProperOverlap[] {
	const candidate = record(project, 'Framescaper V22 project');
