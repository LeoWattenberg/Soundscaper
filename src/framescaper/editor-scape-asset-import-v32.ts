...
import {
	FRAMESCAPER_SCAPE_IMAGE_ASSET_ENCODING_V32,
	collectFramescaperScapeImageAssetReferencesV32,
	validateFramescaperImageAssetBodyV32,
	validateFramescaperScapeImageImportAssetsV32,
	type FramescaperScapeImageAssetReferenceV32,
	type FramescaperScapeImageImportValidationV32,
...
export interface FramescaperScapeImportValidationV32 {
	readonly foundation: Readonly<FramescaperScapeImportValidationV27>;
	readonly images: Readonly<FramescaperScapeImageImportValidationV32>;
}

/** Stage V27 durable state, then every V32 image body under its rebound source identity. */
export async function stageFramescaperScapeImportAssetsV32(
	request: Readonly<ScapeProjectAssetExtensionImportRequest>,
): Promise<void> {
	const supplied = importValidation(request.validation);
	const archiveProject = request.archiveProject as unknown as FramescaperProjectV32;
	const images = validateFramescaperScapeImageImportAssetsV32(archiveProject, request.manifest);
	...
	for (const reference of images.references) {
		...
		if (!entry) throw new Error(`The V32 Scape archive is missing ${descriptor.entry}.`);
	...
	try {
		...
		try {
			...
			throw aggregateScapeErrors(
				error, [cleanupError], 'The V32 Scape image write and cleanup both failed.',
			);
		}
		throw error;
	}
...
async function verifyStoredImageBody(
	...
	if (typeof request.store.loadMediaAsset !== 'function') {
		throw new TypeError('The V32 Scape import requires immutable image-body reads.');
	}
	const body = canonicalMediaContentBlob(await request.store.loadMediaAsset(
		source.storageKey, signalOptions(request.signal),
	));
	await validateFramescaperImageAssetBodyV32(source, body, request.signal);
}
...
function reboundImageSource(
	...
	const sourceValue = records(project.sources, 'rebound V32 sources').find(({ id }) => id === targetId);
	if (!sourceValue || sourceValue.kind !== 'image') {
		throw new Error(`The V32 image body ${reference.sourceId} lost its rebound source.`);
	}
	...
	if (JSON.stringify(originalIdentity) !== JSON.stringify(reference.source)) {
		throw new Error(`The rebound V32 image ${targetId} changed immutable content authority.`);
	}
	return source;
}

function assertStoredImageMetadata(value: unknown, source: FramescaperImageSourceV1): void {
	const metadata = record(value, `stored V32 image ${source.storageKey} metadata`);
	if (metadata.sourceId !== source.storageKey || metadata.size !== source.assetByteLength
		...
		|| (metadata.encoding !== undefined && metadata.encoding !== ''
			&& metadata.encoding !== FRAMESCAPER_SCAPE_IMAGE_ASSET_ENCODING_V32)) {
		throw new Error(`Stored V32 image ${source.storageKey} conflicts with immutable authority.`);
	}
}

function importValidation(value: unknown): FramescaperScapeImportValidationV32 {
	const candidate = record(value, 'V32 Scape import validation');
	if (Reflect.ownKeys(candidate).length !== 2
		|| !Object.hasOwn(candidate, 'foundation') || !Object.hasOwn(candidate, 'images')) {
		throw new TypeError('The exact V32 Scape import validation is required.');
	}
	const foundation = record(candidate.foundation, 'V32 Scape foundation validation');
	const images = record(candidate.images, 'V32 Scape image validation');
	if (!Array.isArray(foundation.references) || !(foundation.descriptorByArchiveId instanceof Map)
		...
		throw new TypeError('The V32 Scape import validation is incomplete.');
	}
	return candidate as unknown as FramescaperScapeImportValidationV32;
}

function assertImageValidation(
	supplied: Readonly<FramescaperScapeImageImportValidationV32>,
	expected: Readonly<FramescaperScapeImageImportValidationV32>,
): void {
	if (JSON.stringify(supplied.references) !== JSON.stringify(expected.references)
		...
		throw new Error('The V32 Scape image validation drifted before staging.');
	}
	for (const [archiveId, descriptor] of expected.descriptorByArchiveId) {
		if (JSON.stringify(supplied.descriptorByArchiveId.get(archiveId)) !== JSON.stringify(descriptor)) {
			throw new Error(`The V32 Scape descriptor ${archiveId} drifted before staging.`);
		}
	}
	const archiveReferences = collectFramescaperScapeImageAssetReferencesV32(
		{ sources: expected.references.map(({ source }) => source) } as unknown as FramescaperProjectV32,
	);
	if (JSON.stringify(archiveReferences) !== JSON.stringify(expected.references)) {
		throw new Error('The V32 Scape image validation no longer closes over its sources.');
	}
}
...
function assertOwnedWriter(value: unknown): asserts value is OwnedMediaAssetWriter {
	...
	if (writer?.maximumChunkBytes !== SCAPE_VIDEO_MAXIMUM_CHUNK_BYTES
		...
		throw new TypeError('The V32 Scape import requires an exact bounded owned media writer.');
	}
}
