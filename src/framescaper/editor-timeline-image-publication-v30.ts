...
export const FRAMESCAPER_TIMELINE_IMAGE_BODY_KIND_V30 = 'timeline-image-asset' as const;
...
export interface FramescaperTimelineImagePublicationV30
	extends FramescaperTimelineImagePublicationTransitionV30 {
	readonly bytes: Uint8Array;
	readonly signal?: AbortSignal;
}

export interface FramescaperTimelineImagePublicationStoreV30 {
...
/** Low-level CAS that roots one already-verified staged body with its exact V30 revision. */
...
export class FramescaperTimelineImagePublisherV30 {
	readonly #profile: unknown;
	...
	async publishIfCurrent(
		...
		const requestValue = closedRecord(
			...
			'V30 timeline-image publication request',
			['signal'],
		);
		...
		try {
			writer = await this.#store.beginMediaAssetWrite(
				publication.source.storageKey,
				{
					...
					kind: FRAMESCAPER_TIMELINE_IMAGE_BODY_KIND_V30,
					encoding: FRAMESCAPER_TIMELINE_IMAGE_BODY_ENCODING_V30,
			...
			try {
				...
				throw new AggregateError(
					[error, cleanupError],
					'V30 timeline-image publication and owned-body rollback both failed.',
				);
			}
			throw error;
		}
	...
	async #cleanupOwnedBody(source: FramescaperImageSourceV1): Promise<void> {
		...
		if (metadata.sha256 !== source.contentSha256
			...
			|| metadata.kind !== FRAMESCAPER_TIMELINE_IMAGE_BODY_KIND_V30
			|| metadata.encoding !== FRAMESCAPER_TIMELINE_IMAGE_BODY_ENCODING_V30
			|| !Object.hasOwn(metadata, 'pendingProjectUntil')) {
			throw new Error('The staged V30 image body lost cleanup ownership.');
		}
...
function preparePublication(profile: unknown, value: unknown): PreparedPublication {
	...
	if (project.revision !== nextRevision) {
		throw new Error('V30 image publication must publish exactly the next revision.');
	}
	assertFreshTimestamp(expected.updatedAt, project.updatedAt);
	const expectedSourceIds = new Set(records(expected.sources, 'V30 base sources').map(stableId));
	const addedSources = records(project.sources, 'V30 target sources')
		.filter((source) => source.kind === 'image' && !expectedSourceIds.has(stableId(source)));
	if (addedSources.length !== 1) throw new Error('V30 image publication requires one fresh image source.');
	...
	if (addedClips.length !== 1) throw new Error('V30 image publication requires one fresh image clip.');
	...
	if (!sameProject(reconstructed, project)) {
		throw new Error('V30 image publication may add only one source, one clip, and its placement.');
	}
...
function freshTrackCommand(
	expected: FramescaperProjectV30,
	project: FramescaperProjectV30,
	clip: FramescaperImageClipV1,
	placement: ReturnType<typeof imagePlacement>,
): Record<string, unknown> | null {
	const expectedIds = new Set(records(expected.tracks, 'V30 base tracks').map(stableId));
	const targetTracks = records(project.tracks, 'V30 target tracks');
	const added = targetTracks.filter((track) => !expectedIds.has(stableId(track)));
	if (added.length === 0) return null;
	if (added.length !== 1 || placement.scope !== 'timeline') {
		throw new Error('V30 image publication may create at most one timeline video track.');
	}
	const track = added[0]!;
	...
	if (trackId !== placement.trackId || track.type !== 'video' || track.locked === true
		...
		throw new Error('The fresh V30 image track must be unlocked and own only the fresh image clip.');
	}
	const primaryId = String(project.primarySequenceId);
	const owners = records(project.sequences, 'V30 target sequences').filter(({ trackIds }) => (
		Array.isArray(trackIds) && trackIds.includes(trackId)
	));
	if (owners.length !== 1 || owners[0]!.id !== primaryId) {
		throw new Error('The fresh V30 image track must belong only to the primary sequence.');
	}
	return {
		type: 'track/add',
...
async function publishIndexedDb(
	...
	return transact(database, ['projects', 'revisions', 'mediaAssets'], 'readwrite', async ({
		...
		if (await request(revisions.get(nextKey)) !== undefined) {
			throw new Error('The V30 image publication next revision is occupied.');
		}
...
function publishMemory(port: StorageRepositoryPort, publication: PreparedPublication): boolean {
	...
	const nextKey = revisionKey(publication.projectId, publication.nextRevision);
	if (memory.revisions.has(nextKey)) throw new Error('The V30 image publication next revision is occupied.');
...
function stagedBody(value: unknown, source: FramescaperImageSourceV1): StorageRecord {
	const row = record(value, 'V30 staged image body') as StorageRecord;
	...
	if (Date.parse(pendingUntil) <= Date.parse(committedAt)
		...
		|| row.kind !== FRAMESCAPER_TIMELINE_IMAGE_BODY_KIND_V30
		|| row.encoding !== FRAMESCAPER_TIMELINE_IMAGE_BODY_ENCODING_V30
		|| row.mimeType !== source.mimeType
		|| row.size !== source.assetByteLength
		|| trustedMediaContentSha256(row) !== source.contentSha256) {
		throw new Error('The staged V30 image body does not match its immutable source authority.');
	}
	return row;
}

function assertBaseRevision(value: unknown, publication: PreparedPublication): void {
	const row = record(value, 'V30 image base revision');
	...
	if (row.key !== key || row.projectId !== publication.projectId
		...
		throw new Error('The exact V30 image base revision is missing or changed.');
	}
}

function imagePlacement(project: FramescaperProjectV30, clipId: string) {
	const timeline = records(project.clips, 'V30 target timeline clips').some(({ id }) => id === clipId);
	const bin = records(record(project.projectBin, 'V30 target project bin').clips, 'V30 target bin clips')
		.some(({ id }) => id === clipId);
	if (timeline === bin) throw new Error('The fresh V30 image clip requires one exact placement.');
	if (bin) return Object.freeze({ scope: 'project-bin' as const });
	const owners = records(project.tracks, 'V30 target tracks').filter(({ clipIds }) => (
		Array.isArray(clipIds) && clipIds.includes(clipId)
	));
	if (owners.length !== 1 || owners[0]!.type !== 'video') {
		throw new Error('The fresh V30 timeline image requires one video-track owner.');
	}
	return Object.freeze({ scope: 'timeline' as const, trackId: stableId(owners[0]!) });
}

function allClips(project: FramescaperProjectV30): Record<string, unknown>[] {
	return [
		...records(project.clips, 'V30 timeline clips'),
		...records(record(project.projectBin, 'V30 project bin').clips, 'V30 bin clips'),
	];
}
...
function snapshotBytes(value: unknown, expectedBytes: number): Uint8Array {
	if (!(value instanceof Uint8Array) || value.byteLength !== expectedBytes) {
		throw new RangeError('V30 image publication bytes must match the exact asset length.');
	}
	return value.slice();
}
...
function assertPort(value: unknown): asserts value is StorageRepositoryPort {
	if (!value || typeof value !== 'object'
		...
		throw new TypeError('A storage repository port is required for V30 image publication.');
	}
}

function assertStore(value: unknown): asserts value is FramescaperTimelineImagePublicationStoreV30 {
	if (!value || typeof value !== 'object') throw new TypeError('A V30 image publication store is required.');
	for (const method of ['beginMediaAssetWrite', 'getMediaAssetMetadata', 'deleteMediaAsset'] as const) {
		if (typeof (value as Record<string, unknown>)[method] !== 'function') {
			throw new TypeError(`The V30 image publication store requires ${method}.`);
		}
	}
}
...
function assertFreshTimestamp(before: unknown, after: unknown): void {
	...
	if (Date.parse(next) <= Date.parse(base)) {
		throw new Error('V30 image publication requires one fresh updatedAt timestamp.');
	}
}

function canonicalTimestamp(value: unknown, name: string): string {
	if (typeof value !== 'string') throw new TypeError(`V30 image ${name} must be a timestamp.`);
	const date = new Date(value);
	if (Number.isNaN(date.getTime()) || date.toISOString() !== value) {
		throw new TypeError(`V30 image ${name} must be canonical.`);
	}
	return value;
}
...
function revision(value: unknown, name: string): number {
	if (!Number.isSafeInteger(value) || Number(value) < 0) {
		throw new RangeError(`The V30 image ${name} revision must be a non-negative safe integer.`);
	}
	return Number(value);
}

function safeNextRevision(value: number): number {
	if (value === Number.MAX_SAFE_INTEGER) throw new RangeError('The V30 image revision cannot increment safely.');
	return value + 1;
}
...
function optionalSignal(value: unknown): AbortSignal | undefined {
	...
	if (!(value instanceof AbortSignal)) throw new TypeError('V30 image publication signal must be an AbortSignal.');
	return value;
}

function throwIfAborted(signal?: AbortSignal): void {
	if (signal?.aborted) throw signal.reason ?? new DOMException('V30 image publication cancelled.', 'AbortError');
}

function stableId(value: Record<string, unknown>): string {
	if (typeof value.id !== 'string' || !value.id) throw new TypeError('V30 image identity must be non-empty.');
	return value.id;
}
