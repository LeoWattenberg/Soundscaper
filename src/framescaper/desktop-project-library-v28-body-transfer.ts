...
import {
	collectFramescaperDesktopV28ExtensionBodyReferences,
	framescaperDesktopV12BodyProjectV28,
	validateFramescaperDesktopV28Bodies,
	type FramescaperDesktopV28BodyDescriptor,
	type FramescaperDesktopV28ExtensionBodyReference,
...
export type { FramescaperDesktopV28BodyDescriptor } from './desktop-project-library-v28-body-contract.ts';
export { validateFramescaperDesktopV28Bodies } from './desktop-project-library-v28-body-contract.ts';

export interface FramescaperDesktopV28PreparedBody {
	readonly descriptor: Readonly<FramescaperDesktopV28BodyDescriptor>;
	readonly blob: Blob;
	readonly metadataIdentity: string;
}

export interface FramescaperDesktopV28BodyBridge {
	readBodyChunk(request: Readonly<{
		body: Readonly<FramescaperDesktopV28BodyDescriptor>;
		offset: number;
		length: number;
	}>): Promise<unknown>;
...
interface TransferReference {
	...
	readonly extension: Readonly<FramescaperDesktopV28ExtensionBodyReference> | null;
}

/** Preflight the complete selected-V28 body graph before main-process admission. */
export async function prepareFramescaperDesktopV28PublicationBodies(
	project: FramescaperProjectV28,
	projectSha256: string,
	store: FramescaperDesktopV12BodyStore,
	signal?: AbortSignal,
): Promise<readonly Readonly<FramescaperDesktopV28PreparedBody>[]> {
	const base = await prepareFramescaperDesktopV12PublicationBodies(
		framescaperDesktopV12BodyProjectV28(project), projectSha256, store, signal,
	);
	const normalizedBase: FramescaperDesktopV28PreparedBody[] = [];
	for (const item of base) {
		throwIfScapeAborted(signal);
		const descriptor = item.descriptor as Readonly<FramescaperDesktopV28BodyDescriptor>;
	...
	const extension: FramescaperDesktopV28PreparedBody[] = [];
	for (const reference of collectFramescaperDesktopV28ExtensionBodyReferences(project)) {
		...
		if (metadataIdentity(current) !== metadataIdentity(metadata)) {
			throw new Error(`Managed V28 ${descriptor.kind} metadata changed during preflight.`);
		}
		extension.push(Object.freeze({ descriptor, blob, metadataIdentity: metadataIdentity(metadata) }));
	}
	const prepared: readonly Readonly<FramescaperDesktopV28PreparedBody>[] = Object.freeze([
		...normalizedBase, ...extension,
	]);
	validateFramescaperDesktopV28Bodies(project, projectSha256,
...
export async function uploadFramescaperDesktopV28PublicationBodies(
	publicationId: string,
	prepared: readonly Readonly<FramescaperDesktopV28PreparedBody>[],
	bridge: Pick<FramescaperDesktopV28BodyBridge, 'writePublicationChunk'>,
	...
	for (const [bodyIndex, body] of prepared.entries()) {
		...
		for (let offset = 0; offset < body.descriptor.byteLength;) {
			...
			if (!(buffer instanceof ArrayBuffer) || buffer.byteLength !== nextOffset - offset) {
				throw new Error(`Managed V28 ${body.descriptor.kind} emitted an inexact chunk.`);
			}
			...
			if (acknowledgement.bodyIndex !== bodyIndex || acknowledgement.nextOffset !== nextOffset
				...
				throw new Error('Framescaper V28 body acknowledgement changed its sequential write.');
			}
			offset = nextOffset;
		}
		if (bytesToHex(digest.digest()) !== body.descriptor.sha256) {
			throw new Error(`Managed V28 ${body.descriptor.kind} changed during upload.`);
		}
		...
		if (metadataIdentity(current) !== body.metadataIdentity) {
			throw new Error(`Managed V28 ${body.descriptor.kind} metadata changed during upload.`);
		}
	}
}

/** Acquire every missing body as one rollback-capable local publication set. */
export async function acquireFramescaperDesktopV28Bodies(
	project: FramescaperProjectV28,
	projectSha256: string,
	bodiesValue: unknown,
	bridge: Pick<FramescaperDesktopV28BodyBridge, 'readBodyChunk'>,
	store: FramescaperDesktopV12BodyStore,
	signal?: AbortSignal,
): Promise<void> {
	const bodies = validateFramescaperDesktopV28Bodies(project, projectSha256, bodiesValue);
	...
	try {
		for (const body of bodies) {
			...
			for (let offset = 0; offset < body.byteLength;) {
				...
				if (!(value instanceof Uint8Array) || value.byteLength !== length) {
					throw new Error('Framescaper desktop V28 body read returned an inexact chunk.');
				}
			...
			if (bytesToHex(digest.digest()) !== body.sha256) {
				throw new Error(`Framescaper desktop V28 ${body.kind} body failed its SHA-256 binding.`);
			}
		...
		if (cleanup.length) throw new AggregateError([error, ...cleanup], 'V28 body acquisition rollback failed.');
		throw error;
	}
}
...
async function extensionDescriptor(
	...
	store: Pick<FramescaperDesktopV12BodyStore, 'getMediaAssetMetadata'>,
	signal?: AbortSignal,
): Promise<Readonly<FramescaperDesktopV28BodyDescriptor>> {
	...
	if (metadata.size > reference.maximumBytes
		...
		throw new Error(`Managed V28 ${reference.kind} body conflicts with project authority.`);
	}
...
function transferReference(
	descriptor: Readonly<FramescaperDesktopV28BodyDescriptor>,
	project: FramescaperProjectV28,
): Readonly<TransferReference> {
	const extension = collectFramescaperDesktopV28ExtensionBodyReferences(project)
		.find((candidate) => bodyKey(candidate) === bodyKey(descriptor)) ?? null;
	if (descriptor.kind.startsWith('framescaper-') && !extension) {
		throw new Error(`V28 desktop ${descriptor.kind} lost its project reference.`);
	}
	return Object.freeze({
		descriptor,
...
function timingByStorageKey(project: FramescaperProjectV28): ReadonlyMap<string, Readonly<VideoTimingAssetReference>> {
...
function addTiming(
	...
	if (prior && JSON.stringify(prior) !== JSON.stringify(value)) {
		throw new Error(`V28 desktop timing body ${value.storageKey} has conflicting references.`);
	}
...
async function loadVerifiedBlob(
	...
	if (blob.size !== metadata.size || await digestMediaContent(blob, { signal }) !== metadata.sha256) {
		throw new Error(`Managed V28 ${reference.descriptor.kind} body failed immutable verification.`);
	}
...
function trustedMetadata(
	...
	if (metadata.sourceId !== descriptor.storageKey || metadata.mimeType !== descriptor.mimeType
		...
		throw new Error(`Managed V28 ${descriptor.kind} metadata conflicts with its project binding.`);
	}
	return metadata;
}
...
function trustedMetadataCandidate(value: unknown, kind: string): TrustedMetadata {
	if (!value || typeof value !== 'object' || Array.isArray(value)) {
		throw new Error(`Managed V28 ${kind} metadata is missing.`);
	}
	...
	if (!metadata.sourceId || !metadata.mimeType || !Number.isSafeInteger(metadata.size) || metadata.size < 1
		...
		throw new Error(`Managed V28 ${kind} metadata is incomplete.`);
	}
	return metadata;
}
...
function assertPublication(
	...
	if (!publication || typeof publication !== 'object' || typeof publication.discardIfCurrent !== 'function'
		...
		throw new Error(`Managed V28 ${body.kind} publication changed its descriptor.`);
	}
}

function storageKind(kind: FramescaperDesktopV28BodyDescriptor['kind']): string {
...
function assertAggregateBytes(bodies: readonly Readonly<{ byteLength: number }>[]): void {
	...
	for (const { byteLength } of bodies) {
		if (byteLength > SCAPE_ARCHIVE_LIMITS.maximumExpandedBytes - total) {
			throw new RangeError('Framescaper desktop V28 bodies exceed their aggregate byte limit.');
		}
		total += byteLength;
	}
}
...
function concatenate(chunks: readonly Uint8Array[], byteLength: number): Uint8Array {
	...
	if (offset !== byteLength) throw new Error('Framescaper desktop V28 semantic body ended early.');
	return output;
}
...
function acknowledgementRecord(value: unknown): Readonly<{
	...
	if (!value || typeof value !== 'object' || Array.isArray(value)) {
		throw new TypeError('Framescaper desktop V28 body acknowledgement is invalid.');
	}
	const record = value as Record<string, unknown>;
	if (Reflect.ownKeys(record).length !== 3 || !Number.isSafeInteger(record.bodyIndex)
		...
		throw new TypeError('Framescaper desktop V28 body acknowledgement changed shape.');
	}
