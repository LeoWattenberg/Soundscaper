...
import {
	acquireFramescaperDesktopV31Bodies,
	prepareFramescaperDesktopV31PublicationBodies,
	uploadFramescaperDesktopV31PublicationBodies,
...
const IDENTITY = Object.freeze({
	label: 'Framescaper desktop V20', librarySchemaVersion: 20,
	databaseUserVersion: 22, scopeVersion: 'v20', projectSchemaVersion: 31,
});
...
interface V20Bundle {
	...
	readonly project: Readonly<V20ProjectRow>;
...
export interface FramescaperDesktopProjectLibraryV20Renderer {
	listProjects(): Promise<readonly Readonly<FramescaperDesktopProjectLibraryV20ProjectSummary>[]>;
...
/** Connect the selected pathless public bridge to one exact durable V31 browser shadow. */
export async function connectFramescaperDesktopProjectLibraryV20Renderer(
	profileValue: EditorProjectRuntimeProfile | unknown,
	storeValue: unknown,
): Promise<FramescaperDesktopProjectLibraryV20Renderer | null> {
	assertFramescaperProjectV31Profile(profileValue);
	...
	if (bridge.handshakeState() !== 'admitted') {
		throw new TypeError('The Framescaper desktop V20 bridge did not retain its admitted handshake.');
	}
...
export function assertFramescaperDesktopProjectLibraryV20RendererComposition(
	profileValue: EditorProjectRuntimeProfile | unknown,
	store: unknown,
	renderer: unknown,
): asserts renderer is FramescaperDesktopProjectLibraryV20Renderer {
	assertFramescaperProjectV31Profile(profileValue);
	...
	if (!composition || composition.profile !== profileValue || composition.store !== store) {
		throw new TypeError('The exact admitted Framescaper desktop V20 renderer composition is required.');
	}
}

class Renderer implements FramescaperDesktopProjectLibraryV20Renderer {
	readonly #profile: EditorProjectRuntimeProfile;
	readonly #bridge: V20Bridge;
	readonly #store: V20Store;
	#tail: Promise<void> = Promise.resolve();

	constructor(profile: EditorProjectRuntimeProfile, bridge: V20Bridge, store: V20Store) {
	...
	readProject(projectIdValue: string, options: Readonly<{ signal?: AbortSignal }> = {}) {
		...
		return this.#exclusive(async () => {
			...
			await acquireFramescaperDesktopV31Bodies(
	...
	publishProject(request: Readonly<{
		...
		return this.#exclusive(async () => {
			const project = cloneFramescaperProjectV31(this.#profile, request.project);
			...
			if (current && !isStrictlyHigherProjectRevision(project.revision, current.project.revision)) {
				throw new Error('Framescaper desktop V20 publication requires a strictly higher revision.');
			}
			...
			if (current && current.bundle.metadataRevision !== metadataRevision) {
				throw new Error('Framescaper desktop V20 catalog changed before publication.');
			}
			const projectSha256 = bytesToHex(sha256(new TextEncoder().encode(JSON.stringify(project))));
			const preparedBodies = await prepareFramescaperDesktopV31PublicationBodies(
			...
			try {
				...
				}), ['publicationId', 'maximumChunkBytes', 'bodyCount'], 'V20 publication admission');
				if (admission.publicationId !== publicationId || admission.maximumChunkBytes !== 4 * 1024 * 1024
					...
					throw new Error('Framescaper V20 publication admission changed.');
				}
				admitted = true;
				await uploadFramescaperDesktopV31PublicationBodies(
	...
	deleteProject(projectIdValue: string): Promise<void> {
		...
		return this.#exclusive(async () => {
			...
			}), ['projectId', 'metadataRevision', 'deleted'], 'V20 delete result');
	...
	duplicateProject(sourceProjectId: string, options: Readonly<{
		...
		return this.#exclusive(async () => {
			...
			if (raw === null) throw new Error('Framescaper V20 duplicate source is unavailable.');
...
function resolveBridge(): V20Bridge | null {
	...
	for (const field of API_FIELDS) {
		if (typeof api[field] !== 'function') throw new TypeError(`The Framescaper desktop V20 bridge requires ${field}.`);
	}
...
function validateHandshake(value: unknown, databaseName: string): void {
	const handshake = exactRecord(value, HANDSHAKE_FIELDS, 'Framescaper desktop V20 handshake');
	if (handshake.kind !== 'framescaper-project-library-handshake' || handshake.version !== 1
		...
		throw new TypeError('The Framescaper desktop V20 handshake identity is unsupported.');
	}
}

function validateBundle(profile: EditorProjectRuntimeProfile, value: unknown, expectedProjectId: string): Readonly<{
	bundle: Readonly<V20Bundle>; project: FramescaperProjectV31;
}> {
	const raw = exactRecord(value, BUNDLE_FIELDS, 'Framescaper desktop V20 bundle');
	const row = projectRow(raw.project, expectedProjectId);
	if (typeof raw.document !== 'string') throw new TypeError('The Framescaper desktop V20 document is invalid.');
	const bytes = new TextEncoder().encode(raw.document);
	if (bytes.byteLength !== row.byteLength || bytesToHex(sha256(bytes)) !== row.sha256) {
		throw new Error('The Framescaper desktop V20 document changed bytes or digest.');
	}
	const project = cloneFramescaperProjectV31(profile, JSON.parse(raw.document) as unknown);
	if (project.id !== row.projectId || project.title !== row.name || project.revision !== row.projectRevision) {
		throw new Error('The Framescaper desktop V20 project disagrees with its descriptor.');
	}
	const bodies = validateFramescaperDesktopV31Bodies(project, row.sha256, raw.bodies);
...
function projectRow(value: unknown, expectedProjectId: string): Readonly<V20ProjectRow> {
	const row = exactRecord(value, ROW_FIELDS, 'Framescaper desktop V20 project row');
	if (row.projectId !== projectIdValue_(expectedProjectId) || row.preferredProduct !== 'framescaper'
		...
		throw new TypeError('The Framescaper desktop V20 project row is invalid.');
	}
	return Object.freeze({
...
function catalog(value: unknown): Readonly<{
	...
	projects: readonly Readonly<FramescaperDesktopProjectLibraryV20ProjectSummary>[];
}> {
	const raw = exactRecord(value, CATALOG_FIELDS, 'Framescaper desktop V20 catalog');
	if (!Array.isArray(raw.projects) || raw.projects.length > 10_000) throw new TypeError('The V20 catalog is invalid.');
	return Object.freeze({
		metadataRevision: nonNegative(raw.metadataRevision, 'metadata revision'),
		projects: Object.freeze(raw.projects.map((value) => {
			const summary = exactRecord(value, SUMMARY_FIELDS, 'Framescaper desktop V20 project summary');
			return Object.freeze({
...
function durableStore(profile: EditorProjectRuntimeProfile, value: unknown): V20Store {
	if (!value || typeof value !== 'object') throw new TypeError('The exact V31 shadow store is required.');
	framescaperProjectStoreAuthorityV31(profile, value);
	const record = value as Record<string, unknown>;
	const status = typeof record.getStatus === 'function'
		? (record.getStatus as () => unknown).call(value) as Record<string, unknown>
		: record;
	if (status?.persistent !== true) throw new Error('The desktop V20 lifecycle requires a durable V31 shadow.');
	const databaseName = typeof record.databaseName === 'string' ? record.databaseName : status.databaseName;
	if (databaseName !== 'kw-media-framescaper-editor-v31') {
		throw new TypeError('The exact V31 shadow database identity is required.');
	}
...
function randomPublicationId(): string {
	...
	if (!globalThis.crypto?.getRandomValues) throw new Error('Web Crypto is required for V20 publication identities.');
...
function instant(value: unknown): string {
	...
	if (!Number.isFinite(Date.parse(result))) throw new TypeError('The Framescaper V20 project timestamp is invalid.');
	return result;
}

function text(value: unknown, label: string): string {
	if (typeof value !== 'string' || value.length < 1 || value.length > 4_096) {
		throw new TypeError(`The Framescaper V20 ${label} is invalid.`);
	}
	return value;
}
...
function nonNegative(value: unknown, label: string): number {
	if (typeof value !== 'number' || !Number.isSafeInteger(value) || value < 0) {
		throw new RangeError(`The Framescaper V20 ${label} is invalid.`);
	}
	return value;
}
...
function positive(value: unknown, label: string): number {
	...
	if (result < 1) throw new RangeError(`The Framescaper V20 ${label} must be positive.`);
	return result;
}
