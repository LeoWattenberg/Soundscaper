...
type LinearImageFrameV30 = Readonly<{
	readonly width: number;
	readonly height: number;
	readonly pixels: Uint8Array<ArrayBuffer>;
}>;

interface CachedImageFrameV30 {
	readonly sourceId: string;
	readonly frameIndex: number;
	readonly frame: LinearImageFrameV30;
}

interface ResolvedImageContextV30 {
	readonly context: ImageContextV30;
	readonly frameIndex: number;
}
...
export async function createFramescaperVideoExportImageExecutionV30(options: Readonly<{
	...
	readonly store?: FramescaperStoredImageAssetStoreV30;
	...
	if (!options.store) throw new Error('Selected V30 image export requires its authenticated asset store.');
	const store = options.store;
	const planned = visible.map((item) => ({
		...item,
		source: framescaperImageSourceForClipV30(project.sources, item.clip),
	}));
	...
	const frameCache = new Map<string, CachedImageFrameV30>();
	...
	try {
		for (const item of planned) {
			...
			if (!reader) {
				...
				reader = await openFramescaperStoredImageFramePackV30(
				...
				if (admittedMetadataBytes === null
					...
					throw new Error('V30 image reader changed its admitted metadata byte estimate.');
				}
...
interface ImageExecutionResourcesV30 {
	readonly maximumAssetBytes: bigint;
	readonly snapshotBytes: bigint;
	readonly readerMetadataBytes: bigint;
}

type ImageExecutionBaseResourcesV30 = Omit<ImageExecutionResourcesV30, 'readerMetadataBytes'>;
...
function admitActiveImageAssets(sources: readonly FramescaperImageSourceV1[]): void {
	...
	if (unique.size > MAXIMUM_ACTIVE_IMAGE_ASSETS) {
		throw new RangeError('V30 active image assets exceed their count bound.');
	}
	let total = 0n;
	for (const source of unique.values()) total += BigInt(source.assetByteLength);
	if (total > BigInt(MAXIMUM_ACTIVE_IMAGE_ASSET_BYTES)) {
		throw new RangeError('V30 active image assets exceed their byte bound.');
	}
}
...
function admitImageExecutionResources(
...
): ImageExecutionBaseResourcesV30 {
	if (planned.length > MAXIMUM_ACTIVE_IMAGE_CONTEXTS) {
		throw new RangeError('V30 active image clips exceed their context bound.');
	}
	...
	if (snapshotBytes + maximumAssetBytes + rangeReadChunkBytes() > maximum) {
		throw new RangeError('V30 active image snapshots exceed their working byte bound.');
	}
	return Object.freeze({ maximumAssetBytes, snapshotBytes });
}

function admitImageReaderMetadata(
	resources: ImageExecutionBaseResourcesV30,
	...
	if (executionBaseBytes(resources, retainedBytes + candidateBytes)
		...
		throw new RangeError('V30 active image reader metadata exceeds its working byte bound.');
	}
}

function admitResolvedImageFrames(
	resolved: readonly ResolvedImageContextV30[],
	resources: ImageExecutionResourcesV30,
	width: number,
	height: number,
): void {
	...
	if (decodedWorkingBytes > maximum) {
		throw new RangeError('V30 resolved image frames exceed their decoded working byte bound.');
	}
	...
	if (decodedWorkingBytes + canvasWorkingBytes > maximum) {
		throw new RangeError('V30 resolved image frames exceed their compositing working byte bound.');
	}
}
...
async function linearFrame(
	...
	frameIndex: number,
	cache: Map<string, CachedImageFrameV30>,
	signal: AbortSignal,
): Promise<LinearImageFrameV30> {
	...
	let linear: LinearImageFrameV30;
	try {
...
function resolvedImageContexts(
	contexts: readonly ImageContextV30[],
	project: FramescaperProjectV30,
	sequenceFrame: number,
): readonly ResolvedImageContextV30[] {
...
function disposeFrameCache(cache: Map<string, CachedImageFrameV30>): void {
