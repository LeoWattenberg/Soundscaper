...
import type { FramescaperVideoExportFinishingAssetStoreV27 } from './video-export-finishing-v27.ts';

export interface FramescaperVideoExportVisualAssetStoreV27 extends FramescaperVideoExportFinishingAssetStoreV27 {
...
export interface FramescaperVideoExportVisualAssetsV27 {
...
export async function loadFramescaperVideoExportVisualAssetsV27(
	...
	if (stillSources.length + lutReferences.length > MAXIMUM_VISUAL_ASSETS) {
		throw new RangeError('V27 visual export assets exceed their count bound.');
	}
	if ((stillSources.length > 0 || lutReferences.length > 0) && request.store === undefined) {
		throw new Error('V27 visual export assets are unavailable.');
	}
	let totalBytes = 0;
	const stills = new Map<string, UnifiedExactRenderVisualRgbaV13>();
	for (const source of stillSources) {
		const blob = await loadBlob(request, stableId(source.storageKey, 'V27 still storage key'));
		totalBytes = boundedTotal(totalBytes, blob.size);
		const bytes = new Uint8Array(await blob.arrayBuffer());
		assertReady(request);
		if (bytesToHex(sha256(bytes)) !== digest(source.contentSha256, 'V27 still digest')) {
			throw new RangeError(`V27 still ${String(source.id)} is missing or stale.`);
		}
		...
		stills.set(stableId(source.id, 'V27 still source'), Object.freeze({
...
async function loadBlob(request: LoadRequest, storageKey: string): Promise<Blob> {
	...
	if (!value || !Number.isSafeInteger(value.size) || value.size < 1
		...
		throw new Error(`V27 visual asset ${storageKey} is missing or stale.`);
	}
...
async function decodeStill(
	...
	if (typeof globalThis.createImageBitmap !== 'function' || !globalThis.document) {
		throw new Error('V27 still decode is unavailable in this browser.');
	}
	...
	try {
		throwIfAborted(signal);
		const width = positiveInteger(source.width, 'V27 still width');
		const height = positiveInteger(source.height, 'V27 still height');
		const canvas = globalThis.document.createElement('canvas');
		canvas.width = width;
		canvas.height = height;
		const context = canvas.getContext('2d', { willReadFrequently: true });
		if (!context) throw new Error('V27 still decode canvas is unavailable.');
...
function assertDecodedStill(source: Readonly<Record<string, unknown>>, frame: UnifiedExactRenderVisualRgbaV13): void {
	if (frame.width !== source.width || frame.height !== source.height
		...
		throw new RangeError(`V27 decoded still ${String(source.id)} geometry is stale.`);
	}
}
...
function assertLut(reference: VideoCubeLutReferenceV1, parsed: ParsedCubeLutV1): void {
	if (reference.sha256 !== parsed.sha256 || reference.byteLength !== parsed.byteLength
		...
		throw new RangeError('V27 visual LUT is missing or stale.');
	}
}
...
function boundedTotal(total: number, added: number): number {
	...
	if (!Number.isSafeInteger(result) || result > MAXIMUM_VISUAL_ASSET_BYTES) {
		throw new RangeError('V27 visual export assets exceed their byte bound.');
	}
	return result;
}
