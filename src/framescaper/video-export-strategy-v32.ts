...
import { createFramescaperVideoExportImageExecutionV32 } from './video-export-image-execution-v32.ts';
import type { FramescaperStoredImageAssetStoreV32 } from './editor-selected-v32-image-frame-source.ts';
import {
...
interface ExportAuthorityV32 {
	readonly canonicalProject: Readonly<Record<string, unknown>>;
	readonly canonicalSnapshot: FramescaperProjectV32;
	readonly inheritedProject: Readonly<Record<string, unknown>>;
	readonly hasTimelineImages: boolean;
}

export type FramescaperVideoExportAssetStoreV32 = FramescaperVideoExportVisualAssetStoreV27
	& FramescaperStoredImageAssetStoreV32;

/** Retain V28 browser delivery and supplement it with authenticated V32 image pictures. */
export function createFramescaperVideoExportStrategyV32(
	profile: unknown,
	...
	const foundationAuthorities = new WeakMap<object, ExportAuthorityV32>();
	const createSupplementalPictureExecution = async ({
		canonicalProject, foundationPlan, signal, assertCurrent,
	}: Readonly<{
		readonly canonicalProject: Readonly<Record<string, unknown>>;
		readonly foundationPlan: Parameters<typeof createFramescaperVideoExportImageExecutionV32>[0]['foundationPlan'];
		readonly signal: AbortSignal;
		readonly assertCurrent: () => void;
	}>) => {
		const authority = foundationAuthorities.get(canonicalProject);
		if (!authority) throw new Error('Selected V32 image export lost its exact project authority.');
		if (!authority.hasTimelineImages) return null;
		return createFramescaperVideoExportImageExecutionV32({
			profile,
	...
	const exports = new WeakMap<object, ExportAuthorityV32>();
	const plans = new WeakMap<object, ExportAuthorityV32>();
	return Object.freeze({
		...
		hasPicture(exportProject: Readonly<Record<string, unknown>>) {
			if (!exports.has(exportProject)) {
				throw new TypeError('Selected V32 picture authority requires an owned export project.');
			}
		...
		captureTimingSourceIds(plan: ProductVideoExportPlan) {
			if (!plans.has(plan)) throw new TypeError('Selected V32 timing closure requires an owned plan.');
...
function projectAuthority(
	...
	const canonicalSnapshot = cloneFramescaperProjectV32(profile, project);
	const hasTimelineImages = records(canonicalSnapshot.clips, 'V32 export clips')
...
function currentAuthority(
	...
	exports: WeakMap<object, ExportAuthorityV32>,
): ExportAuthorityV32 {
	const authority = exports.get(request.exportProject);
	if (!authority || authority.canonicalProject !== request.canonicalProject) {
		throw new TypeError('The browser export projection is not owned by this exact V32 project.');
	}
	const currentSnapshot = cloneFramescaperProjectV32(profile, request.canonicalProject);
	if (!sameProjectSnapshot(currentSnapshot, authority.canonicalSnapshot)) {
		throw new Error('The selected V32 browser export projection is stale.');
	}
	return authority;
}
...
function ownedPlanAuthority(
	...
	request: ProductVideoExportStrategyEncodeRequest,
	exports: WeakMap<object, ExportAuthorityV32>,
	plans: WeakMap<object, ExportAuthorityV32>,
): ExportAuthorityV32 {
	const authority = currentAuthority(profile, request, exports);
	if (plans.get(request.plan) !== authority) {
		throw new TypeError('The V32 export plan is not owned by this exact project snapshot.');
	}
	return authority;
}
...
function imageExportFoundation(project: Readonly<Record<string, unknown>>): FramescaperProjectV28 {
	const foundation = framescaperProjectV28FoundationShapeV32(project) as unknown as Record<string, unknown>;
	const imageClips = records(project.clips, 'V32 export clips')
	...
	const imagesBySourceId = new Map(records(project.sources, 'V32 export sources')
		.filter(({ kind }) => kind === 'image')
		.map((source) => {
			const normalized = normalizeFramescaperImageSourceV1(source);
			return [normalized.id, normalized] as const;
		}));
	const sequences = new Map(records(project.sequences, 'V32 export sequences')
	...
	for (const clip of imageClips) {
		...
		if (!source) throw new ReferenceError(`V32 export image source ${clip.sourceId} is unavailable.`);
		const sequence = sequences.get(clip.sequenceId);
		if (!sequence) throw new ReferenceError(`V32 export image sequence ${clip.sequenceId} is unavailable.`);
		const rate = rational(sequence.rate, `V32 export sequence ${clip.sequenceId} rate`);
	...
	const inheritedClips = new Map(records(foundation.clips, 'V32 inherited export clips')
		.map((clip) => [String(clip.id), clip] as const));
	foundation.clips = records(project.clips, 'V32 canonical export clips').flatMap((clip) => {
	...
	const canonicalTracks = new Map(records(project.tracks, 'V32 canonical export tracks')
		.map((track) => [String(track.id), track] as const));
	foundation.tracks = records(foundation.tracks, 'V32 inherited export tracks').map((track) => {
		const canonical = canonicalTracks.get(String(track.id));
		return canonical ? { ...track, clipIds: [...array(canonical.clipIds, 'V32 export track clip IDs')] } : track;
	});
	foundation.sources = [
		...records(foundation.sources, 'V32 inherited export sources'),
...
function completeProjectRange(
	...
	const runtime = framescaperProjectForRuntimeConsumersV32(profile, project);
	...
	if (!Number.isSafeInteger(range.startFrame) || !Number.isSafeInteger(range.endFrame)
		...
		throw new RangeError('The V32 image export project range is invalid.');
	}
