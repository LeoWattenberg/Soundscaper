...
/** Follow one Scape source collision map through V27 state and exact V32 image authority. */
export function rebindFramescaperSourceIdentitiesV32(
	...
	const sources = records(project.sources, 'V32 rebound sources').map((source) => {
		if (source.kind !== 'image') return source;
		const before = stableId(source.id, 'V32 image source ID');
	...
	assertUniqueIds(sources, 'V32 rebound sources');
	project.sources = sources;
	project.clips = rebindImageClips(project.clips, sourceIdMap, 'V32 rebound clips');
	const bin = record(project.projectBin, 'V32 rebound Project Bin');
	bin.clips = rebindImageClips(bin.clips, sourceIdMap, 'V32 rebound Project Bin clips');
	const imageSourceIds = new Set(sources.filter(({ kind }) => kind === 'image').map(({ id }) => String(id)));
	for (const clip of [
		...records(project.clips, 'V32 rebound clips'),
		...records(bin.clips, 'V32 rebound Project Bin clips'),
	]) {
		if (clip.kind === 'image' && !imageSourceIds.has(String(clip.sourceId))) {
			throw new ReferenceError(`V32 rebound image clip ${String(clip.id)} lost its source.`);
		}
	}
}
...
function assertSourceIdMap(value: ReadonlyMap<string, string>): void {
	if (!value || typeof value !== 'object' || typeof value.get !== 'function'
		...
		throw new TypeError('V32 source rebind requires an exact source identity map.');
	}
	const targets = new Set<string>();
	for (const [beforeValue, afterValue] of value) {
		stableId(beforeValue, 'V32 source rebind origin');
		const after = stableId(afterValue, 'V32 source rebind target');
		if (targets.has(after)) throw new RangeError(`V32 source rebind target ${after} is duplicated.`);
		targets.add(after);
	}
}
