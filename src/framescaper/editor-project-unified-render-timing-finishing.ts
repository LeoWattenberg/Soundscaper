...
export function bindFramescaperUnifiedRenderTimingSidecarsFinishing(
	projectValue: unknown,
	timingViews: ReadonlyMap<string, VideoSourceTimingView>,
): UnifiedExactRenderTimingSidecars {
	if (!(timingViews instanceof Map)) {
		throw new TypeError('Selected finishing exact timing views must be an authenticated Map.');
	}
	if (!projectValue || typeof projectValue !== 'object' || Array.isArray(projectValue)) {
		throw new TypeError('Selected finishing exact timing requires a project.');
	}
	const sources = (projectValue as Readonly<Record<string, unknown>>).sources;
	if (!Array.isArray(sources)) throw new TypeError('Selected finishing exact timing requires project sources.');
	...
	if (timingViews.size !== videos.length) {
		throw new RangeError('Selected finishing exact timing must contain exactly every video source.');
	}
	return new Map(videos.map((source) => {
		const sourceId = source.id;
		if (typeof sourceId !== 'string' || !sourceId) {
			throw new TypeError('Selected finishing video source identities must be text.');
		}
