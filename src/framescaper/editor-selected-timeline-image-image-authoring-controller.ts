...
import {
	importFramescaperTimelineImagesTimelineImage,
	type FramescaperImageImportFileTimelineImage,
	type FramescaperTimelineImageImportResultTimelineImage,
} from './editor-image-import-coordinator-timeline-image.ts';
import {
	createFramescaperTimelineImageCurrentProjectPublicationTimelineImage,
	type FramescaperTimelineImageCurrentProjectPublicationDependenciesTimelineImage,
	type FramescaperTimelineImagePublicationControllerTimelineImage,
...
type ImportImages = typeof importFramescaperTimelineImagesTimelineImage;
type PublicationDependencies = Omit<
	FramescaperTimelineImageCurrentProjectPublicationDependenciesTimelineImage,
	'controller'
>;
...
export interface FramescaperSelectedImageFileServiceTimelineImage {
	...
	withReadDescriptors?<Result>(
		...
		consume: (files: readonly FramescaperImageImportFileTimelineImage[]) => PromiseLike<Result> | Result,
	): Promise<Result>;
}

export interface FramescaperSelectedImageAuthoringControllerTimelineImage
	extends FramescaperTimelineImagePublicationControllerTimelineImage {
	readonly project: FramescaperProjectTimelineImage | null;
	getTelemetrySnapshot(): Readonly<{ readonly positionFrame?: unknown }>;
}

export interface BindFramescaperSelectedImageAuthoringControllerTimelineImageOptions
	extends PublicationDependencies {
	readonly controller: FramescaperSelectedImageAuthoringControllerTimelineImage;
	readonly fileService?: FramescaperSelectedImageFileServiceTimelineImage;
...
export function framescaperSelectedImageImportResultTimelineImageFor(
	owner: unknown,
): FramescaperTimelineImageImportResultTimelineImage | null {
	return owner && (typeof owner === 'object' || typeof owner === 'function')
		? RESULTS.get(owner as object) ?? null : null;
}

/** Bind the timelineImage multi-file timeline import to Generate > Add Images only. */
export function bindFramescaperSelectedImageAuthoringControllerTimelineImage(
...
async function selectImageFiles(
	...
	return new Promise((resolve) => {
		...
		let settled = false;
		const finish = (files: readonly FramescaperImageImportFileTimelineImage[]): void => {
			if (settled) return;
...
function snapshotFiles(value: readonly FramescaperImageImportFileTimelineImage[]): readonly FramescaperImageImportFileTimelineImage[] {
...
function exactProject(value: FramescaperProjectTimelineImage | null): FramescaperProjectTimelineImage {
	assertFramescaperProjectIdentity(value);
	return value as FramescaperProjectTimelineImage;
}
...
function importFailureMessage(result: FramescaperTimelineImageImportResultTimelineImage): string {
