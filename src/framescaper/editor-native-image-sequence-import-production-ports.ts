...
import type {
	FramescaperImageSequenceImportPorts,
...
export type { FramescaperImageSequenceNativeAdmissionRequest } from
	'./editor-native-image-sequence-import.ts';

const TRANSACTION_ID = /^[a-f0-9]{40}$/u;

export interface FramescaperNativeImageSequenceImportRendererPortProfessionalMedia {
...
export interface FramescaperImageSequenceProductionPortsOptionsProfessionalMedia {
	readonly bridge: FramescaperNativeImageSequenceImportRendererPortProfessionalMedia & Readonly<{
...
export interface FramescaperImageSequenceProductionPortsProfessionalMedia
	extends FramescaperImageSequenceImportPorts {
...
export function createFramescaperImageSequenceProductionPortsProfessionalMedia(
	options: FramescaperImageSequenceProductionPortsOptionsProfessionalMedia,
): FramescaperImageSequenceProductionPortsProfessionalMedia {
	...
	return Object.freeze({
		...
		async readCommittedBody(request: Parameters<FramescaperImageSequenceProductionPortsProfessionalMedia['readCommittedBody']>[0]) {
		...
		async admit(request: FramescaperImageSequenceNativeAdmissionRequest): Promise<unknown> {
			const id = activeTransaction(transactionId, disposed);
		...
		async complete(request: FramescaperImageSequenceNativeAdmissionRequest): Promise<void> {
			const id = activeTransaction(transactionId, disposed);
...
async function writeChunks(
	bridge: FramescaperNativeImageSequenceImportRendererPortProfessionalMedia,
	transactionId: string,
	asset: 'pack' | 'inventory',
