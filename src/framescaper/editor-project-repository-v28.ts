...
/** Exact-write V28 repository with opaque V25/V26/future custody and proxy fencing. */
export class FramescaperProjectRepositoryV28 implements ProjectRepositoryPort {
	...
	async save(
		...
		if (currentValue === null) throw new Error('Ordinary V28 save cannot create a project.');
		...
		if (saved === null) throw new Error('The V28 project changed before save.');
	...
	async saveIfCurrent(
		...
		const project = this.#exact(projectValue);
		if (expected.id !== project.id) throw new Error('V28 compare-and-swap cannot change project identity.');
	...
	#exact(value: unknown): FramescaperProjectV28 & ProjectDocument {
		return cloneFramescaperProjectV28(this.#profile, value) as FramescaperProjectV28 & ProjectDocument;
	}
	...
	#custody(value: unknown): ProjectDocument {
		return loadFramescaperProjectV28(this.#profile, value).project as ProjectDocument;
	}
}

function assertDelegate(value: unknown): asserts value is ProjectRepositoryPort {
	if (!value || typeof value !== 'object') throw new TypeError('A V28 repository delegate is required.');
	for (const method of REQUIRED_METHODS) {
		if (typeof (value as Record<string, unknown>)[method] !== 'function') {
			throw new TypeError(`The V28 repository delegate requires ${method}.`);
		}
	}
}

function assertNoAttachments(project: FramescaperProjectV28, operation: string): void {
	if (attachmentAuthority(project).size !== 0) {
		throw new Error(`A proxy-attached V28 ${operation} requires atomic preservation publication.`);
	}
}

function assertSameAttachments(expected: FramescaperProjectV28, project: FramescaperProjectV28): void {
	...
	if (before.size !== after.size || [...before].some(([id, value]) => after.get(id) !== value)) {
		throw new Error('Ordinary V28 save cannot introduce or change a proxy attachment.');
	}
}

function attachmentAuthority(project: FramescaperProjectV28): ReadonlyMap<string, string> {
