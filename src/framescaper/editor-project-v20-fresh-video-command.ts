...
/** Identify the exact new video occurrences that V20 may initialize with neutral keyframes. */
export function framescaperV20ExplicitFreshVideoIds(command: unknown): ReadonlySet<string> {
...
function collectThreePointVideoIds(
	...
	const placements = dataProperty(record, 'placements', `Framescaper V20 ${type} command`);
	if (!Array.isArray(placements)) {
		throw new TypeError(`Framescaper V20 ${type} command.placements must be an array.`);
	}
	for (let index = 0; index < placements.length; index += 1) {
		const placement = dataRecord(
			placements[index],
			`Framescaper V20 ${type} command.placements[${String(index)}]`,
		);
		if (dataProperty(placement, 'kind', `Framescaper V20 ${type} placement`) !== 'video') continue;
		result.add(stableId(
			dataProperty(placement, 'clipId', `Framescaper V20 ${type} placement`),
			`Framescaper V20 ${type} placement.clipId`,
		));
	}
}
