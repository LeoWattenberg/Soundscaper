...
export interface FramescaperProjectV18PreservationPlan {
	readonly sourceId: string;
	readonly plan: VideoProxyPreservationPlan;
}

export interface FramescaperProjectV18PreservationPublication {
	readonly expected: unknown;
	readonly project: unknown;
	readonly plans: readonly FramescaperProjectV18PreservationPlan[];
}

export interface FramescaperProjectV18PreservationDependencies {
...
interface NormalizedPublication {
	readonly expected: FramescaperProjectV18;
...
/** Canonical digest used to bind a claim operation to one exact V18 predecessor. */
export function framescaperProjectFingerprintV18(
 ...
 * introduce a non-null V18 proxy pointer. Ordinary project saves never receive
 * its repository-local, one-use claim capabilities.
 */
export class FramescaperProjectV18PreservationRepository {
	...
	constructor(
		profile: EditorProjectRuntimeProfile | unknown,
		dependenciesValue: FramescaperProjectV18PreservationDependencies | unknown,
	) {
		assertFramescaperProjectV18Profile(profile);
		const dependencies = closedRecord(
			dependenciesValue,
			DEPENDENCY_FIELDS,
			'Framescaper V18 preservation dependencies',
		);
		if (!dependencies.port || typeof dependencies.port !== 'object'
			|| typeof (dependencies.port as StorageRepositoryPort).database !== 'function') {
			throw new TypeError('A storage repository port is required for V18 preservation.');
		}
		if (!(dependencies.claims instanceof VideoProxyClaimRepository)) {
			throw new TypeError('A video proxy claim repository is required for V18 preservation.');
		}
	...
	async publishIfCurrent(
		publicationValue: FramescaperProjectV18PreservationPublication | unknown,
	): Promise<FramescaperProjectV18 | null> {
		const database = await this.#port.database();
		if (!database) {
			throw new Error('Durable storage is required; memory V18 preservation is unsupported.');
		}
	...
	return transact(
		...
		async ({ projects, revisions, mediaAssets, mediaAssetStaging }) => {
			...
			if (!baseRecord
				...
				throw new Error('The exact V18 base revision is missing or conflicts with the current project.');
			}
...
function normalizePublication(
	...
	const raw = closedRecord(value, PUBLICATION_FIELDS, 'Framescaper V18 preservation publication');
	const expected = cloneFramescaperProjectV18(profile, raw.expected);
	...
	if (project.id !== projectId) {
		throw new Error('V18 preservation cannot change project identity.');
	}
	...
	if (project.revision !== nextRevision) {
		throw new Error('V18 preservation must publish exactly the next revision.');
	}
	const attachedSources = collectAttachedSources(project);
	if (attachedSources.length === 0) {
		throw new Error('V18 preservation requires at least one non-null proxy attachment.');
	}
...
function normalizePlans(
...
): readonly FramescaperProjectV18PreservationPlan[] {
	const values = denseArray(value, 'Framescaper V18 preservation plans');
	if (values.length > MAXIMUM_PRESERVATION_PLANS) {
		throw new RangeError('The V18 preservation plan limit was exceeded.');
	}
	const seen = new Set<string>();
	const plans = values.map((entry, index) => {
		const raw = closedRecord(entry, PLAN_FIELDS, `V18 preservation plan ${String(index)}`);
		const sourceId = requiredIdentifier(raw.sourceId, 'preservation source id');
		if (seen.has(sourceId)) throw new Error('A V18 attachment has duplicate preservation plans.');
	...
	if (seen.size !== attachments.size) {
		throw new Error('Every V18 attachment requires one complete preservation plan.');
	}
	return Object.freeze(plans);
}

function collectAttachedSources(project: FramescaperProjectV18): readonly AttachedSource[] {
...
function assertClaimsMatchPublication(
	...
	if (claims.proxy.bodyKind !== 'proxy'
		...
		throw new Error('The proxy claim does not match the exact V18 attachment.');
	}
	if (claims.timing.bodyKind !== 'timing'
		...
		throw new Error('The timing claim does not match the exact V18 attachment.');
	}
}
...
function fingerprint(project: FramescaperProjectV18): string {
	return bytesToHex(sha256(TEXT_ENCODER.encode(serializeScapeProjectDocument(project))));
}

function sameProject(value: unknown, expected: FramescaperProjectV18): boolean {
	try {
...
function revision(value: unknown, label: string): number {
	if (!Number.isSafeInteger(value) || Number(value) < 0) {
		throw new RangeError(`The V18 ${label} revision must be a non-negative safe integer.`);
	}
	return Number(value);
}

function safeNextRevision(value: number): number {
	if (value === Number.MAX_SAFE_INTEGER) throw new RangeError('The V18 revision cannot be incremented safely.');
	return value + 1;
}
