...
export type FramescaperImageOrInheritedClipV30 =
...
export interface FramescaperProjectV30 extends Omit<FramescaperProjectV28,
	...
	readonly clips: readonly FramescaperImageOrInheritedClipV30[];
	...
	readonly projectBin: Readonly<Record<string, unknown>> & Readonly<{
		clips: readonly FramescaperImageOrInheritedClipV30[];
	}>;
...
function validateImageModels(project: Record<string, unknown>): void {
	...
	for (const owner of [...sources, ...timelineClips, ...binClips, ...tracks, ...sequences]) {
		...
		if (identities.has(id)) throw new RangeError(`V30 image identity ${id} is duplicated.`);
		identities.add(id);
	}
	...
	if (Array.isArray(selection.clipIds)) {
		for (const clipId of selection.clipIds.map(String)) {
			if (!timelineClips.some(({ id }) => String(id) === clipId)) {
				throw new ReferenceError(`V30 selection references missing timeline clip ${clipId}.`);
			}
		}
	}
}
...
function assertCanonical(value: unknown, normalized: unknown, name: string): void {
	if (JSON.stringify(value) !== JSON.stringify(normalized)) throw new TypeError(`V30 ${name} is not canonical.`);
}
