...
export interface FramescaperPlaybackProjectServiceV18Options {
...
export function createFramescaperPlaybackProjectServiceV18(
	...
	optionsValue: FramescaperPlaybackProjectServiceV18Options | unknown = {},
): PlaybackProjectService {
	...
	async function prepareProjectForActivation<Project extends object>(
		...
		if (readFramescaperProjectSchemaVersion(project) !== 18) return;
		...
		for (const sourceId of multicameraMemberSourceIds(canonical)) {
			...
			const timingDecision = dataRecord(source.timingDecision, `Multicamera source ${sourceId} timing decision`);
			if (timingDecision.mode !== 'exact') continue;
			const sourceSha256 = nonEmptyString(source.contentSha256, `Multicamera source ${sourceId} digest`);
...
function multicameraMemberSourceIds(project: FramescaperProjectV18): readonly string[] {
	return Object.freeze([...new Set(project.multicameraGroups.flatMap((group) => (
		group.members.map(({ sourceId }) => sourceId)
	)))].sort((left, right) => left.localeCompare(right)));
}

function snapshotOptions(value: unknown): Readonly<FramescaperPlaybackProjectServiceV18Options> {
	if (value === null || typeof value !== 'object' || Array.isArray(value)) {
		throw new TypeError('Framescaper V18 playback options must be a plain record.');
	}
	...
	if (keys.some((key) => key !== 'timingStore')) {
		throw new TypeError('Framescaper V18 playback options contain an unsupported authority field.');
	}
	const descriptor = Object.getOwnPropertyDescriptor(value, 'timingStore');
	if (!descriptor) return Object.freeze({});
	if (!descriptor.enumerable || !Object.hasOwn(descriptor, 'value')) {
		throw new TypeError('The Framescaper V18 timing store must be an own enumerable data property.');
	}
	const store = descriptor.value as Partial<VideoTimingMediaStore> | null;
	if (!store || typeof store.loadMediaAsset !== 'function') {
		throw new TypeError('The Framescaper V18 playback timing store is invalid.');
	}
	return Object.freeze({
