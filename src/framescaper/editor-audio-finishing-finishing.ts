...
export interface FramescaperAudioFinishingFinishing {
	readonly automationLanes: readonly AutomationLaneV21[];
	readonly mixer: MixerGraphV21;
}

export function createDefaultFramescaperAudioFinishingFinishing(
...
export function normalizeFramescaperAudioFinishingFinishing(
	project: Readonly<Record<string, unknown>>,
	...
	if (!Array.isArray(value?.automationLanes) || value.automationLanes.length > 4_096) {
		throw new RangeError('Framescaper finishing automationLanes must contain at most 4096 lanes.');
	}
