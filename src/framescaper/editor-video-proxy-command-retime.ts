...
export interface FramescaperVideoProxyDetachCommandRetime {
	readonly type: 'framescaper/video-proxy-detach';
	readonly sourceId: string;
	readonly expectedAttachment: Readonly<VideoProxyAttachmentV18>;
}

export function isFramescaperVideoProxyDetachCommandRetime(
	value: unknown,
): value is FramescaperVideoProxyDetachCommandRetime {
...
export function snapshotFramescaperVideoProxyDetachCommandRetime(
	value: unknown,
	...
	if (record.type !== 'framescaper/video-proxy-detach') {
		throw new RangeError('The Framescaper retime proxy command type is unsupported.');
	}
	if (typeof record.sourceId !== 'string' || !record.sourceId
		|| record.sourceId.length > 256) {
		throw new TypeError('The Framescaper retime proxy detach source ID is invalid.');
	}
...
/** Mutate only the already-cloned retime source carrier; its owner finalizes the command. */
export function detachFramescaperVideoProxyDraftRetime(
	draft: Record<string, unknown>,
	command: Readonly<FramescaperVideoProxyDetachCommandRetime>,
): void {
	if (!Array.isArray(draft.sources)) {
		throw new TypeError('Framescaper retime project.sources must be an array.');
	}
...
function closedRecord(value: unknown): Readonly<Record<string, unknown>> {
	if (!value || typeof value !== 'object' || Array.isArray(value)
		...
		throw new TypeError('The Framescaper retime proxy detach command must be a plain record.');
	}
	const keys = Reflect.ownKeys(value);
	if (keys.length !== FIELDS.length
		|| keys.some((key) => typeof key !== 'string' || !FIELDS.includes(key))) {
		throw new TypeError('The Framescaper retime proxy detach command has unsupported, missing, or extra fields.');
	}
	for (const field of FIELDS) {
		const descriptor = Object.getOwnPropertyDescriptor(value, field);
		if (!descriptor?.enumerable || !Object.hasOwn(descriptor, 'value')) {
			throw new TypeError(`The Framescaper retime proxy detach command.${field} must be an own data property.`);
		}
	}
