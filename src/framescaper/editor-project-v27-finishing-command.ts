...
export interface FramescaperVideoSourceColorInterpretationSetCommandV27 {
...
export interface FramescaperVideoVisualPresentationSetCommandV27 {
...
export interface FramescaperVideoProcessorStackSetCommandV27 {
...
export interface FramescaperVideoMotionAnalysisSetCommandV27 {
...
export interface FramescaperVideoFinishingPresetSetCommandV27 {
...
export interface FramescaperVideoCaptionTrackSetCommandV27 {
...
export type FramescaperOwnedFinishingCommandV27 =
	...
	| FramescaperVideoProcessorStackSetCommandV27
	| FramescaperVideoMotionAnalysisSetCommandV27
	| FramescaperVideoFinishingPresetSetCommandV27
	| FramescaperVideoCaptionTrackSetCommandV27
	| FramescaperAutomationLaneSetCommandV27
	| FramescaperMixerGraphSetCommandV27;
...
export function snapshotFramescaperOwnedFinishingCommandV27(
	value: unknown,
	...
	const command = readClosedDomainRecord(value, `Framescaper V27 ${commandSpec.label} command`, [
	...
	}) as unknown as FramescaperOwnedFinishingCommandV27;
}

export function applyFramescaperOwnedFinishingCommandV27(
	...
	if (!same(current, expected)) throw new Error(`The expected V27 ${commandSpec.label} is stale.`);
...
function optionalNormalized(
	...
	if (value === null) {
		if (!commandSpec.nullable) throw new RangeError(`A V27 ${commandSpec.label} cannot be removed.`);
		return null;
	}
...
function assertIdentity(
	...
	for (const value of [expected, replacement]) {
		if (value !== null && value[commandSpec.identityField] !== identity) {
			throw new RangeError(`A V27 ${commandSpec.label} command cannot change identity.`);
		}
	}
}

function assertMutation(expected: unknown, replacement: unknown, label: string): void {
	if (same(expected, replacement)) throw new RangeError(`A V27 ${label} command must mutate state; no-op commands are unsupported.`);
}

function commandType(value: unknown): string {
	const command = record(value, 'Framescaper V27 finishing command');
	const type = field(command, 'type');
	if (typeof type !== 'string' || !TYPES.has(type)) {
		throw new RangeError('Framescaper V27 finishing command type is unsupported.');
	}
	return type;
}

function field(value: Readonly<Record<string, unknown>>, name: string): unknown {
	return readClosedDomainField(value, name, 'Framescaper V27 finishing command');
}

function stableId(value: unknown, name: string): string {
	if (typeof value !== 'string' || !ID.test(value)) {
		throw new TypeError(`Framescaper V27 ${name} must be a stable ID.`);
	}
	return value;
}
