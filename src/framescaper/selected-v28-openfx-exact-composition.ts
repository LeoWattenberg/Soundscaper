...
import {
	framescaperOpenFxLinearPlaneV28,
	framescaperOpenFxRgbaPlaneV28,
	type FramescaperSelectedOpenFxExactPlanesV28,
...
export interface FramescaperSelectedOpenFxCompositionV28 {
	...
	disposition(): Readonly<{
		readonly effects: readonly FramescaperOpenFxFrameDispositionV28[];
		readonly reportsDegradation: boolean;
	}>;
}

export function createFramescaperSelectedOpenFxCompositionV28(options: Readonly<{
	readonly planes: FramescaperSelectedOpenFxExactPlanesV28;
...
}>): FramescaperSelectedOpenFxCompositionV28 {
	...
	const initial = new Map([...options.initialPlanes].map(([identity, frame]) => (
		[identity, framescaperOpenFxRgbaPlaneV28(identity, frame)] as const
	)));
	...
	async function contexts(
		...
		for (const context of values) {
			...
			const applied = await options.planes.apply({
				...
				primary: framescaperOpenFxLinearPlaneV28(primaryIdentity, output), named,
	...
	function requiredPlane(identity: string) {
		...
		if (existing) return framescaperOpenFxLinearPlaneV28(identity, existing);
		...
		return framescaperOpenFxRgbaPlaneV28(identity, Object.freeze({
	...
	async function applyTransitions(tracks: Map<string, UnifiedExactLinearCompositionEntryV13[]>) {
		for (const transition of activeTransitions) {
			...
			const result = await options.planes.apply({
				...
				named: Object.freeze([
					framescaperOpenFxLinearPlaneV28(transition.outgoingClipId, outgoing),
					framescaperOpenFxLinearPlaneV28(transition.incomingClipId, incoming),
				]),
