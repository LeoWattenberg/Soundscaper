...
export function reconcileInheritedFramescaperProjectStateFinishing(project: Record<string, unknown>): void {
...
function reconcileAudio(project: Record<string, unknown>): void {
	const audio = reconcileFramescaperAudioFinishingFinishing(project, {
...
function stableId(value: Record<string, unknown>): string {
	if (typeof value.id !== 'string' || !value.id) throw new TypeError('finishing inherited identity must be non-empty.');
	return value.id;
}

function ids(value: unknown): string[] {
	if (!Array.isArray(value)) throw new TypeError('finishing inherited IDs must be an array.');
