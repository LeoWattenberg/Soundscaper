...
import { createFramescaperImageBatchPlacementV32 } from './editor-image-placement-v32.ts';

export interface FramescaperImageImportFileV32 {
...
export interface FramescaperTimelineImagePublicationRequestV32 {
...
export interface FramescaperTimelineImagePublicationPortV32 {
	publish(request: FramescaperTimelineImagePublicationRequestV32): Promise<FramescaperProjectV32>;
}

export type DecodeFramescaperTimelineImageV32 = (request: Readonly<{
...
export interface FramescaperTimelineImageImportRequestV32 {
	readonly project: FramescaperProjectV32;
	readonly files: readonly FramescaperImageImportFileV32[];
	readonly sequenceStartFrame: number;
	readonly createId: (prefix: string) => string;
	readonly publisher: FramescaperTimelineImagePublicationPortV32;
	readonly decode?: DecodeFramescaperTimelineImageV32;
	readonly signal?: AbortSignal;
}

export interface FramescaperTimelineImageImportFileResultV32 {
	readonly fileName: string;
...
export interface FramescaperTimelineImageImportResultV32 {
	readonly project: FramescaperProjectV32;
	readonly files: readonly FramescaperTimelineImageImportFileResultV32[];
}

interface PreparedImage {
	readonly file: FramescaperImageImportFileV32;
	readonly fileName: string;
	readonly mimeTypeHint: string | null;
...
export async function importFramescaperTimelineImagesV32(
	request: FramescaperTimelineImageImportRequestV32,
): Promise<FramescaperTimelineImageImportResultV32> {
	if (request?.project?.schemaVersion !== 32 && request?.project?.schemaVersion !== 31) {
		throw new TypeError('Timeline image import requires a V32-compatible selected project.');
	}
	...
	const results: FramescaperTimelineImageImportFileResultV32[] = [];
	...
	const placement = createFramescaperImageBatchPlacementV32(request.project, {
...
async function defaultDecode(request: Parameters<DecodeFramescaperTimelineImageV32>[0]) {
...
function imageCommand(
	...
	return {
		type: 'batch',
		commands: [
			...(trackCommand ? [trackCommand as FramescaperProjectCommandV32] : []),
...
function primaryRate(project: FramescaperProjectV32): Readonly<{ num: number; den: number }> {
...
function orderedResults(
	files: readonly FramescaperImageImportFileV32[],
	results: readonly FramescaperTimelineImageImportFileResultV32[],
): readonly FramescaperTimelineImageImportFileResultV32[] {
	const queues = new Map<string, FramescaperTimelineImageImportFileResultV32[]>();
...
function appendCancelled(
	files: readonly FramescaperImageImportFileV32[],
	results: FramescaperTimelineImageImportFileResultV32[],
): void {
...
function fileResult(
	fileName: string,
	status: FramescaperTimelineImageImportFileResultV32['status'],
	sourceId: string | null,
	clipId: string | null,
	notices: readonly string[],
	resultMessage: string | null,
): FramescaperTimelineImageImportFileResultV32 {
	return Object.freeze({ fileName, status, sourceId, clipId, notices: Object.freeze([...notices]), message: resultMessage });
}

function fileSize(file: FramescaperImageImportFileV32): number {
