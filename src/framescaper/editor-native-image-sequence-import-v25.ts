...
const ADMISSION_RESPONSE_KEYS = Object.freeze([
	'kind', 'admitted', 'projectId', 'projectRevision', 'sourceId',
	'inventorySha256', 'sourcePackSha256', 'characteristics',
]);

export interface FramescaperSelectedImageSequenceFileV25 {
...
export interface FramescaperImageSequenceSelectionV25 {
	readonly sourceId: string;
	readonly projectBinClipId: string;
	readonly name: string;
	readonly frameRate: NativeMediaImageSequenceRateV1;
	readonly files: readonly FramescaperSelectedImageSequenceFileV25[];
}
...
export interface ComposeFramescaperImageSequenceImportV25Options {
	...
	readonly select: () => Awaitable<FramescaperImageSequenceSelectionV25 | null>;
	readonly ports: FramescaperImageSequenceImportPortsV25;
	readonly commit: (
 ...
 */
export async function composeFramescaperImageSequenceImportV25(
	options: ComposeFramescaperImageSequenceImportV25Options,
): Promise<void> {
	const generation = candidateGeneration(options.profile, options.project);
...
async function assertRuntimeCapability(ports: FramescaperImageSequenceImportPortsV25): Promise<void> {
...
function selection(value: unknown): FramescaperImageSequenceSelectionV25 {
...
function selectedFile(value: unknown): FramescaperSelectedImageSequenceFileV25 {
	...
	return Object.freeze({
		...
		chunks: record.chunks as FramescaperSelectedImageSequenceFileV25['chunks'],
	});
}

async function digestFile(file: FramescaperSelectedImageSequenceFileV25): Promise<string> {
...
function admissionCharacteristics(
	value: unknown,
	request: FramescaperImageSequenceNativeAdmissionRequestV25,
...
function assertPorts(options: ComposeFramescaperImageSequenceImportV25Options): void {
