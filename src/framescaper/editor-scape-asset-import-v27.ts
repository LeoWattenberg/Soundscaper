...
import {
	...
	type FramescaperScapeImportValidationV27,
...
export async function stageFramescaperScapeImportAssetsV27(
	...
	for (const reference of validation.references) {
		...
		if (!entry) throw new Error(`The V27 Scape archive is missing ${descriptor.entry}.`);
...
function motionMaterial(
	...
	const analyses = records(project.videoMotionAnalyses, 'rebound V27 motion analyses');
	const stacks = records(project.videoProcessorStacks, 'rebound V27 processor stacks');
	...
	if (!target || !stack || target.sourceId !== stack.sourceId) {
		throw new Error('The V27 motion analysis could not follow its rebound source identity.');
	}
...
async function readArchiveBytes(
	...
	if (offset !== descriptor.size) throw new Error('A V27 Scape asset ended before its admitted size.');
	return result;
}
...
async function beginWriter(
	...
	if (!writer || typeof writer.commitOwned !== 'function'
		...
		throw new TypeError('The V27 Scape import requires an exact bounded owned media writer.');
	}
...
async function verifyStoredBody(
	...
	if (typeof request.store.loadMediaAsset !== 'function') {
		throw new TypeError('The V27 Scape import requires immutable media-body reads.');
	}
	...
	if (body.size !== material.size || await digestMediaContent(body, {
		...
		throw new Error(`Stored V27 archive body ${material.storageKey} conflicts with immutable content.`);
	}
...
function assertStoredMetadata(value: unknown, material: StageMaterial): void {
	const metadata = record(value, `stored V27 ${material.kind} metadata`);
	if (metadata.sourceId !== material.storageKey || metadata.size !== material.size
		...
		throw new Error(`Stored V27 archive body ${material.storageKey} has conflicting role, size, or digest.`);
	}
}

function validateArchiveBytes(reference: FramescaperScapeAssetReferenceV27, bytes: Uint8Array): void {
	if (reference.timingReference) validateVideoTimingAssetBytes(reference.timingReference, bytes);
	validateFramescaperScapeAssetReferenceBytesV27(reference, bytes);
}
...
function reboundStillStorageKey(
	...
	const source = records(project.sources, 'rebound V27 sources').find(({ id }) => id === targetId);
	if (!source || source.kind !== 'still') {
		throw new Error(`The V27 ${reference.role} body lost its rebound still source.`);
	}
	return stableId(source.storageKey, `rebound V27 ${reference.role} storage key`);
}

function importValidation(value: unknown): FramescaperScapeImportValidationV27 {
	if (!value || typeof value !== 'object' || Array.isArray(value)) {
		throw new TypeError('The exact V27 Scape import validation is required.');
	}
	const candidate = value as Partial<FramescaperScapeImportValidationV27>;
	if (!Array.isArray(candidate.references) || !(candidate.descriptorByArchiveId instanceof Map)) {
		throw new TypeError('The V27 Scape import validation is incomplete.');
	}
	return candidate as FramescaperScapeImportValidationV27;
}

function buffered(reference: FramescaperScapeAssetReferenceV27): boolean {
	return reference.role === 'proxy-timing' || reference.role === 'lut' || reference.role === 'motion';
}

function storageKind(reference: FramescaperScapeAssetReferenceV27): string {
...
async function cleanupWriter(
	...
	try {
		...
		throw aggregateScapeErrors(primary, [cleanupError], 'The V27 Scape body write and cleanup both failed.');
	}
	throw primary;
}
