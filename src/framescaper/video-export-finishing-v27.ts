...
/** Selected V27 adapter from exact keyed RGBA frames into the shared V13 finisher. */
...
export async function createFramescaperVideoExportFinishingV27(
	...
	if (!(request.timingViewsBySourceId instanceof Map)) {
		throw new TypeError('Selected V27 export requires raw authenticated source timing views.');
	}
...
function renderAuthority(request: FramescaperVideoExportFinishingRequestV27) {
	...
	return Object.freeze({
		sequenceId: stableId(project.primarySequenceId, 'Selected V27 primary sequence'),
...
export async function loadFramescaperVideoExportFinishingAssetsV27(
	...
	if (references.length > MAXIMUM_AUXILIARY_ASSETS
		...
		throw new RangeError('Selected V27 finishing assets exceed the browser export bound.');
	}
	if (references.length > 0 && request.store === undefined) {
		throw new Error('Selected V27 finishing assets are unavailable in this browser runtime.');
	}
	...
	if (!Array.isArray(sourceValues)) throw new TypeError('Selected V27 finishing sources are unavailable.');
	const sourceById = new Map(sourceValues.map((sourceValue) => {
		const source = record(sourceValue, 'Selected V27 finishing source');
...
async function loadAsset(
	...
	if (!blob || blob.size !== reference.byteLength || typeof blob.arrayBuffer !== 'function') {
		throw new Error(`Selected V27 finishing asset ${reference.storageKey} is missing or stale.`);
	}
...
function exactOccurrence(
	...
	for (const layerValue of layersValue) {
		const layer = record(layerValue, 'Selected V27 finishing layer');
		if (!Array.isArray(layer.clips)) throw new TypeError('Selected V27 finishing layer clips are unavailable.');
...
function requiredAnalysis(
	...
	if (!reference) throw new ReferenceError(`Selected V27 motion analysis ${id} is unavailable.`);
	return reference;
}
...
function uniqueBy<Value>(values: readonly Value[], key: (value: Value) => string): readonly Value[] {
	...
	for (const value of values) {
		...
		if (current !== undefined && JSON.stringify(current) !== JSON.stringify(value)) {
			throw new Error(`Selected V27 finishing asset ${identity} has conflicting references.`);
		}
...
function assertLutReference(reference: VideoCubeLutReferenceV1, body: ParsedCubeLutV1): void {
	if (reference.sha256 !== body.sha256 || reference.byteLength !== body.byteLength
		...
		throw new RangeError('Selected V27 cube LUT body is missing, stale, or geometrically mismatched.');
	}
}
...
function assertSameSignal(expected: AbortSignal, actual: AbortSignal): void {
	if (actual !== expected) throw new TypeError('Selected V27 finishing requires its exact export AbortSignal.');
}
