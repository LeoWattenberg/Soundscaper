...
/** V30 image-route ceiling; inherited V27 media keeps independent resource ownership. */
export const FRAMESCAPER_IMAGE_PREVIEW_MAXIMUM_WORKING_BYTES_V30 = 512 * 1024 * 1024;
export const FRAMESCAPER_IMAGE_PREVIEW_MAXIMUM_SOURCES_V30 = 512;
export const FRAMESCAPER_IMAGE_PREVIEW_MAXIMUM_CONTEXTS_V30 = 4_096;

export interface FramescaperImagePreviewResourceContextV30 {
	readonly source: FramescaperImageSourceV1;
	readonly width: number;
	readonly height: number;
}

export interface FramescaperImagePreviewResourceBudgetV30 {
 ...
 */
export function admitFramescaperImageTimelinePreviewResourcesV30(
	contexts: readonly FramescaperImagePreviewResourceContextV30[],
): FramescaperImagePreviewResourceBudgetV30 {
...
export function admitFramescaperImageProjectBinThumbnailResourcesV30(
 ...
 */
export function admitFramescaperImageTimelineFilmstripResourcesV30(
	contexts: readonly FramescaperImagePreviewResourceContextV30[],
): FramescaperImagePreviewResourceBudgetV30 {
...
export function assertFramescaperImagePreviewReaderMetadataV30(
	sourceValue: FramescaperImageSourceV1,
	byteLength: number,
): void {
	...
	if (!Number.isSafeInteger(byteLength) || byteLength < 0
		...
		throw new RangeError(`V30 image reader ${source.id} exceeds its preflight metadata reservation.`);
	}
}

interface AdmittedContextsV30 {
	readonly contexts: readonly FramescaperImagePreviewResourceContextV30[];
	readonly sources: ReadonlyMap<string, FramescaperImagePreviewResourceContextV30>;
}

interface RouteResourcesV30 {
	readonly retainedFrameBytes: bigint;
	readonly drawableBackingBytes: bigint;
	readonly requestOutputBytes: bigint;
	readonly uniqueFrameCacheBytes: bigint;
...
function contextsAndSources(
	values: readonly FramescaperImagePreviewResourceContextV30[],
	route: string,
): AdmittedContextsV30 {
	if (!Array.isArray(values) || values.length > FRAMESCAPER_IMAGE_PREVIEW_MAXIMUM_CONTEXTS_V30) {
		throw new RangeError(`V30 image ${route} exceeds its context count bound.`);
	}
	const contexts = values.map((value) => Object.freeze({
		source: normalizeFramescaperImageSourceV1(value?.source),
		width: dimension(value?.width, `V30 image ${route} width`),
		height: dimension(value?.height, `V30 image ${route} height`),
	}));
	const sources = new Map<string, FramescaperImagePreviewResourceContextV30>();
	for (const context of contexts) {
		const prior = sources.get(context.source.id);
		if (prior && (prior.width !== context.width || prior.height !== context.height)) {
			throw new RangeError(`V30 image ${route} source geometry is ambiguous.`);
		}
		sources.set(context.source.id, context);
	}
	if (sources.size > FRAMESCAPER_IMAGE_PREVIEW_MAXIMUM_SOURCES_V30) {
		throw new RangeError(`V30 image ${route} exceeds its source count bound.`);
	}
...
function admit(
	route: string,
	admitted: AdmittedContextsV30,
	resources: RouteResourcesV30,
): FramescaperImagePreviewResourceBudgetV30 {
	...
	if (totalBytes > BigInt(FRAMESCAPER_IMAGE_PREVIEW_MAXIMUM_WORKING_BYTES_V30)) {
		throw new RangeError(`V30 image ${route} exceeds its 512 MiB working byte bound.`);
	}
...
function conservativeReaderMetadataBytes(source: FramescaperImageSourceV1): number {
	...
	if (availableReceiptBytes < 2) {
		throw new RangeError(`V30 image source ${source.id} cannot contain a canonical frame pack.`);
	}
...
function scaledFrameBytes(context: FramescaperImagePreviewResourceContextV30): bigint {
