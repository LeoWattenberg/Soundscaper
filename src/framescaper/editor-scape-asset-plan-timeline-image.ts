...
export const FRAMESCAPER_SCAPE_IMAGE_ASSET_KIND_TIMELINE_IMAGE = 'framescaper-image-asset' as const;
export const FRAMESCAPER_SCAPE_IMAGE_ASSET_ENCODING_TIMELINE_IMAGE = 'framescaper-image-asset-v1' as const;

export interface FramescaperScapeImageAssetReferenceTimelineImage {
	readonly source: FramescaperImageSourceV1;
	readonly sourceId: string;
	readonly archiveId: string;
	readonly kind: typeof FRAMESCAPER_SCAPE_IMAGE_ASSET_KIND_TIMELINE_IMAGE;
	readonly encoding: typeof FRAMESCAPER_SCAPE_IMAGE_ASSET_ENCODING_TIMELINE_IMAGE;
...
export interface FramescaperScapeImageImportValidationTimelineImage {
	readonly references: readonly FramescaperScapeImageAssetReferenceTimelineImage[];
...
/** Plan every timelineImage image source as one exact immutable archive body. */
export async function planFramescaperScapeImageExportAssetsTimelineImage(
	project: FramescaperProjectTimelineImage,
	store: MetadataStore,
	signal?: AbortSignal,
): Promise<readonly PlannedScapeExportAsset[]> {
	const assets: PlannedScapeExportAsset[] = [];
	for (const reference of collectFramescaperScapeImageAssetReferencesTimelineImage(project)) {
		throwIfScapeAborted(signal);
		const metadata = record(
			await awaitScapeOperation(store.getMediaAssetMetadata(reference.storageKey), signal),
			`timelineImage image ${reference.sourceId} metadata`,
		);
...
/** Admit an archive manifest only when it exactly closes over timelineImage image authority. */
export function validateFramescaperScapeImageImportAssetsTimelineImage(
	project: FramescaperProjectTimelineImage,
	manifest: ScapeManifest,
): Readonly<FramescaperScapeImageImportValidationTimelineImage> {
	const references = collectFramescaperScapeImageAssetReferencesTimelineImage(project);
	const descriptors = manifest.assets.filter(({ kind }) => kind === FRAMESCAPER_SCAPE_IMAGE_ASSET_KIND_TIMELINE_IMAGE);
	if (descriptors.length !== references.length) {
		throw new Error('The timelineImage Scape archive has an incomplete image asset inventory.');
	}
	const descriptorByArchiveId = new Map<string, ScapeAssetDescriptor>();
	for (const descriptor of descriptors) {
		if (descriptorByArchiveId.has(descriptor.sourceId)) {
			throw new Error(`The timelineImage Scape image asset ${descriptor.sourceId} is duplicated.`);
		}
	...
	for (const reference of references) {
		...
		if (!descriptor || descriptor.kind !== reference.kind || descriptor.entry !== reference.entry
			...
			throw new Error(`The timelineImage Scape image descriptor for ${reference.sourceId} conflicts with project authority.`);
		}
	}
...
export async function validateFramescaperImageAssetBodyTimelineImage(
	sourceValue: unknown,
	bodyValue: unknown,
	signal?: AbortSignal,
): Promise<Blob> {
	...
	if (body.size !== source.assetByteLength) {
		throw new Error(`timelineImage image body ${source.storageKey} has a conflicting size.`);
	}
...
export async function validateFramescaperScapeImageExportAssetBodyTimelineImage(
	asset: PlannedScapeExportAsset,
	body: Blob,
	signal?: AbortSignal,
): Promise<void> {
	const reference = plannedReference(asset);
	if (body.size !== asset.size) throw new Error('The timelineImage Scape image body changed size after admission.');
	await validateFramescaperImageAssetBodyTimelineImage(reference.source, body, signal);
}

export function collectFramescaperScapeImageAssetReferencesTimelineImage(
	project: FramescaperProjectTimelineImage,
): readonly FramescaperScapeImageAssetReferenceTimelineImage[] {
	const references: FramescaperScapeImageAssetReferenceTimelineImage[] = [];
	...
	for (const value of project.sources) {
		...
		if (archiveIds.has(reference.archiveId) || entries.has(reference.entry)
			...
			throw new Error(`The timelineImage Scape image ${source.id} has a conflicting archive identity.`);
		}
...
function imageReference(source: FramescaperImageSourceV1): FramescaperScapeImageAssetReferenceTimelineImage {
	return Object.freeze({
		source,
		sourceId: source.id,
		archiveId: `framescaper:image:${source.id}`,
		kind: FRAMESCAPER_SCAPE_IMAGE_ASSET_KIND_TIMELINE_IMAGE,
		encoding: FRAMESCAPER_SCAPE_IMAGE_ASSET_ENCODING_TIMELINE_IMAGE,
		entry: `framescaper/timelineImage/image/${safeScapeEntryId(source.id)}/body`,
...
function plannedReference(asset: PlannedScapeExportAsset): FramescaperScapeImageAssetReferenceTimelineImage {
	const source = record(asset.source, 'timelineImage planned image archive source');
	const candidate = record(source.imageAssetReference, 'timelineImage planned image archive reference');
	...
	if (JSON.stringify(candidate) !== JSON.stringify(expected)
		...
		throw new Error('The timelineImage planned image archive asset drifted from its exact reference.');
	}
	return expected;
}
...
function assertImageMetadata(
	...
	if (metadata.sourceId !== reference.storageKey || metadata.size !== reference.byteLength
		...
		throw new Error(`timelineImage image body ${reference.storageKey} is missing or stale.`);
	}
}
