...
export interface FramescaperSelectedTimelineFilmstripOptionsV30
...
export async function createFramescaperSelectedTimelineFilmstripV30(
	...
	const width = evenDimension(options.width, 'V30 timeline filmstrip width');
	const height = evenDimension(options.height, 'V30 timeline filmstrip height');
	...
	const images: ImageFilmstripContextV30[] = [];
	for (const [index, request] of frames.entries()) {
		...
		if (clip.sourceId !== request.sourceId) {
			throw new Error(`V30 timeline filmstrip clip ${clip.id} changed source authority.`);
		}
	...
	try {
		...
		if (imageResult.status === 'rejected' && inheritedResult.status === 'rejected') {
			throw new AggregateError(
				...
				'The V30 timeline filmstrip image and inherited routes both failed.',
			);
		}
		...
		if (inherited.length !== inheritedIndexes.length) {
			throw new Error('The inherited V30 timeline filmstrip returned an incomplete frame set.');
		}
...
function frameRequests(value: unknown): ProductVideoTimelineFilmstripRequest['frames'][number][] {
	if (!Array.isArray(value) || value.length > 4_096) {
		throw new RangeError('V30 timeline filmstrip requests must be a bounded array.');
	}
	return value.map((candidate, index) => {
		if (!candidate || typeof candidate !== 'object' || Array.isArray(candidate)) {
			throw new TypeError(`V30 timeline filmstrip request ${String(index)} must be an object.`);
		}
...
function stableId(value: unknown, name: string): string {
	if (typeof value !== 'string' || !/^[A-Za-z0-9][A-Za-z0-9._:-]{0,127}$/u.test(value)) {
		throw new TypeError(`V30 ${name} must be a stable ID.`);
	}
	return value;
}
...
function text(value: unknown, name: string): string {
	if (typeof value !== 'string' || value.length < 1 || value.length > 4_096) {
		throw new TypeError(`V30 ${name} must be bounded text.`);
	}
	return value;
}
