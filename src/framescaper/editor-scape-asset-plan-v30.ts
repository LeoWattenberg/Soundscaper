...
export const FRAMESCAPER_SCAPE_IMAGE_ASSET_KIND_V30 = 'framescaper-image-asset' as const;
export const FRAMESCAPER_SCAPE_IMAGE_ASSET_ENCODING_V30 = 'framescaper-image-asset-v1' as const;

export interface FramescaperScapeImageAssetReferenceV30 {
	readonly source: FramescaperImageSourceV1;
	readonly sourceId: string;
	readonly archiveId: string;
	readonly kind: typeof FRAMESCAPER_SCAPE_IMAGE_ASSET_KIND_V30;
	readonly encoding: typeof FRAMESCAPER_SCAPE_IMAGE_ASSET_ENCODING_V30;
...
export interface FramescaperScapeImageImportValidationV30 {
	readonly references: readonly FramescaperScapeImageAssetReferenceV30[];
...
/** Plan every V30 image source as one exact immutable archive body. */
export async function planFramescaperScapeImageExportAssetsV30(
	project: FramescaperProjectV30,
	store: MetadataStore,
	signal?: AbortSignal,
): Promise<readonly PlannedScapeExportAsset[]> {
	const assets: PlannedScapeExportAsset[] = [];
	for (const reference of collectFramescaperScapeImageAssetReferencesV30(project)) {
		throwIfScapeAborted(signal);
		const metadata = record(
			await awaitScapeOperation(store.getMediaAssetMetadata(reference.storageKey), signal),
			`V30 image ${reference.sourceId} metadata`,
		);
...
/** Admit an archive manifest only when it exactly closes over V30 image authority. */
export function validateFramescaperScapeImageImportAssetsV30(
	project: FramescaperProjectV30,
	manifest: ScapeManifest,
): Readonly<FramescaperScapeImageImportValidationV30> {
	const references = collectFramescaperScapeImageAssetReferencesV30(project);
	const descriptors = manifest.assets.filter(({ kind }) => kind === FRAMESCAPER_SCAPE_IMAGE_ASSET_KIND_V30);
	if (descriptors.length !== references.length) {
		throw new Error('The V30 Scape archive has an incomplete image asset inventory.');
	}
	const descriptorByArchiveId = new Map<string, ScapeAssetDescriptor>();
	for (const descriptor of descriptors) {
		if (descriptorByArchiveId.has(descriptor.sourceId)) {
			throw new Error(`The V30 Scape image asset ${descriptor.sourceId} is duplicated.`);
		}
	...
	for (const reference of references) {
		...
		if (!descriptor || descriptor.kind !== reference.kind || descriptor.entry !== reference.entry
			...
			throw new Error(`The V30 Scape image descriptor for ${reference.sourceId} conflicts with project authority.`);
		}
	}
...
export async function validateFramescaperImageAssetBodyV30(
	sourceValue: unknown,
	bodyValue: unknown,
	signal?: AbortSignal,
): Promise<Blob> {
	...
	if (body.size !== source.assetByteLength) {
		throw new Error(`V30 image body ${source.storageKey} has a conflicting size.`);
	}
...
export async function validateFramescaperScapeImageExportAssetBodyV30(
	asset: PlannedScapeExportAsset,
	body: Blob,
	signal?: AbortSignal,
): Promise<void> {
	const reference = plannedReference(asset);
	if (body.size !== asset.size) throw new Error('The V30 Scape image body changed size after admission.');
	await validateFramescaperImageAssetBodyV30(reference.source, body, signal);
}

export function collectFramescaperScapeImageAssetReferencesV30(
	project: FramescaperProjectV30,
): readonly FramescaperScapeImageAssetReferenceV30[] {
	const references: FramescaperScapeImageAssetReferenceV30[] = [];
	...
	for (const value of project.sources) {
		...
		if (archiveIds.has(reference.archiveId) || entries.has(reference.entry)
			...
			throw new Error(`The V30 Scape image ${source.id} has a conflicting archive identity.`);
		}
...
function imageReference(source: FramescaperImageSourceV1): FramescaperScapeImageAssetReferenceV30 {
	return Object.freeze({
		source,
		sourceId: source.id,
		archiveId: `framescaper-v30:image:${source.id}`,
		kind: FRAMESCAPER_SCAPE_IMAGE_ASSET_KIND_V30,
		encoding: FRAMESCAPER_SCAPE_IMAGE_ASSET_ENCODING_V30,
		entry: `framescaper/v30/image/${safeScapeEntryId(source.id)}/body`,
...
function plannedReference(asset: PlannedScapeExportAsset): FramescaperScapeImageAssetReferenceV30 {
	const source = record(asset.source, 'V30 planned image archive source');
	const candidate = record(source.imageAssetReference, 'V30 planned image archive reference');
	...
	if (JSON.stringify(candidate) !== JSON.stringify(expected)
		...
		throw new Error('The V30 planned image archive asset drifted from its exact reference.');
	}
	return expected;
}
...
function assertImageMetadata(
	...
	if (metadata.sourceId !== reference.storageKey || metadata.size !== reference.byteLength
		...
		throw new Error(`V30 image body ${reference.storageKey} is missing or stale.`);
	}
}
