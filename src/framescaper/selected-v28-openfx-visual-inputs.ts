...
import type { createFramescaperSelectedOpenFxExactPlanesV28 } from './selected-v28-openfx-exact-planes.ts';

export async function materializeFramescaperSelectedOpenFxVisualsV28(
	entries: readonly UnifiedExactRenderVisualFrameEntryV13[],
	stills: ReadonlyMap<string, UnifiedExactRenderVisualRgbaV13>,
	width: number,
	height: number,
	signal: AbortSignal,
	openFx: ReturnType<typeof createFramescaperSelectedOpenFxExactPlanesV28> | null,
	...
	for (const entry of entries) {
		...
		if (external && !openFx?.has('generator', sourceId) && !openFx?.has('general', sourceId)) {
			throw new Error('An external generator requires an exact selected V28 OpenFX node.');
		}
...
export function orderedFramescaperSelectedOpenFxVisualEntriesV28(
	...
	while (output.length < values.length) {
		...
		if (ready.length === 0) throw new Error('Selected V28 external-generator dependencies cannot be ordered.');
...
export function framescaperSelectedOpenFxFrozenFrameResolverV28(
	plan: FramescaperSelectedOpenFxExecutionV28['plan'],
	sourceFrames: FramescaperVideoFrameAddressV27,
): NonNullable<FramescaperSelectedOpenFxExecutionV28['resolveFrozenFrame']> {
