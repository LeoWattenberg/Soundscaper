...
import {
	...
	type FramescaperExactSequenceFrameSequence,
	type FramescaperFlattenedSequenceClipSequence,
} from './editor-project-sequence-nested-sequence.ts';
import { assertFramescaperProjectSequenceProfile } from './editor-domain-runtime-profile.ts';
import { materializeFramescaperNestedVideoRetimeSequence } from './editor-project-sequence-nested-retime.ts';
import {
...
const ID_PREFIX = 'framescaper-flat';
...
interface MaterializedClipOccurrence {
	readonly occurrence: Readonly<FramescaperFlattenedSequenceClipSequence>;
 ...
 * Persisted sequence identity and aliases remain untouched; each reachable occurrence
 * receives deterministic transient clip and track IDs on the primary frame grid.
 */
export function materializeFramescaperNestedPlaybackFoundationSequence(
	...
	const flattened = flattenFramescaperSequenceSequence(profile, project, project.primarySequenceId);
...
function materializeClipOccurrence(
	...
	occurrence: Readonly<FramescaperFlattenedSequenceClipSequence>,
...
function materializeVideoClip(
	...
	const retimeMap = materializeFramescaperNestedVideoRetimeSequence({
...
function sequenceFrameToSample(
	frame: Readonly<FramescaperExactSequenceFrameSequence>,
...
function exactSafeInteger(value: FramescaperExactSequenceFrameSequence, name: string): number {
...
function occurrenceIdentity(occurrence: Readonly<FramescaperFlattenedSequenceClipSequence>): string {
...
function trackOccurrenceIdentity(occurrence: Readonly<FramescaperFlattenedSequenceClipSequence>): string {
	const key = JSON.stringify([
		occurrence.trackId,
		occurrence.sequencePath,
		occurrence.subsequencePath,
...
function sequenceById(project: FramescaperProjectSequence, id: string): Readonly<Record<string, unknown>> {
