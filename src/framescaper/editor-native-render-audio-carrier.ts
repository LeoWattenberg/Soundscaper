...
export interface FramescaperNativeAudioCarrierStreamSinkNativeMedia {
	write(bytes: Uint8Array): PromiseLike<void> | void;
}

export interface FramescaperNativeAudioCarrierStreamResultNativeMedia {
	readonly byteLength: number;
	readonly sha256: string;
	readonly chunkCount: number;
}

export interface FramescaperNativeAudioCarrierOptionsNativeMedia {
...
export async function createFramescaperNativeAudioCarrierNativeMedia(
	plan: UnifiedExactRenderPlanV14,
	project: Readonly<Record<string, unknown>>,
	operation: ProductNativeRenderInputOperation,
	options: FramescaperNativeAudioCarrierOptionsNativeMedia = {},
	...
	try {
		const streamed = await streamFramescaperNativeAudioCarrierNativeMedia(
		...
		if (completed.byteLength !== streamed.byteLength || completed.sha256 !== streamed.sha256) {
			throw new Error('Selected nativeMedia durable audio spool changed its live stream identity.');
		}
		...
		try { await spool.abort(); }
			...
			throw new AggregateError([error, cleanupError], 'nativeMedia PCM rendering and spool cleanup failed.', { cause: error });
		}
		throw error;
	}
}
...
export function framescaperNativeAudioCarrierNativeMediaByteLength(
	plan: UnifiedExactRenderPlanV14,
	project: Readonly<Record<string, unknown>>,
): number {
	...
	if (!Number.isSafeInteger(layout.byteLength) || layout.byteLength > MAXIMUM_AUDIO_BYTES) {
		throw new RangeError('Selected nativeMedia float32 WAV exceeds its 16 TiB live reservation.');
	}
	return layout.byteLength;
}

/** Stream float32 WAV directly into the acknowledged main/helper input reservation. */
export async function streamFramescaperNativeAudioCarrierNativeMedia(
	plan: UnifiedExactRenderPlanV14,
	project: Readonly<Record<string, unknown>>,
	operation: ProductNativeRenderInputOperation,
	...
	if (!sink || typeof sink.write !== 'function' || typeof operation.renderAudioToSink !== 'function') {
		throw new Error('Selected nativeMedia bounded PCM stream authority is unavailable.');
	}
	const audioLayout = exactAudioLayout(plan);
	const channelCount = outputChannelCount(project, audioLayout);
	const byteLength = framescaperNativeAudioCarrierNativeMediaByteLength(plan, project);
	...
	const result = await operation.renderAudioToSink(project, renderRange(plan), async (channels, metadata) => {
		...
		if (mapped.length !== channelCount || mapped.some((channel) => channel.length !== frames)) {
			throw new Error('Selected nativeMedia PCM mapping changed exact channel geometry.');
		}
	...
	if (result.sampleRate !== plan.timebase.sampleRate
		...
		throw new Error('Selected nativeMedia audio streaming changed exact render geometry.');
	}
	...
	if (!finalized || typeof finalized !== 'object' || finalized.byteLength !== byteLength
		...
		throw new Error('Selected nativeMedia float32 WAV length changed.');
	}
...
function exactAudioLayout(plan: UnifiedExactRenderPlanV14): 'mono' | 'stereo' | 'preserve' {
	if (!plan.output.includeAudio || plan.codecs.audio !== 'pcm_s16le'
		...
		throw new Error('Selected nativeMedia audio carrier is not bound to exact PCM/MOV authority.');
	}
...
function outputChannelCount(project: Readonly<Record<string, unknown>>, layout: 'mono' | 'stereo' | 'preserve'): number {
	...
	if (!Number.isSafeInteger(masterChannels) || masterChannels < 1 || masterChannels > 32) {
		throw new RangeError('Selected nativeMedia audio project has invalid master-channel geometry.');
	}
...
function exactPcmChunk(
	...
	if (!Array.isArray(channels) || channels.length < 1 || channels.length > 32
		...
		throw new Error('Selected nativeMedia audio renderer emitted a non-dense PCM chunk.');
	}
...
function assertReady(operation: ProductNativeRenderInputOperation): void {
	if (operation.signal.aborted) throw operation.signal.reason
		?? new DOMException('nativeMedia audio carrier production was cancelled.', 'AbortError');
