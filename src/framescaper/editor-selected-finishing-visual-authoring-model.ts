...
export type FramescaperSelectedVisualAuthoringSurface = FramescaperSelectedVisualAuthoringSurfaceFinishing;
export const createFramescaperSelectedVisualAuthoringModel =
	createFramescaperSelectedVisualAuthoringModelFinishing;

type Data = Readonly<Record<string, unknown>>;

export const FRAMESCAPER_SELECTED_FINISHING_DIALOG_AUTHORING_SURFACES = Object.freeze([
	'video-transition', 'video-transition-dissolve', 'video-adjustment-layer',
	'video-visual-preset', 'video-mask-matte', 'video-freeze',
] as const);

export type FramescaperSelectedVisualAuthoringSurfaceFinishing =
	(typeof FRAMESCAPER_SELECTED_FINISHING_DIALOG_AUTHORING_SURFACES)[number];

export interface FramescaperSelectedVisualAuthoringFenceFinishing {
...
export interface FramescaperSelectedTransitionPairFinishing {
	readonly id: string;
...
export interface FramescaperSelectedVisualAuthoringModelFinishing {
...
export function createFramescaperSelectedVisualAuthoringFenceFinishing(input: Readonly<{
	readonly project: unknown;
	readonly selectedClipId: string | null;
	readonly playheadSample: number;
}>): FramescaperSelectedVisualAuthoringFenceFinishing {
	assertFramescaperProjectIdentity(input.project);
	const project = record(input.project, 'selected finishing authoring project');
...
export function assertFramescaperSelectedVisualAuthoringFenceFinishing(
	projectValue: unknown,
	fenceValue: unknown,
	selectedClipId?: string | null,
): asserts fenceValue is FramescaperSelectedVisualAuthoringFenceFinishing {
	assertFramescaperProjectIdentity(projectValue);
	const project = record(projectValue, 'selected finishing authoring project');
	const fence = record(fenceValue, 'selected finishing authoring fence');
	if (fence.schemaFamily !== FRAMESCAPER_PROJECT_SCHEMA_FAMILY
		|| fence.schemaVersion !== PROJECT_SCHEMA_VERSION
		|| fence.projectId !== project.id || fence.projectRevision !== project.revision) {
...
export function assertFramescaperSelectedVisualAuthoringRuntimeFenceFinishing(input: Readonly<{
	readonly project: unknown;
	...
	assertFramescaperSelectedVisualAuthoringFenceFinishing(input.project, input.fence, selectedClipId);
	const fence = input.fence as FramescaperSelectedVisualAuthoringFenceFinishing;
...
function createTransitionPairs(project: Data, clips: readonly Data[]): readonly FramescaperSelectedTransitionPairFinishing[] {
...
function authoringSurface(value: unknown): FramescaperSelectedVisualAuthoringSurfaceFinishing {
	if (!FRAMESCAPER_SELECTED_FINISHING_DIALOG_AUTHORING_SURFACES.includes(value as never)) {
		throw new RangeError('The selected finishing visual authoring surface is unsupported.');
	}
	return value as FramescaperSelectedVisualAuthoringSurfaceFinishing;
}
