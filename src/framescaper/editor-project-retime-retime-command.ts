...
export type FramescaperVideoRetimeScopeRetime = 'timeline' | 'project-bin';

interface FramescaperVideoRetimeCommandBaseRetime {
	readonly scope: FramescaperVideoRetimeScopeRetime;
	readonly clipId: string;
	readonly expectedRetimeMap: VideoRetimeCurveV16 | null;
}

export type FramescaperVideoRetimeCommandRetime = Readonly<
	| (FramescaperVideoRetimeCommandBaseRetime & {
		readonly type: 'video-retime/set';
		readonly retimeMap: VideoRetimeCurveV16;
	})
	| (FramescaperVideoRetimeCommandBaseRetime & { readonly type: 'video-retime/reset' })
	| (FramescaperVideoRetimeCommandBaseRetime & { readonly type: 'video-retime/constant' })
	| (FramescaperVideoRetimeCommandBaseRetime & { readonly type: 'video-retime/reverse' })
	| (FramescaperVideoRetimeCommandBaseRetime & {
...
type CommandType = FramescaperVideoRetimeCommandRetime['type'];
type CommandInput<T extends CommandType> = Omit<Extract<FramescaperVideoRetimeCommandRetime, {
	readonly type: T;
}>, 'type' | 'scope'> & Readonly<{ scope?: FramescaperVideoRetimeScopeRetime }>;

const BASE_FIELDS = ['type', 'scope', 'clipId', 'expectedRetimeMap'] as const;

export function createFramescaperVideoRetimeSetCommandRetime(
	value: CommandInput<'video-retime/set'> | unknown,
): Extract<FramescaperVideoRetimeCommandRetime, { readonly type: 'video-retime/set' }> {
	return createCommand('video-retime/set', value);
}

export function createFramescaperVideoRetimeResetCommandRetime(
	value: CommandInput<'video-retime/reset'> | unknown,
): Extract<FramescaperVideoRetimeCommandRetime, { readonly type: 'video-retime/reset' }> {
	return createCommand('video-retime/reset', value);
}

export function createFramescaperVideoRetimeConstantCommandRetime(
	value: CommandInput<'video-retime/constant'> | unknown,
): Extract<FramescaperVideoRetimeCommandRetime, { readonly type: 'video-retime/constant' }> {
	return createCommand('video-retime/constant', value);
}

export function createFramescaperVideoRetimeReverseCommandRetime(
	value: CommandInput<'video-retime/reverse'> | unknown,
): Extract<FramescaperVideoRetimeCommandRetime, { readonly type: 'video-retime/reverse' }> {
	return createCommand('video-retime/reverse', value);
}

export function createFramescaperVideoRetimeFreezeCommandRetime(
	value: CommandInput<'video-retime/freeze'> | unknown,
): Extract<FramescaperVideoRetimeCommandRetime, { readonly type: 'video-retime/freeze' }> {
	return createCommand('video-retime/freeze', value);
}

export function createFramescaperVideoRetimeRampCommandRetime(
	value: CommandInput<'video-retime/ramp'> | unknown,
): Extract<FramescaperVideoRetimeCommandRetime, { readonly type: 'video-retime/ramp' }> {
...
export function snapshotFramescaperVideoRetimeCommandRetime(
	value: FramescaperVideoRetimeCommandRetime | unknown,
): FramescaperVideoRetimeCommandRetime {
	const record = plainRecord(value, 'Framescaper retime video-retime command');
	const type = dataField(record, 'type', 'Framescaper retime video-retime command');
	if (!isFramescaperVideoRetimeCommandTypeRetime(type)) {
		throw new RangeError('Framescaper retime video-retime command.type is unsupported.');
	}
	return createCommand(type, record, true);
}

export function isFramescaperVideoRetimeCommandRetime(
	value: unknown,
): value is FramescaperVideoRetimeCommandRetime {
	if (!value || typeof value !== 'object' || Array.isArray(value)) return false;
	const descriptor = Object.getOwnPropertyDescriptor(value, 'type');
	return Boolean(descriptor?.enumerable && Object.hasOwn(descriptor, 'value')
		&& isFramescaperVideoRetimeCommandTypeRetime(descriptor.value));
}

/** Resolve one authored operation against the occurrence's exact persisted bounds. */
export function resolveFramescaperVideoRetimeMapRetime(
...
function createCommand<T extends CommandType>(
	...
	serialized = false,
): Extract<FramescaperVideoRetimeCommandRetime, { readonly type: T }> {
	const name = 'Framescaper retime video-retime command';
	...
	if (type === 'video-retime/set') {
		...
		return Object.freeze({ ...base, retimeMap }) as Extract<FramescaperVideoRetimeCommandRetime, { readonly type: T }>;
	}
	if (type === 'video-retime/freeze') {
		return Object.freeze({
			...base, sourceFrame: inputRational(dataField(record, 'sourceFrame', name), `${name}.sourceFrame`),
		}) as Extract<FramescaperVideoRetimeCommandRetime, { readonly type: T }>;
	}
	...
	return Object.freeze(base) as Extract<FramescaperVideoRetimeCommandRetime, { readonly type: T }>;
}
...
function isFramescaperVideoRetimeCommandTypeRetime(value: unknown): value is CommandType {
...
function scopeValue(value: unknown): FramescaperVideoRetimeScopeRetime {
	if (value !== 'timeline' && value !== 'project-bin') {
		throw new RangeError('Framescaper retime video-retime command.scope must be timeline or project-bin.');
	}
	return value;
}
...
function identifier(value: unknown): string {
	if (typeof value !== 'string' || value.length === 0 || value !== value.trim()) {
		throw new TypeError('Framescaper retime video-retime command.clipId must be a canonical non-empty string.');
	}
	return value;
}
