...
export const FRAMESCAPER_SCAPE_ASSET_KINDS = Object.freeze([
...
export type FramescaperScapeAssetKind = typeof FRAMESCAPER_SCAPE_ASSET_KINDS[number];

type AssetRole = 'still' | 'freeze-render' | 'proxy' | 'proxy-timing' | 'lut' | 'motion';

export interface FramescaperScapeAssetReference {
...
export interface FramescaperScapeImportValidation {
...
export async function planFramescaperScapeExportAssets(
	...
	const references = collectFramescaperScapeAssetReferences(project);
	...
	for (const reference of references) {
		...
		const metadata = record(
			...
			`Framescaper ${reference.role} archive metadata`,
		);
		const size = positiveInteger(metadata.size, `Framescaper ${reference.role} archive size`);
		if (size > reference.maximumBytes
			|| (reference.byteLength !== null && size !== reference.byteLength)
			|| metadata.sha256 !== reference.sha256) {
			throw new Error(`Framescaper ${reference.role} body ${reference.storageKey} is missing or stale.`);
		}
		const storedMime = String(metadata.mimeType ?? '');
		if (storedMime && storedMime !== reference.mimeType) {
			throw new Error(`Framescaper ${reference.role} body ${reference.storageKey} has a conflicting media type.`);
		}
...
export function validateFramescaperScapeImportAssets(
	...
	const references = collectFramescaperScapeAssetReferences(project);
	const kinds = new Set<string>(FRAMESCAPER_SCAPE_ASSET_KINDS);
	const descriptors = manifest.assets.filter(({ kind }) => kinds.has(kind));
	if (descriptors.length !== references.length) {
		throw new Error('The Framescaper Scape archive has an incomplete durable finishing asset inventory.');
	}
	const descriptorByArchiveId = new Map<string, ScapeAssetDescriptor>();
	for (const descriptor of descriptors) {
		if (descriptorByArchiveId.has(descriptor.sourceId)) {
			throw new Error(`The Framescaper Scape asset ${descriptor.sourceId} is duplicated.`);
		}
	...
	for (const reference of references) {
		...
		if (!descriptor || descriptor.kind !== reference.kind
			...
			throw new Error(`The Framescaper Scape ${reference.role} descriptor is missing or conflicts with project authority.`);
		}
	}
...
export async function validateFramescaperScapeExportAssetBody(
	...
	if (body.size !== asset.size || body.size > reference.maximumBytes
		...
		throw new Error(`The Framescaper ${reference.role} archive body changed after admission.`);
	}
...
export function collectFramescaperScapeAssetReferences(
	...
	const references: FramescaperScapeAssetReference[] = [];
	const byArchiveId = new Map<string, FramescaperScapeAssetReference>();
	const byEntry = new Map<string, FramescaperScapeAssetReference>();
	const byStorageKey = new Map<string, FramescaperScapeAssetReference>();
	...
	const stackById = new Map(records(project.videoProcessorStacks, 'videoProcessorStacks')
		.map((stack) => [String(stack.id), stack as unknown as VideoProcessorStackV1]));
	for (const value of records(project.videoMotionAnalyses, 'videoMotionAnalyses')) {
		const motion = value as unknown as VideoMotionAnalysisReferenceV1;
		const stack = stackById.get(motion.processorStackId);
		if (!stack) throw new Error(`Framescaper motion analysis ${motion.id} has no processor stack.`);
...
function stillReference(
	...
	freeze: boolean,
): FramescaperScapeAssetReference {
	const sourceId = stableId(source.id, 'Framescaper still source ID');
	const mimeType = String(source.mimeType);
	if (!STILL_MIME.test(mimeType)) throw new TypeError(`Framescaper still ${sourceId} has an invalid media type.`);
	const role = freeze ? 'freeze-render' as const : 'still' as const;
	const digest = sha(source.contentSha256, `Framescaper ${role} digest`);
	return reference({
		...
		mimeType, storageKey: stableId(source.storageKey, `Framescaper ${role} storage key`),
...
function proxyReference(attachment: Readonly<VideoProxyAttachmentV18>): FramescaperScapeAssetReference {
	return reference({
...
function proxyTimingReference(timing: Readonly<VideoTimingAssetReference>): FramescaperScapeAssetReference {
	return reference({
...
function lutReference(lut: Readonly<VideoCubeLutReferenceV1>): FramescaperScapeAssetReference {
	return reference({
...
function addReference(
	value: FramescaperScapeAssetReference,
	references: FramescaperScapeAssetReference[],
	byArchiveId: Map<string, FramescaperScapeAssetReference>,
	byEntry: Map<string, FramescaperScapeAssetReference>,
	byStorageKey: Map<string, FramescaperScapeAssetReference>,
): void {
	const matches = [byArchiveId.get(value.archiveId), byEntry.get(value.entry), byStorageKey.get(value.storageKey)]
		.filter((candidate): candidate is FramescaperScapeAssetReference => candidate !== undefined);
	if (matches.length > 0) {
		if (matches.every((candidate) => sameReference(candidate, value))) return;
		throw new Error(`Framescaper Scape asset ${value.archiveId} has a conflicting identity or role.`);
	}
...
function sameReference(left: FramescaperScapeAssetReference, right: FramescaperScapeAssetReference): boolean {
	return JSON.stringify(left) === JSON.stringify(right);
}

function reference(value: FramescaperScapeAssetReference): FramescaperScapeAssetReference {
	if (!SHA256.test(value.sha256) || value.maximumBytes < 1
		...
		throw new RangeError(`Framescaper ${value.role} archive reference exceeds its digest or size bound.`);
	}
	return Object.freeze({ ...value });
}

function plannedReference(asset: PlannedScapeExportAsset): FramescaperScapeAssetReference {
	const source = record(asset.source, 'Framescaper planned archive source');
	const referenceValue = source.archiveReference;
	if (!referenceValue || typeof referenceValue !== 'object' || Array.isArray(referenceValue)) {
		throw new TypeError('The Framescaper planned archive asset lost its exact reference.');
	}
	const reference = referenceValue as FramescaperScapeAssetReference;
	if (reference.archiveId !== asset.sourceId || reference.kind !== asset.kind
		...
		throw new Error('The Framescaper planned archive asset drifted from its reference.');
	}
	return reference;
}

function validateLut(reference: FramescaperScapeAssetReference, bytes: Uint8Array): void {
	...
	if (parsed.sha256 !== expected.sha256 || parsed.byteLength !== expected.byteLength
		...
		throw new Error('The Framescaper cube LUT archive body conflicts with its project reference.');
	}
}

function validateMotion(reference: FramescaperScapeAssetReference, bytes: Uint8Array): void {
