...
/** Closed menu intent for selected-baseline pathless import and native delivery. */
...
export interface FramescaperNativeImageSequenceImportRequestNativeMedia {
	readonly frameRate: NativeMediaImageSequenceRateV1;
}

export const FRAMESCAPER_NATIVE_MEDIA_IMAGE_SEQUENCE_EXPORT_FORMATS = Object.freeze([
	'png', 'tiff', 'openexr',
] as const);

export type FramescaperNativeMediaImageSequenceExportFormat =
	(typeof FRAMESCAPER_NATIVE_MEDIA_IMAGE_SEQUENCE_EXPORT_FORMATS)[number];

export type FramescaperNativeMediaImageSequenceEncodeProfile =
	| 'encode-png-sequence' | 'encode-tiff-sequence' | 'encode-openexr-sequence';

export type FramescaperNativeRenderDeliveryRequestNativeMedia =
	| Readonly<{ readonly kind: 'encoded-mov' }>
	| Readonly<{
		readonly kind: 'image-sequence';
		readonly format: FramescaperNativeMediaImageSequenceExportFormat;
		readonly frameRate: NativeMediaImageSequenceRateV1;
		readonly preserveAlpha: true;
	}>;

export interface FramescaperNativeMediaImageSequenceDeliveryDescriptor {
	readonly format: FramescaperNativeMediaImageSequenceExportFormat;
	readonly profile: FramescaperNativeMediaImageSequenceEncodeProfile;
...
const IMAGE_SEQUENCE_DESCRIPTORS: Readonly<Record<
	FramescaperNativeMediaImageSequenceExportFormat,
	FramescaperNativeMediaImageSequenceDeliveryDescriptor
>> = Object.freeze({
...
export function snapshotFramescaperNativeImageSequenceImportRequestNativeMedia(
	value: unknown,
): FramescaperNativeImageSequenceImportRequestNativeMedia {
	const request = exactRecord(value, ['frameRate'], 'user-selected nativeMedia image-sequence import request');
...
export function snapshotFramescaperNativeRenderDeliveryRequestNativeMedia(
	value: unknown,
): FramescaperNativeRenderDeliveryRequestNativeMedia {
	if (value === undefined) return Object.freeze({ kind: 'encoded-mov' as const });
	const candidate = exactRecord(value, undefined, 'selected nativeMedia native delivery request');
	const kind = ownData(candidate, 'kind', 'selected nativeMedia native delivery request');
	if (kind === 'encoded-mov') {
		exactFields(candidate, ['kind'], 'selected nativeMedia MOV delivery request');
		return Object.freeze({ kind: 'encoded-mov' as const });
	}
	if (kind !== 'image-sequence') {
		throw new RangeError('The selected nativeMedia native delivery format is unsupported.');
	}
	exactFields(
		candidate, ['kind', 'format', 'frameRate', 'preserveAlpha'],
		'selected nativeMedia image-sequence delivery request',
	);
	const format = imageSequenceFormat(candidate.format);
	if (candidate.preserveAlpha !== true) {
		throw new RangeError('Selected nativeMedia image-sequence delivery must preserve alpha.');
	}
	const frameRate = exactRate(candidate.frameRate, 'image-sequence delivery frame rate');
	const descriptor = framescaperNativeMediaImageSequenceDeliveryDescriptor(format);
	...
	if (!dispatch.imageSequence || !dispatch.supportsAlpha || dispatch.audioEncoder !== null
		...
		throw new Error('The selected nativeMedia image-sequence profile lost its alpha output-tree authority.');
	}
...
export function framescaperNativeMediaNativeDeliveryProfile(
	...
	return delivery.kind === 'encoded-mov'
		...
		: framescaperNativeMediaImageSequenceDeliveryDescriptor(delivery.format).profile;
}

export function framescaperNativeMediaImageSequenceDeliveryDescriptor(
	format: FramescaperNativeMediaImageSequenceExportFormat,
): FramescaperNativeMediaImageSequenceDeliveryDescriptor {
	const row = IMAGE_SEQUENCE_DESCRIPTORS[format];
	if (!row) throw new RangeError('The selected nativeMedia image-sequence output format is unsupported.');
	return row;
}

/** Reconstruct only a selected menu delivery from an already normalized V14 plan. */
export function framescaperNativeRenderDeliveryRequestFromPlanNativeMedia(
	...
	if (!descriptor) throw new RangeError('The V14 plan is not a selected nativeMedia native delivery.');
	return snapshotFramescaperNativeRenderDeliveryRequestNativeMedia({
...
function descriptor(
	format: FramescaperNativeMediaImageSequenceExportFormat,
	profile: FramescaperNativeMediaImageSequenceEncodeProfile,
	extension: FramescaperNativeMediaImageSequenceDeliveryDescriptor['extension'],
	mimeType: FramescaperNativeMediaImageSequenceDeliveryDescriptor['mimeType'],
): FramescaperNativeMediaImageSequenceDeliveryDescriptor {
	return Object.freeze({ format, profile, extension, mimeType });
}

function imageSequenceFormat(value: unknown): FramescaperNativeMediaImageSequenceExportFormat {
	if (typeof value !== 'string'
		|| !(FRAMESCAPER_NATIVE_MEDIA_IMAGE_SEQUENCE_EXPORT_FORMATS as readonly string[]).includes(value)) {
		throw new RangeError('The selected nativeMedia image-sequence output format is unsupported.');
	}
	return value as FramescaperNativeMediaImageSequenceExportFormat;
}
