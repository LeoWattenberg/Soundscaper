...
export interface FramescaperAudioEditorControllerPresentationV19 {
...
/**
 * Bind the common editor controller to the exact V19 browser environment.
 * V18 nested-sequence and multicamera action factories remain the inherited
 * command surface; all command execution itself belongs to the V19 runtime.
 */
export function createFramescaperAudioEditorControllerV19(
	environmentValue: FramescaperEditorProjectEnvironmentV19 | unknown,
	presentationValue: FramescaperAudioEditorControllerPresentationV19 | unknown = {},
	...
	const scapeProjectRuntime = createFramescaperScapeNativeRuntimeV19(environment.runtime.profile);
...
function snapshotPresentation(value: unknown): FramescaperAudioEditorControllerPresentationV19 {
	if (value === null || typeof value !== 'object' || Array.isArray(value)) {
		throw new TypeError('Framescaper V19 controller presentation must be a plain record.');
	}
	...
	if (keys.some((key) => typeof key !== 'string' || !PRESENTATION_FIELDS.includes(
		...
		throw new TypeError('Framescaper V19 controller presentation contains an unsupported authority option.');
	}
	...
	for (const key of PRESENTATION_FIELDS) {
		...
		if (!descriptor.enumerable || !Object.hasOwn(descriptor, 'value')) {
			throw new TypeError(`Framescaper V19 controller presentation ${key} must be an own data property.`);
		}
		output[key] = descriptor.value;
	}
	if (output.locale !== undefined && typeof output.locale !== 'string') {
		throw new TypeError('Framescaper V19 controller locale must be a string.');
	}
	if (output.copy !== undefined && (
		!output.copy || typeof output.copy !== 'object' || Array.isArray(output.copy)
	)) {
		throw new TypeError('Framescaper V19 controller copy must be an object.');
	}
	return output as FramescaperAudioEditorControllerPresentationV19;
}
