...
export type FramescaperMotionAnalysisProgress = FramescaperMotionAnalysisProgressFinishing;
export type FramescaperMotionAnalysisTarget = FramescaperMotionAnalysisTargetFinishing;
export const framescaperMotionAnalysisActionsFor = framescaperMotionAnalysisActionsFinishingFor;

export type FramescaperMotionAnalysisPhaseFinishing = 'decoding' | 'tracking' | 'publishing' | 'complete';

export interface FramescaperMotionAnalysisProgressFinishing {
	readonly phase: FramescaperMotionAnalysisPhaseFinishing;
	readonly completed: number;
	readonly total: number;
}

export interface FramescaperMotionAnalysisTargetFinishing {
...
export interface FramescaperMotionAnalysisFrameRequestFinishing {
	readonly projectId: string;
	readonly source: Readonly<Record<string, unknown>>;
	readonly startFrame: number;
	readonly endFrame: number;
	readonly signal?: AbortSignal;
	readonly onProgress: (progress: FramescaperMotionAnalysisProgressFinishing) => void;
}

export type FramescaperMotionAnalysisFrameProviderFinishing = (
	request: FramescaperMotionAnalysisFrameRequestFinishing,
...
interface MotionAnalysisOwnerFinishing {
...
interface MotionAnalysisStoreFinishing {
	...
	): PromiseLike<MotionAnalysisWriterFinishing>;
}

interface MotionAnalysisWriterFinishing {
	...
	commitOwned(options?: Readonly<{ readonly signal?: AbortSignal }>): PromiseLike<MotionAnalysisPublicationFinishing>;
	abort(): PromiseLike<void>;
}

interface MotionAnalysisPublicationFinishing {
	readonly metadata: Readonly<Record<string, unknown>>;
	discardIfCurrent(): PromiseLike<boolean>;
}

export interface FramescaperMotionAnalysisActionsFinishing {
	targets(): readonly FramescaperMotionAnalysisTargetFinishing[];
...
const RUNTIMES = new WeakMap<object, FramescaperMotionAnalysisActionsFinishing>();

export function createFramescaperMotionAnalysisActionsFinishing(options: Readonly<{
	readonly owner: MotionAnalysisOwnerFinishing;
	readonly store: MotionAnalysisStoreFinishing;
	readonly frameProvider: FramescaperMotionAnalysisFrameProviderFinishing;
}>): FramescaperMotionAnalysisActionsFinishing {
	if (!options.owner || typeof options.owner !== 'object'
		|| typeof options.owner.actions?.edit?.commit !== 'function') {
		throw new TypeError('Selected finishing motion analysis requires a controller owner.');
	}
	if (!options.store || typeof options.store.getMediaAssetMetadata !== 'function'
		|| typeof options.store.beginMediaAssetWrite !== 'function') {
		throw new TypeError('Selected finishing motion analysis requires an exact asset store.');
	}
	if (typeof options.frameProvider !== 'function') {
		throw new TypeError('Selected finishing motion analysis requires a frame provider.');
	}
	let active = false;
	const runtime: FramescaperMotionAnalysisActionsFinishing = Object.freeze({
		targets: () => targets(options.owner.project),
		analyze: async (request: Parameters<FramescaperMotionAnalysisActionsFinishing['analyze']>[0]) => {
			if (active) throw new Error('A selected finishing motion analysis is already running.');
			active = true;
			try {
...
export function bindFramescaperMotionAnalysisActionsFinishing(
	owner: object,
	runtime: FramescaperMotionAnalysisActionsFinishing,
): void {
	if (!owner || typeof owner !== 'object') throw new TypeError('A finishing motion-analysis owner is required.');
	RUNTIMES.set(owner, runtime);
}

export function framescaperMotionAnalysisActionsFinishingFor(
	owner: unknown,
): FramescaperMotionAnalysisActionsFinishing | null {
...
async function executeAnalysis(
	...
	const onProgress = (value: FramescaperMotionAnalysisProgressFinishing): void => {
	...
	const frames = await options.frameProvider({
		projectId: stableId(initial.id, 'finishing project ID'), source,
		...range,
	...
	const result = await analysisModule.analyzeVideoMotionV1({
		analysisId,
		inputSha256: digest(source.contentSha256, 'finishing motion-analysis source digest'),
...
async function publishAnalysis(
	options: Readonly<{ readonly owner: MotionAnalysisOwnerFinishing; readonly store: MotionAnalysisStoreFinishing }>,
	...
	onProgress: (progress: FramescaperMotionAnalysisProgressFinishing) => void,
): Promise<VideoMotionAnalysisReferenceV1> {
	...
	let created: MotionAnalysisPublicationFinishing | null = null;
	if (priorBody === null) {
	...
	try {
		...
		const command = snapshotFramescaperOwnedFinishingCommandFinishing({
...
async function writeAnalysisBody(
	store: MotionAnalysisStoreFinishing,
	result: VideoMotionAnalysisResultV1,
	signal: AbortSignal | undefined,
): Promise<MotionAnalysisPublicationFinishing> {
	...
	let publication: MotionAnalysisPublicationFinishing | null = null;
	try {
...
function targets(value: unknown): readonly FramescaperMotionAnalysisTargetFinishing[] {
	const project = projectRecord(value);
	const sources = records(project.sources, 'finishing motion-analysis sources');
	const analyses = records(project.videoMotionAnalyses, 'finishing motion analyses')
		.map(normalizeVideoMotionAnalysisReferenceV1);
	return Object.freeze(records(project.videoProcessorStacks, 'finishing processor stacks').flatMap((value) => {
		...
		const sourceEndFrame = positiveInteger(source.sourceFrameCount, 'finishing video source frame count');
...
function assertCurrent(
	...
	if (digest(currentSource.contentSha256, 'finishing motion-analysis source digest')
		!== digest(source.contentSha256, 'finishing motion-analysis source digest')
		|| JSON.stringify(currentStack) !== JSON.stringify(stack)
		|| JSON.stringify(currentAnalysis) !== JSON.stringify(expectedAnalysis)) {
		throw new Error('The selected finishing motion-analysis source, settings, or reference changed before publication.');
	}
}
...
function analysisForStack(
	...
	const matches = records(project.videoMotionAnalyses, 'finishing motion analyses')
		.map(normalizeVideoMotionAnalysisReferenceV1)
		.filter((analysis) => analysis.sourceId === stack.sourceId && analysis.processorStackId === stack.id);
	if (matches.length > 1) throw new Error('A finishing processor stack has multiple motion analyses.');
	return matches[0] ?? null;
}

function stackById(project: Readonly<Record<string, unknown>>, stackId: string): VideoProcessorStackV1 {
	const id = stableId(stackId, 'finishing processor stack ID');
	const value = records(project.videoProcessorStacks, 'finishing processor stacks').find((item) => item.id === id);
	if (!value) throw new ReferenceError(`finishing processor stack ${id} is unavailable.`);
...
function sourceById(
	...
	const value = records(project.sources, 'finishing motion-analysis sources').find(({ id }) => id === sourceId);
	if (!value || value.kind !== 'video') throw new ReferenceError(`finishing video source ${sourceId} is unavailable.`);
...
function analysisRange(startValue: number, endValue: number, source: Readonly<Record<string, unknown>>) {
	...
	const sourceEnd = positiveInteger(source.sourceFrameCount, 'finishing video source frame count');
	...
	if (endFrame - startFrame > MAXIMUM_ANALYSIS_FRAMES) {
		throw new RangeError(`A selected finishing motion-analysis run is limited to ${String(MAXIMUM_ANALYSIS_FRAMES)} frames.`);
	}
...
function projectRecord(value: unknown): Readonly<Record<string, unknown>> {
	...
	const project = record(value, 'Selected finishing motion-analysis project');
	return project;
}
