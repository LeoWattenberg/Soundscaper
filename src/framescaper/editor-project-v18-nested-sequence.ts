...
export interface FramescaperNestedSequenceFrameRequestV18 {
	readonly rootSequenceId: string;
	readonly subsequenceIds: readonly string[];
	readonly sourceFrame: number;
}

export interface FramescaperFlattenedSequenceClipV18 {
...
export function mapFramescaperNestedSequenceFrameV18(
	profile: unknown,
	projectValue: FramescaperProjectV18 | unknown,
	requestValue: FramescaperNestedSequenceFrameRequestV18 | unknown,
): Readonly<FramescaperExactSequenceFrameV18> {
	...
	if (request.subsequenceIds.length < 1
		|| request.subsequenceIds.length > FRAMESCAPER_V18_MAXIMUM_NESTING_DEPTH) {
	...
	const path: FramescaperSubsequenceV18[] = [];
...
export function flattenFramescaperSequenceV18(
	profile: unknown,
	projectValue: FramescaperProjectV18 | unknown,
	rootSequenceIdValue?: string,
): readonly Readonly<FramescaperFlattenedSequenceClipV18>[] {
	validateFramescaperProjectV18(profile, projectValue);
	const project = projectValue as FramescaperProjectV18;
	...
	const outgoing = new Map<string, FramescaperSubsequenceV18[]>();
...
function sequenceBindings(project: FramescaperProjectV18): ReadonlyMap<string, SequenceBinding> {
...
function sequenceRates(project: FramescaperProjectV18): ReadonlyMap<string, SequenceBinding['rate']> {
...
function requestRecord(value: unknown): FramescaperNestedSequenceFrameRequestV18 {
...
function exactFrame(value: Fraction): Readonly<FramescaperExactSequenceFrameV18> {
	return Object.freeze({ numerator: value.numerator, denominator: value.denominator });
}

function compareFlattened(left: FramescaperFlattenedSequenceClipV18, right: FramescaperFlattenedSequenceClipV18): number {
...
function freezeFlattened(value: FramescaperFlattenedSequenceClipV18): Readonly<FramescaperFlattenedSequenceClipV18> {
