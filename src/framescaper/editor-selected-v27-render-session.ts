...
export interface FramescaperSelectedRenderSessionOptionsV27 {
	readonly profile: unknown;
	readonly project: unknown;
	readonly authority: unknown;
}

export interface FramescaperSelectedRenderSessionV27 {
	readonly status: 'selected-v27-render-session';
	readonly generation: 27;
...
export interface FramescaperSelectedRenderSessionRuntimeV27 {
	create(authority: unknown): FramescaperSelectedRenderSessionV27;
}

const OWNER_RUNTIMES = new WeakMap<object, FramescaperSelectedRenderSessionRuntimeV27>();

/** Capture the selected V13 plan once for every preview/export picture consumer. */
export function createFramescaperSelectedRenderSessionV27(
	value: FramescaperSelectedRenderSessionOptionsV27,
): FramescaperSelectedRenderSessionV27 {
	...
	return Object.freeze({
		status: 'selected-v27-render-session' as const,
		generation: 27 as const,
		plan,
...
/** Bind a factory to the selected controller without exposing it as a visible UI control. */
export function bindFramescaperSelectedRenderSessionRuntimeV27(
	profile: unknown,
	controller: Readonly<{ readonly project: unknown }>,
): void {
	if (!controller || typeof controller !== 'object') {
		throw new TypeError('The selected V27 render-session owner must be a controller.');
	}
	OWNER_RUNTIMES.set(controller, Object.freeze({
		create: (authority: unknown) => createFramescaperSelectedRenderSessionV27({
			profile,
...
export function framescaperSelectedRenderSessionRuntimeV27For(
	owner: unknown,
): FramescaperSelectedRenderSessionRuntimeV27 | null {
...
function bindExactTiming(
	...
	const project = record(projectValue, 'Selected V27 render project');
	const sources = array(project.sources, 'Selected V27 render sources');
	const result = new Map<string, BoundVideoSourceTimingView>();
	for (const sourceValue of sources) {
		const source = record(sourceValue, 'Selected V27 render source');
		if (source.kind !== 'video') continue;
		if (typeof source.id !== 'string' || !source.id || result.has(source.id)) {
			throw new TypeError('Selected V27 render video source identities must be unique.');
		}
...
function exactOptions(value: unknown): FramescaperSelectedRenderSessionOptionsV27 {
	const options = record(value, 'Selected V27 render-session options');
	const fields = ['profile', 'project', 'authority'];
	const keys = Reflect.ownKeys(options);
	if (keys.length !== fields.length
		|| keys.some((key) => typeof key !== 'string' || !fields.includes(key))) {
		throw new TypeError('Selected V27 render-session options must be an exact record.');
	}
	for (const field of fields) {
		const descriptor = Object.getOwnPropertyDescriptor(options, field);
		if (!descriptor?.enumerable || !Object.hasOwn(descriptor, 'value')) {
			throw new TypeError(`Selected V27 render-session ${field} must be an own data property.`);
		}
	}
	return options as unknown as FramescaperSelectedRenderSessionOptionsV27;
}
