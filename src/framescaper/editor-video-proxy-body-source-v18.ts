...
export interface FramescaperVideoProxyBodyStoreV18 {
	loadMediaAsset(
		storageKey: string,
...
export interface FramescaperVideoProxyBodySourceDependenciesV18 {
	readonly store: FramescaperVideoProxyBodyStoreV18;
...
export function createFramescaperVideoProxyBodySourceV18(
	dependencies: FramescaperVideoProxyBodySourceDependenciesV18,
): (request: Readonly<FramescaperVideoProxyBodyRequestV18>) => Promise<FramescaperVideoProxyBodyLeaseV18> {
	const store = dependencies?.store;
	const getProject = dependencies?.getProject;
	if (!store || typeof store.loadMediaAsset !== 'function') {
		throw new TypeError('A Framescaper V18 proxy body source requires a media store.');
	}
	if (typeof getProject !== 'function') {
		throw new TypeError('A Framescaper V18 proxy body source requires the live project.');
	}
	...
	return async function acquireBody(request) {
		...
		if (!expected || typeof expected !== 'object') {
			throw new TypeError('A Framescaper V18 proxy body request states what it expects.');
		}
		...
		} as FramescaperVideoProxyBodyLeaseV18);
	};
}
...
function nonEmpty(value: unknown, name: string): string {
	if (typeof value !== 'string' || !value) {
		throw new TypeError(`A Framescaper V18 proxy body ${name} is required.`);
	}
	return value;
}
