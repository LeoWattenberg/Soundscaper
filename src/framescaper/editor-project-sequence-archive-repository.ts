...
} from './editor-project-sequence-preservation-repository.ts';
import {
...
export interface FramescaperProjectSequenceArchivePreservationPlan {
	readonly sourceId: string;
	readonly plan: VideoProxyPreservationPlan;
}

export interface FramescaperProjectSequenceArchivePublication {
	readonly mode: FramescaperProjectSequenceArchivePublicationMode;
	readonly origin: unknown;
	readonly expected: unknown | null;
	readonly project: unknown;
	readonly plans: readonly FramescaperProjectSequenceArchivePreservationPlan[];
}

export interface FramescaperProjectSequenceArchiveRepositoryDependencies {
...
interface NormalizedPublication {
	readonly mode: FramescaperProjectSequenceArchivePublicationMode;
	readonly origin: FramescaperProjectSequence;
	readonly expected: FramescaperProjectSequence | null;
...
/** Atomic create/collision-copy/replacement owner for imported attached sequence documents. */
export class FramescaperProjectSequenceArchiveRepository {
	...
	constructor(
		...
		dependenciesValue: FramescaperProjectSequenceArchiveRepositoryDependencies | unknown,
	) {
		assertFramescaperProjectSequenceProfile(profile);
		const dependencies = closedRecord(
			dependenciesValue,
			DEPENDENCY_FIELDS,
			'Framescaper sequence archive repository dependencies',
		);
		if (!dependencies.port || typeof dependencies.port !== 'object'
			|| typeof (dependencies.port as StorageRepositoryPort).database !== 'function') {
			throw new TypeError('A durable repository port is required for sequence archive publication.');
		}
		if (!(dependencies.claims instanceof VideoProxyClaimRepository)) {
			throw new TypeError('The sequence archive repository requires its exact claim authority.');
		}
	...
	async publish(
		publicationValue: FramescaperProjectSequenceArchivePublication | unknown,
	): Promise<FramescaperProjectSequence | null> {
		const publication = normalizePublication(this.#profile, publicationValue);
		const database = await this.#port.database();
		if (!database) throw new Error('Durable storage is required for sequence archive publication.');
...
function normalizePublication(
	...
	const raw = closedRecord(value, PUBLICATION_FIELDS, 'Framescaper sequence archive publication');
	const mode = publicationMode(raw.mode);
	const origin = cloneFramescaperProjectSequence(profile, raw.origin);
	const project = cloneFramescaperProjectSequence(profile, raw.project);
	const expected = raw.expected === null ? null : cloneFramescaperProjectSequence(profile, raw.expected);
	...
	return {
		...
		originFingerprint: framescaperProjectFingerprintSequence(profile, origin),
		projectId,
...
): readonly FramescaperProjectSequenceArchivePreservationPlan[] {
	const input = denseArray(value, 'Framescaper sequence archive preservation plans');
...
function assertSamePreservedAttachments(
	origin: FramescaperProjectSequence,
	target: FramescaperProjectSequence,
): void {
	const inventory = (project: FramescaperProjectSequence): string[] => [...attachedSources(project).values()]
...
function publicationMode(value: unknown): FramescaperProjectSequenceArchivePublicationMode {
	if (value === 'create' || value === 'copy' || value === 'compare-and-swap') return value;
	throw new TypeError('A supported sequence archive publication mode is required.');
}
...
function projectIdentifier(project: FramescaperProjectSequence): string {
	return identifier(project.id, 'project id');
}

function projectRevision(project: FramescaperProjectSequence): number {
	if (!Number.isSafeInteger(project.revision) || Number(project.revision) < 0) {
		throw new RangeError('A non-negative sequence project revision is required.');
	}
