...
import {
	assertFramescaperDesktopProjectLibraryV20RendererComposition,
...
export interface FramescaperDesktopProjectStoreV20Local {
...
export type FramescaperDesktopProjectStoreV20Adapter<Store> = Store & Readonly<{
	createProjectIfAbsent(project: unknown): Promise<FramescaperProjectV31 | null>;
}>;

/** Overlay selected V20 main authority without exposing its generation through the public API. */
export function createFramescaperDesktopProjectStoreV20Adapter<
	Store extends FramescaperDesktopProjectStoreV20Local,
>(
...
): Store | FramescaperDesktopProjectStoreV20Adapter<Store> {
	...
	framescaperProjectStoreAuthorityV31(profileValue, localStore);
	const renderer = composition.desktopProjectLibrary as FramescaperDesktopProjectLibraryV20Renderer | null;
	if (renderer === null) return localStore;
	assertFramescaperDesktopProjectLibraryV20RendererComposition(profileValue, localStore, renderer);
	return new Proxy(localStore, proxyHandler(profileValue, localStore, renderer)) as
		FramescaperDesktopProjectStoreV20Adapter<Store>;
}

function proxyHandler<Store extends FramescaperDesktopProjectStoreV20Local>(
	profile: EditorProjectRuntimeProfile,
	localStore: Store,
	renderer: FramescaperDesktopProjectLibraryV20Renderer,
): ProxyHandler<Store> {
...
function assertLocalStore(value: unknown): asserts value is FramescaperDesktopProjectStoreV20Local {
	if (!value || typeof value !== 'object') throw new TypeError('An exact Framescaper V31 local store is required.');
	for (const method of ['ready', 'estimateStorage', 'loadProject', 'listProjects'] as const) {
		if (typeof inheritedData(value, method) !== 'function') {
			throw new TypeError(`The Framescaper V31 local store requires ${method}.`);
		}
	}
}
