...
const STORE_AUTHORITIES = new WeakMap<AudioEditorProjectStore, FramescaperProjectStoreFoundationAuthority>();

export interface FramescaperProjectStoreFoundationAuthority {
	readonly port: StorageRepositoryPort;
	readonly opfs: OpfsRepository | null;
}

export interface FramescaperProjectStoreFoundationOptions extends AudioEditorProjectStoreOptions {
	readonly store?: AudioEditorProjectStore;
}

export interface FramescaperProjectStoreFoundationDefinition {
...
/** Construct one authenticated Framescaper project store from an exact generation definition. */
export function createFramescaperProjectStoreFoundation(
	definition: FramescaperProjectStoreFoundationDefinition,
	optionsValue: FramescaperProjectStoreFoundationOptions | unknown = {},
	...
	let authority: FramescaperProjectStoreFoundationAuthority | null = null;
	const store = createProjectStore({
		...copyCreationOptions(optionsValue),
		projectStorageProfile: definition.storageProfile,
		repositoryFactory: foundationRepositoryFactory(
			definition,
...
export function framescaperProjectStoreFoundationAuthority(
	definition: FramescaperProjectStoreFoundationDefinition,
	store: unknown,
): Readonly<FramescaperProjectStoreFoundationAuthority> {
	definition.authenticate(definition.profile);
	if (!(store instanceof AudioEditorProjectStore)
		|| STORE_GENERATIONS.get(store) !== definition.token
...
function foundationRepositoryFactory(
	definition: FramescaperProjectStoreFoundationDefinition,
...
function copyCreationOptions(value: unknown): AudioEditorProjectStoreOptions {
	...
	for (const key of Reflect.ownKeys(value)) {
		if (typeof key !== 'string') throw new TypeError('Framescaper store options cannot contain symbols.');
