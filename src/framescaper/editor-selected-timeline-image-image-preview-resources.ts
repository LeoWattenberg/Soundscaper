...
/** timelineImage image-route ceiling; inherited finishing media keeps independent resource ownership. */
export const FRAMESCAPER_IMAGE_PREVIEW_MAXIMUM_WORKING_BYTES_TIMELINE_IMAGE = 512 * 1024 * 1024;
export const FRAMESCAPER_IMAGE_PREVIEW_MAXIMUM_SOURCES_TIMELINE_IMAGE = 512;
export const FRAMESCAPER_IMAGE_PREVIEW_MAXIMUM_CONTEXTS_TIMELINE_IMAGE = 4_096;

export interface FramescaperImagePreviewResourceContextTimelineImage {
	readonly source: FramescaperImageSourceV1;
	readonly width: number;
	readonly height: number;
}

export interface FramescaperImagePreviewResourceBudgetTimelineImage {
 ...
 */
export function admitFramescaperImageTimelinePreviewResourcesTimelineImage(
	contexts: readonly FramescaperImagePreviewResourceContextTimelineImage[],
): FramescaperImagePreviewResourceBudgetTimelineImage {
...
export function admitFramescaperImageProjectBinThumbnailResourcesTimelineImage(
 ...
 */
export function admitFramescaperImageTimelineFilmstripResourcesTimelineImage(
	contexts: readonly FramescaperImagePreviewResourceContextTimelineImage[],
): FramescaperImagePreviewResourceBudgetTimelineImage {
...
export function assertFramescaperImagePreviewReaderMetadataTimelineImage(
	sourceValue: FramescaperImageSourceV1,
	byteLength: number,
): void {
	...
	if (!Number.isSafeInteger(byteLength) || byteLength < 0
		...
		throw new RangeError(`timelineImage image reader ${source.id} exceeds its preflight metadata reservation.`);
	}
}

interface AdmittedContextsTimelineImage {
	readonly contexts: readonly FramescaperImagePreviewResourceContextTimelineImage[];
	readonly sources: ReadonlyMap<string, FramescaperImagePreviewResourceContextTimelineImage>;
}

interface RouteResourcesTimelineImage {
	readonly retainedFrameBytes: bigint;
	readonly drawableBackingBytes: bigint;
	readonly requestOutputBytes: bigint;
	readonly uniqueFrameCacheBytes: bigint;
...
function contextsAndSources(
	values: readonly FramescaperImagePreviewResourceContextTimelineImage[],
	route: string,
): AdmittedContextsTimelineImage {
	if (!Array.isArray(values) || values.length > FRAMESCAPER_IMAGE_PREVIEW_MAXIMUM_CONTEXTS_TIMELINE_IMAGE) {
		throw new RangeError(`timelineImage image ${route} exceeds its context count bound.`);
	}
	const contexts = values.map((value) => Object.freeze({
		source: normalizeFramescaperImageSourceV1(value?.source),
		width: dimension(value?.width, `timelineImage image ${route} width`),
		height: dimension(value?.height, `timelineImage image ${route} height`),
	}));
	const sources = new Map<string, FramescaperImagePreviewResourceContextTimelineImage>();
	for (const context of contexts) {
		const prior = sources.get(context.source.id);
		if (prior && (prior.width !== context.width || prior.height !== context.height)) {
			throw new RangeError(`timelineImage image ${route} source geometry is ambiguous.`);
		}
		sources.set(context.source.id, context);
	}
	if (sources.size > FRAMESCAPER_IMAGE_PREVIEW_MAXIMUM_SOURCES_TIMELINE_IMAGE) {
		throw new RangeError(`timelineImage image ${route} exceeds its source count bound.`);
	}
...
function admit(
	route: string,
	admitted: AdmittedContextsTimelineImage,
	resources: RouteResourcesTimelineImage,
): FramescaperImagePreviewResourceBudgetTimelineImage {
	...
	if (totalBytes > BigInt(FRAMESCAPER_IMAGE_PREVIEW_MAXIMUM_WORKING_BYTES_TIMELINE_IMAGE)) {
		throw new RangeError(`timelineImage image ${route} exceeds its 512 MiB working byte bound.`);
	}
...
function conservativeReaderMetadataBytes(source: FramescaperImageSourceV1): number {
	...
	if (availableReceiptBytes < 2) {
		throw new RangeError(`timelineImage image source ${source.id} cannot contain a canonical frame pack.`);
	}
...
function scaledFrameBytes(context: FramescaperImagePreviewResourceContextTimelineImage): bigint {
