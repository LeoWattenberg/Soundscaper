...
import {
	...
	type FramescaperAudioFinishingFinishing,
} from './editor-audio-finishing-finishing.ts';

/**
 * Reconcile only references invalidated by an inherited structural edit. The
 * selected finishing mixer is authored state: unrelated nodes, routes, output names,
 * VCA membership, and automation must never disappear because one track did.
 */
export function reconcileFramescaperAudioFinishingFinishing(
	project: Readonly<Record<string, unknown>>,
	value: Readonly<{ readonly automationLanes: unknown; readonly mixer: unknown }>,
): FramescaperAudioFinishingFinishing {
	...
	return normalizeFramescaperAudioFinishingFinishing(project, {
		automationLanes,
		mixer: reconciledMixer,
	});
}

interface ProjectAudioReferencesFinishing {
...
function projectReferences(
	project: Readonly<Record<string, unknown>>,
	mixer: MixerGraphV21,
): ProjectAudioReferencesFinishing {
...
function edgeExists(edge: MixerEdgeV21, refs: ProjectAudioReferencesFinishing): boolean {
...
function endpointExists(
	endpoint: Exclude<MixerEdgeV21['destination'], { readonly kind: 'effect-sidechain' }>
		| MixerEdgeV21['source'],
	refs: ProjectAudioReferencesFinishing,
): boolean {
...
function stripExists(strip: StripRef, refs: ProjectAudioReferencesFinishing): boolean {
...
function addRoutesForNewTracks(
	...
	const defaults = createDefaultFramescaperAudioFinishingFinishing(project).mixer.edges;
...
function automationArray(value: unknown): readonly unknown[] {
	if (!Array.isArray(value)) throw new TypeError('finishing automation lanes must be an array.');
	return value;
}
