...
export const FRAMESCAPER_NATIVE_MEDIA_SOURCE_CHARACTERISTICS_REQUIREMENT: ProjectFeatureRequirement = Object.freeze({
...
const NATIVE_REQUIREMENTS = Object.freeze([
	FRAMESCAPER_NATIVE_MEDIA_SOURCE_CHARACTERISTICS_REQUIREMENT,
	FRAMESCAPER_OPENFX_OPENFX_REQUIREMENT,
]);

export function reconcileFramescaperProjectFeatureRequirementsNativeMedia(
	profile: unknown,
	project: unknown,
): ProjectFeatureRequirementsManifest {
	assertFramescaperProjectNativeMediaProfile(profile);
	const candidate = record(project, 'Framescaper nativeMedia project');
	const actual = normalizeManifest(candidate);
	assertNoNativeRequirementConflict(actual);
	const baseline = framescaperProjectFinishingFoundationShapeNativeMedia(candidate).featureRequirements;
	...
	if (videoSources.some(({ characteristics }) => videoSourceCharacteristicsV25AreReported(characteristics))) {
		appendOnce(requirements, FRAMESCAPER_NATIVE_MEDIA_SOURCE_CHARACTERISTICS_REQUIREMENT);
	}
	if (array(candidate, 'ofxEffects').length > 0) appendOnce(requirements, FRAMESCAPER_OPENFX_OPENFX_REQUIREMENT);
	return Object.freeze({ schemaVersion: baseline.schemaVersion, requirements: Object.freeze(requirements) });
}

export function validateFramescaperProjectFeatureRequirementsNativeMedia(
	profile: unknown,
	project: unknown,
): ProjectFeatureRequirementsManifest {
	assertFramescaperProjectNativeMediaProfile(profile);
	...
	const actual = normalizeManifest(candidate);
	assertNoNativeRequirementConflict(actual);
	const expected = reconcileFramescaperProjectFeatureRequirementsNativeMedia(profile, candidate);
	if (JSON.stringify(actual) !== JSON.stringify(expected)) {
		throw new TypeError('Framescaper nativeMedia feature requirements do not match finishing/native state.');
	}
	return actual;
}

export function createFramescaperProjectFeatureCompatibilityServiceNativeMedia(profile: unknown) {
	assertFramescaperProjectNativeMediaProfile(profile);
	...
	for (const row of manifest.requirements) {
		for (const owned of NATIVE_REQUIREMENTS) {
			...
			if (ownsId !== ownsFeature || ((ownsId || ownsFeature) && JSON.stringify(row) !== JSON.stringify(owned))) {
				throw new TypeError('A publisher requirement cannot replace Framescaper nativeMedia native ownership.');
			}
		}
	}
}
