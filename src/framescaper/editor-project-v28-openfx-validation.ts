...
export const FRAMESCAPER_PROJECT_V28_MAXIMUM_OPENFX_EFFECTS = 100_000;

export function validateFramescaperOpenFxOwnershipV28(project: Record<string, unknown>): void {
	const effects = array(project.ofxEffects, 'ofxEffects');
	if (effects.length > FRAMESCAPER_PROJECT_V28_MAXIMUM_OPENFX_EFFECTS) {
		throw new RangeError('A Framescaper V28 project exceeds its OpenFX effect ceiling.');
	}
