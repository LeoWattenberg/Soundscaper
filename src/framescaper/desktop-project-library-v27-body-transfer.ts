...
import {
	collectFramescaperDesktopV27ExtensionBodyReferences,
	validateFramescaperDesktopV27Bodies,
	type FramescaperDesktopV27BodyDescriptor,
	type FramescaperDesktopV27ExtensionBodyReference,
} from './desktop-project-library-v27-body-contract.ts';
...
export type { FramescaperDesktopV27BodyDescriptor } from './desktop-project-library-v27-body-contract.ts';
export { validateFramescaperDesktopV27Bodies } from './desktop-project-library-v27-body-contract.ts';

export interface FramescaperDesktopV27PreparedBody {
	readonly descriptor: Readonly<FramescaperDesktopV27BodyDescriptor>;
	readonly blob: Blob;
	readonly metadataIdentity: string;
}

export interface FramescaperDesktopV27BodyBridge {
	readBodyChunk(request: Readonly<{
		body: Readonly<FramescaperDesktopV27BodyDescriptor>;
		offset: number;
		length: number;
	}>): Promise<unknown>;
...
interface TransferReference {
	...
	readonly extension: Readonly<FramescaperDesktopV27ExtensionBodyReference> | null;
}

/** Preflight the complete selected-V27 body graph before main-process admission. */
export async function prepareFramescaperDesktopV27PublicationBodies(
	project: FramescaperProjectV27,
	projectSha256: string,
	store: FramescaperDesktopV12BodyStore,
	signal?: AbortSignal,
): Promise<readonly Readonly<FramescaperDesktopV27PreparedBody>[]> {
	const base = await prepareFramescaperDesktopV12PublicationBodies(
		project as unknown as FramescaperProjectV20, projectSha256, store, signal,
	);
	const normalizedBase: FramescaperDesktopV27PreparedBody[] = [];
	for (const item of base) {
		throwIfScapeAborted(signal);
		const descriptor = item.descriptor as Readonly<FramescaperDesktopV27BodyDescriptor>;
	...
	const extension: FramescaperDesktopV27PreparedBody[] = [];
	for (const reference of collectFramescaperDesktopV27ExtensionBodyReferences(project)) {
		...
		if (metadataIdentity(current) !== metadataIdentity(metadata)) {
			throw new Error(`Managed V27 ${descriptor.kind} metadata changed during preflight.`);
		}
		extension.push(Object.freeze({ descriptor, blob, metadataIdentity: metadataIdentity(metadata) }));
	}
	const prepared: readonly Readonly<FramescaperDesktopV27PreparedBody>[] = Object.freeze([
		...normalizedBase, ...extension,
	]);
	validateFramescaperDesktopV27Bodies(project, projectSha256,
...
export async function uploadFramescaperDesktopV27PublicationBodies(
	publicationId: string,
	prepared: readonly Readonly<FramescaperDesktopV27PreparedBody>[],
	bridge: Pick<FramescaperDesktopV27BodyBridge, 'writePublicationChunk'>,
	...
	for (const [bodyIndex, body] of prepared.entries()) {
		...
		for (let offset = 0; offset < body.descriptor.byteLength;) {
			...
			if (!(buffer instanceof ArrayBuffer) || buffer.byteLength !== nextOffset - offset) {
				throw new Error(`Managed V27 ${body.descriptor.kind} emitted an inexact chunk.`);
			}
			...
			if (acknowledgement.bodyIndex !== bodyIndex || acknowledgement.nextOffset !== nextOffset
				...
				throw new Error('Framescaper V27 body acknowledgement changed its sequential write.');
			}
			offset = nextOffset;
		}
		if (bytesToHex(digest.digest()) !== body.descriptor.sha256) {
			throw new Error(`Managed V27 ${body.descriptor.kind} changed during upload.`);
		}
		...
		if (metadataIdentity(current) !== body.metadataIdentity) {
			throw new Error(`Managed V27 ${body.descriptor.kind} metadata changed during upload.`);
		}
	}
}

/** Acquire every missing body as one rollback-capable local publication set. */
export async function acquireFramescaperDesktopV27Bodies(
	project: FramescaperProjectV27,
	projectSha256: string,
	bodiesValue: unknown,
	bridge: Pick<FramescaperDesktopV27BodyBridge, 'readBodyChunk'>,
	store: FramescaperDesktopV12BodyStore,
	signal?: AbortSignal,
): Promise<void> {
	const bodies = validateFramescaperDesktopV27Bodies(project, projectSha256, bodiesValue);
	...
	try {
		for (const body of bodies) {
			...
			for (let offset = 0; offset < body.byteLength;) {
				throwIfScapeAborted(signal);
				const length = Math.min(FRAMESCAPER_DESKTOP_V12_MAXIMUM_BODY_CHUNK_BYTES,
					body.byteLength - offset);
				const value = await bridge.readBodyChunk({ body, offset, length });
				if (!(value instanceof Uint8Array) || value.byteLength !== length) {
					throw new Error('Framescaper desktop V27 body read returned an inexact chunk.');
				}
			...
			if (bytesToHex(digest.digest()) !== body.sha256) {
				throw new Error(`Framescaper desktop V27 ${body.kind} body failed its SHA-256 binding.`);
			}
		...
		if (cleanup.length) throw new AggregateError([error, ...cleanup], 'V27 body acquisition rollback failed.');
		throw error;
	}
}
...
async function extensionDescriptor(
	...
	store: Pick<FramescaperDesktopV12BodyStore, 'getMediaAssetMetadata'>,
	signal?: AbortSignal,
): Promise<Readonly<FramescaperDesktopV27BodyDescriptor>> {
	...
	if (metadata.size > reference.maximumBytes
		...
		throw new Error(`Managed V27 ${reference.kind} body conflicts with project authority.`);
	}
...
function transferReference(
	descriptor: Readonly<FramescaperDesktopV27BodyDescriptor>,
	project: FramescaperProjectV27,
): Readonly<TransferReference> {
	const extension = collectFramescaperDesktopV27ExtensionBodyReferences(project)
		.find((candidate) => bodyKey(candidate) === bodyKey(descriptor)) ?? null;
	if (descriptor.kind.startsWith('framescaper-') && !extension) {
		throw new Error(`V27 desktop ${descriptor.kind} lost its project reference.`);
	}
	return Object.freeze({
		descriptor,
...
function timingByStorageKey(project: FramescaperProjectV27): ReadonlyMap<string, Readonly<VideoTimingAssetReference>> {
...
function addTiming(
	...
	if (prior && JSON.stringify(prior) !== JSON.stringify(value)) {
		throw new Error(`V27 desktop timing body ${value.storageKey} has conflicting references.`);
	}
...
async function loadVerifiedBlob(
	...
	if (blob.size !== metadata.size || await digestMediaContent(blob, { signal }) !== metadata.sha256) {
		throw new Error(`Managed V27 ${reference.descriptor.kind} body failed immutable verification.`);
	}
...
function validateSemanticBytes(reference: Readonly<TransferReference>, bytes: Uint8Array): void {
	...
	if (reference.extension) {
		validateFramescaperScapeAssetReferenceBytesV27(reference.extension.archiveReference, bytes);
	}
}
...
function trustedMetadata(
	...
	if (metadata.sourceId !== descriptor.storageKey || metadata.mimeType !== descriptor.mimeType
		...
		throw new Error(`Managed V27 ${descriptor.kind} metadata conflicts with its project binding.`);
	}
	return metadata;
}
...
function trustedMetadataCandidate(value: unknown, kind: string): TrustedMetadata {
	if (!value || typeof value !== 'object' || Array.isArray(value)) {
		throw new Error(`Managed V27 ${kind} metadata is missing.`);
	}
	...
	if (!metadata.sourceId || !metadata.mimeType || !Number.isSafeInteger(metadata.size) || metadata.size < 1
		...
		throw new Error(`Managed V27 ${kind} metadata is incomplete.`);
	}
	return metadata;
}
...
function assertPublication(
	...
	if (!publication || typeof publication !== 'object' || typeof publication.discardIfCurrent !== 'function'
		...
		throw new Error(`Managed V27 ${body.kind} publication changed its descriptor.`);
	}
}

function storageKind(kind: FramescaperDesktopV27BodyDescriptor['kind']): string {
...
function assertAggregateBytes(bodies: readonly Readonly<{ byteLength: number }>[]): void {
	...
	for (const { byteLength } of bodies) {
		if (byteLength > SCAPE_ARCHIVE_LIMITS.maximumExpandedBytes - total) {
			throw new RangeError('Framescaper desktop V27 bodies exceed their aggregate byte limit.');
		}
		total += byteLength;
	}
}
...
function concatenate(chunks: readonly Uint8Array[], byteLength: number): Uint8Array {
	...
	if (offset !== byteLength) throw new Error('Framescaper desktop V27 semantic body ended early.');
	return output;
}
...
function acknowledgementRecord(value: unknown): Readonly<{
	...
	if (!value || typeof value !== 'object' || Array.isArray(value)) {
		throw new TypeError('Framescaper desktop V27 body acknowledgement is invalid.');
	}
	const record = value as Record<string, unknown>;
	if (Reflect.ownKeys(record).length !== 3 || !Number.isSafeInteger(record.bodyIndex)
		...
		throw new TypeError('Framescaper desktop V27 body acknowledgement changed shape.');
	}
