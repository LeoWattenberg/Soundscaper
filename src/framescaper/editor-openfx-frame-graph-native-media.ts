...
/** Context-aware selected-baseline OpenFX execution over explicit intermediate RGBA planes. */
...
export interface FramescaperOpenFxFrameNativeMedia {
	readonly width: number;
	readonly height: number;
	readonly pixels: Uint8Array<ArrayBuffer>;
}

export interface FramescaperOpenFxNamedPlaneNativeMedia {
	readonly identity: string;
	readonly rgba: FramescaperOpenFxFrameNativeMedia;
}

export type FramescaperOpenFxStandardParametersNativeMedia = Readonly<{
	readonly Transition?: number;
	readonly SourceTime?: Readonly<{ readonly num: number; readonly den: number }>;
}>;

export interface FramescaperOpenFxFrameExecutionRequestNativeMedia {
	...
	readonly standardParameters: FramescaperOpenFxStandardParametersNativeMedia;
	readonly signal: AbortSignal;
}

export type FramescaperOpenFxFrameExecutionResultNativeMedia = Readonly<
	| {
...
export interface FramescaperOpenFxFrameDispositionNativeMedia {
	readonly instanceId: string;
	readonly context: OfxContext;
	readonly outputOrdinal: number;
	readonly mode: FramescaperOpenFxFrameExecutionResultNativeMedia['mode'];
	readonly reportsDegradation: boolean;
...
export interface FramescaperOpenFxFrameGraphNativeMedia {
	apply(request: Readonly<{
		...
		readonly outputOrdinal: number;
		readonly primary: FramescaperOpenFxNamedPlaneNativeMedia | null;
...
export function createFramescaperOpenFxFrameGraphNativeMedia(options: Readonly<{
	readonly plan: UnifiedExactRenderPlanV14;
	...
	readonly resolveFrozenFrame?: (
		...
		signal: AbortSignal,
	) => PromiseLike<FramescaperOpenFxFrameNativeMedia | null>;
	readonly allowRepeatedFrames?: boolean;
}>): FramescaperOpenFxFrameGraphNativeMedia {
	const plan = admittedPlan(options.plan);
	if (typeof options.assertCurrent !== 'function' || typeof options.execute !== 'function') {
		throw new TypeError('Selected nativeMedia OpenFX frame execution requires exact authority ports.');
	}
	...
	async function apply(
		requestValue: Parameters<FramescaperOpenFxFrameGraphNativeMedia['apply']>[0],
	) {
		if (active) throw new Error('Selected nativeMedia OpenFX frame execution cannot overlap.');
		active = true;
		try {
...
function admittedPlan(value: unknown): UnifiedExactRenderPlanV14 {
	if (!value || typeof value !== 'object' || Array.isArray(value)) {
		throw new TypeError('Selected nativeMedia OpenFX frame execution requires a V14 plan.');
	}
...
function checkpoint(
	value: Parameters<FramescaperOpenFxFrameGraphNativeMedia['apply']>[0],
	...
	if (!Array.isArray(value.namedPlanes) || value.namedPlanes.length > 16) {
		throw new RangeError('Selected nativeMedia OpenFX named input planes are invalid.');
	}
	...
	if (new Set(namedPlanes.map(({ identity }) => identity)).size !== namedPlanes.length) {
		throw new Error('Selected nativeMedia OpenFX named plane identities must be unique.');
	}
	if (value.context === 'transition' && unit(value.transitionProgress, 'Transition') === null) {
		throw new Error('Selected nativeMedia OpenFX Transition requires its exact standard parameter.');
	}
...
function namedPlane(value: unknown, plan: UnifiedExactRenderPlanV14, name: string) {
	...
	const candidate = value as Partial<FramescaperOpenFxNamedPlaneNativeMedia>;
...
function bindInputs(
	...
	planes: readonly FramescaperOpenFxNamedPlaneNativeMedia[],
	primary: FramescaperOpenFxNamedPlaneNativeMedia | null,
	current: FramescaperOpenFxFrameNativeMedia,
	plan: UnifiedExactRenderPlanV14,
) {
...
): FramescaperOpenFxStandardParametersNativeMedia {
...
function disposition(
	state: OfxEffectStateV26,
	outputOrdinal: number,
	mode: FramescaperOpenFxFrameExecutionResultNativeMedia['mode'],
...
function frame(value: unknown, plan: UnifiedExactRenderPlanV14, name: string): FramescaperOpenFxFrameNativeMedia {
	if (!value || typeof value !== 'object' || Array.isArray(value)) throw new TypeError(`${name} must be RGBA.`);
	const candidate = value as Partial<FramescaperOpenFxFrameNativeMedia>;
	...
	return cloneFrame(candidate as FramescaperOpenFxFrameNativeMedia);
}

function cloneFrame(value: FramescaperOpenFxFrameNativeMedia): FramescaperOpenFxFrameNativeMedia {
	return Object.freeze({ width: value.width, height: value.height, pixels: value.pixels.slice() as Uint8Array<ArrayBuffer> });
}

function transparent(plan: UnifiedExactRenderPlanV14): FramescaperOpenFxFrameNativeMedia {
	return Object.freeze({
