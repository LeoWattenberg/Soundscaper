...
import { normalizeFramescaperAudioFinishingFinishing } from './editor-audio-finishing-finishing.ts';
import {
	framescaperProjectFeatureRequirementsForVisualFoundationFinishing,
	stripFramescaperProjectFinishingState,
	validateFramescaperProjectFeatureRequirementsFinishing,
...
export const FRAMESCAPER_PROJECT_FINISHING_SCHEMA_VERSION = 1 as const;

export interface FramescaperProjectFinishing extends Omit<FramescaperProjectVisual,
...
export function validateFramescaperProjectFinishing(
	profile: unknown,
	project: unknown,
): project is FramescaperProjectFinishing {
	assertFramescaperProjectFinishingProfile(profile);
	const candidate = exactProject(project);
	if (data(candidate, 'schemaVersion') !== FRAMESCAPER_PROJECT_FINISHING_SCHEMA_VERSION) {
	...
	validateFramescaperProjectVisual(
		...
		framescaperProjectVisualFoundationFinishing(profile, candidate),
	);
	validateFinishingState(candidate);
	validateFramescaperProjectFeatureRequirementsFinishing(profile, candidate);
	return true;
}

export function framescaperProjectVisualFoundationFinishing(
	profile: unknown,
	project: unknown,
	...
	stripFramescaperProjectFinishingState(result);
	result.featureRequirements = framescaperProjectFeatureRequirementsForVisualFoundationFinishing(
		profile,
		candidate,
	);
	return result as unknown as FramescaperProjectVisual;
}

export function normalizeFramescaperProjectFinishingStateFinishing(
	...
	const audio = normalizeFramescaperAudioFinishingFinishing(project, {
...
function validateFinishingState(project: Record<string, unknown>): void {
	...
	const normalizedAudio = normalizeFramescaperAudioFinishingFinishing(project, {
...
function assertNoDormantNativeState(project: Record<string, unknown>): void {
	for (const field of ['nativeVideoSources', 'ofxEffects']) {
		if (Object.hasOwn(project, field)) throw new TypeError(`Framescaper finishing does not admit dormant native state ${field}.`);
	}
	for (const source of records(data(project, 'sources'), 'sources')) {
		if (Object.hasOwn(source, 'imageSequence')) {
			throw new TypeError('Framescaper finishing does not inherit professionalMedia native image-sequence state.');
		}
	}
}

function exactProject(value: unknown): Record<string, unknown> {
	const project = record(value, 'Framescaper finishing project');
	...
	if (keys.length !== expected.size || keys.some((key) => typeof key !== 'string' || !expected.has(key))) {
		...
		throw new TypeError(`Framescaper finishing project contains unsupported field ${String(unexpected)}.`);
	}
...
function addIdentity(identities: Set<string>, id: string, name: string): void {
	if (identities.has(id)) throw new RangeError(`finishing ${name} identity ${id} collides with project identity.`);
	identities.add(id);
}

function assertCanonical(actual: unknown, expected: unknown, name: string): void {
	if (JSON.stringify(actual) !== JSON.stringify(expected)) throw new TypeError(`Framescaper finishing ${name} is not canonical.`);
}
