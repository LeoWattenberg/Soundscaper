...
const IDENTITY = Object.freeze({
	label: 'Framescaper desktop V18', librarySchemaVersion: 18,
	databaseUserVersion: 20, scopeVersion: 'v18', projectSchemaVersion: 27,
});
...
export interface FramescaperDesktopProjectLibraryV18ProjectSummary {
	readonly id: string;
...
export interface FramescaperDesktopProjectLibraryV18Renderer {
	listProjects(): Promise<readonly Readonly<FramescaperDesktopProjectLibraryV18ProjectSummary>[]>;
	readProject(projectId: string, options?: Readonly<{ signal?: AbortSignal }>): Promise<FramescaperProjectV27 | null>;
	publishProject(request: Readonly<{
		readonly project: unknown;
		readonly signal?: AbortSignal;
		readonly beforeFinish?: () => PromiseLike<void> | void;
	}>): Promise<FramescaperProjectV27>;
...
/** Connect the selected pathless public bridge to one exact durable V27 browser shadow. */
export async function connectFramescaperDesktopProjectLibraryV18Renderer(
	profileValue: EditorProjectRuntimeProfile | unknown,
	storeValue: unknown,
): Promise<FramescaperDesktopProjectLibraryV18Renderer | null> {
	...
	if (bridge.handshakeState() !== 'admitted') {
		throw new TypeError('The Framescaper desktop V18 bridge did not retain its admitted handshake.');
	}
...
export function assertFramescaperDesktopProjectLibraryV18RendererComposition(
	profileValue: EditorProjectRuntimeProfile | unknown,
	store: unknown,
	renderer: unknown,
): asserts renderer is FramescaperDesktopProjectLibraryV18Renderer {
	assertFramescaperProjectV27Profile(profileValue);
	...
	if (!composition || composition.profile !== profileValue || composition.store !== store) {
		throw new TypeError('The exact admitted Framescaper desktop V18 renderer composition is required.');
	}
}

class Renderer implements FramescaperDesktopProjectLibraryV18Renderer {
	readonly #profile: EditorProjectRuntimeProfile;
	readonly #bridge: V18Bridge;
	readonly #store: V18Store;
	#tail: Promise<void> = Promise.resolve();

	constructor(profile: EditorProjectRuntimeProfile, bridge: V18Bridge, store: V18Store) {
	...
	publishProject(request: Readonly<{
		...
		return this.#exclusive(async () => {
			const project = cloneFramescaperProjectV27(this.#profile, request.project);
			...
			if (current && !isStrictlyHigherProjectRevision(project.revision, current.project.revision)) {
				throw new Error('Framescaper desktop V18 publication requires a strictly higher revision.');
			}
			...
			try {
				...
				}), ['publicationId', 'maximumChunkBytes', 'bodyCount'], 'V18 publication admission');
				...
				if (JSON.stringify(result.project) !== JSON.stringify(project)) {
					throw new Error('Framescaper V18 publication readback changed the project.');
				}
	...
	deleteProject(projectIdValue: string): Promise<void> {
		...
		return this.#exclusive(async () => {
			...
			}), ['projectId', 'metadataRevision', 'deleted'], 'V18 delete result');
			if (result.projectId !== projectId || result.deleted !== true) {
				throw new Error('Framescaper V18 delete acknowledgement changed.');
			}
		});
	}
	...
	duplicateProject(sourceProjectId: string, options: Readonly<{
		...
		return this.#exclusive(async () => {
			...
			if (raw === null) throw new Error('Framescaper V18 duplicate source is unavailable.');
...
function resolveBridge(): V18Bridge | null {
	...
	for (const field of API_FIELDS) {
		if (typeof api[field] !== 'function') throw new TypeError(`The Framescaper desktop V18 bridge requires ${field}.`);
	}
...
function validateHandshake(value: unknown, databaseName: string): void {
	const handshake = exactRecord(value, HANDSHAKE_FIELDS, 'Framescaper desktop V18 handshake');
	if (handshake.kind !== 'framescaper-project-library-handshake' || handshake.version !== 1
		...
		throw new TypeError('The Framescaper desktop V18 handshake identity is unsupported.');
	}
}

function validateBundle(profile: EditorProjectRuntimeProfile, value: unknown, expectedProjectId: string): Readonly<{
	bundle: Readonly<V18Bundle>; project: FramescaperProjectV27;
}> {
	const raw = exactRecord(value, BUNDLE_FIELDS, 'Framescaper desktop V18 bundle');
	const row = projectRow(raw.project, expectedProjectId);
	if (typeof raw.document !== 'string') throw new TypeError('The Framescaper desktop V18 document is invalid.');
	const bytes = new TextEncoder().encode(raw.document);
	if (bytes.byteLength !== row.byteLength || bytesToHex(sha256(bytes)) !== row.sha256) {
		throw new Error('The Framescaper desktop V18 document changed bytes or digest.');
	}
	const project = cloneFramescaperProjectV27(profile, JSON.parse(raw.document) as unknown);
...
function projectRow(value: unknown, expectedProjectId: string): Readonly<V18ProjectRow> {
	const row = exactRecord(value, ROW_FIELDS, 'Framescaper desktop V18 project row');
	if (row.projectId !== projectIdValue_(expectedProjectId) || row.preferredProduct !== 'framescaper'
		...
		throw new TypeError('The Framescaper desktop V18 project row is invalid.');
	}
	return Object.freeze({
...
function catalog(value: unknown): Readonly<{
	...
	projects: readonly Readonly<FramescaperDesktopProjectLibraryV18ProjectSummary>[];
}> {
	const raw = exactRecord(value, CATALOG_FIELDS, 'Framescaper desktop V18 catalog');
	if (!Array.isArray(raw.projects) || raw.projects.length > 10_000) throw new TypeError('The V18 catalog is invalid.');
	return Object.freeze({
		metadataRevision: nonNegative(raw.metadataRevision, 'metadata revision'),
		projects: Object.freeze(raw.projects.map((value) => {
			const summary = exactRecord(value, SUMMARY_FIELDS, 'Framescaper desktop V18 project summary');
			return Object.freeze({
...
function durableStore(profile: EditorProjectRuntimeProfile, value: unknown): V18Store {
	if (!value || typeof value !== 'object') throw new TypeError('The exact V27 shadow store is required.');
	framescaperProjectStoreAuthorityV27(profile, value);
	const record = value as Record<string, unknown>;
	const status = typeof record.getStatus === 'function'
		? (record.getStatus as () => unknown).call(value) as Record<string, unknown>
		: record;
	if (status?.persistent !== true) throw new Error('The desktop V18 lifecycle requires a durable V27 shadow.');
	const databaseName = typeof record.databaseName === 'string' ? record.databaseName : status.databaseName;
	if (databaseName !== 'kw-media-framescaper-editor-v27') {
		throw new TypeError('The exact V27 shadow database identity is required.');
	}
	const methods = Object.freeze(Object.fromEntries([
		...
		if (typeof method !== 'function') throw new TypeError(`The exact V27 body store requires ${field}.`);
		return [field, (...args: unknown[]) => method.apply(value, args)];
	})));
	return Object.freeze({ databaseName, ...methods }) as unknown as V18Store;
}
...
function randomPublicationId(): string {
	...
	if (!globalThis.crypto?.getRandomValues) throw new Error('Web Crypto is required for V18 publication identities.');
...
function instant(value: unknown): string {
	...
	if (!Number.isFinite(Date.parse(result))) throw new TypeError('The Framescaper V18 project timestamp is invalid.');
	return result;
}

function text(value: unknown, label: string): string {
	if (typeof value !== 'string' || value.length < 1 || value.length > 4_096) {
		throw new TypeError(`The Framescaper V18 ${label} is invalid.`);
	}
	return value;
}
...
function nonNegative(value: unknown, label: string): number {
	if (typeof value !== 'number' || !Number.isSafeInteger(value) || value < 0) {
		throw new RangeError(`The Framescaper V18 ${label} is invalid.`);
	}
	return value;
}
...
function positive(value: unknown, label: string): number {
	...
	if (result < 1) throw new RangeError(`The Framescaper V18 ${label} must be positive.`);
	return result;
}
