...
import {
	...
	validateFramescaperProjectFeatureRequirementsSequence,
...
import {
	validateFramescaperMulticameraGroupsSequence,
	type FramescaperMulticameraGroupSequence,
} from './editor-project-sequence-multicam.ts';
import {
	validateFramescaperSubsequencesSequence,
	type FramescaperSubsequenceSequence,
} from './editor-project-sequence-subsequence.ts';

export const FRAMESCAPER_PROJECT_SEQUENCE_SCHEMA_VERSION = 1 as const;

export interface FramescaperProjectSequence extends Record<string, unknown> {
	...
	readonly sources: readonly FramescaperProjectSourceSequence[];
...
export type FramescaperProjectSourceSequence = Readonly<Record<string, unknown>> & (
	| { readonly kind: 'video'; readonly proxyAttachment: Readonly<VideoProxyAttachmentV18> | null }
	| { readonly kind: 'audio' }
);

export type FramescaperProjectSequenceValidationOptions = AudioEditorProjectV17ValidationOptions;

/** Validate exact Framescaper sequence while reusing the unchanged exact-V17 foundation. */
export function validateFramescaperProjectSequence(
	profile: EditorProjectRuntimeProfile | unknown,
	project: unknown,
	options: FramescaperProjectSequenceValidationOptions = {},
): project is FramescaperProjectSequence {
	...
	if (schemaVersion !== FRAMESCAPER_PROJECT_SEQUENCE_SCHEMA_VERSION) {
		throw new RangeError(`Unsupported Framescaper project schema version: ${String(schemaVersion)}.`);
	}
	if (dataProperty(candidate, 'schemaFamily', 'Framescaper project')
		!== FRAMESCAPER_PROJECT_SCHEMA_FAMILY) {
		throw new RangeError('Unsupported Framescaper project schema family.');
	}
	validateFramescaperSubsequencesSequence(profile, candidate);
	...
	validateFramescaperProjectFeatureRequirementsSequence(profile, candidate);
	return true;
}

export function framescaperProjectSequenceHasProxyAttachment(project: FramescaperProjectSequence): boolean {
