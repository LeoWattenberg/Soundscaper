...
import type {
	FramescaperMotionAnalysisFrameProviderV27,
	FramescaperMotionAnalysisProgressV27,
} from './editor-motion-analysis-actions-v27.ts';

interface MotionFrameStoreV27 {
	loadMediaAsset(
		storageKey: string,
...
interface MotionFrameExtractorV27 {
	capture(
...
type MotionFrameExtractorFactoryV27 = (
	body: Blob,
	options?: Readonly<{ readonly signal?: AbortSignal }>,
) => PromiseLike<MotionFrameExtractorV27> | MotionFrameExtractorV27;

type MotionGrayDecoderV27 = (
	body: Blob,
	frameNumber: number,
	signal?: AbortSignal,
...
/** Build authenticated source-domain gray frames for the selected V27 built-in analyzer. */
export function createFramescaperMotionAnalysisFrameProviderV27(options: Readonly<{
	readonly store: MotionFrameStoreV27;
	readonly createExtractor?: MotionFrameExtractorFactoryV27;
	readonly decodeGray?: MotionGrayDecoderV27;
}>): FramescaperMotionAnalysisFrameProviderV27 {
	if (!options.store || typeof options.store.loadMediaAsset !== 'function') {
		throw new TypeError('The selected V27 motion frame provider requires an original-media store.');
	}
	...
	return async (request) => {
		...
		const storageKey = stableId(source.storageKey ?? source.id, 'V27 motion source storage key');
		const expectedDigest = digest(source.contentSha256, 'V27 motion source digest');
		...
		throwIfAborted(request.signal);
		if (body === null) {
			throw new Error('The selected V27 motion-analysis original is offline or unavailable.');
		}
		if (await digestMediaContent(body, request.signal ? { signal: request.signal } : {}) !== expectedDigest) {
			throw new Error('The selected V27 motion-analysis original changed before authentication.');
		}
		throwIfAborted(request.signal);
		...
		if (!extractor || typeof extractor.capture !== 'function' || typeof extractor.dispose !== 'function') {
			throw new TypeError('The selected V27 motion frame extractor is invalid.');
		}
		...
		try {
			...
			for (let frameNumber = request.startFrame; frameNumber < request.endFrame; frameNumber += 1) {
				...
				if (!(captured?.blob instanceof Blob)) {
					throw new TypeError('The V27 motion frame extractor returned no pathless image body.');
				}
...
async function sourceTimestampResolver(
	...
	if (source.timingAsset !== null && source.timingAsset !== undefined) {
		...
		if (loaded.status !== 'available' || !loaded.index) {
			throw new Error(`The selected V27 motion-analysis timing asset is ${loaded.status}.`);
		}
...
function midpointSeconds(
	...
	if (!Number.isFinite(seconds) || seconds < 0) {
		throw new RangeError('The selected V27 motion-analysis presentation timestamp is unsupported.');
	}
	return seconds;
}
...
async function defaultExtractor(
	body: Blob,
	options: Readonly<{ readonly signal?: AbortSignal }> = {},
): Promise<MotionFrameExtractorV27> {
	const module = await import('../common/editor/video-media.js');
	return module.createAudioEditorVideoFrameExtractor(body, options) as Promise<MotionFrameExtractorV27>;
}
...
async function decodeGrayImage(
	...
	if (typeof globalThis.createImageBitmap !== 'function') {
		throw new Error('Browser image decoding is unavailable for selected V27 motion analysis.');
	}
	...
	try {
		...
		if (!context || typeof context.drawImage !== 'function' || typeof context.getImageData !== 'function') {
			throw new Error('Canvas pixel readback is unavailable for selected V27 motion analysis.');
		}
...
function createCanvas(width: number, height: number): HTMLCanvasElement {
	...
	if (!document?.createElement) {
		throw new Error('Browser canvas creation is unavailable for selected V27 motion analysis.');
	}
...
function sourceRecord(value: unknown): Readonly<Record<string, unknown>> {
	if (!value || typeof value !== 'object' || Array.isArray(value)) {
		throw new TypeError('A selected V27 video source is required.');
	}
	const source = value as Readonly<Record<string, unknown>>;
	if (source.kind !== undefined && source.kind !== 'video') {
		throw new TypeError('Selected V27 motion analysis requires a video source.');
	}
	return source;
}
...
function rationalRate(value: unknown): Readonly<{ readonly num: number; readonly den: number }> {
	if (!value || typeof value !== 'object' || Array.isArray(value)) {
		throw new TypeError('The V27 motion source frame rate is unavailable.');
	}
	const rate = value as Readonly<Record<string, unknown>>;
	const num = positiveInteger(rate.num, 'V27 motion source rate numerator');
	const den = positiveInteger(rate.den, 'V27 motion source rate denominator');
	return Object.freeze({ num, den });
}

function progress(
	listener: (value: FramescaperMotionAnalysisProgressV27) => void,
	value: FramescaperMotionAnalysisProgressV27,
): void {
