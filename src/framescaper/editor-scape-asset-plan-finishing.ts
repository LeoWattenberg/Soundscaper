...
export const FRAMESCAPER_SCAPE_ASSET_KINDS_FINISHING = Object.freeze([
...
export type FramescaperScapeAssetKindFinishing = typeof FRAMESCAPER_SCAPE_ASSET_KINDS_FINISHING[number];

type AssetRole = 'still' | 'freeze-render' | 'proxy' | 'proxy-timing' | 'lut' | 'motion';

export interface FramescaperScapeAssetReferenceFinishing {
...
export interface FramescaperScapeImportValidationFinishing {
...
export async function planFramescaperScapeExportAssetsFinishing(
	...
	const references = collectFramescaperScapeAssetReferencesFinishing(project);
	...
	for (const reference of references) {
		...
		const metadata = record(
			...
			`finishing ${reference.role} archive metadata`,
		);
		const size = positiveInteger(metadata.size, `finishing ${reference.role} archive size`);
		if (size > reference.maximumBytes
			|| (reference.byteLength !== null && size !== reference.byteLength)
			|| metadata.sha256 !== reference.sha256) {
			throw new Error(`finishing ${reference.role} body ${reference.storageKey} is missing or stale.`);
		}
		const storedMime = String(metadata.mimeType ?? '');
		if (storedMime && storedMime !== reference.mimeType) {
			throw new Error(`finishing ${reference.role} body ${reference.storageKey} has a conflicting media type.`);
		}
...
export function validateFramescaperScapeImportAssetsFinishing(
	...
	const references = collectFramescaperScapeAssetReferencesFinishing(project);
	const kinds = new Set<string>(FRAMESCAPER_SCAPE_ASSET_KINDS_FINISHING);
	const descriptors = manifest.assets.filter(({ kind }) => kinds.has(kind));
	if (descriptors.length !== references.length) {
		throw new Error('The finishing Scape archive has an incomplete durable finishing asset inventory.');
	}
	const descriptorByArchiveId = new Map<string, ScapeAssetDescriptor>();
	for (const descriptor of descriptors) {
		if (descriptorByArchiveId.has(descriptor.sourceId)) {
			throw new Error(`The finishing Scape asset ${descriptor.sourceId} is duplicated.`);
		}
	...
	for (const reference of references) {
		...
		if (!descriptor || descriptor.kind !== reference.kind
			...
			throw new Error(`The finishing Scape ${reference.role} descriptor is missing or conflicts with project authority.`);
		}
	}
...
export async function validateFramescaperScapeExportAssetBodyFinishing(
	...
	if (body.size !== asset.size || body.size > reference.maximumBytes
		...
		throw new Error(`The finishing ${reference.role} archive body changed after admission.`);
	}
...
export function collectFramescaperScapeAssetReferencesFinishing(
	...
	const references: FramescaperScapeAssetReferenceFinishing[] = [];
	const byArchiveId = new Map<string, FramescaperScapeAssetReferenceFinishing>();
	const byEntry = new Map<string, FramescaperScapeAssetReferenceFinishing>();
	const byStorageKey = new Map<string, FramescaperScapeAssetReferenceFinishing>();
	...
	for (const motion of project.videoMotionAnalyses) {
		...
		if (!stack) throw new Error(`finishing motion analysis ${motion.id} has no processor stack.`);
...
function stillReference(
	...
	freeze: boolean,
): FramescaperScapeAssetReferenceFinishing {
	const sourceId = stableId(source.id, 'finishing still source ID');
	const mimeType = String(source.mimeType);
	if (!STILL_MIME.test(mimeType)) throw new TypeError(`finishing still ${sourceId} has an invalid media type.`);
	const role = freeze ? 'freeze-render' as const : 'still' as const;
	const digest = sha(source.contentSha256, `finishing ${role} digest`);
	return reference({
		...
		mimeType, storageKey: stableId(source.storageKey, `finishing ${role} storage key`),
...
function proxyReference(attachment: Readonly<VideoProxyAttachmentV18>): FramescaperScapeAssetReferenceFinishing {
	return reference({
...
function proxyTimingReference(timing: Readonly<VideoTimingAssetReference>): FramescaperScapeAssetReferenceFinishing {
	return reference({
...
function lutReference(lut: Readonly<VideoCubeLutReferenceV1>): FramescaperScapeAssetReferenceFinishing {
	return reference({
...
function addReference(
	value: FramescaperScapeAssetReferenceFinishing,
	references: FramescaperScapeAssetReferenceFinishing[],
	byArchiveId: Map<string, FramescaperScapeAssetReferenceFinishing>,
	byEntry: Map<string, FramescaperScapeAssetReferenceFinishing>,
	byStorageKey: Map<string, FramescaperScapeAssetReferenceFinishing>,
): void {
	const matches = [byArchiveId.get(value.archiveId), byEntry.get(value.entry), byStorageKey.get(value.storageKey)]
		.filter((candidate): candidate is FramescaperScapeAssetReferenceFinishing => candidate !== undefined);
	if (matches.length > 0) {
		if (matches.every((candidate) => sameReference(candidate, value))) return;
		throw new Error(`finishing Scape asset ${value.archiveId} has a conflicting identity or role.`);
	}
...
function sameReference(left: FramescaperScapeAssetReferenceFinishing, right: FramescaperScapeAssetReferenceFinishing): boolean {
	return JSON.stringify(left) === JSON.stringify(right);
}

function reference(value: FramescaperScapeAssetReferenceFinishing): FramescaperScapeAssetReferenceFinishing {
	if (!SHA256.test(value.sha256) || value.maximumBytes < 1
		...
		throw new RangeError(`finishing ${value.role} archive reference exceeds its digest or size bound.`);
	}
	return Object.freeze({ ...value });
}

function plannedReference(asset: PlannedScapeExportAsset): FramescaperScapeAssetReferenceFinishing {
	const source = record(asset.source, 'finishing planned archive source');
	const referenceValue = source.archiveReference;
	if (!referenceValue || typeof referenceValue !== 'object' || Array.isArray(referenceValue)) {
		throw new TypeError('The finishing planned archive asset lost its exact reference.');
	}
	const reference = referenceValue as FramescaperScapeAssetReferenceFinishing;
	if (reference.archiveId !== asset.sourceId || reference.kind !== asset.kind
		...
		throw new Error('The finishing planned archive asset drifted from its reference.');
	}
	return reference;
}

function validateLut(reference: FramescaperScapeAssetReferenceFinishing, bytes: Uint8Array): void {
	...
	if (parsed.sha256 !== expected.sha256 || parsed.byteLength !== expected.byteLength
		...
		throw new Error('The finishing cube LUT archive body conflicts with its project reference.');
	}
}

function validateMotion(reference: FramescaperScapeAssetReferenceFinishing, bytes: Uint8Array): void {
