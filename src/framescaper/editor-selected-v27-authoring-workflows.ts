...
export interface FramescaperSelectedPreparedAuthoringV27 {
	readonly command: unknown;
	readonly rollback?: () => Promise<void>;
}

/** Lazily prepare one selected V27 command and any owned media it roots. */
export async function prepareFramescaperSelectedAuthoringV27(
	surface: FramescaperCandidateAuthoringSurface,
	projectValue: unknown,
	store: AudioEditorProjectStore,
): Promise<Readonly<FramescaperSelectedPreparedAuthoringV27> | null> {
	const project = record(projectValue, 'Framescaper V27 project');
	if (project.schemaVersion !== 27) throw new TypeError('Selected authoring requires schema V27.');
	...
	throw new RangeError(`Selected V27 authoring does not activate ${surface}.`);
}
...
async function prepareFreezeFrame(
	project: ReadonlyData,
	store: AudioEditorProjectStore,
): Promise<Readonly<FramescaperSelectedPreparedAuthoringV27>> {
...
): Readonly<FramescaperSelectedPreparedAuthoringV27> {
