...
export type FramescaperImageClipPlacementV30 =
	| Readonly<{ readonly scope: 'timeline'; readonly trackId: string }>
	| Readonly<{ readonly scope: 'project-bin' }>;

export interface FramescaperImageSourceSetCommandV30 {
...
export interface FramescaperImageClipSetCommandV30 {
	readonly type: 'image-clip/set';
	readonly clipId: string;
	readonly expectedClip: FramescaperImageClipV1 | null;
	readonly expectedPlacement: FramescaperImageClipPlacementV30 | null;
	readonly clip: FramescaperImageClipV1 | null;
	readonly placement: FramescaperImageClipPlacementV30 | null;
}

export type FramescaperImageCommandV30 =
	| FramescaperImageSourceSetCommandV30
	| FramescaperImageClipSetCommandV30;

const ID = /^[A-Za-z0-9][A-Za-z0-9._:-]{0,127}$/u;

export function snapshotFramescaperImageCommandV30(value: unknown): FramescaperImageCommandV30 {
	...
	throw new RangeError('Framescaper V30 image command type is unsupported.');
}

export function applyFramescaperImageCommandV30(
	project: Record<string, unknown>,
	command: FramescaperImageCommandV30,
): void {
	if (command.type === 'image-source/set') applySource(project, command);
	else applyClip(project, command);
}

function sourceCommand(value: unknown): FramescaperImageSourceSetCommandV30 {
	const command = readClosedDomainRecord(value, 'Framescaper V30 image source command', [
...
function clipCommand(value: unknown): FramescaperImageClipSetCommandV30 {
	const command = readClosedDomainRecord(value, 'Framescaper V30 image clip command', [
	...
	if ((expectedClip === null) !== (expectedPlacement === null)
		...
		throw new RangeError('A V30 image clip and its placement are present or absent together.');
	}
...
function applySource(project: Record<string, unknown>, command: FramescaperImageSourceSetCommandV30): void {
	...
	if (!same(current, command.expectedSource)) throw new Error('The expected V30 image source is stale.');
...
function applyClip(project: Record<string, unknown>, command: FramescaperImageClipSetCommandV30): void {
	...
	if (!same(current, command.expectedClip) || !same(currentPlacement, command.expectedPlacement)) {
		throw new Error('The expected V30 image clip or placement is stale.');
	}
	...
	if (command.clip !== null && command.placement?.scope === 'timeline') {
		...
		if (!target || target.type !== 'video') throw new ReferenceError('A V30 image clip requires a video track.');
...
function optionalPlacement(value: unknown): FramescaperImageClipPlacementV30 | null {
	if (value === null) return null;
	const discriminant = readClosedDomainRecord(value, 'V30 image placement', ['scope', 'trackId'], ['scope']);
	const scope = field(discriminant, 'scope');
	if (scope === 'project-bin') {
		readClosedDomainRecord(value, 'V30 image Project Bin placement', ['scope']);
		return Object.freeze({ scope });
	}
	if (scope === 'timeline') {
		const placement = readClosedDomainRecord(value, 'V30 image timeline placement', ['scope', 'trackId']);
		return Object.freeze({
			scope, trackId: stableId(field(placement, 'trackId'), 'placement.trackId'),
		});
	}
	throw new RangeError('V30 image placement scope is unsupported.');
}
...
function assertMutation(before: unknown, after: unknown, name: string): void {
	if (before === null && after === null) throw new RangeError(`A V30 ${name} command must mutate state.`);
}
...
function assertIdentity(id: string, before: unknown, after: unknown, name: string): void {
	for (const value of [before, after]) {
		if (value !== null && (value as Readonly<{ id: string }>).id !== id) {
			throw new RangeError(`A V30 ${name} command cannot change identity.`);
		}
	}
}

function commandType(value: unknown): string {
	const command = record(value, 'Framescaper V30 image command');
	const type = readClosedDomainField(command, 'type', 'Framescaper V30 image command');
	if (typeof type !== 'string') throw new TypeError('Framescaper V30 image command.type must be a string.');
	return type;
}

function field(recordValue: Readonly<Record<string, unknown>>, name: string): unknown {
	return readClosedDomainField(recordValue, name, 'Framescaper V30 image command');
}

function stableId(value: unknown, name: string): string {
	if (typeof value !== 'string' || !ID.test(value)) throw new TypeError(`Framescaper V30 ${name} must be a stable ID.`);
	return value;
}

function assertUnlocked(track: Record<string, unknown> | undefined, name: string): void {
	if (track?.locked === true) throw new Error(`The V30 image clip ${name} track is locked.`);
}
