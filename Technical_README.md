...
locale under `src/common/i18n/audacity/`, generated by
`scripts/audacity-qt-translations.mjs commit`. At runtime a locale's catalog
is lazy-imported through the generated `src/common/i18n/audacity/index.js`
...
one JSON catalog per locale at `src/common/i18n/audacity/<locale>.json`, with
...
`src/common/i18n/audacity/`, embedding the upstream commit, artifact, and
mapping provenance in each catalog, and a test
(`tests/i18n-audacity-catalog.test.js`) holds the result to the runtime's
rules before anything is committed.

When the layer changed, the workflow commits `src/common/i18n/audacity/`
...
`commit` writes directly into the checkout at `src/common/i18n/audacity/`;
review the result with `git diff -- src/common/i18n/audacity` before
committing it.
