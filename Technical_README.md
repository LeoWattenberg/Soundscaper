...
Audacity-derived locale strings are committed source: one JSON catalog per
locale under `src/common/i18n/translations/`, generated by
`scripts/audacity-qt-translations.mjs commit`. At runtime a locale's catalog
is lazy-imported through the generated `src/common/i18n/translations/index.js`
loader, the same way as the machine catalogs, and laid over them.
...
browser tests and write a report. The build reuses pinned, already published
AI runtimes without compiling them. Publish changed archives through **Update
AI assets** and refresh the committed test runtime snapshots first.
...
### 4. Audacity translation sync

Audacity's reviewed translations are committed source, not a runtime fetch:
one JSON catalog per locale at `src/common/i18n/translations/<locale>.json`, with
a generated `index.js` loader, both produced by
`scripts/audacity-qt-translations.mjs commit`. No R2 bucket, custom domain, or
CORS/cache configuration is needed for translations; the only remaining
per-product R2 bucket is `soundscaper-assets` for the retained legacy FFmpeg
publication tooling, described above.

[`sync-audacity-translations.yml`](.github/workflows/sync-audacity-translations.yml)
runs at 03:37 UTC every Monday and can also be dispatched manually. It needs no
repository secret: its `Discover and verify the latest upstream artifact` step
authenticates only with the workflow's own `GITHUB_TOKEN` to select the newest
successful scheduled run of Audacity's `translate_tx_pull_to_s3.yml` workflow,
then downloads and checks its artifact's official GitHub byte length and
SHA-256 digest. The `Convert the reviewed strings into the committed layer`
step then runs `scripts/audacity-qt-translations.mjs commit` to write
`src/common/i18n/translations/`, embedding the upstream commit, artifact, and
mapping provenance in each catalog, and a test
(`tests/i18n-translation-catalog.test.js`) holds the result to the runtime's
rules before anything is committed.

When the layer changed, the workflow commits `src/common/i18n/translations/`
directly to `main` using the `contents: write` permission granted to the
workflow, then explicitly dispatches `quality.yml` on that commit: a push made
with the default `GITHUB_TOKEN` starts no `push` or `pull_request` workflow of
its own, so the quality gate is asked for by name rather than left to trigger
itself. A rollback is an ordinary `git revert` of the sync commit; there is no
separate release to roll back.

To run the same discover-and-commit steps locally:

```bash
work="$(mktemp -d)"
node scripts/manage-audacity-translation-release.mjs discover \
  --output "$work/source" --github-env "$work/source.env"
set -a
. "$work/source.env"
set +a
node scripts/audacity-qt-translations.mjs commit \
...
`discover` reads an optional `GITHUB_TOKEN` from the environment to raise its
GitHub API rate limit but needs no credential against a public repository.
`commit` writes directly into the checkout at `src/common/i18n/translations/`;
review the result with `git diff -- src/common/i18n/translations` before
committing it.
