...
**Status:** **The complete menu-reached Milestone 7 workflow layer is implemented on selected Soundscaper S30 and Framescaper F31, but new model-backed routes are only conditionally active and production qualification remains open.** The versioned workflow contract, aggregate fences, guided and Advanced recipes, stage consent/progress, semantic review, disposable indexes, and ordinary undoable publishers cover the planned 7A/7B surface. Existing Parakeet, Silero, diarization, cleanup, and model-free fast-shot execution remains active.
Conditional runtime-family workers cover Whisper/alignment, DeepFilterNet enhancement, TIGER separation, PANNs reactions, Beat This beats, TransNetV2 accurate shots, nomic/SigLIP search, OCR, subject/saliency reframe, deterministic highlights, and bounded Qwen editorial output. A route runs only with its exact installed, signed-catalog model and authenticated target runtime; missing evidence returns typed unavailability without substitution, implicit install, or canonical mutation. Review starts unselected and acceptance revalidates the aggregate fence before publishing ordinary transcript, labels, derived audio, tempo, annotations, indexes, crop/keyframes, or secondary sequences in one undo step.
Production release evidence is still incomplete: TIGER/PANNs/Beat This/TransNetV2 conversions and parity, new externally signed catalog entries, all five target closures for ONNX Runtime/whisper.cpp/llama.cpp, the Windows-arm64 Sherpa Node addon, live EU R2 publication and full-digest public read-back, five-target packaged canaries, and owner-lab results remain pending. Authenticated preseed remains the established zero-network path.
