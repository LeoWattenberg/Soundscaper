...
export function createFramescaperOpenFxBuildCommand(phase, executable, args, cwd, environment) {
...
export function runFramescaperOpenFxHostRecipeCli(options) {
...
function parseCli(argv, optionFields) {
	...
	for (let index = 0; index < argv.length; index += 1) {
		...
		if (!key?.startsWith('--') || index + 1 >= argv.length) {
			throw new TypeError('The OpenFX-host recipe arguments are invalid.');
		}
