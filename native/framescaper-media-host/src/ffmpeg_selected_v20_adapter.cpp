...
// Closed adapter identity retained for audit: selected-v20-v7-keyed-rgba.
...
[[nodiscard]] audio_input open_audio(
	...
	av_dict_set(&options, "protocol_whitelist", stream_fd >= 0 ? "pipe" : "file", 0);
	av_dict_set(&options, "format_whitelist", "wav", 0);
	const auto source = stream_fd >= 0 ? "pipe:" + std::to_string(stream_fd) : path.string();
	const AVInputFormat* format = stream_fd >= 0 ? av_find_input_format("wav") : nullptr;
	auto status = avformat_open_input(&input.format, source.c_str(), format, &options);
...
struct mux_session final {
	AVFormatContext* format{};
	std::unique_ptr<ffmpeg_video_encode_session> video_binding;
	...
	mux_session(mux_session&& other) noexcept
		: format{std::exchange(other.format, nullptr)},
		video_binding{std::move(other.video_binding)}, video{std::exchange(other.video, nullptr)},
	...
	~mux_session() {
		av_audio_fifo_free(audio_fifo); swr_free(&audio_resampler);
		avcodec_free_context(&audio); video = nullptr; video_binding.reset();
...
[[nodiscard]] std::array<std::string_view, 4> quality_settings(
	...
	if (container == "mp4" && quality == "draft") return {"28", "veryfast", "", "128000"};
	if (container == "mp4" && quality == "balanced") return {"23", "medium", "", "192000"};
	if (container == "mp4" && quality == "high") return {"18", "slow", "", "256000"};
	if (container == "mov" && quality == "draft") return {"0", "", "", ""};
	if (container == "mov" && quality == "balanced") return {"2", "", "", ""};
	if (container == "mov" && quality == "high") return {"3", "", "", ""};
...
void configure_video(
	...
	const selected_v20_execution_plan& plan, const std::array<std::string_view, 4>&
) {
	const AVRational time_base{
		plan.output_rate.denominator().convert_to<int>(),
		plan.output_rate.numerator().convert_to<int>(),
	};
	output.video_binding = ffmpeg_video_encode_session::open({
		job.admitted_plan, job.backend, plan.width, plan.height,
		time_base, {time_base.den, time_base.num},
		(output.format->oformat->flags & AVFMT_GLOBALHEADER) != 0,
	});
	output.video = output.video_binding->context();
...
void configure_audio(
	...
	const auto encoder_name = job.admitted_plan.audio_encoder.empty() ?
		(job.admitted_plan.container == "mp4" ? "aac" : "libopus") : job.admitted_plan.audio_encoder;
	...
	if (!quality[3].empty()) output.audio->bit_rate = std::stoll(std::string{quality[3]});
	...
	output.audio_fifo = av_audio_fifo_alloc(output.audio->sample_fmt,
		output.audio->ch_layout.nb_channels, std::max(1, output.audio->frame_size * 2));
	if (output.audio_fifo == nullptr)
		throw adapter_failure("audio-allocation", "The audio FIFO cannot be allocated.");
}
...
void write_video_frame(mux_session& output, const selected_v20_output_frame& frame) {
	...
	auto* encoded = output.video_binding->prepare(
		planes.data(), strides.data(), output.video->width, output.video->height,
		AV_PIX_FMT_RGBA, static_cast<std::int64_t>(frame.output_ordinal), 1
	);
	const auto status = avcodec_send_frame(output.video, encoded);
	if (status < 0 && output.video_binding->hardware()) {
		throw ffmpeg_encode_failure(
			"hardware-encoder-failed", "Send one exact hardware-selected frame: " + ffmpeg_error(status), 78
		);
	}
	require(status, "Send a selected-V20 video frame");
...
void reauthenticate_sources(const invocation& job) {
	for (std::size_t index = 0; index < job.sources.size(); ++index) {
		not_cancelled();
		if (job.source_stream_fds.at(index) >= 0) continue;
...
engine_result execute_selected_v20_keyed_adapter(
	...
	try {
		...
		std::unique_ptr<selected_v20_frame_pack> carrier;
		if (job.source_stream_fds.at(carrier_index) == 0) {
			carrier = std::make_unique<selected_v20_frame_pack>(
				std::cin, job.source_byte_lengths.at(carrier_index)
			);
		} else {
			carrier = std::make_unique<selected_v20_frame_pack>(
		...
		if (audio_index) audio.emplace(open_audio(
			job.sources.at(*audio_index), job.source_stream_fds.at(*audio_index), plan.sample_rate
		));
		...
		if (job.backend != "native-cpu" && error.status() != 75) {
...
bool self_test_selected_v20_keyed_adapter() noexcept {
	return selected_v20_maximum_frame_bytes == 8U * 1024U * 1024U
		&& ffmpeg_professional_cpu_encoder_set_available()
		&& av_get_pix_fmt("rgba") == AV_PIX_FMT_RGBA
		&& av_get_pix_fmt("yuv420p") == AV_PIX_FMT_YUV420P
		&& av_get_pix_fmt("yuv422p10le") == AV_PIX_FMT_YUV422P10LE;
}
