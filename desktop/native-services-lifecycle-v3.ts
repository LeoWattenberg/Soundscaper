...
} from './native-services-checkpoint-recovery-v3.ts';
...
export interface FramescaperNativeServicesLifecycleV3Options {
	...
	readonly onQueueEnqueued?: (record: NativeQueueRecordV3) => void;
}
 ...
 */
export class FramescaperNativeServicesLifecycleV3 {
	readonly #queue: FramescaperNativeQueueRepository;
	readonly #roots: FramescaperNativeRootRepository;
	readonly #watch: FramescaperNativeWatchRepository;
	readonly #scratch: FramescaperNativeScratchRepository;
	...
	readonly #mintOpaqueId: () => string;
	readonly #mintJobId: () => string;
	readonly #selectRoot: FramescaperNativeServicesLifecycleV3Options['selectRoot'];
	readonly #probeRoot: FramescaperNativeServicesLifecycleV3Options['probeRoot'];
	readonly #reauthorizeQueue: FramescaperNativeServicesLifecycleV3Options['reauthorizeQueue'];
	readonly #watchCoordinator: FramescaperNativeWatchCoordinatorPort;
	readonly #scratchCleanup: FramescaperNativeServicesLifecycleV3Options['scratchCleanup'];
	readonly #publicationPortFor: FramescaperNativeServicesLifecycleV3Options['publicationPortFor'];
	readonly #publicationFenceFor: FramescaperNativeServicesLifecycleV3Options['publicationFenceFor'];
	readonly #removeRenderInputs: FramescaperNativeServicesLifecycleV3Options['removeRenderInputs'];
	readonly #checkpointInspectFor: FramescaperNativeServicesLifecycleV3Options['checkpointInspectFor'];
	readonly #checkpointStore: FramescaperNativeServicesLifecycleV3Options['checkpointStore'];
	readonly #externalDisplay: FramescaperNativeServicesLifecycleV3Options['externalDisplay'];
	readonly #onQueueEnqueued: FramescaperNativeServicesLifecycleV3Options['onQueueEnqueued'];

	constructor(options: FramescaperNativeServicesLifecycleV3Options) {
		...
		this.#reauthorizeQueue = options.reauthorizeQueue;
	...
	enqueue(value: unknown): NativeQueueRecordV3 {
	...
	async reauthorizeQueueRoot(value: unknown): Promise<NativeQueueRecordV3 | null> {
		if (!this.#selectRoot || !this.#reauthorizeQueue) throw unavailable('reauthorize a queue destination');
		const { jobId } = framescaperNativeLifecycleIdRequest(value, 'jobId');
		const current = this.#queue.read(jobId);
		if (current === null) throw new Error('The native queue job does not exist.');
		if (current.state !== 'needs-authorization') {
			throw new Error('Only a queue job awaiting destination authorization may replace its root grant.');
		}
		const oldRoot = this.#roots.read(current.rootGrantId);
		if (oldRoot === null) throw new Error('The former destination identity is unavailable for comparison.');
		const selection = await this.#selectRoot();
		if (selection === null) return null;
		if (selection.rootPath !== oldRoot.rootPath
			|| selection.volumeIdentity !== oldRoot.volumeIdentity
			|| selection.directoryIdentity !== oldRoot.directoryIdentity) {
			throw new Error('Queue reauthorization requires the same directory path and physical identity.');
		}
		const candidate = Object.freeze({ ...selection, revokedAtMs: null });
		const revalidation = await this.#reauthorizeQueue(Object.freeze({
			...current, rootGrantId: selection.grantId,
		}), candidate);
		if (!queueRevalidationCleared(revalidation)) {
			throw new Error('The queue job changed while its destination was being reauthorized.');
		}
		const authorized = this.#queue.reauthorizeRoot(
			current, selection, this.#lease(), this.#now(),
		);
		this.#onQueueEnqueued?.(authorized);
		return authorized;
	}
	...
	async checkpoint(value: unknown): Promise<NativeImageSequenceCheckpointResultV1> {
		...
		const grant = this.#roots.requireActive(job.rootGrantId);
		return verifyAndStoreNativeImageSequenceCheckpointV3(
