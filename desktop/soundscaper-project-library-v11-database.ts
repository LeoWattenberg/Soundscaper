...
import {
	DESKTOP_PROJECT_LIBRARY_V11_APPLICATION_ID,
	DESKTOP_PROJECT_LIBRARY_V11_DATABASE_VERSION,
} from './soundscaper-project-library-v11-contract.ts';
import { emptySoundscaperDesktopLibraryV11Metadata } from './soundscaper-project-library-v11-metadata.ts';

export function initializeSoundscaperDesktopProjectLibraryV11Database(
	database: DatabaseSync,
): void {
	const applicationId = pragmaNumber(database, 'application_id');
	if (applicationId !== 0 && applicationId !== DESKTOP_PROJECT_LIBRARY_V11_APPLICATION_ID) {
		throw new Error('Soundscaper desktop V11 database belongs to another application');
	}
	const userVersion = pragmaNumber(database, 'user_version');
	if (userVersion !== 0 && userVersion !== DESKTOP_PROJECT_LIBRARY_V11_DATABASE_VERSION) {
		throw new Error('Unsupported Soundscaper desktop V11 database version');
	}
	...
	try {
		...
		`);
		const empty = emptySoundscaperDesktopLibraryV11Metadata();
		...
		`).run();
		database.exec(`
		PRAGMA application_id = ${String(DESKTOP_PROJECT_LIBRARY_V11_APPLICATION_ID)};
		PRAGMA user_version = ${String(DESKTOP_PROJECT_LIBRARY_V11_DATABASE_VERSION)};
		COMMIT;
		`);
	} catch (error) {
...
export function assertSoundscaperDesktopProjectLibraryV11DatabaseIdentity(
	database: DatabaseSync,
): void {
	if (pragmaNumber(database, 'application_id') !== DESKTOP_PROJECT_LIBRARY_V11_APPLICATION_ID
		|| pragmaNumber(database, 'user_version') !== DESKTOP_PROJECT_LIBRARY_V11_DATABASE_VERSION) {
		throw new Error('Soundscaper desktop V11 database identity is invalid');
	}
}
...
function pragmaNumber(database: DatabaseSync, name: 'application_id' | 'user_version'): number {
	...
	if (typeof value !== 'number' || !Number.isSafeInteger(value) || value < 0) {
		throw new RangeError(`Soundscaper desktop V11 PRAGMA ${name} is invalid`);
	}
	return value;
}
