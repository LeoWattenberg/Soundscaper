...
import {
	encodeSoundscaperDesktopProjectLibraryV10MetadataRow as encodeMetadataRow,
	freezeSoundscaperDesktopProjectLibraryV10Lease as freezeLease,
	sameSoundscaperDesktopProjectLibraryV10Lease as sameLease,
	sameSoundscaperDesktopProjectLibraryV10MetadataSnapshot as sameMetadataSnapshot,
	type SoundscaperDesktopProjectLibraryV10JournalRow as JournalRow,
	type SoundscaperDesktopProjectLibraryV10Lease,
	type SoundscaperDesktopProjectLibraryV10MetadataRow as MetadataRow,
	validateSoundscaperDesktopProjectLibraryV10JournalRow as validateJournalRow,
	validateSoundscaperDesktopProjectLibraryV10LeaseToken as validateLeaseToken,
	validateSoundscaperDesktopProjectLibraryV10MetadataIntegrity as validateMetadataIntegrity,
	validateSoundscaperDesktopProjectLibraryV10MetadataRow as validateMetadataRow,
	validateSoundscaperDesktopProjectLibraryV10OpaqueId as leaseId,
...
export type { SoundscaperDesktopProjectLibraryV10Lease };
...
export type SoundscaperDesktopProjectLibraryV10Checkpoint =
	| 'prepared'
	| 'committed'
	| 'complete';

export interface SoundscaperDesktopProjectLibraryV10Recovery {
...
interface CreateOptions {
	...
	readonly checkpoint?: (phase: SoundscaperDesktopProjectLibraryV10Checkpoint) => void;
	readonly now?: () => number;
...
interface PersistedLeaseRow {
	...
	readonly lease: SoundscaperDesktopProjectLibraryV10Lease | null;
}

/** Main-process V10 catalog. A remote V21 handshake must admit every operation. */
export class SoundscaperDesktopProjectLibraryV10Catalog {
	readonly owner: Readonly<SoundscaperDesktopProjectLibraryV10Owner>;
	#checkpoint: (phase: SoundscaperDesktopProjectLibraryV10Checkpoint) => void;
	#database: DatabaseSync;
	#gate = createSoundscaperDesktopProjectLibraryV10HandshakeGate();
	...
	private constructor(options: CreateOptions) {
		...
		this.owner = validateSoundscaperDesktopProjectLibraryV10Owner(options.owner);
		this.#checkpoint = options.checkpoint ?? (() => {});
		this.#now = options.now ?? Date.now;
		this.#randomId = options.randomId ?? (() => randomBytes(24).toString('hex'));
	}

	static create(value: unknown): SoundscaperDesktopProjectLibraryV10Catalog {
		const record = snapshotOptions(value);
		if (!(record.database instanceof DatabaseSync)) {
			throw new TypeError('Soundscaper desktop V10 catalog requires a SQLite database');
		}
		for (const field of ['checkpoint', 'now', 'randomId'] as const) {
			if (record[field] !== undefined && typeof record[field] !== 'function') {
				throw new TypeError(`Soundscaper desktop V10 catalog ${field} must be a function`);
			}
		}
		const catalog = new SoundscaperDesktopProjectLibraryV10Catalog({
	...
	readMetadata(): Readonly<SoundscaperDesktopLibraryV10Metadata> {
		this.#assertReady();
		return this.#validatedMetadataRow().metadata;
	}

	acquireLease(options: { readonly ttlMs: number }): SoundscaperDesktopProjectLibraryV10Lease {
		this.#assertReady();
		...
		return this.#transaction(() => {
			...
			if (result.changes !== 1) throw new Error('Soundscaper desktop V10 lease acquisition lost its compare-and-swap');
			return lease;
		});
	}
	...
	releaseLease(lease: SoundscaperDesktopProjectLibraryV10Lease): boolean {
		this.#assertReady();
		const token = validateLeaseToken(lease);
		return this.#transaction(() => {
			const current = this.#leaseRow();
	...
	publishMetadata(options: {
		...
		readonly metadata: SoundscaperDesktopLibraryV10Metadata;
		...
		const lease = validateLeaseToken(record.lease as SoundscaperDesktopProjectLibraryV10Lease);
		...
		if (next.revision !== increment(expectedRevision, 'V10 metadata revision')) {
			throw new RangeError('Soundscaper desktop V10 metadata revision must advance by exactly one');
		}
	...
	recoverMetadata(options: {
		readonly lease: SoundscaperDesktopProjectLibraryV10Lease;
	}): Readonly<SoundscaperDesktopProjectLibraryV10Recovery> {
		this.#assertReady();
		...
		return this.#exclusiveMetadataOperation(() => this.#transaction(() => {
			...
			`).all();
			if (pending.length > 1) throw new Error('Soundscaper desktop V10 catalog has conflicting recovery journals');
			...
			throw new Error('Soundscaper desktop V10 metadata changed outside its recovery journal');
		}));
	}
	...
	#preparePublication(
		...
		`).get()) throw new Error('Soundscaper desktop V10 metadata recovery is required before publishing');
		if (this.#database.prepare(`
			SELECT 1 AS pending FROM publication_journal
			WHERE state IN ('prepared', 'materialized', 'committed') LIMIT 1
		`).get()) throw new Error('Soundscaper desktop V10 body publication recovery is required before publishing');
		const previous = this.#validatedMetadataRow().row;
		if (previous.revision !== expectedRevision) {
			throw new Error('Soundscaper desktop V10 metadata failed its expected revision compare-and-swap');
		}
	...
	#commitPublication(transactionId: string, lease: SoundscaperDesktopProjectLibraryV10Lease): void {
		this.#assertLeaseOwned(lease);
		...
		if (!journal || journal.state !== 'prepared'
			...
			throw new Error('Soundscaper desktop V10 publication journal is not owned and prepared');
		}
		const current = this.#validatedMetadataRow().row;
		if (!sameMetadataSnapshot(current, journal.previous)) {
			throw new Error('Soundscaper desktop V10 metadata changed after journal preparation');
		}
		...
		if (metadataUpdate.changes !== 1) {
			throw new Error('Soundscaper desktop V10 metadata lost its compare-and-swap');
		}
		...
		if (journalUpdate.changes !== 1) {
			throw new Error('Soundscaper desktop V10 publication journal could not commit');
		}
		this.#assertLeaseOwned(lease);
	}

	#settlePublication(transactionId: string, lease: SoundscaperDesktopProjectLibraryV10Lease): void {
		this.#assertLeaseOwned(lease);
		const result = this.#database.prepare(`
		...
		if (result.changes !== 1) throw new Error('Soundscaper desktop V10 publication journal could not complete');
		this.#pruneJournals();
		this.#assertLeaseOwned(lease);
	}
	...
	#completeRecoveryJournal(transactionId: string, state: 'complete' | 'recovered'): void {
		...
		if (result.changes !== 1) throw new Error('Soundscaper desktop V10 recovery journal could not settle');
		this.#pruneJournals();
	}
	...
	#validatedMetadataRow(): {
		...
		`).get();
		if (!raw) throw new Error('Soundscaper desktop V10 metadata row is missing');
		const row = validateMetadataRow(raw, 'persisted V10 metadata');
		const metadata = validateMetadataIntegrity(row, 'Soundscaper desktop V10 metadata');
		return { metadata, row };
	}
	...
	#leaseRow(): PersistedLeaseRow {
		...
		`).get();
		if (!raw) throw new Error('Soundscaper desktop V10 lease row is missing');
		...
		if (fencingToken === 0) {
			if (active || raw.leaseId !== null || raw.ownerProduct !== null
				...
				throw new TypeError('Initial Soundscaper desktop V10 lease row is invalid');
			}
		...
		const lease = freezeLease({
			...
			fencingToken,
			owner: validateSoundscaperDesktopProjectLibraryV10Owner({
	...
	#assertLeaseOwned(
		token: SoundscaperDesktopProjectLibraryV10Lease,
		now = this.#timestamp(),
	): SoundscaperDesktopProjectLibraryV10Lease {
		const current = this.#leaseRow();
		if (!current.active || !current.lease || current.lease.expiresAtMs <= now
	...
	#exclusiveMetadataOperation<Result>(operation: () => Result): Result {
		if (this.#metadataOperationActive) {
			throw new Error('A Soundscaper desktop V10 metadata operation is already active');
		}
	...
	#timestamp(): number {
		return nonNegativeInteger(this.#now(), 'Soundscaper desktop V10 clock');
	}
...
function snapshotOptions(value: unknown): Record<typeof CREATE_FIELDS[number], unknown> {
	const record = snapshotRecord(value, CREATE_FIELDS, 'Soundscaper desktop V10 catalog options');
	if (record.database === undefined || record.owner === undefined) {
		throw new TypeError('Soundscaper desktop V10 catalog options are incomplete');
	}
	return record;
}
...
function freezeRecovery(
	value: SoundscaperDesktopProjectLibraryV10Recovery,
): Readonly<SoundscaperDesktopProjectLibraryV10Recovery> {
...
function leaseTtl(value: unknown): number {
	return boundedInteger(value, MINIMUM_LEASE_TTL_MS, MAXIMUM_LEASE_TTL_MS,
		'Soundscaper desktop V10 lease TTL');
}
...
function leaseLost(): Error {
	return new Error('Soundscaper desktop V10 lease holder no longer owns the lease');
}
