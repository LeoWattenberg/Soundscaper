...
export interface FramescaperNativeQueueCapacityContextV1 {
	readonly queue: readonly NativeQueueRecordV2[];
	readonly scratch: readonly FramescaperNativeScratchReservation[];
}

export type FramescaperNativeQueueCapacityProvider = (
	context: FramescaperNativeQueueCapacityContextV1,
) => Promise<NativeQueueCapacityV1>;

export interface FramescaperNativeQueueCapacityProviderOptions {
 ...
 */
export function createFramescaperNativeQueueCapacityProvider(
	options: FramescaperNativeQueueCapacityProviderOptions,
...
function queueRecords(value: unknown): readonly NativeQueueRecordV2[] {
	if (!Array.isArray(value)) throw new TypeError('A native queue capacity context requires queue rows.');
	for (const record of value) assertNativeQueueRecordV2(record);
	return value as readonly NativeQueueRecordV2[];
}
...
function committedScratchBytes(
	running: readonly NativeQueueRecordV2[],
