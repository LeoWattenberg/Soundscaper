...
import type { SoundscaperDesktopProjectLibraryV11Handshake } from './soundscaper-project-library-v11-contract.ts';
import type {
	SoundscaperDesktopProjectLibraryV11CatalogSnapshot,
	SoundscaperDesktopProjectLibraryV11DeleteResult,
...
import type { SoundscaperDesktopProjectLibraryV11PublicationHost } from './soundscaper-project-library-v11-publication-host.ts';
...
import {
	type SoundscaperDesktopProjectLibraryV11TransferService,
	type SoundscaperDesktopProjectLibraryV11TransferSession,
} from './soundscaper-project-library-v11-transfer-service.ts';

const MAXIMUM_ACTIVE_SESSIONS = 16;

export interface SoundscaperDesktopProjectLibraryV11MainSession {
	listProjects(): Promise<Readonly<SoundscaperDesktopProjectLibraryV11CatalogSnapshot>>;
	readProjectBundle(projectId: string): Promise<Readonly<SoundscaperDesktopProjectLibraryV11TransferBundle> | null>;
...
export class SoundscaperDesktopProjectLibraryV11MainSessionService {
	...
	readonly #transfer: SoundscaperDesktopProjectLibraryV11TransferService;
	...
	constructor(
		...
		lease: SoundscaperDesktopProjectLibraryV11Lease,
		transfer: SoundscaperDesktopProjectLibraryV11TransferService,
	) {
		this.#host = host;
	...
	updateLease(lease: SoundscaperDesktopProjectLibraryV11Lease): void {
		this.#lease = lease;
	}
	...
	openSession(value: unknown): SoundscaperDesktopProjectLibraryV11MainSession {
		...
		if (this.#sessions.size >= MAXIMUM_ACTIVE_SESSIONS) {
			throw new RangeError('Soundscaper V11 main session capacity is exhausted');
		}
		const session = new MainSession(this, transfer);
		this.#sessions.add(session);
		return Object.freeze(session) as SoundscaperDesktopProjectLibraryV11MainSession;
	}

	async close(reason: unknown = new Error('Soundscaper V11 main service is closed')): Promise<void> {
	...
	async begin(session: MainSession, value: unknown) {
		...
		if (this.#active || this.#lifecycleActive) {
			throw new RangeError('Soundscaper V11 publication capacity is exhausted');
		}
		...
		try {
			await upload.start();
			return validateSoundscaperDesktopProjectLibraryV11PublicationAdmission({
				publicationId: upload.id,
				maximumChunkBytes: MAXIMUM_SOUNDSCAPER_V11_TRANSFER_CHUNK_BYTES,
	...
	async write(session: MainSession, value: unknown) {
		...
		return validateSoundscaperDesktopProjectLibraryV11PublicationChunkAcknowledgement(result, request);
	}
	...
	async abort(session: MainSession, value: unknown): Promise<boolean> {
		...
		try {
			await upload.abort(new Error('Soundscaper V11 renderer publication was aborted'));
			return true;
		} finally {
	...
	#ownedUpload(session: MainSession, publicationId: string): PublicationUpload {
		...
		if (!upload || upload.id !== publicationId || !upload.belongsTo(session)) {
			throw new Error('Soundscaper V11 publication does not belong to this main session');
		}
		return upload;
	}
	...
	#assertSession(session: MainSession): void {
		this.#assertAccepting();
		if (!this.#sessions.has(session)) throw new Error('Soundscaper V11 main session is closed');
	}

	#assertAccepting(): void {
		if (this.#closed) throw new Error('Soundscaper V11 main service is closed');
		if (this.#fenced !== null) {
			throw new Error('Soundscaper V11 main service lost its writer fence', { cause: this.#fenced });
		}
	}
}

class MainSession implements SoundscaperDesktopProjectLibraryV11MainSession {
	readonly #controller = new AbortController();
	readonly #operations = new Set<Promise<unknown>>();
	readonly #service: SoundscaperDesktopProjectLibraryV11MainSessionService;
	readonly #transfer: SoundscaperDesktopProjectLibraryV11TransferSession;
	#closePromise: Promise<void> | null = null;
	#closed = false;

	constructor(
		service: SoundscaperDesktopProjectLibraryV11MainSessionService,
		transfer: SoundscaperDesktopProjectLibraryV11TransferSession,
	) {
	...
	close(): Promise<void> {
		return this.closeFromService(new Error('Soundscaper V11 main session is closed'));
	}
	...
	#admit<Result>(operation: () => Promise<Result>): Promise<Result> {
		if (this.#closed) return Promise.reject(new Error('Soundscaper V11 main session is closed'));
...
class PublicationUpload {
	...
	readonly #request: ReturnType<typeof validateSoundscaperDesktopProjectLibraryV11PublicationBeginRequest>;
	readonly #streams: PublicationByteStream[];
	#bodyIndex = 0;
	#offset = 0;
	#result: Promise<Readonly<SoundscaperDesktopProjectLibraryV11TransferBundle>> | null = null;
	...
	constructor(
		...
		host: SoundscaperDesktopProjectLibraryV11PublicationHost,
		lease: SoundscaperDesktopProjectLibraryV11Lease,
		request: ReturnType<typeof validateSoundscaperDesktopProjectLibraryV11PublicationBeginRequest>,
	) {
		this.id = id;
		this.#owner = owner;
		this.#host = host;
	...
	async write(bodyIndex: number, offset: number, bytes: Uint8Array) {
		...
		if (!descriptor || bodyIndex !== this.#bodyIndex || offset !== this.#offset) {
			throw new Error('Soundscaper V11 publication chunks must be sequential by body and offset');
		}
		if (bytes.byteLength > descriptor.byteLength - this.#offset) {
			throw new RangeError('Soundscaper V11 publication chunk exceeds its declared body');
		}
	...
	async finish() {
		if (!this.readyToFinish) {
			throw new Error('Soundscaper V11 publication bodies are incomplete');
		}
		if (!this.#result) throw new Error('Soundscaper V11 publication did not start');
		return validateSoundscaperDesktopProjectLibraryV11PublicationResult(
			await this.#result,
			this.#projectId,
		);
	}
...
class PublicationByteStream implements AsyncIterable<Uint8Array>, AsyncIterator<Uint8Array> {
	...
	offer(bytes: Uint8Array): Promise<void> {
		...
		if (this.#complete) return Promise.reject(new Error('Soundscaper V11 publication body is complete'));
		if (this.#pending) return Promise.reject(new Error('Soundscaper V11 publication stream capacity is exhausted'));
