...
export function validateFramescaperDesktopProjectLibraryV10CatalogSnapshot(
	value: unknown,
): Readonly<FramescaperDesktopProjectLibraryV10CatalogSnapshot> {
	const raw = closedRecord(value, CATALOG_FIELDS, 'Framescaper V10 catalog snapshot');
	const projects = denseArray(raw.projects, 'Framescaper V10 project summaries', MAXIMUM_PROJECTS)
		.map(projectSummary);
	if (new Set(projects.map(({ id }) => id)).size !== projects.length) {
		throw new TypeError('Framescaper V10 project summaries contain duplicate identities');
	}
...
export function validateFramescaperDesktopProjectLibraryV10DeleteRequest(
	value: unknown,
): Readonly<FramescaperDesktopProjectLibraryV10DeleteRequest> {
...
export function validateFramescaperDesktopProjectLibraryV10DeleteResult(
	value: unknown,
	expectedProjectId?: string,
): Readonly<FramescaperDesktopProjectLibraryV10DeleteResult> {
	const raw = closedRecord(value, DELETE_RESULT_FIELDS, 'Framescaper V10 delete result');
	const projectId = validateFramescaperDesktopProjectLibraryV10ProjectId(raw.projectId);
	if (expectedProjectId !== undefined
		&& projectId !== validateFramescaperDesktopProjectLibraryV10ProjectId(expectedProjectId)) {
		throw new Error('Framescaper V10 delete returned another project identity');
	}
	if (raw.deleted !== true) throw new TypeError('Framescaper V10 delete result is invalid');
	return Object.freeze({
		projectId,
		metadataRevision: nonNegativeInteger(raw.metadataRevision, 'metadata revision'),
		deleted: true,
...
export function validateFramescaperDesktopProjectLibraryV10DuplicateRequest(
	value: unknown,
): Readonly<FramescaperDesktopProjectLibraryV10DuplicateRequest> {
...
function projectSummary(value: unknown): Readonly<FramescaperDesktopProjectLibraryV10ProjectSummary> {
	const raw = closedRecord(value, SUMMARY_FIELDS, 'Framescaper V10 project summary');
	return Object.freeze({
		id: validateFramescaperDesktopProjectLibraryV10ProjectId(raw.id),
		title: boundedTitle(raw.title),
...
function boundedTitle(value: unknown): string {
	if (typeof value !== 'string' || !value.trim()
		...
		throw new TypeError('Framescaper V10 project title is invalid');
	}
	return value;
}

function canonicalTimestamp(value: unknown): string {
	if (typeof value !== 'string') throw new TypeError('Framescaper V10 project timestamp is invalid');
	const time = Date.parse(value);
	if (!Number.isFinite(time) || new Date(time).toISOString() !== value) {
		throw new TypeError('Framescaper V10 project timestamp is invalid');
	}
	return value;
}
