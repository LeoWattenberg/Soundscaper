...
/** Runs only after the maintained renderer has activated its controller and V10 store. */
export async function runFramescaperV18ArtifactRendererSmoke(scope, expected) {
	const fail = (message) => { throw new Error(`Framescaper V18 artifact smoke ${message}`); };
	...
	if (preload.handshakeState() !== 'admitted') fail('requires an admitted V10 preload handshake');
	...
	if (preload.handshakeState() !== 'admitted'
		...
		|| handshake.projectSchemaVersion !== 18
		|| JSON.stringify(handshake.scapeFormatVersions) !== '[1,2]'
		|| handshake.attachedScapeFormatVersion !== 2
		|| handshake.storageDatabaseName !== 'kw-media-framescaper-editor-v18'
		|| handshake.desktopLibrarySchemaVersion !== 10
		|| handshake.desktopDatabaseUserVersion !== 12
		|| JSON.stringify(handshake.desktopLibraryScope) !== '["kw.media","scape-project-library","v10"]') {
		fail('received a drifted V10 handshake');
	}
	...
	if (!Array.isArray(bundle.bodies) || bundle.bodies.length !== 0) {
		fail('requires a source-free V18 package fixture');
	}
	if (typeof bundle.document !== 'string' || bundle.document.length === 0) {
		fail('requires a V18 project document');
	}
	let document;
	try { document = JSON.parse(bundle.document); }
	catch { fail('requires a JSON V18 project document'); }
	if (JSON.stringify(document) !== bundle.document) fail('requires a canonical V18 project document');
	if (document?.schemaVersion !== 18 || document?.id !== projectId
		...
		fail('UI and V18 bundle do not match');
	}
	if (!Number.isSafeInteger(bundle.metadataRevision) || bundle.metadataRevision < 1
		...
		fail('received an invalid V10 project descriptor');
	}
	...
	if (documentBytes.byteLength !== row.byteLength || sha256 !== row.sha256) {
		fail('V18 bundle bytes do not match the V10 descriptor');
	}
	return {
...
export function joinFramescaperV18ArtifactEvidence(rendererValue, mainValue) {
	const renderer = validateFramescaperV18ArtifactRendererEvidence(rendererValue);
	const main = validateFramescaperV18ArtifactMainEvidence(mainValue, renderer.project);
	return Object.freeze({ ...renderer, main });
}

export function validateFramescaperV18ArtifactEvidence(value) {
	const record = closedRecord(value, ARTIFACT_FIELDS, 'Framescaper V18 artifact evidence');
	return joinFramescaperV18ArtifactEvidence({
...
export function validateFramescaperV18ArtifactRendererEvidence(value) {
	const record = closedRecord(value, RENDERER_FIELDS, 'Framescaper V18 renderer evidence');
	const preloadBridge = exactStringArray(record.preloadBridge, PRELOAD_BRIDGE, 'Framescaper V10 preload bridge');
	const handshake = validateHandshake(record.handshake);
	const project = validateProject(record.project);
	const uiRecord = closedRecord(record.ui, UI_FIELDS, 'Framescaper V18 UI evidence');
	const ui = Object.freeze({
		projectId: text(uiRecord.projectId, 'Framescaper V18 UI project identity'),
		title: text(uiRecord.title, 'Framescaper V18 UI title'),
		trackCount: nonNegativeInteger(uiRecord.trackCount, 'Framescaper V18 UI track count'),
		clipCount: nonNegativeInteger(uiRecord.clipCount, 'Framescaper V18 UI clip count'),
	});
	if (ui.projectId !== project.projectId || ui.title !== project.title) {
		throw new Error('Framescaper V18 UI and renderer readback do not match');
	}
	return Object.freeze({ preloadBridge, handshake, ui, project });
}

export function validateFramescaperV18ArtifactMainEvidence(value, expectedProject) {
	const record = closedRecord(value, MAIN_FIELDS, 'Framescaper V18 main evidence');
	const hostRecord = closedRecord(record.host, HOST_FIELDS, 'Framescaper V18 main host evidence');
	if (hostRecord.product !== 'framescaper' || hostRecord.closed !== false
		|| hostRecord.fenced !== false || hostRecord.activePublication !== false) {
		throw new Error('Framescaper V18 main host was not active and quiescent');
	}
	...
	for (const field of PROJECT_FIELDS) {
		if (project[field] !== expected[field]) {
			throw new Error('Framescaper V18 renderer and main readback do not match');
		}
	}
...
function validateProject(value) {
	const record = closedRecord(value, PROJECT_FIELDS, 'Framescaper V18 project evidence');
	const project = Object.freeze({
		projectId: text(record.projectId, 'Framescaper V18 project identity'),
		title: text(record.title, 'Framescaper V18 project title'),
		projectSchemaVersion: record.projectSchemaVersion,
		projectRevision: nonNegativeInteger(record.projectRevision, 'Framescaper V18 project revision'),
		metadataRevision: positiveInteger(record.metadataRevision, 'Framescaper V18 metadata revision'),
		byteLength: positiveInteger(record.byteLength, 'Framescaper V18 project byte length'),
		sha256: record.sha256,
		bodyCount: record.bodyCount,
	});
	if (project.projectSchemaVersion !== 18 || typeof project.sha256 !== 'string'
		|| !DIGEST.test(project.sha256) || project.bodyCount !== 0) {
		throw new Error('Framescaper V18 project evidence is invalid');
	}
	return project;
}

function validateHandshake(value) {
	const record = closedRecord(value, HANDSHAKE_FIELDS, 'Framescaper V10 handshake evidence');
	if (record.kind !== 'framescaper-project-library-handshake' || record.version !== 1
		|| record.owner !== 'framescaper' || record.projectSchemaVersion !== 18
		|| record.attachedScapeFormatVersion !== 2
		|| record.storageDatabaseName !== 'kw-media-framescaper-editor-v18'
		|| record.desktopLibrarySchemaVersion !== 10 || record.desktopDatabaseUserVersion !== 12) {
		throw new Error('Framescaper V10 handshake evidence is invalid');
	}
	return Object.freeze({
		...
		scapeFormatVersions: exactNumberArray(
			record.scapeFormatVersions, [1, 2], 'Framescaper V10 Scape format versions',
		),
		...
		desktopLibraryScope: exactStringArray(
			...
			['kw.media', 'scape-project-library', 'v10'],
			'Framescaper V10 library scope',
		),
	});
}
