	...
	return Object.freeze({
		...
		processAnalyzerPcm: async (
			...
			const answer = await instance.session.request(VAMP_PEER_OPERATION.process,
				(writer) => writeVampAnalyzerPcm(writer, admitted));
...
async function openSession(
	...
	const request = async (operation: number, build?: (writer: VampPeerWriter) => void) => {
		const perform = async () => {
			...
			return vampPeerResponse(await launch.control!.receive(), operation);
		};
	...
	const close = async () => {
		...
		if (failures.length > 1) throw new AggregateError(failures, 'The Vamp analyzer peer close phases failed.');
	};
...
async function temporary<T>(session: VampPeerSession, operation: () => Promise<T>): Promise<T> {
	...
	if (operationError !== null && closeError !== null) {
		throw new AggregateError([operationError, closeError], 'Vamp scan and peer shutdown both failed.');
	}
