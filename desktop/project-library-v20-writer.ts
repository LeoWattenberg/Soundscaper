...
import { initializeFramescaperDesktopProjectLibraryV20LifecycleDatabase } from './project-library-v20-database.ts';
import { importFramescaperDesktopProjectLibraryV19IntoV20 } from './project-library-v20-import.ts';

const PRODUCTION_LEASE_TTL_MS = 30_000;
const PRODUCTION_RENEW_INTERVAL_MS = 10_000;

export interface FramescaperDesktopProjectLibraryV20Qualification {
...
export interface FramescaperDesktopProjectLibraryV20WriterSnapshot {
...
interface WriterOptions {
	...
	readonly qualification: Readonly<FramescaperDesktopProjectLibraryV20Qualification> | null;
}

export function createFramescaperDesktopProjectLibraryV20Extension(
	...
	return Object.freeze({
		async start(context: Parameters<FramescaperDesktopProjectLibraryExactGenerationExtension['start']>[0]) {
			initializeFramescaperDesktopProjectLibraryV20LifecycleDatabase(context.database);
			const writer = await FramescaperDesktopProjectLibraryV20Writer.acquire({
				...context,
			...
			try {
				...
				await importFramescaperDesktopProjectLibraryV19IntoV20({
				...
				try { await writer.close(); }
					...
					throw new AggregateError([error, cleanupError], 'Framescaper V20 startup cleanup failed');
				}
				throw error;
			}
		},
...
class FramescaperDesktopProjectLibraryV20Writer implements FramescaperDesktopProjectLibraryExactGenerationLifecycle {
	...
	#recovery: FramescaperDesktopProjectLibraryV20WriterSnapshot['recovery'] = Object.freeze({
	...
	static async acquire(value: Readonly<{
		...
		renewIntervalMs: number;
		checkpoint: ((phase: FramescaperDesktopProjectLibraryPublicationCheckpoint) => void) | null;
	}>): Promise<FramescaperDesktopProjectLibraryV20Writer> {
		...
		while (!lease) {
			...
			if (attempt.lease) lease = attempt.lease;
				...
				if (remaining <= 0) throw new Error('Framescaper desktop V20 writer lease is busy');
				await delay(Math.min(remaining, Math.max(10, attempt.retryAtMs - Date.now() + 1)));
			}
		}
		return new FramescaperDesktopProjectLibraryV20Writer({
	...
	assertCanUse(): void {
		if (this.#closed) throw new Error('Framescaper desktop V20 writer is closed');
		if (this.#fenced !== null) throw new Error('Framescaper desktop V20 writer is fenced', { cause: this.#fenced });
		if (Date.now() >= this.#lease.expiresAtMs) {
			const error = new Error('Framescaper desktop V20 writer lease expired before renewal');
			this.#fence(error);
			throw error;
		}
	}

	snapshot(): Readonly<{ fenced: boolean; writer: Readonly<FramescaperDesktopProjectLibraryV20WriterSnapshot> }> {
		return Object.freeze({
	...
	assertLeaseInTransaction(database: DatabaseSync): void {
		this.assertCanUse();
		if (database !== this.#database) throw new Error('Framescaper V20 lease checked against another database');
		...
		if (row?.active !== 1 || row.leaseId !== this.#lease.leaseId
			...
			const error = new Error('Framescaper V20 publication writer lease no longer owns its fence');
			this.#fence(error);
			throw error;
		}
	}
	...
	async preparePublication(value: Readonly<FramescaperDesktopProjectLibraryExactPublicationDeclaration>): Promise<void> {
		...
		if (this.#activePublicationId !== null && this.#activePublicationId !== value.publicationId) {
			throw new Error('Framescaper V20 already owns an active publication');
		}
		this.#transaction(() => {
	...
	assertCanCommit(
		...
		if (row?.state !== 'materialized' || row.leaseId !== this.#lease.leaseId
			|| row.fencingToken !== this.#lease.fencingToken) {
			throw new Error('Framescaper V20 publication journal no longer owns its fence');
		}
	}
	...
	async publicationComplete(value: Readonly<FramescaperDesktopProjectLibraryExactPublicationDeclaration>): Promise<void> {
		this.#transaction(() => {
			...
			`).run(value.publicationId, this.#lease.leaseId, this.#lease.fencingToken).changes !== 1) {
				throw new Error('Framescaper V20 publication journal could not complete');
			}
		});
	...
	async recover(): Promise<void> {
		...
		if (rows.length > 1) throw new Error('Framescaper V20 has conflicting publication recovery journals');
		if (!rows[0]) return;
		...
		try {
			...
			this.#transaction(() => {
				...
				if (released.changes !== 1) {
					throw new Error('Framescaper V20 writer lease release lost ownership');
				}
			});
	...
	#updateJournal(publicationId: string, previous: string, next: string, result: string | null): void {
		this.#transaction(() => {
			...
			if (updated.changes !== 1) throw new Error(`Framescaper V20 publication journal could not enter ${next}`);
		});
	}
	...
	#renew(): void {
		...
		try {
			...
			if (renewed.changes !== 1) throw new Error('Framescaper V20 writer lease renewal lost ownership');
...
function acquireLease(
	...
	try {
		...
		if (!Number.isSafeInteger(fencingToken) || fencingToken < 1) throw new Error('Framescaper V20 fencing token overflow');
		...
		if (typeof ownerJson !== 'string') throw new Error('Framescaper V20 writer owner is not serializable');
		if (database.prepare(`
		...
		).changes !== 1) throw new Error('Framescaper V20 writer lease acquisition raced');
		database.exec('COMMIT');
		return Object.freeze({
...
function timingOptions(qualification: Readonly<FramescaperDesktopProjectLibraryV20Qualification> | null) {
	...
	if (renewIntervalMs >= leaseTtlMs) throw new RangeError('Framescaper V20 renewal must precede lease expiry');
...
function lowerOnly(value: unknown, ceiling: number, label: string): number {
	if (!Number.isSafeInteger(value) || Number(value) < 1 || Number(value) > ceiling) {
		throw new RangeError(`Framescaper V20 qualification ${label} is outside its lower-only range`);
	}
	return Number(value);
}

function sqlText(value: unknown, label: string): string {
	if (typeof value !== 'string' || value.length < 1) throw new Error(`Framescaper V20 ${label} is invalid`);
	return value;
}

function sqlNonNegative(value: unknown, label: string): number {
	if (!Number.isSafeInteger(value) || Number(value) < 0) {
		throw new Error(`Framescaper V20 ${label} is invalid`);
	}
	return Number(value);
}

function containedDocument(root: string, value: string): string {
	if (!value || value.includes('\0')) throw new Error('Framescaper V20 journal document is invalid');
	const result = resolve(root, value);
	const relation = relative(resolve(root), result);
	if (!relation || relation === '..' || relation.startsWith(`..${sep}`)) {
		throw new Error('Framescaper V20 journal document leaves its root');
	}
	return result;
}
