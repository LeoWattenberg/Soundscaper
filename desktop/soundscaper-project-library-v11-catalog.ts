...
import {
	encodeSoundscaperDesktopProjectLibraryV11MetadataRow as encodeMetadataRow,
	freezeSoundscaperDesktopProjectLibraryV11Lease as freezeLease,
	sameSoundscaperDesktopProjectLibraryV11Lease as sameLease,
	sameSoundscaperDesktopProjectLibraryV11MetadataSnapshot as sameMetadataSnapshot,
	type SoundscaperDesktopProjectLibraryV11JournalRow as JournalRow,
	type SoundscaperDesktopProjectLibraryV11Lease,
	type SoundscaperDesktopProjectLibraryV11MetadataRow as MetadataRow,
	validateSoundscaperDesktopProjectLibraryV11JournalRow as validateJournalRow,
	validateSoundscaperDesktopProjectLibraryV11LeaseToken as validateLeaseToken,
	validateSoundscaperDesktopProjectLibraryV11MetadataIntegrity as validateMetadataIntegrity,
	validateSoundscaperDesktopProjectLibraryV11MetadataRow as validateMetadataRow,
	validateSoundscaperDesktopProjectLibraryV11OpaqueId as leaseId,
...
export type { SoundscaperDesktopProjectLibraryV11Lease };
...
export type SoundscaperDesktopProjectLibraryV11Checkpoint =
	| 'prepared'
	| 'committed'
	| 'complete';

export interface SoundscaperDesktopProjectLibraryV11Recovery {
...
interface CreateOptions {
	...
	readonly checkpoint?: (phase: SoundscaperDesktopProjectLibraryV11Checkpoint) => void;
	readonly now?: () => number;
...
interface PersistedLeaseRow {
	...
	readonly lease: SoundscaperDesktopProjectLibraryV11Lease | null;
}

/** Main-process V11 catalog. A remote V21 handshake must admit every operation. */
export class SoundscaperDesktopProjectLibraryV11Catalog {
	readonly owner: Readonly<SoundscaperDesktopProjectLibraryV11Owner>;
	#checkpoint: (phase: SoundscaperDesktopProjectLibraryV11Checkpoint) => void;
	#database: DatabaseSync;
	#gate = createSoundscaperDesktopProjectLibraryV11HandshakeGate();
	...
	private constructor(options: CreateOptions) {
		...
		this.owner = validateSoundscaperDesktopProjectLibraryV11Owner(options.owner);
		this.#checkpoint = options.checkpoint ?? (() => {});
		this.#now = options.now ?? Date.now;
		this.#randomId = options.randomId ?? (() => randomBytes(24).toString('hex'));
	}

	static create(value: unknown): SoundscaperDesktopProjectLibraryV11Catalog {
		const record = snapshotOptions(value);
		if (!(record.database instanceof DatabaseSync)) {
			throw new TypeError('Soundscaper desktop V11 catalog requires a SQLite database');
		}
		for (const field of ['checkpoint', 'now', 'randomId'] as const) {
			if (record[field] !== undefined && typeof record[field] !== 'function') {
				throw new TypeError(`Soundscaper desktop V11 catalog ${field} must be a function`);
			}
		}
		const catalog = new SoundscaperDesktopProjectLibraryV11Catalog({
	...
	readMetadata(): Readonly<SoundscaperDesktopLibraryV11Metadata> {
		this.#assertReady();
		return this.#validatedMetadataRow().metadata;
	}

	acquireLease(options: { readonly ttlMs: number }): SoundscaperDesktopProjectLibraryV11Lease {
		this.#assertReady();
		const record = snapshotClosedRecord(options, ACQUIRE_FIELDS, 'V11 lease acquisition');
		...
		return this.#transaction(() => {
			...
			const fencingToken = increment(previousFence, 'V11 lease fencing token');
			const lease = freezeLease({
				leaseId: this.#newId(),
				fencingToken,
				owner: this.owner,
				acquiredAtMs: now,
				expiresAtMs: checkedAdd(now, ttlMs, 'V11 lease expiry'),
			...
			if (result.changes !== 1) throw new Error('Soundscaper desktop V11 lease acquisition lost its compare-and-swap');
			return lease;
		});
	}
	...
	renewLease(
		...
		const record = snapshotClosedRecord(options, RENEW_FIELDS, 'V11 lease renewal');
		const ttlMs = leaseTtl(record.ttlMs);
		return this.#transaction(() => {
			const now = this.#timestamp();
			const current = this.#assertLeaseOwned(token, now);
			const expiresAtMs = checkedAdd(now, ttlMs, 'V11 lease expiry');
			const result = this.#database.prepare(`
	...
	releaseLease(lease: SoundscaperDesktopProjectLibraryV11Lease): boolean {
		this.#assertReady();
		const token = validateLeaseToken(lease);
		return this.#transaction(() => {
			const current = this.#leaseRow();
	...
	publishMetadata(options: {
		...
		readonly metadata: SoundscaperDesktopLibraryV11Metadata;
	}): Readonly<SoundscaperDesktopLibraryV11Metadata> {
		this.#assertReady();
		const record = snapshotClosedRecord(options, PUBLISH_FIELDS, 'V11 metadata publication');
		const expectedRevision = nonNegativeInteger(record.expectedRevision, 'expected metadata revision');
		const lease = validateLeaseToken(record.lease as SoundscaperDesktopProjectLibraryV11Lease);
		...
		if (next.revision !== increment(expectedRevision, 'V11 metadata revision')) {
			throw new RangeError('Soundscaper desktop V11 metadata revision must advance by exactly one');
		}
	...
	recoverMetadata(options: {
		readonly lease: SoundscaperDesktopProjectLibraryV11Lease;
	}): Readonly<SoundscaperDesktopProjectLibraryV11Recovery> {
		this.#assertReady();
		const record = snapshotClosedRecord(options, RECOVER_FIELDS, 'V11 metadata recovery');
		const lease = validateLeaseToken(record.lease as SoundscaperDesktopProjectLibraryV11Lease);
		return this.#exclusiveMetadataOperation(() => this.#transaction(() => {
			...
			`).all();
			if (pending.length > 1) throw new Error('Soundscaper desktop V11 catalog has conflicting recovery journals');
			...
			throw new Error('Soundscaper desktop V11 metadata changed outside its recovery journal');
		}));
	}
	...
	#preparePublication(
		...
		`).get()) throw new Error('Soundscaper desktop V11 metadata recovery is required before publishing');
		if (this.#database.prepare(`
			SELECT 1 AS pending FROM publication_journal
			WHERE state IN ('prepared', 'materialized', 'committed') LIMIT 1
		`).get()) throw new Error('Soundscaper desktop V11 body publication recovery is required before publishing');
		const previous = this.#validatedMetadataRow().row;
		if (previous.revision !== expectedRevision) {
			throw new Error('Soundscaper desktop V11 metadata failed its expected revision compare-and-swap');
		}
	...
	#commitPublication(transactionId: string, lease: SoundscaperDesktopProjectLibraryV11Lease): void {
		this.#assertLeaseOwned(lease);
		...
		if (!journal || journal.state !== 'prepared'
			...
			throw new Error('Soundscaper desktop V11 publication journal is not owned and prepared');
		}
		const current = this.#validatedMetadataRow().row;
		if (!sameMetadataSnapshot(current, journal.previous)) {
			throw new Error('Soundscaper desktop V11 metadata changed after journal preparation');
		}
		...
		if (metadataUpdate.changes !== 1) {
			throw new Error('Soundscaper desktop V11 metadata lost its compare-and-swap');
		}
		...
		if (journalUpdate.changes !== 1) {
			throw new Error('Soundscaper desktop V11 publication journal could not commit');
		}
		this.#assertLeaseOwned(lease);
	}

	#settlePublication(transactionId: string, lease: SoundscaperDesktopProjectLibraryV11Lease): void {
		this.#assertLeaseOwned(lease);
		const result = this.#database.prepare(`
		...
		if (result.changes !== 1) throw new Error('Soundscaper desktop V11 publication journal could not complete');
		this.#pruneJournals();
		this.#assertLeaseOwned(lease);
	}
	...
	#completeRecoveryJournal(transactionId: string, state: 'complete' | 'recovered'): void {
		...
		if (result.changes !== 1) throw new Error('Soundscaper desktop V11 recovery journal could not settle');
		this.#pruneJournals();
	}
	...
	#validatedMetadataRow(): {
		...
		`).get();
		if (!raw) throw new Error('Soundscaper desktop V11 metadata row is missing');
		const row = validateMetadataRow(raw, 'persisted V11 metadata');
		const metadata = validateMetadataIntegrity(row, 'Soundscaper desktop V11 metadata');
		return { metadata, row };
	}
	...
	#leaseRow(): PersistedLeaseRow {
		...
		`).get();
		if (!raw) throw new Error('Soundscaper desktop V11 lease row is missing');
		const active = booleanInteger(raw.active, 'persisted V11 lease active');
		const fencingToken = nonNegativeInteger(raw.fencingToken, 'persisted V11 fencing token');
		if (fencingToken === 0) {
			if (active || raw.leaseId !== null || raw.ownerProduct !== null
				...
				throw new TypeError('Initial Soundscaper desktop V11 lease row is invalid');
			}
			return Object.freeze({ active: false, lease: null });
		}
		const lease = freezeLease({
			leaseId: leaseId(raw.leaseId, 'persisted V11 lease id'),
			fencingToken,
			owner: validateSoundscaperDesktopProjectLibraryV11Owner({
				product: raw.ownerProduct,
				processId: raw.ownerProcessId,
				instanceId: raw.ownerInstanceId,
			}),
			acquiredAtMs: nonNegativeInteger(raw.acquiredAtMs, 'persisted V11 lease acquisition'),
			expiresAtMs: nonNegativeInteger(raw.expiresAtMs, 'persisted V11 lease expiry'),
			tookOverStaleLease: booleanInteger(raw.tookOver, 'persisted V11 lease takeover'),
		});
		if (lease.expiresAtMs <= lease.acquiredAtMs) {
			throw new TypeError('Persisted Soundscaper desktop V11 lease expiry is invalid');
		}
		return Object.freeze({ active, lease });
	}

	#assertLeaseOwned(
		token: SoundscaperDesktopProjectLibraryV11Lease,
		now = this.#timestamp(),
	): SoundscaperDesktopProjectLibraryV11Lease {
		const current = this.#leaseRow();
		if (!current.active || !current.lease || current.lease.expiresAtMs <= now
	...
	#exclusiveMetadataOperation<Result>(operation: () => Result): Result {
		if (this.#metadataOperationActive) {
			throw new Error('A Soundscaper desktop V11 metadata operation is already active');
		}
	...
	#newId(): string {
		...
		return leaseId(value, 'V11 lease or journal id');
	}

	#timestamp(): number {
		return nonNegativeInteger(this.#now(), 'Soundscaper desktop V11 clock');
	}
...
function snapshotOptions(value: unknown): Record<typeof CREATE_FIELDS[number], unknown> {
	const record = snapshotRecord(value, CREATE_FIELDS, 'Soundscaper desktop V11 catalog options');
	if (record.database === undefined || record.owner === undefined) {
		throw new TypeError('Soundscaper desktop V11 catalog options are incomplete');
	}
	return record;
}
...
function freezeRecovery(
	value: SoundscaperDesktopProjectLibraryV11Recovery,
): Readonly<SoundscaperDesktopProjectLibraryV11Recovery> {
...
function leaseTtl(value: unknown): number {
	return boundedInteger(value, MINIMUM_LEASE_TTL_MS, MAXIMUM_LEASE_TTL_MS,
		'Soundscaper desktop V11 lease TTL');
}
...
function leaseLost(): Error {
	return new Error('Soundscaper desktop V11 lease holder no longer owns the lease');
}
