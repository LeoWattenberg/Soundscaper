...
/** Main-private V28/V14 project, source-body, backend, and publication authority. */
...
import {
	selectedV28OriginalBodyForInput,
	selectedV28ProjectBody,
	selectedV28ProjectPlanBodyMetadataMatches,
	type SelectedV28ProjectMediaBody,
} from './native-services-selected-v28-body-custody.ts';
import type {
...
} from './native-services-selected-v28-watch-authority.ts';
import { FRAMESCAPER_SELECTED_V28_WATCH_BIN_ID } from '../src/common/editor/native-watch-target.ts';
...
interface ProjectRecord {
	...
	readonly bodies: readonly Readonly<SelectedV28ProjectMediaBody>[];
}
...
export interface FramescaperNativeSelectedV28ProjectAuthorityOptions {
	...
	readonly watch: Readonly<{
		...
		watchProject(projectId: string): unknown;
...
/** V28 jobs never pass paths or project bodies through the renderer/helper request schema. */
export class FramescaperNativeSelectedV28ProjectAuthority {
	readonly #options: FramescaperNativeSelectedV28ProjectAuthorityOptions;
	...
	watchImportAlreadyPresent(projectId: string, digest: string): Promise<boolean> {
		return this.watchImportState(
			projectId, FRAMESCAPER_SELECTED_V28_WATCH_BIN_ID, digest,
	...
	queueReservations(
		...
		const project = projectRecord(this.#options.project.projectRecord(request.projectId));
		const bodiesMatch = project !== null && project.projectRevision === request.projectRevision
			&& (request.taskKind === 'proxy-generation'
				...
				: selectedV28ProjectPlanBodyMetadataMatches(plan, request.inputFingerprints, project.bodies));
		...
		for (const input of request.inputFingerprints) {
			const body = selectedV28OriginalBodyForInput(plan, input, project!.bodies);
			...
			const awaitingCarrier = record.state === 'paused'
				...
				inputsMatch = project !== null
					&& (record.taskKind === 'proxy-generation'
						...
						: selectedV28ProjectPlanBodyMetadataMatches(plan, record.inputFingerprints, project.bodies))
	...
	async prepare(
		...
		if (bundle.project.projectRevision !== record.projectRevision
			...
			|| !(record.taskKind === 'proxy-generation'
				...
				: selectedV28ProjectPlanBodyMetadataMatches(plan, record.inputFingerprints, bundle.bodies))) {
	...
	async #loadBodies(
		...
		for (const input of record.inputFingerprints) {
			const body = selectedV28OriginalBodyForInput(plan, input, bodies);
	...
	async #readBody(body: Readonly<SelectedV28ProjectMediaBody>): Promise<Uint8Array> {
...
function projectRecord(value: unknown): ProjectRecord | null {
	if (value === null) return null;
	if (!value || typeof value !== 'object' || Array.isArray(value)) throw new TypeError('A selected V14 project row is malformed.');
	...
	if (typeof row.projectId !== 'string' || !Number.isSafeInteger(row.projectRevision)
		...
		throw new TypeError('A selected V14 project row has invalid identity fields.');
	}
	return Object.freeze({
...
function projectBundle(value: unknown): ProjectBundle {
	if (!value || typeof value !== 'object' || Array.isArray(value)) throw new TypeError('A selected V14 project bundle is unavailable.');
	const row = value as Record<string, unknown>;
	const project = row.project as Record<string, unknown> | null;
	if (!project || !Number.isSafeInteger(project.projectRevision)
		|| typeof project.sha256 !== 'string' || !SHA256.test(project.sha256)
		|| !Array.isArray(row.bodies) || row.bodies.length > MAXIMUM_BODIES) {
		throw new TypeError('A selected V14 project bundle is malformed.');
	}
	return Object.freeze({
...
function onlyBody(bodies: readonly Readonly<SelectedV28ProjectMediaBody>[],
	predicate: (body: Readonly<SelectedV28ProjectMediaBody>) => boolean, label: string): Readonly<SelectedV28ProjectMediaBody> {
