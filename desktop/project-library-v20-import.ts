...
export async function importFramescaperDesktopProjectLibraryV19IntoV20(value: Readonly<{
	appDataPath: string;
	...
	const sourcePaths = createFramescaperDesktopProjectLibraryV19Paths(value.appDataPath);
	...
	try {
		...
		if (finalManifest.digest !== manifest.digest) {
			throw new Error('Framescaper V19 source catalog changed during copy-forward');
		}
...
function importAlreadyComplete(database: DatabaseSync): boolean {
	const stored = database.prepare('SELECT state FROM v19_import WHERE singleton = 1').get() as
		Record<string, unknown> | undefined;
	return stored?.state === 'complete';
}

function validateV19Identity(database: DatabaseSync): void {
	...
	if (applicationId !== 0x46534350 || userVersion !== 21
		|| identity?.schemaVersion !== 19 || identity.projectSchemaVersion !== 28) {
		throw new Error('Framescaper V19 copy-forward source identity is unsupported');
	}
	const lineage = database.prepare('SELECT state FROM v18_import WHERE singleton = 1').get() as
		Record<string, unknown> | undefined;
	...
	if (lineage?.state !== 'complete' || lease?.active !== 0 || journals !== 0) {
		throw new Error('Framescaper V19 copy-forward source lifecycle is not settled');
	}
}
...
function prepareProgress(
	...
	try {
		...
		const stored = database.prepare('SELECT * FROM v19_import WHERE singleton = 1').get() as
			Record<string, unknown> | undefined;
		if (!stored) {
			if (Number(database.prepare('SELECT COUNT(*) AS count FROM projects').get()?.count) !== 0) {
				throw new Error('Framescaper V20 import destination is not empty');
			}
			database.prepare(`
				INSERT INTO v19_import (
		...
		if (stored.source_catalog_sha256 !== manifest.digest
			...
			throw new Error('Framescaper V19 copy-forward source changed after durable admission');
		}
		const next = nonNegative(stored.next_project_index, 'V19 import cursor');
		if (next > manifest.rows.length) throw new Error('Framescaper V19 import cursor is invalid');
		database.exec('COMMIT');
		return next;
	} catch (error) {
...
async function migrateRow(
	...
	if (!Number.isSafeInteger(row.byte_length) || row.byte_length < 1 || !digest(row.sha256)) {
		throw new Error('Framescaper V19 project row has invalid file geometry');
	}
	const sourceDocument = containedPath(sourcePaths.projectsRoot, row.document_file, 'V19 document');
	...
	const sourceProject = validateFramescaperDesktopV19CurrentProjectV28(JSON.parse(sourceText) as unknown);
	...
	if (project.id !== row.project_id || project.title !== row.title
		...
		throw new Error('Framescaper V28 to V31 reimport changed catalog identity');
	}
	const document = JSON.stringify(project);
	const bytes = new TextEncoder().encode(document);
	const sha256 = createHash('sha256').update(bytes).digest('hex');
	if (!/^[a-f0-9]{48}$/u.test(row.entry_id)) {
		throw new Error('Framescaper V19 project row has an invalid entry identity');
	}
...
function decodeDocument(bytes: Uint8Array): string {
	try {
		...
		throw new Error('Framescaper V19 project document is not valid UTF-8', { cause: error });
	}
}
	...
	try {
		...
		if (existing) {
			for (const field of Object.keys(row) as Array<keyof SourceRow>) {
				if (existing[field] !== row[field]) throw new Error('Framescaper V19 import row conflicts');
			}
		} else {
			database.prepare(`
		...
		database.prepare(`
			UPDATE v19_import SET next_project_index = ?
...
function completeImport(
	...
	try {
		assertLease(database);
		const progress = database.prepare('SELECT * FROM v19_import WHERE singleton = 1').get() as Record<string, unknown>;
		if (progress.next_project_index !== manifest.rows.length) throw new Error('Framescaper V19 import is incomplete');
		database.prepare('UPDATE library_identity SET metadata_revision = ? WHERE singleton = 1')
			.run(manifest.metadataRevision);
		database.prepare(`
			UPDATE v19_import SET state = 'complete', completed_at_ms = ? WHERE singleton = 1
		`).run(Date.now());
		database.exec('COMMIT');
	} catch (error) {
		database.exec('ROLLBACK');
...
function completeAbsentImport(database: DatabaseSync, assertLease: (database: DatabaseSync) => void): void {
	...
	try {
		assertLease(database);
		const existing = database.prepare('SELECT state FROM v19_import WHERE singleton = 1').get() as
			Record<string, unknown> | undefined;
		if (existing?.state !== undefined && existing.state !== 'complete') {
			throw new Error('Framescaper V19 source is unavailable after durable import admission');
		}
		if (!existing) {
			if (Number(database.prepare('SELECT COUNT(*) AS count FROM projects').get()?.count) !== 0) {
				throw new Error('Framescaper V20 import destination is not empty');
