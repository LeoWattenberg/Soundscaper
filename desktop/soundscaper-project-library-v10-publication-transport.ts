...
export interface SoundscaperDesktopProjectLibraryV10PublicationExpectedProject {
	readonly projectRevision: number;
	readonly projectSha256: string;
}

export interface SoundscaperDesktopProjectLibraryV10PublicationBeginRequest {
	readonly publicationId: string;
	readonly expectedMetadataRevision: number;
	readonly expectedProject: Readonly<SoundscaperDesktopProjectLibraryV10PublicationExpectedProject> | null;
	readonly project: unknown;
	readonly bodies: readonly Readonly<SoundscaperDesktopProjectLibraryV10TransferBody>[];
}

export interface SoundscaperDesktopProjectLibraryV10PublicationAdmission {
	readonly publicationId: string;
	readonly maximumChunkBytes: number;
	readonly bodyCount: number;
}

export interface SoundscaperDesktopProjectLibraryV10PublicationChunkRequest {
	readonly publicationId: string;
...
export interface SoundscaperDesktopProjectLibraryV10PublicationChunkAcknowledgement {
	readonly bodyIndex: number;
	readonly nextOffset: number;
	readonly complete: boolean;
}

export interface SoundscaperDesktopProjectLibraryV10PublicationCompletionRequest {
	readonly publicationId: string;
}
...
export function validateSoundscaperDesktopProjectLibraryV10PublicationBeginRequest(
	value: unknown,
): Readonly<SoundscaperDesktopProjectLibraryV10PublicationBeginRequest> {
	const record = snapshotClosedRecord(value, BEGIN_FIELDS, 'Soundscaper V10 publication begin');
	...
	const bodies = denseArray(
		record.bodies,
		'Soundscaper V10 publication body descriptors',
...
export function validateSoundscaperDesktopProjectLibraryV10PublicationAdmission(
	value: unknown,
	expectedBodyCount: number,
): Readonly<SoundscaperDesktopProjectLibraryV10PublicationAdmission> {
	const record = snapshotClosedRecord(value, ADMISSION_FIELDS, 'Soundscaper V10 publication admission');
	...
	if (bodyCount !== expectedBodyCount || bodyCount > MAXIMUM_BODY_DESCRIPTORS) {
		throw new Error('Soundscaper V10 publication admission body count changed');
	}
	if (record.maximumChunkBytes !== MAXIMUM_SOUNDSCAPER_V10_TRANSFER_CHUNK_BYTES) {
		throw new Error('Soundscaper V10 publication admission chunk bound changed');
	}
	return Object.freeze({
		publicationId: publicationId(record.publicationId),
...
export function validateSoundscaperDesktopProjectLibraryV10PublicationChunkRequest(
	value: unknown,
): Readonly<SoundscaperDesktopProjectLibraryV10PublicationChunkRequest> {
	const record = snapshotClosedRecord(value, CHUNK_FIELDS, 'Soundscaper V10 publication chunk');
	const bytes = binary(record.bytes);
	if (bytes.byteLength < 1
		|| bytes.byteLength > MAXIMUM_SOUNDSCAPER_V10_TRANSFER_CHUNK_BYTES) {
		throw new RangeError('Soundscaper V10 publication chunk exceeds its byte limit');
	}
	return Object.freeze({
		publicationId: publicationId(record.publicationId),
...
export function validateSoundscaperDesktopProjectLibraryV10PublicationChunkAcknowledgement(
	value: unknown,
	request: Readonly<SoundscaperDesktopProjectLibraryV10PublicationChunkRequest>,
): Readonly<SoundscaperDesktopProjectLibraryV10PublicationChunkAcknowledgement> {
	const record = snapshotClosedRecord(
		value,
		ACKNOWLEDGEMENT_FIELDS,
		'Soundscaper V10 publication chunk acknowledgement',
	);
	...
	if (record.bodyIndex !== request.bodyIndex || record.nextOffset !== expectedOffset
		...
		throw new Error('Soundscaper V10 publication acknowledgement changed its sequential write');
	}
	return Object.freeze({
...
export function validateSoundscaperDesktopProjectLibraryV10PublicationCompletionRequest(
	value: unknown,
): Readonly<SoundscaperDesktopProjectLibraryV10PublicationCompletionRequest> {
	const record = snapshotClosedRecord(
		value,
		COMPLETION_FIELDS,
		'Soundscaper V10 publication completion',
	);
	return Object.freeze({ publicationId: publicationId(record.publicationId) });
}

export function validateSoundscaperDesktopProjectLibraryV10PublicationResult(
	value: unknown,
	expectedProjectId: string,
): Readonly<SoundscaperDesktopProjectLibraryV10TransferBundle> {
	return validateSoundscaperDesktopProjectLibraryV10TransferBundle(value, expectedProjectId);
}

export function validateSoundscaperDesktopProjectLibraryV10PublicationAbortResult(
	value: unknown,
): boolean {
	if (typeof value !== 'boolean') {
		throw new TypeError('Soundscaper V10 publication abort result must be a boolean');
	}
	return value;
}
...
function validateExpectedProject(
	value: unknown,
): Readonly<SoundscaperDesktopProjectLibraryV10PublicationExpectedProject> | null {
	...
	if (typeof record.projectSha256 !== 'string' || !DIGEST.test(record.projectSha256)) {
		throw new TypeError('Soundscaper V10 expected project digest is invalid');
	}
	return Object.freeze({
...
function publicationId(value: unknown): string {
	if (typeof value !== 'string' || !PUBLICATION_ID.test(value)) {
		throw new TypeError('Soundscaper V10 publication id is invalid');
	}
	return value;
}
...
function binary(value: unknown): Uint8Array {
	...
	throw new TypeError('Soundscaper V10 publication chunk must be binary data');
}
...
function checkedAdd(left: number, right: number, label: string): number {
	if (left > Number.MAX_SAFE_INTEGER - right) {
		throw new RangeError(`Soundscaper V10 ${label} exceeds the safe integer range`);
	}
	return left + right;
}
