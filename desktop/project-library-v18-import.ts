...
import { createFramescaperDesktopProjectLibraryV17Paths } from './project-library-v17-contract.ts';
...
export async function importFramescaperDesktopProjectLibraryV17IntoV18(value: Readonly<{
	appDataPath: string;
	...
	try {
		...
		if (finalManifest.digest !== manifest.digest) {
			throw new Error('Framescaper V17 source catalog changed during copy-forward');
		}
...
function validateV17Identity(database: DatabaseSync): void {
	...
	if (applicationId !== 0x46534350 || userVersion !== 19
		|| identity?.schemaVersion !== 17 || identity.projectSchemaVersion !== 20) {
		throw new Error('Framescaper V17 copy-forward source identity is unsupported');
	}
	const lineage = database.prepare('SELECT state FROM v12_import WHERE singleton = 1').get() as
	...
	if (lineage?.state !== 'complete' || lease?.active !== 0 || journals !== 0) {
		throw new Error('Framescaper V17 copy-forward source lifecycle is not settled');
	}
}
...
function prepareProgress(
	...
	try {
		...
		const stored = database.prepare('SELECT * FROM v17_import WHERE singleton = 1').get() as
			Record<string, unknown> | undefined;
		if (!stored) {
		...
		if (stored.source_catalog_sha256 !== manifest.digest
			...
			throw new Error('Framescaper V17 copy-forward source changed after durable admission');
		}
		const next = nonNegative(stored.next_project_index, 'V17 import cursor');
...
async function migrateRow(
	...
	if (!Number.isSafeInteger(row.byte_length) || row.byte_length < 1 || !digest(row.sha256)) {
		throw new Error('Framescaper V17 project row has invalid file geometry');
	}
	const sourceDocument = containedPath(sourcePaths.projectsRoot, row.document_file, 'V17 document');
	...
	const sourceProject = validateFramescaperDesktopV17CurrentProjectV20(JSON.parse(sourceText) as unknown);
	if (sourceProject.id !== row.project_id || sourceProject.title !== row.title
		...
		throw new Error('Framescaper V17 project row disagrees with its exact document');
	}
	const project = reimportFramescaperProjectV27(FRAMESCAPER_V27_PROJECT_RUNTIME_PROFILE, sourceProject);
	if (project.id !== row.project_id || project.title !== row.title
		...
		throw new Error('Framescaper V20 to V27 reimport changed catalog identity');
	}
	const document = JSON.stringify(project);
	const bytes = new TextEncoder().encode(document);
	const sha256 = createHash('sha256').update(bytes).digest('hex');
	if (!/^[a-f0-9]{48}$/u.test(row.entry_id)) {
		throw new Error('Framescaper V17 project row has an invalid entry identity');
	}
	const documentFile = `${row.entry_id}/${String(row.project_revision)}-${sha256}.json`;
	const destinationDocument = containedPath(destinationPaths.projectsRoot, documentFile, 'V18 document');
	await writeExactFile(destinationDocument, bytes, sha256);
	const bodies = parseFramescaperDesktopExactBodies(row.bodies_json, 'Framescaper V17 import');
...
function decodeDocument(bytes: Uint8Array): string {
	try {
		...
		throw new Error('Framescaper V17 project document is not valid UTF-8', { cause: error });
	}
}
...
function publishRow(
	...
	try {
		...
		database.prepare(`
			UPDATE v17_import SET next_project_index = ?
...
function completeImport(
	...
	try {
		assertLease(database);
		const progress = database.prepare('SELECT * FROM v17_import WHERE singleton = 1').get() as Record<string, unknown>;
		...
		database.prepare(`
			UPDATE v17_import SET state = 'complete', completed_at_ms = ? WHERE singleton = 1
		`).run(Date.now());
		database.exec('COMMIT');
	} catch (error) {
		database.exec('ROLLBACK');
