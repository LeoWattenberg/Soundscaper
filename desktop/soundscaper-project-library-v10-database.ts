...
import { emptySoundscaperDesktopLibraryV10Metadata } from './soundscaper-project-library-v10-metadata.ts';

export function initializeSoundscaperDesktopProjectLibraryV10Database(
	...
	if (applicationId !== 0 && applicationId !== DESKTOP_PROJECT_LIBRARY_V10_APPLICATION_ID) {
		throw new Error('Soundscaper desktop V10 database belongs to another application');
	}
	...
	if (userVersion !== 0 && userVersion !== DESKTOP_PROJECT_LIBRARY_V10_DATABASE_VERSION) {
		throw new Error('Unsupported Soundscaper desktop V10 database version');
	}
	...
	try {
		...
		`);
		const empty = emptySoundscaperDesktopLibraryV10Metadata();
...
export function assertSoundscaperDesktopProjectLibraryV10DatabaseIdentity(
	database: DatabaseSync,
): void {
	if (pragmaNumber(database, 'application_id') !== DESKTOP_PROJECT_LIBRARY_V10_APPLICATION_ID
		...
		throw new Error('Soundscaper desktop V10 database identity is invalid');
	}
}
...
function pragmaNumber(database: DatabaseSync, name: 'application_id' | 'user_version'): number {
	...
	if (typeof value !== 'number' || !Number.isSafeInteger(value) || value < 0) {
		throw new RangeError(`Soundscaper desktop V10 PRAGMA ${name} is invalid`);
	}
	return value;
}
