...
const CHANNELS = Object.freeze({
	...
	framescaperNativeSelectImageSequence: 'framescaper:v1:native-services:image-sequence:select', framescaperNativeReadImageSequenceFile: 'framescaper:v1:native-services:image-sequence:read', framescaperNativeReleaseImageSequence: 'framescaper:v1:native-services:image-sequence:release',
	framescaperProjectHandshake: 'framescaper:v12:projects:handshake', framescaperProjectBundle: 'framescaper:v12:projects:bundle', framescaperProjectBodyRead: 'framescaper:v12:projects:bodies:read', framescaperProjectList: 'framescaper:v12:projects:list',
	framescaperProjectDelete: 'framescaper:v12:projects:delete', framescaperProjectDuplicate: 'framescaper:v12:projects:duplicate', framescaperProjectBegin: 'framescaper:v12:projects:publication:begin',
	framescaperProjectChunk: 'framescaper:v12:projects:publication:chunk', framescaperProjectFinish: 'framescaper:v12:projects:publication:finish', framescaperProjectAbort: 'framescaper:v12:projects:publication:abort',
...
const FRAMESCAPER_PROJECT_LIBRARY_HANDSHAKE = Object.freeze({ kind: 'framescaper-project-library-handshake', version: 1, owner: 'framescaper', projectSchemaVersion: 20,
	scapeFormatVersions: Object.freeze([1, 2]), attachedScapeFormatVersion: 2, storageDatabaseName: 'kw-media-framescaper-editor-v20', desktopLibrarySchemaVersion: 12,
	desktopDatabaseUserVersion: 14, desktopLibraryScope: Object.freeze(['kw.media', 'scape-project-library', 'v12']) });
...
const bridge = Object.freeze({ v1: api }); const framescaperBridge = Object.freeze({ v1: Object.freeze({ ...api, projectLibrary: framescaperProjectLibrary }) });
...
function nativeAudioAvailability(value) { const payload = value?.payload;
	return Object.freeze({ enabled: value?.enabled === true, quarantined: value?.quarantined === true, payload: Object.freeze({ status: payload?.status === 'available' ? 'available' : 'unavailable', reason: payload?.reason == null ? null : text(payload.reason, 64), detail: text(payload?.detail ?? '', 512) }),
		backends: Object.freeze(bounded(value?.backends, 16).map((backend) => text(backend, 32))) }); }
function nativeAudioInventory(value) { const inventory = value?.inventory; if (value?.status !== 'described') return Object.freeze({ status: 'failed', code: text(value?.code ?? 'helper-failed', 32), message: text(value?.message ?? '', 512) });
	return Object.freeze({ status: 'described', inventory: Object.freeze({ backend: text(inventory?.backend, 32), status: text(inventory?.status, 32), detail: text(inventory?.detail ?? '', 512), devices: Object.freeze(bounded(inventory?.devices, 128).map((device) => Object.freeze({ handle: text(device?.handle, 256), label: text(device?.label, 256), direction: text(device?.direction, 16) }))) }) }); }
function bounded(value, maximum) { return Array.isArray(value) ? value.slice(0, maximum) : []; }
function nativePluginStatus(value, depth = 0) { if (typeof value === 'string') return text(value, 1024);
	if (typeof value === 'number' || typeof value === 'boolean' || value === null) return value; if (depth >= 8) return null;
	if (Array.isArray(value)) return Object.freeze(bounded(value, 512).map((entry) => nativePluginStatus(entry, depth + 1))); if (!value || typeof value !== 'object') return null;
	const output = {}; for (const key of Object.keys(value).slice(0, 64)) { if (PLUGIN_PATH_KEYS.has(key)) continue; output[text(key, 64)] = nativePluginStatus(value[key], depth + 1); }
...
function nativeWatchProjection(value) { const mode = value?.importMode; if (mode !== 'link' && mode !== 'copy') throw new TypeError('Invalid Framescaper watch import mode');
	return Object.freeze({ ruleId: nativeIdentifier(value?.ruleId, 'watch rule id'), grantId: nativeIdentifier(value?.grantId, 'grant id'), projectId: text(value?.projectId, 4096), extensions: Object.freeze(nativeArray(value?.extensions, 32, 'watch extensions').map((item) => text(item, 32))), importMode: mode, generateProxies: value?.generateProxies === true, enabled: value?.enabled === true }); }
...
function nativeWatchEnabledRequest(value) { nativeRecord(value, ['ruleId', 'enabled'], 'watch enabled request'); if (typeof value.enabled !== 'boolean') throw new TypeError('Invalid Framescaper watch enabled request'); return Object.freeze({ ruleId: nativeIdentifier(value.ruleId, 'watch rule id'), enabled: value.enabled }); } function nativeWatchImportClaimRequest(value) { nativeRecord(value, ['projectId', 'projectRevision'], 'watch import claim request'); return Object.freeze({ projectId: nativeBoundedText(value.projectId, 128, 'watch project id'), projectRevision: safeInteger(value.projectRevision) }); } function nativeWatchImportCompletionRequest(value) { nativeRecord(value, ['claimId', 'projectId', 'expectedProjectRevision', 'committedProjectRevision', 'success'], 'watch import completion request'); if (typeof value.success !== 'boolean') throw new TypeError('Invalid watch import completion result'); return Object.freeze({ claimId: nativeIdentifier(value.claimId, 'watch claim id'), projectId: nativeBoundedText(value.projectId, 128, 'watch project id'), expectedProjectRevision: safeInteger(value.expectedProjectRevision), committedProjectRevision: safeInteger(value.committedProjectRevision), success: value.success }); } function nativeWatchImportClaim(value) { nativeRecord(value, ['claimId', 'projectId', 'projectRevision', 'importMode', 'locatorId', 'locatorRevision', 'name', 'size', 'mimeType', 'lastModified', 'contentSha256'], 'watch import claim'); if (!['link', 'copy'].includes(value.importMode) || !String(value.mimeType).startsWith('video/')) throw new TypeError('Invalid Framescaper watch import claim'); const size = safeInteger(value.size); if (!size) throw new RangeError('Invalid Framescaper watch import size'); return Object.freeze({ claimId: nativeIdentifier(value.claimId, 'watch claim id'), projectId: nativeBoundedText(value.projectId, 128, 'watch project id'), projectRevision: safeInteger(value.projectRevision), importMode: value.importMode, locatorId: nativeIdentifier(value.locatorId, 'watch locator id'), locatorRevision: nativeIdentifier(value.locatorRevision, 'watch locator revision'), name: nativeBoundedText(value.name, 255, 'watch file name', /[\0/\\]/u), size, mimeType: nativeBoundedText(value.mimeType, 128, 'watch MIME type'), lastModified: safeInteger(value.lastModified), contentSha256: nativeDigest(value.contentSha256, 'watch content') }); }
...
function nativeRenderInputPortReply(port, message, transfer) { return new Promise((resolve, reject) => { let settled = false; const finish = (error, value) => { if (settled) return; settled = true; clearTimeout(timeout); port.onmessage = null; port.onmessageerror = null; if (error) reject(error); else resolve(value); }; const timeout = setTimeout(() => finish(new Error('Framescaper render-input transfer timed out')), 30000); port.onmessage = (event) => finish(null, event.data); port.onmessageerror = () => finish(new Error('Framescaper render-input MessagePort failed')); try { port.postMessage(message, transfer); } catch (error) { finish(error); } }); } function nativeQueueEnqueueRequest(value) { nativeRecord(value, ['taskKind', 'planVersion', 'derivedInputStageId', 'planFingerprint', 'planPayload', 'projectId', 'projectRevision', 'inputFingerprints', 'rootGrantId', 'relativeDestination', 'reservations', 'recoveryClass'], 'native queue enqueue request');
	if (!['encoded-export', 'image-sequence-export', 'proxy-generation'].includes(value.taskKind) || ![7, 8, 9, 10, 11, 12].includes(value.planVersion) || typeof value.planPayload !== 'string' || !value.planPayload.length || utf8Bytes(value.planPayload, 65536) > 65536 || !['atomic-restart', 'verified-frame-checkpoint'].includes(value.recoveryClass)) throw new TypeError('Invalid Framescaper native queue enqueue plan');
...
function nativeImageSequenceReleaseRequest(value) { nativeRecord(value, ['selectionId'], 'image-sequence release request'); return Object.freeze({ selectionId: nativeImageSequenceId(value.selectionId) }); }
function nativeImageSequenceId(value) { if (typeof value !== 'string' || !/^[a-f0-9]{40}$/u.test(value)) throw new TypeError('Invalid Framescaper image-sequence opaque id'); return value; }
function nativeRecord(value, fields, label) { const keys = value && typeof value === 'object' && !Array.isArray(value) ? Reflect.ownKeys(value) : [];
	if (keys.length !== fields.length || keys.some((key) => typeof key !== 'string' || !fields.includes(key))) throw new TypeError(`Invalid ${label} fields`); return value; }
