...
import type {
	SoundscaperDesktopProjectLibraryV10PublicationPlan,
	SoundscaperDesktopProjectLibraryV10PlannedBody,
} from './soundscaper-project-library-v10-publication-contract.ts';
import type {
	SoundscaperDesktopProjectLibraryV10PublicationStage,
...
import {
	encodeSoundscaperDesktopProjectLibraryV10MetadataRow,
	sameSoundscaperDesktopProjectLibraryV10Lease,
	type SoundscaperDesktopProjectLibraryV10Lease,
	type SoundscaperDesktopProjectLibraryV10MetadataRow,
	validateSoundscaperDesktopProjectLibraryV10LeaseToken,
	validateSoundscaperDesktopProjectLibraryV10MetadataIntegrity,
	validateSoundscaperDesktopProjectLibraryV10MetadataRow,
...
export type SoundscaperDesktopProjectLibraryV10PublicationJournalState =
	| 'prepared'
	| 'materialized'
	| 'committed'
	| 'complete';

export interface SoundscaperDesktopProjectLibraryV10PersistedBody {
	readonly bodyId: string;
	readonly mediaRelativeFile: string;
	readonly descriptor: Readonly<SoundscaperDesktopProjectLibraryV10TransferBody>;
}

export interface SoundscaperDesktopProjectLibraryV10PersistedPublication {
	readonly transactionId: string;
	readonly state: SoundscaperDesktopProjectLibraryV10PublicationJournalState;
	...
	readonly bodies: readonly Readonly<SoundscaperDesktopProjectLibraryV10PersistedBody>[];
	readonly stages: readonly Readonly<SoundscaperDesktopProjectLibraryV10PublicationStage>[];
}
...
export function readSoundscaperDesktopProjectLibraryV10MetadataSnapshot(
	database: DatabaseSync,
): Readonly<{
	readonly row: SoundscaperDesktopProjectLibraryV10MetadataRow;
	readonly metadata: Readonly<SoundscaperDesktopLibraryV10Metadata>;
}> {
	...
	`).get();
	if (!value) throw new Error('Soundscaper V10 metadata row is missing');
	const row = validateSoundscaperDesktopProjectLibraryV10MetadataRow(
		value as Record<string, unknown>,
		'Soundscaper V10 publication metadata',
	);
	return Object.freeze({
		row,
		metadata: validateSoundscaperDesktopProjectLibraryV10MetadataIntegrity(
			row,
			'Soundscaper V10 publication metadata',
		),
	});
...
export function assertSoundscaperDesktopProjectLibraryV10PublicationLease(
	database: DatabaseSync,
	value: SoundscaperDesktopProjectLibraryV10Lease,
	now: number,
): SoundscaperDesktopProjectLibraryV10Lease {
	const lease = validateSoundscaperDesktopProjectLibraryV10LeaseToken(value);
	...
	const current = validateSoundscaperDesktopProjectLibraryV10LeaseToken({
		leaseId: row.leaseId,
	...
	} as SoundscaperDesktopProjectLibraryV10Lease);
	if (current.expiresAtMs <= now || !sameSoundscaperDesktopProjectLibraryV10Lease(current, lease)) {
		throw leaseLost();
	}
	return current;
}

export function prepareSoundscaperDesktopProjectLibraryV10Publication(
	database: DatabaseSync,
	transactionId: string,
	plan: Readonly<SoundscaperDesktopProjectLibraryV10PublicationPlan>,
	stages: readonly Readonly<SoundscaperDesktopProjectLibraryV10PublicationStage>[],
	now: number,
): void {
	if (!TRANSACTION_ID.test(transactionId)) {
		throw new TypeError('Soundscaper V10 publication transaction id is invalid');
	}
	transaction(database, () => {
		assertSoundscaperDesktopProjectLibraryV10PublicationLease(database, plan.lease, now);
		const current = readSoundscaperDesktopProjectLibraryV10MetadataSnapshot(database);
		...
		if (current.row.revision !== plan.expectedMetadataRevision) {
			throw new Error('Soundscaper V10 metadata revision changed before journal preparation');
		}
		if (database.prepare(`
		...
		const next = encodeSoundscaperDesktopProjectLibraryV10MetadataRow(plan.nextMetadata, now);
		database.prepare(`
...
export function markSoundscaperDesktopProjectLibraryV10PublicationMaterialized(
	database: DatabaseSync,
	transactionId: string,
	...
	transaction(database, () => {
		const current = assertSoundscaperDesktopProjectLibraryV10PublicationLease(database, lease, now);
		...
		if (result.changes !== 1) throw new Error('Soundscaper V10 prepared publication is unavailable');
	});
}

export function commitSoundscaperDesktopProjectLibraryV10Publication(
	database: DatabaseSync,
	publication: Readonly<SoundscaperDesktopProjectLibraryV10PersistedPublication>,
	lease: SoundscaperDesktopProjectLibraryV10Lease,
	now: number,
): void {
	transaction(database, () => {
		const currentLease = assertSoundscaperDesktopProjectLibraryV10PublicationLease(database, lease, now);
		const current = readSoundscaperDesktopProjectLibraryV10MetadataSnapshot(database);
		...
		if (current.row.revision !== publication.expectedMetadataRevision) {
			throw new Error('Soundscaper V10 metadata revision changed before publication commit');
		}
		const result = database.prepare(`
		...
		if (result.changes !== 1) throw new Error('Soundscaper V10 project revision could not publish');
		...
		const next = encodeSoundscaperDesktopProjectLibraryV10MetadataRow(publication.nextMetadata, now);
		...
		if (metadata.changes !== 1) throw new Error('Soundscaper V10 publication lost metadata compare-and-swap');
		...
		if (journal.changes !== 1) throw new Error('Soundscaper V10 publication journal could not commit');
	});
}

export function settleSoundscaperDesktopProjectLibraryV10Publication(
	database: DatabaseSync,
	transactionId: string,
	lease: SoundscaperDesktopProjectLibraryV10Lease,
	now: number,
	...
	transaction(database, () => {
		assertSoundscaperDesktopProjectLibraryV10PublicationLease(database, lease, now);
		const result = database.prepare(`
		...
		if (result.changes !== 1) throw new Error('Soundscaper V10 publication journal could not complete');
		database.prepare(`
...
export function readSoundscaperDesktopProjectLibraryV10PendingPublication(
	database: DatabaseSync,
): Readonly<SoundscaperDesktopProjectLibraryV10PersistedPublication> | null {
	...
	if (rows.length > 1) throw new Error('Soundscaper V10 has conflicting pending publications');
	return rows.length === 0 ? null : parsePublicationRow(rows[0]!);
}

export function readSoundscaperDesktopProjectLibraryV10PublicationById(
	database: DatabaseSync,
	transactionId: string,
): Readonly<SoundscaperDesktopProjectLibraryV10PersistedPublication> {
	...
	if (!row) throw new Error('Soundscaper V10 publication journal is missing');
...
function parsePublicationRow(
	...
	if (!TRANSACTION_ID.test(transactionId)) throw new TypeError('Soundscaper V10 publication id is invalid');
	...
	if (createHash('sha256').update(previousJson, 'utf8').digest('hex') !== previousDigest) {
		throw new Error('Soundscaper V10 previous metadata journal digest is invalid');
	}
	...
	if (createHash('sha256').update(nextJson, 'utf8').digest('hex') !== nextDigest) {
		throw new Error('Soundscaper V10 next metadata journal digest is invalid');
	}
	const previousMetadata = parseSoundscaperDesktopLibraryV10MetadataJson(previousJson);
	const nextMetadata = parseSoundscaperDesktopLibraryV10MetadataJson(nextJson);
	if (previousMetadata.revision !== expectedMetadataRevision
		|| nextMetadata.revision !== expectedMetadataRevision + 1) {
		throw new Error('Soundscaper V10 publication journal metadata revisions are invalid');
	}
	...
	if (new TextEncoder().encode(document).byteLength !== projectByteLength
		...
		throw new Error('Soundscaper V10 publication project journal integrity is invalid');
	}
	...
	if (!project || project.projectRevision !== projectRevision || project.sha256 !== projectSha256) {
		throw new Error('Soundscaper V10 publication project disagrees with next metadata');
	}
	...
	if (project.id !== entryId || project.metadataFile !== projectRelativeFile) {
		throw new Error('Soundscaper V10 publication project path disagrees with metadata');
	}
...
function parseBodies(value: unknown): readonly Readonly<SoundscaperDesktopProjectLibraryV10PersistedBody>[] {
	const parsed = parseJson(value, 'publication bodies');
	if (!Array.isArray(parsed) || parsed.length > MAXIMUM_BODIES) {
		throw new TypeError('Soundscaper V10 publication bodies are invalid');
	}
	return Object.freeze(parsed.map((value) => {
		...
		if (bodyId !== expectedBodyId) throw new Error('Soundscaper V10 publication body id is invalid');
		...
		if (mediaRelativeFile !== expectedFile) {
			throw new Error('Soundscaper V10 publication media path is invalid');
		}
...
function parseStages(
	...
	bodies: readonly Readonly<SoundscaperDesktopProjectLibraryV10PersistedBody>[],
): readonly Readonly<SoundscaperDesktopProjectLibraryV10PublicationStage>[] {
	const parsed = parseJson(value, 'publication stages');
	if (!Array.isArray(parsed) || parsed.length !== bodies.length + 1) {
		throw new TypeError('Soundscaper V10 publication stages are incomplete');
	}
	return Object.freeze(parsed.map((value, index) => {
		...
		if (record.role !== role || record.bodyId !== bodyId
			...
			throw new Error('Soundscaper V10 publication stage identity is invalid');
		}
		return Object.freeze({
			role,
			bodyId,
...
function publishBody(
	database: DatabaseSync,
	body: Readonly<SoundscaperDesktopProjectLibraryV10PersistedBody>,
	now: number,
): void {
	...
	if (existing) {
		if (existing.kind !== body.descriptor.kind || existing.encoding !== body.descriptor.encoding
			...
			throw new Error('Soundscaper V10 managed body identity conflicts with existing publication');
		}
		return;
	}
	database.prepare(`
...
function persistedBody(
	body: Readonly<SoundscaperDesktopProjectLibraryV10PlannedBody>,
): SoundscaperDesktopProjectLibraryV10PersistedBody {
	return {
		bodyId: body.bodyId,
...
function assertMetadataSnapshot(
	left: Readonly<SoundscaperDesktopLibraryV10Metadata>,
	right: Readonly<SoundscaperDesktopLibraryV10Metadata>,
	message: string,
): void {
	if (JSON.stringify(left) !== JSON.stringify(right)) {
		throw new Error(`Soundscaper V10 metadata ${message}`);
	}
}
...
function publicationState(value: unknown): SoundscaperDesktopProjectLibraryV10PublicationJournalState {
	if (value !== 'prepared' && value !== 'materialized' && value !== 'committed' && value !== 'complete') {
		throw new TypeError('Soundscaper V10 publication journal state is invalid');
	}
	return value;
}
...
function parseJson(value: unknown, label: string): unknown {
	...
	catch (error) { throw new TypeError(`Soundscaper V10 ${label} JSON is invalid`, { cause: error }); }
}
...
function closedRecord<const Field extends string>(
	...
	if (!value || typeof value !== 'object' || Array.isArray(value)
		...
		throw new TypeError(`Soundscaper V10 ${label} must be a plain record`);
	}
	const keys = Reflect.ownKeys(value);
	if (keys.length !== fields.length || keys.some(
		(key) => typeof key !== 'string' || !fields.includes(key as Field),
	)) throw new TypeError(`Soundscaper V10 ${label} has invalid fields`);
	const record = Object.create(null) as Record<Field, unknown>;
	for (const field of fields) {
		const descriptor = Object.getOwnPropertyDescriptor(value, field);
		if (!descriptor?.enumerable || !Object.hasOwn(descriptor, 'value')) {
			throw new TypeError(`Soundscaper V10 ${label}.${field} must be a data property`);
		}
...
function digest(value: unknown, label: string): string {
	...
	if (!DIGEST.test(result)) throw new TypeError(`Soundscaper V10 ${label} digest is invalid`);
	return result;
}
...
function leaseLost(): Error {
	return new Error('Soundscaper V10 publication writer lease no longer owns its fence');
}
