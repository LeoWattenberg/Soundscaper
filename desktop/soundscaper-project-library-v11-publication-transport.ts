...
export interface SoundscaperDesktopProjectLibraryV11PublicationExpectedProject {
	readonly projectRevision: number;
	readonly projectSha256: string;
}

export interface SoundscaperDesktopProjectLibraryV11PublicationBeginRequest {
	readonly publicationId: string;
	readonly expectedMetadataRevision: number;
	readonly expectedProject: Readonly<SoundscaperDesktopProjectLibraryV11PublicationExpectedProject> | null;
	readonly project: unknown;
	readonly bodies: readonly Readonly<SoundscaperDesktopProjectLibraryV11TransferBody>[];
}

export interface SoundscaperDesktopProjectLibraryV11PublicationAdmission {
	readonly publicationId: string;
	readonly maximumChunkBytes: number;
	readonly bodyCount: number;
}

export interface SoundscaperDesktopProjectLibraryV11PublicationChunkRequest {
	readonly publicationId: string;
...
export interface SoundscaperDesktopProjectLibraryV11PublicationChunkAcknowledgement {
	readonly bodyIndex: number;
	readonly nextOffset: number;
	readonly complete: boolean;
}

export interface SoundscaperDesktopProjectLibraryV11PublicationCompletionRequest {
	readonly publicationId: string;
}
...
export function validateSoundscaperDesktopProjectLibraryV11PublicationBeginRequest(
	value: unknown,
): Readonly<SoundscaperDesktopProjectLibraryV11PublicationBeginRequest> {
	const record = snapshotClosedRecord(value, BEGIN_FIELDS, 'Soundscaper V11 publication begin');
	...
	const bodies = denseArray(
		record.bodies,
		'Soundscaper V11 publication body descriptors',
...
export function validateSoundscaperDesktopProjectLibraryV11PublicationAdmission(
	value: unknown,
	expectedBodyCount: number,
): Readonly<SoundscaperDesktopProjectLibraryV11PublicationAdmission> {
	const record = snapshotClosedRecord(value, ADMISSION_FIELDS, 'Soundscaper V11 publication admission');
	...
	if (bodyCount !== expectedBodyCount || bodyCount > MAXIMUM_BODY_DESCRIPTORS) {
		throw new Error('Soundscaper V11 publication admission body count changed');
	}
	if (record.maximumChunkBytes !== MAXIMUM_SOUNDSCAPER_V11_TRANSFER_CHUNK_BYTES) {
		throw new Error('Soundscaper V11 publication admission chunk bound changed');
	}
	return Object.freeze({
		publicationId: publicationId(record.publicationId),
...
export function validateSoundscaperDesktopProjectLibraryV11PublicationChunkRequest(
	value: unknown,
): Readonly<SoundscaperDesktopProjectLibraryV11PublicationChunkRequest> {
	const record = snapshotClosedRecord(value, CHUNK_FIELDS, 'Soundscaper V11 publication chunk');
	const bytes = binary(record.bytes);
	if (bytes.byteLength < 1
		|| bytes.byteLength > MAXIMUM_SOUNDSCAPER_V11_TRANSFER_CHUNK_BYTES) {
		throw new RangeError('Soundscaper V11 publication chunk exceeds its byte limit');
	}
	return Object.freeze({
		publicationId: publicationId(record.publicationId),
...
export function validateSoundscaperDesktopProjectLibraryV11PublicationChunkAcknowledgement(
	value: unknown,
	request: Readonly<SoundscaperDesktopProjectLibraryV11PublicationChunkRequest>,
): Readonly<SoundscaperDesktopProjectLibraryV11PublicationChunkAcknowledgement> {
	const record = snapshotClosedRecord(
		value,
		ACKNOWLEDGEMENT_FIELDS,
		'Soundscaper V11 publication chunk acknowledgement',
	);
	...
	if (record.bodyIndex !== request.bodyIndex || record.nextOffset !== expectedOffset
		...
		throw new Error('Soundscaper V11 publication acknowledgement changed its sequential write');
	}
	return Object.freeze({
...
export function validateSoundscaperDesktopProjectLibraryV11PublicationCompletionRequest(
	value: unknown,
): Readonly<SoundscaperDesktopProjectLibraryV11PublicationCompletionRequest> {
	const record = snapshotClosedRecord(
		value,
		COMPLETION_FIELDS,
		'Soundscaper V11 publication completion',
	);
	return Object.freeze({ publicationId: publicationId(record.publicationId) });
}

export function validateSoundscaperDesktopProjectLibraryV11PublicationResult(
	value: unknown,
	expectedProjectId: string,
): Readonly<SoundscaperDesktopProjectLibraryV11TransferBundle> {
	return validateSoundscaperDesktopProjectLibraryV11TransferBundle(value, expectedProjectId);
}

export function validateSoundscaperDesktopProjectLibraryV11PublicationAbortResult(
	value: unknown,
): boolean {
	if (typeof value !== 'boolean') {
		throw new TypeError('Soundscaper V11 publication abort result must be a boolean');
	}
	return value;
}
...
function validateExpectedProject(
	value: unknown,
): Readonly<SoundscaperDesktopProjectLibraryV11PublicationExpectedProject> | null {
	...
	if (typeof record.projectSha256 !== 'string' || !DIGEST.test(record.projectSha256)) {
		throw new TypeError('Soundscaper V11 expected project digest is invalid');
	}
	return Object.freeze({
...
function publicationId(value: unknown): string {
	if (typeof value !== 'string' || !PUBLICATION_ID.test(value)) {
		throw new TypeError('Soundscaper V11 publication id is invalid');
	}
	return value;
}
...
function binary(value: unknown): Uint8Array {
	...
	throw new TypeError('Soundscaper V11 publication chunk must be binary data');
}
...
function checkedAdd(left: number, right: number, label: string): number {
	if (left > Number.MAX_SAFE_INTEGER - right) {
		throw new RangeError(`Soundscaper V11 ${label} exceeds the safe integer range`);
	}
	return left + right;
}
