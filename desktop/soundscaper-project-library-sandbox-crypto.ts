...
/** Browser-safe seam for the narrow Node createHash usage in the dedicated preload. */
export function createHash(algorithm: string): Readonly<{
	...
	const hash = {
		...
		digest(encoding: 'hex') {
			if (encoding !== 'hex') {
				throw new TypeError('The sandbox preload supports only hexadecimal digests')
			}
