...
import type {
	SoundscaperDesktopProjectLibraryV11PublicationPlan,
	SoundscaperDesktopProjectLibraryV11PlannedBody,
} from './soundscaper-project-library-v11-publication-contract.ts';
import type {
	SoundscaperDesktopProjectLibraryV11PublicationStage,
...
import {
	encodeSoundscaperDesktopProjectLibraryV11MetadataRow,
	sameSoundscaperDesktopProjectLibraryV11Lease,
	type SoundscaperDesktopProjectLibraryV11Lease,
	type SoundscaperDesktopProjectLibraryV11MetadataRow,
	validateSoundscaperDesktopProjectLibraryV11LeaseToken,
	validateSoundscaperDesktopProjectLibraryV11MetadataIntegrity,
	validateSoundscaperDesktopProjectLibraryV11MetadataRow,
...
export type SoundscaperDesktopProjectLibraryV11PublicationJournalState =
	| 'prepared'
	| 'materialized'
	| 'committed'
	| 'complete';

export interface SoundscaperDesktopProjectLibraryV11PersistedBody {
	readonly bodyId: string;
	readonly mediaRelativeFile: string;
	readonly descriptor: Readonly<SoundscaperDesktopProjectLibraryV11TransferBody>;
}

export interface SoundscaperDesktopProjectLibraryV11PersistedPublication {
	readonly transactionId: string;
	readonly state: SoundscaperDesktopProjectLibraryV11PublicationJournalState;
	...
	readonly bodies: readonly Readonly<SoundscaperDesktopProjectLibraryV11PersistedBody>[];
	readonly stages: readonly Readonly<SoundscaperDesktopProjectLibraryV11PublicationStage>[];
}
...
export function readSoundscaperDesktopProjectLibraryV11MetadataSnapshot(
	database: DatabaseSync,
): Readonly<{
	readonly row: SoundscaperDesktopProjectLibraryV11MetadataRow;
	readonly metadata: Readonly<SoundscaperDesktopLibraryV11Metadata>;
}> {
	...
	`).get();
	if (!value) throw new Error('Soundscaper V11 metadata row is missing');
	const row = validateSoundscaperDesktopProjectLibraryV11MetadataRow(
		value as Record<string, unknown>,
		'Soundscaper V11 publication metadata',
	);
	return Object.freeze({
		row,
		metadata: validateSoundscaperDesktopProjectLibraryV11MetadataIntegrity(
			row,
			'Soundscaper V11 publication metadata',
		),
	});
...
export function assertSoundscaperDesktopProjectLibraryV11PublicationLease(
	database: DatabaseSync,
	value: SoundscaperDesktopProjectLibraryV11Lease,
	now: number,
): SoundscaperDesktopProjectLibraryV11Lease {
	const lease = validateSoundscaperDesktopProjectLibraryV11LeaseToken(value);
	...
	const current = validateSoundscaperDesktopProjectLibraryV11LeaseToken({
		leaseId: row.leaseId,
	...
	} as SoundscaperDesktopProjectLibraryV11Lease);
	if (current.expiresAtMs <= now || !sameSoundscaperDesktopProjectLibraryV11Lease(current, lease)) {
		throw leaseLost();
	}
	return current;
}

export function prepareSoundscaperDesktopProjectLibraryV11Publication(
	database: DatabaseSync,
	transactionId: string,
	plan: Readonly<SoundscaperDesktopProjectLibraryV11PublicationPlan>,
	stages: readonly Readonly<SoundscaperDesktopProjectLibraryV11PublicationStage>[],
	now: number,
): void {
	if (!TRANSACTION_ID.test(transactionId)) {
		throw new TypeError('Soundscaper V11 publication transaction id is invalid');
	}
	transaction(database, () => {
		assertSoundscaperDesktopProjectLibraryV11PublicationLease(database, plan.lease, now);
		const current = readSoundscaperDesktopProjectLibraryV11MetadataSnapshot(database);
		...
		if (current.row.revision !== plan.expectedMetadataRevision) {
			throw new Error('Soundscaper V11 metadata revision changed before journal preparation');
		}
		if (database.prepare(`
		...
		const next = encodeSoundscaperDesktopProjectLibraryV11MetadataRow(plan.nextMetadata, now);
		database.prepare(`
...
export function markSoundscaperDesktopProjectLibraryV11PublicationMaterialized(
	database: DatabaseSync,
	transactionId: string,
	...
	transaction(database, () => {
		const current = assertSoundscaperDesktopProjectLibraryV11PublicationLease(database, lease, now);
		...
		if (result.changes !== 1) throw new Error('Soundscaper V11 prepared publication is unavailable');
	});
}

export function commitSoundscaperDesktopProjectLibraryV11Publication(
	database: DatabaseSync,
	publication: Readonly<SoundscaperDesktopProjectLibraryV11PersistedPublication>,
	lease: SoundscaperDesktopProjectLibraryV11Lease,
	now: number,
): void {
	transaction(database, () => {
		const currentLease = assertSoundscaperDesktopProjectLibraryV11PublicationLease(database, lease, now);
		const current = readSoundscaperDesktopProjectLibraryV11MetadataSnapshot(database);
		...
		if (current.row.revision !== publication.expectedMetadataRevision) {
			throw new Error('Soundscaper V11 metadata revision changed before publication commit');
		}
		const result = database.prepare(`
		...
		if (result.changes !== 1) throw new Error('Soundscaper V11 project revision could not publish');
		...
		const next = encodeSoundscaperDesktopProjectLibraryV11MetadataRow(publication.nextMetadata, now);
		...
		if (metadata.changes !== 1) throw new Error('Soundscaper V11 publication lost metadata compare-and-swap');
		...
		if (journal.changes !== 1) throw new Error('Soundscaper V11 publication journal could not commit');
	});
}

export function settleSoundscaperDesktopProjectLibraryV11Publication(
	database: DatabaseSync,
	transactionId: string,
	lease: SoundscaperDesktopProjectLibraryV11Lease,
	now: number,
	...
	transaction(database, () => {
		assertSoundscaperDesktopProjectLibraryV11PublicationLease(database, lease, now);
		const result = database.prepare(`
		...
		if (result.changes !== 1) throw new Error('Soundscaper V11 publication journal could not complete');
		database.prepare(`
...
export function readSoundscaperDesktopProjectLibraryV11PendingPublication(
	database: DatabaseSync,
): Readonly<SoundscaperDesktopProjectLibraryV11PersistedPublication> | null {
	...
	if (rows.length > 1) throw new Error('Soundscaper V11 has conflicting pending publications');
	return rows.length === 0 ? null : parsePublicationRow(rows[0]!);
}

export function readSoundscaperDesktopProjectLibraryV11PublicationById(
	database: DatabaseSync,
	transactionId: string,
): Readonly<SoundscaperDesktopProjectLibraryV11PersistedPublication> {
	...
	if (!row) throw new Error('Soundscaper V11 publication journal is missing');
...
function parsePublicationRow(
	...
	if (!TRANSACTION_ID.test(transactionId)) throw new TypeError('Soundscaper V11 publication id is invalid');
	...
	if (createHash('sha256').update(previousJson, 'utf8').digest('hex') !== previousDigest) {
		throw new Error('Soundscaper V11 previous metadata journal digest is invalid');
	}
	...
	if (createHash('sha256').update(nextJson, 'utf8').digest('hex') !== nextDigest) {
		throw new Error('Soundscaper V11 next metadata journal digest is invalid');
	}
	const previousMetadata = parseSoundscaperDesktopLibraryV11MetadataJson(previousJson);
	const nextMetadata = parseSoundscaperDesktopLibraryV11MetadataJson(nextJson);
	if (previousMetadata.revision !== expectedMetadataRevision
		|| nextMetadata.revision !== expectedMetadataRevision + 1) {
		throw new Error('Soundscaper V11 publication journal metadata revisions are invalid');
	}
	...
	if (new TextEncoder().encode(document).byteLength !== projectByteLength
		...
		throw new Error('Soundscaper V11 publication project journal integrity is invalid');
	}
	...
	if (!project || project.projectRevision !== projectRevision || project.sha256 !== projectSha256) {
		throw new Error('Soundscaper V11 publication project disagrees with next metadata');
	}
	...
	if (project.id !== entryId || project.metadataFile !== projectRelativeFile) {
		throw new Error('Soundscaper V11 publication project path disagrees with metadata');
	}
...
function parseBodies(value: unknown): readonly Readonly<SoundscaperDesktopProjectLibraryV11PersistedBody>[] {
	const parsed = parseJson(value, 'publication bodies');
	if (!Array.isArray(parsed) || parsed.length > MAXIMUM_BODIES) {
		throw new TypeError('Soundscaper V11 publication bodies are invalid');
	}
	return Object.freeze(parsed.map((value) => {
		...
		if (bodyId !== expectedBodyId) throw new Error('Soundscaper V11 publication body id is invalid');
		...
		if (mediaRelativeFile !== expectedFile) {
			throw new Error('Soundscaper V11 publication media path is invalid');
		}
...
function parseStages(
	...
	bodies: readonly Readonly<SoundscaperDesktopProjectLibraryV11PersistedBody>[],
): readonly Readonly<SoundscaperDesktopProjectLibraryV11PublicationStage>[] {
	const parsed = parseJson(value, 'publication stages');
	if (!Array.isArray(parsed) || parsed.length !== bodies.length + 1) {
		throw new TypeError('Soundscaper V11 publication stages are incomplete');
	}
	return Object.freeze(parsed.map((value, index) => {
		...
		if (record.role !== role || record.bodyId !== bodyId
			...
			throw new Error('Soundscaper V11 publication stage identity is invalid');
		}
		return Object.freeze({
			role,
			bodyId,
...
function publishBody(
	database: DatabaseSync,
	body: Readonly<SoundscaperDesktopProjectLibraryV11PersistedBody>,
	now: number,
): void {
	...
	if (existing) {
		if (existing.kind !== body.descriptor.kind || existing.encoding !== body.descriptor.encoding
			...
			throw new Error('Soundscaper V11 managed body identity conflicts with existing publication');
		}
		return;
	}
	database.prepare(`
...
function persistedBody(
	body: Readonly<SoundscaperDesktopProjectLibraryV11PlannedBody>,
): SoundscaperDesktopProjectLibraryV11PersistedBody {
	return {
		bodyId: body.bodyId,
...
function assertMetadataSnapshot(
	left: Readonly<SoundscaperDesktopLibraryV11Metadata>,
	right: Readonly<SoundscaperDesktopLibraryV11Metadata>,
	message: string,
): void {
	if (JSON.stringify(left) !== JSON.stringify(right)) {
		throw new Error(`Soundscaper V11 metadata ${message}`);
	}
}
...
function publicationState(value: unknown): SoundscaperDesktopProjectLibraryV11PublicationJournalState {
	if (value !== 'prepared' && value !== 'materialized' && value !== 'committed' && value !== 'complete') {
		throw new TypeError('Soundscaper V11 publication journal state is invalid');
	}
	return value;
}
...
function parseJson(value: unknown, label: string): unknown {
	...
	catch (error) { throw new TypeError(`Soundscaper V11 ${label} JSON is invalid`, { cause: error }); }
}
...
function closedRecord<const Field extends string>(
	...
	if (!value || typeof value !== 'object' || Array.isArray(value)
		...
		throw new TypeError(`Soundscaper V11 ${label} must be a plain record`);
	}
	const keys = Reflect.ownKeys(value);
	if (keys.length !== fields.length || keys.some(
		(key) => typeof key !== 'string' || !fields.includes(key as Field),
	)) throw new TypeError(`Soundscaper V11 ${label} has invalid fields`);
	const record = Object.create(null) as Record<Field, unknown>;
	for (const field of fields) {
		const descriptor = Object.getOwnPropertyDescriptor(value, field);
		if (!descriptor?.enumerable || !Object.hasOwn(descriptor, 'value')) {
			throw new TypeError(`Soundscaper V11 ${label}.${field} must be a data property`);
		}
...
function digest(value: unknown, label: string): string {
	...
	if (!DIGEST.test(result)) throw new TypeError(`Soundscaper V11 ${label} digest is invalid`);
	return result;
}
...
function leaseLost(): Error {
	return new Error('Soundscaper V11 publication writer lease no longer owns its fence');
}
