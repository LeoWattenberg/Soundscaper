...
export interface SoundscaperDesktopLibraryV11Project {
	readonly id: string;
...
export interface SoundscaperDesktopLibraryV11Media {
	readonly id: string;
...
export interface SoundscaperDesktopLibraryV11Metadata {
	readonly schemaVersion: 11;
	readonly revision: number;
	readonly projects: readonly Readonly<SoundscaperDesktopLibraryV11Project>[];
	readonly media: readonly Readonly<SoundscaperDesktopLibraryV11Media>[];
}

export function emptySoundscaperDesktopLibraryV11Metadata():
...
export function validateSoundscaperDesktopLibraryV11Metadata(
	value: unknown,
): Readonly<SoundscaperDesktopLibraryV11Metadata> {
	const record = closedRecord(value, METADATA_FIELDS, 'Soundscaper desktop V11 metadata');
	if (record.schemaVersion !== SOUNDSCAPER_DESKTOP_LIBRARY_SCHEMA_VERSION) {
		throw new TypeError('Soundscaper desktop V11 metadata has an unsupported schema version');
	}
	...
	if (Buffer.byteLength(JSON.stringify(metadata), 'utf8') > MAXIMUM_METADATA_BYTES) {
		throw new RangeError('Soundscaper desktop V11 metadata exceeds its byte limit');
	}
	return metadata;
}

export function parseSoundscaperDesktopLibraryV11MetadataJson(
	value: unknown,
): Readonly<SoundscaperDesktopLibraryV11Metadata> {
	if (typeof value !== 'string' || Buffer.byteLength(value, 'utf8') > MAXIMUM_METADATA_BYTES) {
		throw new RangeError('Persisted Soundscaper desktop V11 metadata exceeds its byte limit');
	}
	let parsed: unknown;
	try {
		parsed = JSON.parse(value) as unknown;
	} catch (error) {
		throw new TypeError('Persisted Soundscaper desktop V11 metadata is not valid JSON', { cause: error });
	}
	return validateSoundscaperDesktopLibraryV11Metadata(parsed);
}

function validateProject(value: unknown): Readonly<SoundscaperDesktopLibraryV11Project> {
	const record = closedRecord(value, PROJECT_FIELDS, 'Soundscaper desktop V11 project');
	if (record.preferredProduct !== 'soundscaper') {
		throw new TypeError('Soundscaper desktop V11 project must be owned by Soundscaper');
	}
	if (record.projectSchemaVersion !== SOUNDSCAPER_DESKTOP_LIBRARY_PROJECT_SCHEMA_VERSION) {
		throw new TypeError('Soundscaper desktop V11 project has an unsupported schema version');
	}
	...
	if (byteLength > MAXIMUM_PROJECT_DOCUMENT_BYTES) {
		throw new RangeError('Soundscaper desktop V11 project exceeds its document byte limit');
	}
	const metadataFile = `${id}/${String(projectRevision)}-${sha256}.json`;
	if (record.metadataFile !== metadataFile) {
		throw new TypeError('Soundscaper desktop V11 project metadataFile does not match its descriptor');
	}
...
function validateMedia(value: unknown): Readonly<SoundscaperDesktopLibraryV11Media> {
	const record = closedRecord(value, MEDIA_FIELDS, 'Soundscaper desktop V11 media');
	if (record.category !== 'audio-freeze' || !isSoundscaperDesktopLibraryFreezeMediaBindingId(record.id)) {
		throw new TypeError('Soundscaper desktop V11 media must be an exact freeze binding');
	}
	const relativeFile = freezeRelativeFileForSoundscaperDesktopLibraryBinding(record.id);
	if (record.relativeFile !== relativeFile) {
		throw new TypeError('Soundscaper desktop V11 media path does not match its freeze binding');
	}
	return Object.freeze({
		id: record.id,
		relativeFile,
...
function opaqueId(value: unknown, name: string): string {
	if (typeof value !== 'string' || !OPAQUE_ID.test(value)) {
		throw new TypeError(`Soundscaper desktop V11 ${name} is invalid`);
	}
	return value;
}
...
function digest(value: unknown, name: string): string {
	if (typeof value !== 'string' || !DIGEST.test(value)) {
		throw new TypeError(`Soundscaper desktop V11 ${name} digest is invalid`);
	}
	return value;
}
...
function nonNegativeSafeInteger(value: unknown, name: string): number {
	if (typeof value !== 'number' || !Number.isSafeInteger(value) || value < 0) {
		throw new TypeError(`Soundscaper desktop V11 ${name} must be a non-negative safe integer`);
	}
	return value;
}
...
function positiveSafeInteger(value: unknown, name: string): number {
	...
	if (result === 0) throw new TypeError(`Soundscaper desktop V11 ${name} must be positive`);
	return result;
}

function assertUnique(values: readonly string[], name: string): void {
	if (new Set(values).size !== values.length) {
		throw new TypeError(`Soundscaper desktop V11 metadata has a duplicate ${name}`);
	}
}
