...
import { initializeFramescaperDesktopProjectLibraryLifecycleDatabase } from './framescaper-project-library-database.ts';
...
export interface FramescaperDesktopProjectLibraryWriterSnapshot {
...
export function createFramescaperDesktopProjectLibraryExtension(
	...
	return Object.freeze({
		async start(context: Parameters<FramescaperDesktopProjectLibraryExactGenerationExtension['start']>[0]) {
			...
			const writer = await FramescaperDesktopProjectLibraryWriter.acquire({
				...context,
			...
			try {
				...
				try { await writer.close(); }
					...
					throw new AggregateError([error, cleanupError], 'Framescaper 1.0 startup cleanup failed');
				}
				throw error;
			}
		},
...
class FramescaperDesktopProjectLibraryWriter implements FramescaperDesktopProjectLibraryExactGenerationLifecycle {
	...
	#recovery: FramescaperDesktopProjectLibraryWriterSnapshot['recovery'] = Object.freeze({
	...
	static async acquire(value: Readonly<{
		...
		renewIntervalMs: number;
		checkpoint: ((phase: FramescaperDesktopProjectLibraryPublicationCheckpoint) => void) | null;
	}>): Promise<FramescaperDesktopProjectLibraryWriter> {
		...
		while (!lease) {
			...
			if (attempt.lease) lease = attempt.lease;
				...
				if (remaining <= 0) throw new Error('Framescaper desktop 1.0 writer lease is busy');
	...
	assertCanUse(): void {
		if (this.#closed) throw new Error('Framescaper desktop 1.0 writer is closed');
		if (this.#fenced !== null) throw new Error('Framescaper desktop 1.0 writer is fenced', { cause: this.#fenced });
		if (Date.now() >= this.#lease.expiresAtMs) {
			const error = new Error('Framescaper desktop 1.0 writer lease expired before renewal');
			this.#fence(error);
			throw error;
		}
	}

	snapshot(): Readonly<{ fenced: boolean; writer: Readonly<FramescaperDesktopProjectLibraryWriterSnapshot> }> {
		return Object.freeze({
	...
	assertLeaseInTransaction(database: DatabaseSync): void {
		this.assertCanUse();
		if (database !== this.#database) throw new Error('Framescaper 1.0 lease checked against another database');
		...
		if (row?.active !== 1 || row.leaseId !== this.#lease.leaseId
			...
			const error = new Error('Framescaper 1.0 publication writer lease no longer owns its fence');
			this.#fence(error);
			throw error;
		}
	}
	...
	async preparePublication(value: Readonly<FramescaperDesktopProjectLibraryExactPublicationDeclaration>): Promise<void> {
		...
		if (this.#activePublicationId !== null && this.#activePublicationId !== value.publicationId) {
			throw new Error('Framescaper 1.0 already owns an active publication');
		}
		this.#transaction(() => {
	...
	assertCanCommit(
		...
		if (row?.state !== 'materialized' || row.leaseId !== this.#lease.leaseId
			|| row.fencingToken !== this.#lease.fencingToken) {
			throw new Error('Framescaper 1.0 publication journal no longer owns its fence');
		}
	}
	...
	async recover(): Promise<void> {
		...
		if (rows.length > 1) throw new Error('Framescaper 1.0 has conflicting publication recovery journals');
		if (!rows[0]) return;
		...
		try {
			...
			this.#transaction(() => {
				...
				if (released.changes !== 1) {
					throw new Error('Framescaper 1.0 writer lease release lost ownership');
				}
			});
	...
	#updateJournal(publicationId: string, previous: string, next: string, result: string | null): void {
		this.#transaction(() => {
			...
			if (updated.changes !== 1) throw new Error(`Framescaper 1.0 publication journal could not enter ${next}`);
		});
	}
	...
	#renew(): void {
		...
		try {
			...
			if (renewed.changes !== 1) throw new Error('Framescaper 1.0 writer lease renewal lost ownership');
...
function acquireLease(
	...
	try {
		...
		if (!Number.isSafeInteger(fencingToken) || fencingToken < 1) throw new Error('Framescaper 1.0 fencing token overflow');
		...
		if (typeof ownerJson !== 'string') throw new Error('Framescaper 1.0 writer owner is not serializable');
		if (database.prepare(`
...
function sqlText(value: unknown, label: string): string {
	if (typeof value !== 'string' || value.length < 1) throw new Error(`Framescaper 1.0 ${label} is invalid`);
	return value;
}

function sqlNonNegative(value: unknown, label: string): number {
	if (!Number.isSafeInteger(value) || Number(value) < 0) {
		throw new Error(`Framescaper 1.0 ${label} is invalid`);
	}
	return Number(value);
}

function containedDocument(root: string, value: string): string {
	if (!value || value.includes('\0')) throw new Error('Framescaper 1.0 journal document is invalid');
