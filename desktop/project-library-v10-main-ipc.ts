...
export interface FramescaperDesktopProjectLibraryV10MainIpcRegistration {
...
export function registerFramescaperDesktopProjectLibraryV10MainIpc(
	...
	const options = snapshotClosedRecord(value, OPTION_FIELDS, 'Framescaper V10 main IPC options');
	if (typeof options.handle !== 'function' || typeof options.removeHandler !== 'function'
		|| typeof options.ownerFor !== 'function'
		|| !(options.main instanceof FramescaperDesktopProjectLibraryV10Main)) {
		throw new TypeError('Framescaper V10 main IPC requires exact Electron seams and main owner');
	}
	...
	const connections = new Map<object, FramescaperDesktopProjectLibraryV10MainSession>();
	const refused = new WeakSet<object>();
	const registered: string[] = [];
	let disposed = false;

	const owner = (event: unknown): object => {
		if (disposed) throw new Error('Framescaper V10 main IPC is disposed');
		return objectReference(ownerFor(event));
	};
	const connection = (event: unknown): FramescaperDesktopProjectLibraryV10MainSession => {
	...
	try {
		register(FRAMESCAPER_DESKTOP_PROJECT_LIBRARY_V10_MAIN_CHANNELS.handshake, (event, handshake) => {
			const renderer = owner(event);
		...
		register(FRAMESCAPER_DESKTOP_PROJECT_LIBRARY_V10_MAIN_CHANNELS.readProjectBundle,
		...
		register(FRAMESCAPER_DESKTOP_PROJECT_LIBRARY_V10_MAIN_CHANNELS.finishPublication,
			(event, request) => connection(event).finishPublication(request));
		register(FRAMESCAPER_DESKTOP_PROJECT_LIBRARY_V10_MAIN_CHANNELS.abortPublication,
...
async function closeSessions(
	sessions: readonly FramescaperDesktopProjectLibraryV10MainSession[],
): Promise<void> {
	...
	if (failures.length > 1) throw new AggregateError(failures, 'Framescaper V10 IPC cleanup failed');
}

function objectReference(value: unknown): object {
	if ((typeof value !== 'object' || value === null) && typeof value !== 'function') {
		throw new TypeError('Framescaper V10 renderer owner must be an object reference');
	}
	return value;
}
