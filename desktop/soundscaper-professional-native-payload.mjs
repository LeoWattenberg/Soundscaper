...
export async function describeSoundscaperProfessionalNativePayload(location, readFileImpl = readFile) {
	...
	if (selected.productionReadiness !== null) {
		try {
			...
			assertReadinessArtifacts(productionReadiness, pluginPeer, isolation);
	...
	return Object.freeze({ status: 'available', descriptor: Object.freeze({
		...
		productionReadiness, pluginPeer, isolation: Object.freeze({ ...isolation, entrypoint }),
	}) });
}
	...
	for (const target of value.targets) {
		...
		if (target.status === 'built' && (target.blockedBy !== null
			...
			|| (target.productionReadiness !== null
				&& !validReadinessReference(target.productionReadiness, target.id)))) {
...
async function runtimeArtifact(location, manifest, target, descriptor, readFileImpl) {
...
function assertReadinessArtifacts(verified, peer, isolation) {
	...
	if (launcher.peerPayloadSha256 !== peer.sha256
		...
		throw new Error('Signed readiness does not bind the exact professional peer/isolation closure.');
	}
}
