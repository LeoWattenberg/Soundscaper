...
import type {
	...
	SoundscaperDesktopProjectLibraryV10TransferSession,
} from './soundscaper-project-library-v10-transfer-service.ts';

export const SOUNDSCAPER_DESKTOP_PROJECT_LIBRARY_V10_CHANNELS = Object.freeze({
	handshake: 'soundscaper:v10:projects:handshake',
	readProjectBundle: 'soundscaper:v10:projects:bundle',
	readBodyChunk: 'soundscaper:v10:projects:bodies:read',
} as const);
...
interface Connection {
	...
	readonly session: SoundscaperDesktopProjectLibraryV10TransferSession;
}

export interface SoundscaperDesktopProjectLibraryV10IpcRegistration {
...
export function registerSoundscaperDesktopProjectLibraryV10Ipc(value: unknown):
	SoundscaperDesktopProjectLibraryV10IpcRegistration {
	const options = snapshotClosedRecord(value, OPTIONS_FIELDS, 'Soundscaper V10 IPC options');
	if (typeof options.handle !== 'function' || typeof options.ownerFor !== 'function'
		|| !options.service || typeof options.service !== 'object') {
		throw new TypeError('Soundscaper V10 IPC requires handler, owner, and service seams');
	}
	...
	const service = options.service as SoundscaperDesktopProjectLibraryV10TransferService;
	if (typeof service.openSession !== 'function') {
		throw new TypeError('Soundscaper V10 IPC requires a transfer service');
	}
	...
	const owner = (event: unknown): object => {
		if (disposed) throw new Error('Soundscaper V10 IPC is disposed');
		const result = ownerFor(event);
		if (!result || (typeof result !== 'object' && typeof result !== 'function')) {
			throw new TypeError('Soundscaper V10 renderer owner is invalid');
		}
		return result;
	};
	...
	handle(SOUNDSCAPER_DESKTOP_PROJECT_LIBRARY_V10_CHANNELS.handshake, (event, handshake) => {
		const renderer = owner(event);
	...
	handle(SOUNDSCAPER_DESKTOP_PROJECT_LIBRARY_V10_CHANNELS.readProjectBundle,
		(event, projectId) => {
			const admitted = connection(event);
			return admitted.session.readProjectBundle(projectId as string, admitted.controller.signal);
		});
	handle(SOUNDSCAPER_DESKTOP_PROJECT_LIBRARY_V10_CHANNELS.readBodyChunk,
		(event, request) => {
			const admitted = connection(event);
	...
	return Object.freeze({
		async dispose(): Promise<void> {
			...
			for (const { controller } of connections.values()) {
				controller.abort(new Error('Soundscaper V10 IPC was disposed'));
			}
			connections.clear();
		},
		async revokeOwner(renderer: object): Promise<void> {
			...
			refused.add(renderer);
			admitted.controller.abort(new Error('Soundscaper V10 renderer owner was revoked'));
		},
	});
}
...
function withSignal(value: unknown, signal: AbortSignal): unknown {
	if (!value || typeof value !== 'object' || Array.isArray(value)) {
		throw new TypeError('Soundscaper V10 body read must be a plain record');
	}
	...
	if (keys.includes('signal') || keys.some((key) => typeof key !== 'string')) {
		throw new TypeError('Soundscaper V10 body read has unsupported fields');
	}
	const result: Record<string, unknown> = {};
	for (const key of keys) {
		const descriptor = Object.getOwnPropertyDescriptor(value, key);
		if (!descriptor?.enumerable || !Object.hasOwn(descriptor, 'value')) {
			throw new TypeError(`Soundscaper V10 body read.${String(key)} must be a data property`);
		}
