...
import {
	constants, access, chmod, copyFile, mkdir, readFile, rename, rm, stat, writeFile,
...
import { createFramescaperDesktopProjectLibraryV18Paths } from './project-library-v18-contract.ts';
...
export async function importFramescaperDesktopProjectLibraryV18IntoV19(value: Readonly<{
	appDataPath: string;
	...
	const sourcePaths = createFramescaperDesktopProjectLibraryV18Paths(value.appDataPath);
	...
	try {
		...
		for (let index = progress; index < manifest.rows.length; index += 1) {
			const row = await migrateRow(sourcePaths, value.destinationPaths, manifest.rows[index]!);
		...
		if (finalManifest.digest !== manifest.digest) {
			throw new Error('Framescaper V18 source catalog changed during copy-forward');
		}
...
function validateV18Identity(database: DatabaseSync): void {
	...
	if (applicationId !== 0x46534350 || userVersion !== 20
		|| identity?.schemaVersion !== 18 || identity.projectSchemaVersion !== 27) {
		throw new Error('Framescaper V18 copy-forward source identity is unsupported');
	}
	const lineage = database.prepare('SELECT state FROM v17_import WHERE singleton = 1').get() as
		Record<string, unknown> | undefined;
	const lease = database.prepare('SELECT active FROM library_lease WHERE singleton = 1').get() as
		Record<string, unknown> | undefined;
	const journals = Number(database.prepare('SELECT COUNT(*) AS count FROM publication_journal').get()?.count);
	if (lineage?.state !== 'complete' || lease?.active !== 0 || journals !== 0) {
		throw new Error('Framescaper V18 copy-forward source lifecycle is not settled');
	}
}
...
function prepareProgress(
	...
	try {
		...
		const stored = database.prepare('SELECT * FROM v18_import WHERE singleton = 1').get() as
			Record<string, unknown> | undefined;
		if (!stored) {
			if (Number(database.prepare('SELECT COUNT(*) AS count FROM projects').get()?.count) !== 0) {
				throw new Error('Framescaper V19 import destination is not empty');
			}
			database.prepare(`
				INSERT INTO v18_import (
		...
		if (stored.source_catalog_sha256 !== manifest.digest
			...
			throw new Error('Framescaper V18 copy-forward source changed after durable admission');
		}
		const next = nonNegative(stored.next_project_index, 'V18 import cursor');
		if (next > manifest.rows.length) throw new Error('Framescaper V18 import cursor is invalid');
		database.exec('COMMIT');
		return next;
	} catch (error) {
...
async function migrateRow(
	...
	if (!Number.isSafeInteger(row.byte_length) || row.byte_length < 1 || !digest(row.sha256)) {
		throw new Error('Framescaper V18 project row has invalid file geometry');
	}
	const sourceDocument = containedPath(sourcePaths.projectsRoot, row.document_file, 'V18 document');
	...
	const sourceProject = validateFramescaperDesktopV18CurrentProjectV27(JSON.parse(sourceText) as unknown);
	if (sourceProject.id !== row.project_id || sourceProject.title !== row.title
		...
		throw new Error('Framescaper V18 project row disagrees with its exact document');
	}
	const project = reimportFramescaperProjectV28(FRAMESCAPER_V28_PROJECT_RUNTIME_PROFILE, sourceProject);
	if (project.id !== row.project_id || project.title !== row.title
		|| project.revision !== row.project_revision
		|| Date.parse(String(project.updatedAt)) !== row.updated_at_ms) {
		throw new Error('Framescaper V27 to V28 reimport changed catalog identity');
	}
	const document = JSON.stringify(project);
	const bytes = new TextEncoder().encode(document);
	const sha256 = createHash('sha256').update(bytes).digest('hex');
	if (!/^[a-f0-9]{48}$/u.test(row.entry_id)) {
		throw new Error('Framescaper V18 project row has an invalid entry identity');
	}
	const documentFile = `${row.entry_id}/${String(row.project_revision)}-${sha256}.json`;
	const destinationDocument = containedPath(destinationPaths.projectsRoot, documentFile, 'V19 document');
	await writeExactFile(destinationDocument, bytes, sha256);
	const bodies = parseFramescaperDesktopExactBodies(row.bodies_json, 'Framescaper V18 import');
...
async function writeExactFile(destination: string, bytes: Uint8Array, sha256: string): Promise<void> {
	try {
		await verifyFile(destination, bytes.byteLength, sha256);
		return;
	} catch (error) {
		if ((error as NodeJS.ErrnoException).code !== 'ENOENT') throw error;
	}
	...
	try {
		...
		await verifyFile(temporary, bytes.byteLength, sha256);
		await rename(temporary, destination);
	} finally {
		await rm(temporary, { force: true });
	}
...
function decodeDocument(bytes: Uint8Array): string {
	try {
		...
		throw new Error('Framescaper V18 project document is not valid UTF-8', { cause: error });
	}
}
	...
	try {
		...
		if (existing) {
			for (const field of Object.keys(row) as Array<keyof SourceRow>) {
				if (existing[field] !== row[field]) throw new Error('Framescaper V18 import row conflicts');
			}
		} else {
			database.prepare(`
		...
		database.prepare(`
			UPDATE v18_import SET next_project_index = ?
...
function completeImport(
	...
	try {
		assertLease(database);
		const progress = database.prepare('SELECT * FROM v18_import WHERE singleton = 1').get() as Record<string, unknown>;
		if (progress.next_project_index !== manifest.rows.length) throw new Error('Framescaper V18 import is incomplete');
		database.prepare('UPDATE library_identity SET metadata_revision = ? WHERE singleton = 1')
			.run(manifest.metadataRevision);
		database.prepare(`
			UPDATE v18_import SET state = 'complete', completed_at_ms = ? WHERE singleton = 1
		`).run(Date.now());
		database.exec('COMMIT');
	} catch (error) {
		database.exec('ROLLBACK');
...
function completeAbsentImport(database: DatabaseSync, assertLease: (database: DatabaseSync) => void): void {
	...
	try {
		assertLease(database);
		const existing = database.prepare('SELECT state FROM v18_import WHERE singleton = 1').get() as
			Record<string, unknown> | undefined;
		if (existing?.state !== undefined && existing.state !== 'complete') {
			throw new Error('Framescaper V18 source is unavailable after durable import admission');
		}
		if (!existing) {
			if (Number(database.prepare('SELECT COUNT(*) AS count FROM projects').get()?.count) !== 0) {
				throw new Error('Framescaper V19 import destination is not empty');
