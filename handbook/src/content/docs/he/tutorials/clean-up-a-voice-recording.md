...
description: "הסרת הרעש מההקלטה, חיתוך הרעם, הגעה לרמת עוצמה של פודקאסט וייצוא כקובץ MP3."
editUrl: false
sidebar:
  order: 2
head:
...
---
<!-- docs-ai-provenance: {"factPacketSha256":"96727487ae82c7f76b646047856630f73eb756ee7832615587e24e1e6db0b0ee","model":"aya-expanse:32b","modelDigest":"1603440383bd5504dc7afd01c0407b425b988dde650a8dc1a737433baa3cd432","operation":"translate","promptVersion":"docs-translate-v1","schemaVersion":1,"sourceLocale":"en","sourceSha256":"96727487ae82c7f76b646047856630f73eb756ee7832615587e24e1e6db0b0ee","targetLocale":"he"} -->

<!-- Generated by `node scripts/docs-reference.mjs`. Do not edit. -->

רוב ההקלטות שנעשות בבית דורשות שלוש תיקונים זהים: רעש רקע קבוע להסרה, רעש נמוך לסנן החוצה, ורמה שצריך להעלות לסטנדרט. המדריך הזה מבצע את שלושת התיקונים הללו על קטע דוגמה של שלוש שניות, שחציו הראשון הוא רק רעש חדר, ואז מייצא את התוצאה כקובץ MP3.
...
כל שלב למטה עובד על הקבצים האלה בדיוק כפי שהם, אז מה שאתם רואים צריך להתאים למה שהמדריך אומר. Soundscaper פועל בדפדפן; אין צורך בהתקנה.
:::

## מה תלמדו

- למה הפחתת רעש זקוקה לפרופיל, ואיך ליצור אחד.
- מה מסנן מעבר גבוה מסיר ואיפה להגדיר אותו עבור דיבור.
- ההבדל בין רמת שיא ללודיות, ואיך להגיע ליעד לודיות.
- איך לייצא קובץ MP3.

## שלבים

1. פתחו את Soundscaper. פרויקט חדש וריק מוכן ברגע שהעורך נטען.
2. בחרו **קובץ → יבוא** ובחרו את `guide-noisy-take.wav` — קטע קצר שחציו הראשון הוא רעש חדר לפני שהקול מתחיל. הוא נוחת כקטע על מסלול משלו.
3. לחצו על **הפעלה** להאזנה, ואז **עצירה**.
   *אתם צריכים לראות:* חצי שנייה של שריקה, ואז צליל קבוע המייצג קול, עם השריקה מתחתיו.
4. גררו את הסרגל מעל הקטע, מההתחלה לסימן 15%, כדי לבחור את ההקדמה של הרעש בלבד. הפרופיל חייב להכיל רק את הרעש שאותו אתם רוצים להסיר — ללא קול כלל.
5. בחרו **אפקט → הסרת ותיקון רעש → הפחתת רעש** ולחצו על **קבלת פרופיל רעש**. שורת הסטטוס מדווחת שהפרופיל מוכן. לחצו על **סגירה** כדי לעזוב את הדיאלוג בינתיים.
6. בחרו **בחירה → בחר הכל**. הפרופיל נשמר; כעת האפקט צריך לדעת מה לנקות.
7. בחרו **אפקט → הסרת ותיקון רעש → הפחתת רעש**. בדיאלוג **הפחתת רעש**, הגדירו את **הפחתת רעש** ל-`12`, ואז לחצו על **החל על הבחירה**. שתים-עשרה דציבלים הם הגדרה טובה ראשונית. יותר מסיר יותר רעש אבל גורם לקולות להישמע חלולים.
   *אתם צריכים לראות:* ההקדמה שטוחה כמעט לחלוטין והצליל לא נוגע.
8. בחרו **אפקט → אפקטים ישנים → מסננים קלאסיים**. בדיאלוג **מסננים קלאסיים**, בחרו **מעבר גבוה** עבור **סוג מסנן** והגדירו את **תדירות החיתוך** ל-`100`, ואז לחצו על **החל על הבחירה**. הכל מתחת ל-100 הרץ — תנועה, טיפול, מיזוג אוויר — נחלש. דיבור חי מעליו היטב.
9. בחרו **אפקט → נפח ודחיסה → נורמליזציה של לודיות**. בדיאלוג **נורמליזציה של לודיות**, הגדירו את **לודיות יעד** ל-`-16`, ואז לחצו על **החל על הבחירה**. −16 LUFS הוא היעד הנפוץ לפודקאסטים סטריאו. לודיות מודדת כמה חזק כל הקטע נשמע, לא כמה גבוהים השיאים שלו.
   *אתם צריכים לראות:* הגלומה גבוהה יותר והקטע מנוגן ברמה נוחה.
10. לחצו על **הפעלה** להאזנה, ואז **עצירה**.
   *אתם צריכים לראות:* קטע נקי ורציף עם הקדמה שקטה.
11. בחרו **קובץ → ייצוא אודיו**, הגדירו את **פורמט** ל**MP3**, ולחצו על **ייצוא**. הקובץ יורד ברגע שהעיבוד מסתיים, וקישורו נשאר בדיאלוג. הקובץ מקודד בדפדפן; שום דבר לא עוזב את המחשב שלכם.

## לאן הלאה

- עשו זאת על הקטע שלכם עם מדריכי ה-how-to: [הסרת רעש רקע](/guides/cleaning-up/remove-background-noise/), [הסרת רעש נמוך](/guides/cleaning-up/remove-low-rumble/) ו-[נורמליזציה של לודיות לפודקאסט](/guides/volume/normalize-loudness-for-podcasts/).
- בדקו את התוצאה בדרך שבה פלטפורמה הייתה עושה זאת: [מדידת כמה חזק המיקס שלכם הוא](/guides/analysis/measure-loudness/).

## מדריכים נוספים

[פרויקט Soundscaper הראשון שלכם](/tutorials/your-first-project/) — ייבאו הקלטה, האזינו לה, פיצלו אותה, דעכו אותה החוצה, ייצאו קובץ ושמרו את הפרויקט.
...
המדריך הזה מופעל, שלב אחר שלב ועל קבצי הדוגמה האלה, מול כל בניית Soundscaper על ידי סוויטת הדפדפן (`tests/browser/soundscaper-tutorials.spec.js`). אם שלב מפסיק לעבוד, הבנייה נכשלת עד שהמדריך מתוקן, אז מה שאתם קוראים הוא מה שהעורך עושה.
