...
Once the model is admitted in the signed catalog and its native runtime is available, the intended workflow is **Framescaper: Tools → Advanced Local Processing → Shot detection**. The steps below describe that workflow; this installation cannot currently complete.

1. Select video in Framescaper.
2. Install TransNetV2 through Model Manager once its signed catalog entry is available and choose model-based shot detection.
