...
Once the model is admitted in the catalog and its native runtime is available, the intended workflow is **Tools → Advanced Local Processing → Beat tracking**. The steps below describe that workflow; this installation cannot currently complete.

1. Select a passage of rhythmic music.
2. Install Beat This small through Model Manager once its catalog entry is available and select that exact model for beat tracking.
