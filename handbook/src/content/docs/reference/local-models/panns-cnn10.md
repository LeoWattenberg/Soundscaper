...
Once the model is admitted in the signed catalog and its native runtime is available, the intended workflow is **Tools → Advanced Local Processing → Audio tagging**. The steps below describe that workflow; this installation cannot currently complete.

1. Select the audio you want to analyze.
2. Install PANNs through Model Manager once its signed catalog entry is available, then run audio tagging.
