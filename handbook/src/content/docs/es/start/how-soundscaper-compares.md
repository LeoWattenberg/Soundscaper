...
la que normalmente se miden ambas. Esta página compara las tres para que puedas
determinar cuál de ellas ya realiza la tarea que necesitas.

## Cómo leer esta página

Cada celda indica **Sí**, **Parcial** o **No**, seguido del detalle que lo
matiza.

**Parcial** cubre tres situaciones diferentes, y la nota indica cuál se aplica:
la capacidad existe pero es más limitada que en otras, existe pero depende de
algo que debes proporcionar, o solo es accesible mediante un trabajo de
contorneo de una ausencia.
...
- Las filas de **Soundscaper** provienen de este repositorio: los perfiles de
capacidades del producto, el manifiesto de acciones de tiempo de ejecución y el
registro de formatos de exportación. Las cargas de destino nativas de escritorio
...
| --- | --- | --- | --- |
| Licencia | Sí — AGPL-3.0-only | Sí — GPL, código abierto | No — propietario y cerrado |
| Coste | Sí — gratis | Sí — gratis | No — suscripción a Creative Cloud |
...
| Funciona sin cuenta | Sí — no existe ninguna cuenta | Sí — inicio de sesión solo para audio.com | No — se requiere suscripción iniciada sesión |
...
| Abre proyectos de Audacity | Sí — importación y exportación AUP4 | Sí — nativo | No |
| Línea de tiempo de clips no destructiva | Sí | Sí | Sí — editor multicanal |
| Editor de archivo único dedicado | Parcial — la edición de muestras se realiza en la línea de tiempo | Parcial — las ediciones se aplican en su lugar en la línea de tiempo | Sí — editor de forma de onda |
...
| Bandeja de proyecto | Sí — organiza archivos y funciona como portapapeles | No | Parcial — el panel Archivos lista los archivos abiertos |
| Autoguardado y recuperación de errores | Sí — autoguardado, bloqueos y sobres de recuperación | Sí | Sí |
| Marcadores y regiones con nombre | Sí — de primera clase, con navegación y comportamiento de flujo | Parcial — pistas de etiquetas | Sí — marcadores y rangos |
...
| Brocha espectral | Sí | Sí | Sí — pincel y reparación de puntos |
...
| Herramientas para clics, zumbidos y sibilancias | Parcial — solo Click Removal | Parcial — solo Click Removal | Sí — DeClicker, DeHummer, DeEsser, Click/Pop Eliminator |
...
| Scripting Nyquist | Sí — complementos incluidos y el prompt de Nyquist | Sí — complementos incluidos y el prompt de Nyquist | No |
| Paquetes de efectos en sandbox | Parcial — paquetes WebAssembly revisados, uno se distribuye y los externos están aislados | No | No |
| Instrumentos virtuales | No — después de 1.0 | No | No |
...
| Sends | Sí — pre y post fader, múltiples asignaciones | No | Sí — pre y post fader |
| Grupos VCA | Sí | No | No |
| Entrada de sidechain | Sí | No | Sí — a través de sends |
| Mezclas de cue y sala de control | Sí | No | No |
| Compensación de retardo de complementos | Sí — reproducción, monitoreo, buses, sidechains, render y congelación | Parcial — no expuesto en las fuentes fijadas | Sí |
| Carriles de automatización | Sí — ganancia, panorámica, silencio, sends, buses y parámetros de complementos | No — sin carriles y sin herramienta de envolvente en la compilación fijada | Sí — volumen, panorámica y parámetros de efectos |
| Modos de automatización | Sí — lectura, ajuste, tacto, retención y escritura | No | Parcial — lectura, escritura, retención y tacto, sin ajuste |
...
| Capacidad | Soundscaper | Audacity 4 | Audition |
| --- | --- | --- | --- |
| Medidor de sonoridad | Sí — estilo EBU R 128, con historial | No — un efecto de Normalización de Sonoridad pero sin medidor | Sí — Loudness Radar según ITU-R BS.1770 |
| Medidor de fase y correlación | Sí | No | Sí — medidor de fase y análisis |
| Medición de sonido envolvente | Sí | No | Parcial — hasta 5.1 |
| Gráfico de espectro | Sí — Plot Spectrum | Parcial — registrado, pero la versión fijada lo comenta fuera del menú Analizar | Sí — Frequency Analysis |
| Clipping y RMS en la forma de onda | Sí — ambos, conmutados por proyecto | Sí — ambos, conmutados por proyecto | Parcial — indicadores de clipping, RMS en Amplitude Statistics |
| Contraste de inteligibilidad del habla | Sí — analizador de contraste | Parcial — registrado, pero la versión fijada lo comenta fuera del menú Analizar | No |
...
| Autoría y passthrough de ADM | Sí — BW64/ADM con comprobaciones de conformidad | No | No |
| Renderizado binaural | Sí — un modelo binaural nombrado | No | Parcial — binauralizador para ambisonics |
| Ambisonics | No | No | Sí — primer orden, con un panner VR |

## Exportación y entrega

| Capacidad | Soundscaper | Audacity 4 | Audition |
| --- | --- | --- | --- |
| Salida sin pérdida | Sí — WAV, AIFF, BWF y BW64 escritos nativamente | Sí — WAV, AIFF y FLAC | Sí — WAV, AIFF, FLAC y más |
| Salida con pérdida | Parcial — MP3, AAC, Opus, Vorbis, MP2, FLAC y WavPack, todos a través del tiempo de ejecución FFmpeg | Parcial — MP3 integrado, el resto a través de una instalación opcional de FFmpeg | Sí — integrado |
| Configuraciones personalizadas de codificador | Sí — un destino FFmpeg personalizado | Sí — un destino FFmpeg personalizado | Sí — opciones por formato |
| Cola de exportación | Sí — pausar, cancelar, reintentar y reordenar | No — una exportación a la vez | Parcial — Batch Process sin control de cola |
...
| Normalización de sonoridad en la exportación | Sí — parte del plan de entrega | Parcial — ejecutar el efecto primero | Sí — Match Loudness |
...
| Ida y vuelta con un editor de vídeo | Parcial — entrega el mismo proyecto a Framescaper sin copiar los medios | No | Sí — Dynamic Link con Premiere Pro |
...
| Exportación de vídeo | Sí — MP4 y WebM a través del tiempo de ejecución FFmpeg | No | No — solo audio |
| Composición, corrección de color y efectos | Parcial — en Framescaper, en el mismo proyecto | No | No |

## Asistencia por máquina

| Capacidad | Soundscaper | Audacity 4 | Audition |
| --- | --- | --- | --- |
| Mejora de voz | Parcial — solo escritorio, una vez instalado el paquete del modelo | No | Sí — Enhance Speech |
| Transcripción y diarización | Parcial — solo escritorio, modelos opcionales | No | No — las transcripciones están en Premiere Pro |
| Separación de fuente en tallos | Parcial — solo escritorio, modelos opcionales | No | No |
| Atenuación automática | Sí — efecto Auto Duck | Sí — efecto Auto Duck | Sí — atenuación de Essential Sound |
| Detección de compás y plano | Parcial — solo escritorio, modelos opcionales | No | Parcial — Remix reprograma la música automáticamente |
| Se ejecuta por completo en tu máquina | Sí — la inferencia es solo de escritorio y sin conexión tras la instalación | Sí — sin inferencia en absoluto | Parcial — algunas funciones se procesan en la nube de Adobe |
...
Audacity 4 es un editor de un solo paso. No tiene buses, ni envíos, ni carriles de automatización, ni macros en la versión fijada. Soundscaper mantiene ese modelo de edición y añade la capa de mezcla, automatización y entrega sobre él, además de grabación, vídeo e intercambio de trabajo que Audacity no intenta.
