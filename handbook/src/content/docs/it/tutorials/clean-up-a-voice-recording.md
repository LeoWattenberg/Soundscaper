...
---
<!-- docs-ai-provenance: {"factPacketSha256":"96727487ae82c7f76b646047856630f73eb756ee7832615587e24e1e6db0b0ee","model":"aya-expanse:32b","modelDigest":"1603440383bd5504dc7afd01c0407b425b988dde650a8dc1a737433baa3cd432","operation":"translate","promptVersion":"docs-translate-v1","schemaVersion":1,"sourceLocale":"en","sourceSha256":"96727487ae82c7f76b646047856630f73eb756ee7832615587e24e1e6db0b0ee","targetLocale":"it"} -->

<!-- Generated by `node scripts/docs-reference.mjs`. Do not edit. -->

La maggior parte delle registrazioni fatte in casa necessitano delle stesse tre riparazioni: un rumore di fondo costante da rimuovere, un ronzio basso da filtrare e un livello da portare a uno standard. Questo tutorial esegue tutte e tre le operazioni su un esempio di tre secondi, di cui il primo mezzo secondo è solo rumore ambientale, ed esporta il risultato come MP3.

:::tip[Cosa ti serve]
- Scarica [`guide-noisy-take.wav`](https://assets.soundscaper.org/guides/examples/guide-noisy-take.wav) — una breve registrazione il cui primo mezzo secondo è rumore ambientale prima che inizi la voce.
...
2. Scegli **File → Importa** e seleziona `guide-noisy-take.wav` — una breve registrazione il cui primo mezzo secondo è rumore ambientale prima che inizi la voce. Viene inserito come clip nella sua traccia.
...
4. Trascina la riga sopra il clip, dall'inizio al 15% del tempo, per selezionare l'introduzione di solo rumore. Il profilo deve contenere solo il rumore che vuoi rimuovere — nessuna voce.
5. Scegli **Effetto → Rimozione e riparazione del rumore → Riduzione del rumore** e premi **Ottieni profilo rumore**. La riga di stato indica che il profilo è pronto. Premi **Chiudi** per lasciare la finestra di dialogo per ora.
6. Scegli **Seleziona → Seleziona tutto**. Il profilo viene mantenuto; ora l'effetto deve sapere cosa pulire.
7. Scegli **Effetto → Rimozione e riparazione del rumore → Riduzione del rumore**. Nella finestra di dialogo **Riduzione del rumore**, imposta **Riduzione del rumore** a `12`, quindi premi **Applica alla selezione**. Dodici decibel è una buona impostazione iniziale. Una quantità maggiore rimuove più rumore ma rende le voci cavernose.
   *Dovresti vedere:* L'introduzione è quasi piatta e il tono rimane inalterato.
8. Scegli **Effetto → Effetti legacy → Filtri classici**. Nella finestra di dialogo **Filtri classici**, seleziona **Passa-alto** per **Tipo di filtro** e imposta **Frequenza di taglio** a `100`, quindi premi **Applica alla selezione**. Tutto ciò che è al di sotto di 100 Hz — traffico, maneggiamento, aria condizionata — viene attenuato. La voce si trova ben al di sopra di questa frequenza.
9. Scegli **Effetto → Volume e compressione → Normalizzazione del volume**. Nella finestra di dialogo **Normalizzazione del volume**, imposta **Volume target** a `-16`, quindi premi **Applica alla selezione**. −16 LUFS è l'obiettivo comune per i podcast stereo. Il volume misura quanto forte sembra l'intera registrazione, non quanto alti sono i suoi picchi.
...
11. Scegli **File → Esporta audio**, imposta **Formato** su **MP3** e premi **Esporta**. Il file viene scaricato non appena il rendering è completato, e il suo link rimane nella finestra di dialogo. Il file viene codificato nel browser; nulla lascia il tuo computer.

## Passaggi successivi

- Applica queste operazioni alla tua registrazione utilizzando le guide pratiche: [Rimuovere il rumore di fondo](/guides/cleaning-up/remove-background-noise/), [Rimuovere il ronzio basso](/guides/cleaning-up/remove-low-rumble/) e [Normalizzare il volume per un podcast](/guides/volume/normalize-loudness-for-podcasts/).
- Controlla il risultato come farebbe una piattaforma: [Misurare quanto è forte il tuo mix](/guides/analysis/measure-loudness/).
...
- [Ogni parametro degli effetti utilizzati qui, con il suo valore predefinito e l'intervallo, si trova nel riferimento degli effetti audio.](/reference/generated/audio-effects/#parameters)
...
Questo tutorial viene riprodotto, passo dopo passo e su questi stessi file, contro ogni versione di Soundscaper dal browser suite (`tests/browser/soundscaper-tutorials.spec.js`). Se un passaggio smette di funzionare, la versione fallisce fino a quando il tutorial non viene corretto, quindi ciò che leggi è ciò che fa l'editor.
