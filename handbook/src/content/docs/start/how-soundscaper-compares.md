...
- **Soundscaper** rows come from this repository: the product capability
  profiles, the runtime action manifest, the export format registry, and the
  browser and desktop codec capability gates.
  Desktop-native target payloads are generated by repository CI or target
  packaging. A package enables one only after staging and verifying the exact
  matching result; those rows say when a payload is still required.
...
| Microphone and desktop audio together | Partial — built in where the browser and operating system expose display audio | No | Partial — needs an operating-system loopback device |
...
| De-reverb | Partial — desktop Reduce Reverb workflow, with optional model and engine installed | No | Yes — DeReverb |
| Click, hum, and sibilance tools | Partial — Click Removal and De-esser; no dedicated hum remover | Partial — Click Removal only | Yes — DeClicker, DeHummer, DeEsser, Click/Pop Eliminator |
...
| Capability | Soundscaper | Audacity 4 | Audition |
| --- | --- | --- | --- |
| Built-in effect suite | Yes — Audacity-derived effects, bundled Nyquist plug-ins, and first-party effects such as Bitcrusher and De-esser | Yes — 30 built-in effects in the pinned build | Yes — around fifty, including multiband dynamics |
...
| Capability | Soundscaper | Audacity 4 | Audition |
| --- | --- | --- | --- |
| Lossless output | Yes — native WAV, AIFF, BWF, and BW64; FLAC and WavPack through dedicated codecs | Yes — WAV, AIFF, and FLAC | Yes — WAV, AIFF, FLAC, and more |
| Lossy output | Partial — MP3, MP2, Opus, and Ogg Vorbis through dedicated codecs; AAC depends on the browser or desktop provider | Partial — MP3 built in, the rest through an optional FFmpeg install | Yes — built in |
| Custom encoder settings | Partial — per-format controls; custom FFmpeg arguments are unavailable | Yes — a custom FFmpeg target | Yes — per-format options |
...
| Video export | Partial — MP4 and WebM with compatible browser WebCodecs or a verified desktop FFmpeg provider | No | No — audio only |
| Compositing, grading, and effects | Partial — in Framescaper, on the same project | No | No |

## Machine assistance

Desktop assistance requires optional model weights and a matching native engine.
...
| Capability | Soundscaper | Audacity 4 | Audition |
| --- | --- | --- | --- |
| Speech enhancement | Partial — desktop only, after installing the model and engine | No | Yes — Enhance Speech |
...
| Beat and shot detection | Partial — desktop beat detection with an optional model; shot detection is in Framescaper | No | Partial — Remix retimes music automatically |
