...
---
<!-- docs-ai-provenance: {"factPacketSha256":"96727487ae82c7f76b646047856630f73eb756ee7832615587e24e1e6db0b0ee","model":"aya-expanse:32b","modelDigest":"1603440383bd5504dc7afd01c0407b425b988dde650a8dc1a737433baa3cd432","operation":"translate","promptVersion":"docs-translate-v1","schemaVersion":1,"sourceLocale":"en","sourceSha256":"96727487ae82c7f76b646047856630f73eb756ee7832615587e24e1e6db0b0ee","targetLocale":"ko"} -->

<!-- Generated by `node scripts/docs-reference.mjs`. Do not edit. -->

대부분의 가정용 녹음에는 세 가지 수정이 필요합니다: 제거해야 할 지속적인 배경 소음, 필터링해야 할 저주파 잡음, 그리고 표준 수준으로 올려야 하는 레벨입니다. 이 튜토리얼은 3초 예제 테이크를 사용하여 이 세 가지를 모두 수행하고, 첫 0.5초는 방의 소음만 포함하며, 결과를 MP3로 내보냅니다.

:::tip[필요 사항]
- [`guide-noisy-take.wav`](https://assets.soundscaper.org/guides/examples/guide-noisy-take.wav) 다운로드 — 목소리가 시작되기 전 첫 0.5초가 방의 소음으로만 구성된 짧은 테이크.

아래의 모든 단계는 이 파일 그대로 작동하므로, 보는 내용이 튜토리얼과 일치해야 합니다. Soundscaper는 브라우저에서 실행되므로 설치할 필요가 없습니다.
:::

## 배우게 될 내용

- 노이즈 감소가 왜 프로필을 필요로 하는지, 그리고 어떻게 프로필을 생성하는지.
- 하이패스 필터가 제거하는 것, 그리고 음성용 최적의 설정 위치.
- 피크 레벨과 청각의 차이, 그리고 청각 목표를 달성하는 방법.
- MP3 내보내기 방법.

## 단계

1. Soundscaper 열기. 편집기가 로드되면 새로운 빈 프로젝트가 준비됩니다.
2. **파일 → 가져오기**를 선택하고 [`guide-noisy-take.wav`] — 목소리가 시작되기 전 첫 0.5초가 방의 소음으로만 구성된 짧은 테이크를 선택합니다. 이는 자체 트랙에 클립으로 추가됩니다.
3. **재생**을 눌러 들어본 후 **중지**합니다.
   *보여야 할 것:* 0.5초의 노이즈, 그 후 일정 음조의 톤(목소리를 대체함), 그리고 그 아래의 노이즈.
4. 클립 위의 눈금자를 드래그하여 시작부터 15% 지점까지 선택합니다. 프로필에는 제거하고자 하는 노이즈만 포함되어야 하며, 목소리는 전혀 없어야 합니다.
...
7. **효과 → 노이즈 제거 및 복구 → 노이즈 감소**를 선택합니다. **노이즈 감소** 대화 상자에서 **노이즈 감소**를 `12`로 설정하고 **선택 영역에 적용**을 누릅니다. 12 데시벨은 좋은 첫 번째 설정입니다. 더 높은 값은 더 많은 노이즈를 제거하지만 목소리가 공명하게 만듭니다.
   *보여야 할 것:* 선두 부분은 거의 평평해지고 톤은 그대로입니다.
8. **효과 → 레거시 효과 → 클래식 필터**를 선택합니다. **클래식 필터** 대화 상자에서 **필터 유형**을 **하이패스**로 선택하고 **커트오프 주파수**를 `100`로 설정합니다. **선택 영역에 적용**을 누릅니다. 100Hz 이하의 모든 것(교통, 핸들링, 에어컨 등)이 감소됩니다. 음성은 100Hz 이상에 잘 존재합니다.
9. **효과 → 볼륨 및 압축 → 청각 정규화**를 선택합니다. **청각 정규화** 대화 상자에서 **타겟 청각**을 `-16`로 설정하고 **선택 영역에 적용**을 누릅니다. -16 LUFS는 스테레오 팟캐스트의 일반적인 타겟입니다. 청각은 피크 높이가 아닌 전체 테이크의 청각적 느낌을 측정합니다.
   *보여야 할 것:* 웨이브폼이 더 높아지고 테이크가 편안한 수준으로 재생됩니다.
10. **재생**을 눌러 들어본 후 **중지**합니다.
   *보여야 할 것:* 깨끗하고 평평한 테이크와 조용한 선두 부분.
...
- [배경 소음 제거](/guides/cleaning-up/remove-background-noise/), [저주파 잡음 제거](/guides/cleaning-up/remove-low-rumble/), [팟캐스트용 청각 정규화](/guides/volume/normalize-loudness-for-podcasts/) 튜토리얼을 사용하여 자신의 테이크에 적용해 보세요.
- [믹스의 청각 측정](/guides/analysis/measure-loudness/)을 통해 플랫폼이 결과를 확인하는 방법을 알아보세요.

## 다른 튜토리얼

[첫 번째 Soundscaper 프로젝트](/tutorials/your-first-project/) — 녹음 가져오기, 듣기, 분할, 페이드 아웃, 파일 내보내기 및 프로젝트 저장.
[음성 아래에 음악 배치](/tutorials/put-music-under-a-voice/) — 두 트랙을 겹치고, 하나를 다른 하나 아래로 자동으로 덕킹하고, 믹스다운 및 내보내기.

## 참고 자료

- [이 튜토리얼에서 사용된 효과 매개변수, 기본값 및 범위는 오디오 효과 참조](/reference/generated/audio-effects/#parameters)에 있습니다.
...
이 튜토리얼은 브라우저 스위트(`tests/browser/soundscaper-tutorials.spec.js`)를 사용하여 Soundscaper의 각 빌드에 대해 단계별로 재현됩니다. 단계가 작동하지 않으면 빌드가 실패하고 튜토리얼이 수정될 때까지 유지되므로, 읽는 내용은 편집기가 수행하는 내용과 동일합니다.
