...
이것은 Audacity의 **분석 → 대조 (Audacity 3; Audacity 4에는 대조 분석기가 없습니다)** 입니다. 아래의 이름은 Soundscaper의 자체 이름이며, 때때로 다를 수 있습니다.
:::
...
7. **대조** 패널에서 **전경 측정**을 누릅니다. 패널은 전경 수준, 배경 수준 및 그 차이를 보고합니다.
   *보여져야 할 것:* 전경 및 배경 수준, 그 차이(dB), 그리고 차이가 권장되는 20dB를 충족하는지 여부.

## 팁

- 차이가 부족한 경우, [배경 소음을 줄이거나](/guides/cleaning-up/remove-background-noise/) [압축기를 사용하여 음성을 고르게](/guides/volume/even-out-volume-with-a-compressor/) 한 다음 다시 측정합니다.
- 측정은 선택 범위에서 RMS 평균이므로, 단 한 마디의 큰 소리는 조용한 녹음을 구제하지 못합니다.
