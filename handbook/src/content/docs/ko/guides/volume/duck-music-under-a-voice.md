...
팟캐스트와 비디오는 누군가 말할 때마다 음악을 낮추고 침묵 사이에 다시 올림으로써 말과 음악이 함께 흐르도록 합니다. 오토 덕(Auto Duck)은 제어 트랙에서 이를 수행합니다. 제어 트랙이 임계값보다 더 크게 재생되는 곳마다 선택된 트랙이 낮춰지며, 가장자리에 페이드가 적용됩니다.
...
2. **파일 → 가져오기**를 선택하고 음악을 선택합니다. 이는 자체 트랙에 클립으로 배치됩니다.
3. **파일 → 가져오기**를 선택하고 목소리를 선택합니다. 이는 자체 트랙에 클립으로 배치됩니다.
...
6. **재생**을 눌러 들어보세요. 그런 다음 **중지**를 누릅니다.
  *다음과 같은 것을 보게 될 것입니다:* 다른 트랙이 재생되는 동안 음악이 떨어지고 조용해지면 원래 수준으로 회복됩니다.
...
더 많은 [볼륨 및 동역학](/guides/volume/) 가이드:

- [조용한 녹음을 크게 만들기](/guides/volume/make-a-recording-louder/) — Amplify를 사용하여 녹음을 일정한 데시벨 수치로 올립니다.
- [피크 정규화하기](/guides/volume/normalize-peaks/) — 녹화의 가장 큰 지점을 전체 스케일 아래의 정확한 수준으로 가져옵니다.
