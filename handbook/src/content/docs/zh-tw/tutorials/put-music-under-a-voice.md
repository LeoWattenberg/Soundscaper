...
2. 選擇**檔案 → 匯入**，並選擇`guide-music-loop.wav` — 一個兩秒立體聲循環。該檔案會以剪貼的形式出現在獨立的軌上。
3. 選擇**檔案 → 匯入**，並選擇`guide-second-loop.wav` — 另一個兩秒循環，用於疊加在上面。該檔案會以剪貼的形式出現在另一個獨立的軌上。
4. 按下**播放**聆聽，然後按下**停止**。
