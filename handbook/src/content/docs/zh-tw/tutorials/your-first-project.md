...
2. 選擇**檔案 → 匯入**，並選擇`guide-music-loop.wav` — 一個兩秒的立體聲循環。檔案會以剪輯的形式出現在其自己的軌道上。
3. 按下**播放**聆聽，然後按下**停止**。
