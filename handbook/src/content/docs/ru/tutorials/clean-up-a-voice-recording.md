...
2. Выберите **Файл → Импорт** и выберите `guide-noisy-take.wav` — короткую запись, первая половина секунды которой представляет собой шум помещения до начала голоса. Файл загружается как клип на отдельной дорожке.
