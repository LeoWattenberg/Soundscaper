...
2. 选择**文件 → 导入**，并选择`guide-music-loop.wav` — 一个两秒钟的立体声循环。文件会作为剪辑落在自己的轨道上。
3. 按**播放**听一下，然后按**停止**。
