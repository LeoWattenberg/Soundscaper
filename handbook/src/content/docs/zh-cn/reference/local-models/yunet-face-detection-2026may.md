...
在具备所需原生运行时的平台上打开 **Framescaper：效果 → 视频效果 → 重新构图**。本地辅助功能在桌面编辑器中运行。

1. 在 Framescaper 中选择一个视频片段。
2. 安装 YuNet 和 D-FINE 进行主体检测。引导式 重新构图 任务还使用 U²-Net-P 显著性检测。
3. 在本地运行并检查建议的裁切。“工具 → 高级本地处理”可单独运行主体检测。

此操作还需要 [D-FINE Nano 目标检测](/reference/local-models/dfine-nano-coco/)。
