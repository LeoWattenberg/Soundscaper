...
sidebar:
  order: 4
---
<!-- docs-ai-provenance: {"factPacketSha256":"a92eef7d909ae85318c6f4052e734d9eb2e21c8d848d98174fdece51bba0b044","model":"gpt-5.6-luna","modelDigest":"manual","operation":"translate","promptVersion":"docs-translate-v1","schemaVersion":1,"sourceLocale":"en","sourceSha256":"a92eef7d909ae85318c6f4052e734d9eb2e21c8d848d98174fdece51bba0b044","targetLocale":"en-GB"} -->
...
| Loudness Normalisation | `audacity-loudness-normalization` | Volume and dynamics | No | Yes | — |
...
| Normalise | `audacity-normalize` | Volume and dynamics | No | Yes | — |
...
Vocoder uses 10 to 240 logarithmically spaced bands from 20 Hz to just below half the sample rate, with the same band-width settings as the bundled Nyquist effect. Distance controls an eighth-order envelope low-pass filter: higher values smooth the voice envelope more slowly. In stereo, the left channel supplies the voice envelope and the right channel supplies the carrier. Mono input uses sine carriers at the band centres, mixed with optional white noise and radar pulses. Both channels preserves the voice on the left and puts vocoded audio on the right; Vocoded audio duplicates the processed signal into the first stereo pair. Additional channels pass through unchanged. The processor normalises against its largest output peak since playback began, with a bounded gain, then applies Output gain. Unlike Nyquist’s whole-selection normalisation, this cannot anticipate later peaks, so the beginning may be louder. The output keeps the input channel count. Rack playback and export include its filter and envelope release, subject to the existing combined 10-second tail limit.
...
| Loudness Normalisation | `audacity-loudness-normalization` | The gain depends on complete-program loudness. |
| Normalise | `audacity-normalize` | DC offset and peak gain depend on complete-selection statistics. |
...
| Loudness Normalisation | `audacity-loudness-normalization` | Normalise | Perceived loudness | Perceived loudness; RMS | — |
| Loudness Normalisation | `audacity-loudness-normalization` | Target loudness | -23 | -145 to 0 | LUFS |
| Loudness Normalisation | `audacity-loudness-normalization` | Target RMS | -20 | -145 to 0 | dB |
| Loudness Normalisation | `audacity-loudness-normalization` | Normalise stereo channels independently | Off | On or off | — |
| Loudness Normalisation | `audacity-loudness-normalization` | Treat mono as dual-mono | On | On or off | — |
...
| Normalise | `audacity-normalize` | Peak amplitude | -1 | -145 to 0 | dBFS |
| Normalise | `audacity-normalize` | Remove DC offset (centre on 0.0 vertically) | On | On or off | — |
| Normalise | `audacity-normalize` | Normalise peak amplitude to | On | On or off | — |
| Normalise | `audacity-normalize` | Normalise stereo channels independently | Off | On or off | — |
