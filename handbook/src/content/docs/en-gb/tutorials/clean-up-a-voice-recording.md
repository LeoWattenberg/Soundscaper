...
---
<!-- docs-ai-provenance: {"factPacketSha256":"96727487ae82c7f76b646047856630f73eb756ee7832615587e24e1e6db0b0ee","model":"gpt-5.6-luna","modelDigest":"manual","operation":"translate","promptVersion":"docs-translate-v1","schemaVersion":1,"sourceLocale":"en","sourceSha256":"96727487ae82c7f76b646047856630f73eb756ee7832615587e24e1e6db0b0ee","targetLocale":"en-GB"} -->
...
2. Choose **File → Import** and pick `guide-noisy-take.wav` — a short take whose first half second is room noise before the voice starts. It lands as a clip on its own track.
...
5. Choose **Effect → Noise removal and repair → Noise Reduction** and press **Get noise profile**. The status line reports that the profile is ready. Press **Close** to leave the dialogue for now.
6. Choose **Select → Select all**. The profile is kept; now the effect needs to know what to clean.
7. Choose **Effect → Noise removal and repair → Noise Reduction**. In the **Noise Reduction** dialogue, set **Noise reduction** to `12`, then press **Apply to selection**. Twelve decibels is a good first setting. More removes more noise but makes voices sound hollow.
   *You should see:* The lead-in is nearly flat and the tone is untouched.
8. Choose **Effect → Legacy effects → Classic Filters**. In the **Classic Filters** dialogue, choose **High-pass** for **Filter type** and set **Cutoff frequency** to `100`, then press **Apply to selection**. Everything below 100 Hz — traffic, handling, air conditioning — is rolled off. Speech lives well above it.
9. Choose **Effect → Volume and compression → Loudness Normalisation**. In the **Loudness Normalisation** dialogue, set **Target loudness** to `-16`, then press **Apply to selection**. −16 LUFS is the common target for stereo podcasts. Loudness measures how loud the whole take feels, not how tall its peaks are.
...
11. Choose **File → Export audio**, set **Format** to **MP3**, and press **Export**. The file downloads as soon as the render finishes, and its link stays in the dialogue. The file is encoded in the browser; nothing leaves your computer.

## Where next

- Do it on your own take with the how-to guides: [Remove background noise](/guides/cleaning-up/remove-background-noise/), [Remove low rumble](/guides/cleaning-up/remove-low-rumble/) and [Normalise loudness for a podcast](/guides/volume/normalize-loudness-for-podcasts/).
