...
2. Choose **File → Import audio** and pick the passage to stretch. The file lands as a clip on its own track.
3. Choose **Select → Select all**.
4. Choose **Effect → Pitch and tempo → Sliding stretch**. In the **Sliding stretch** dialog, set **Final tempo change** to `25`, then press **Apply to selection**. The initial tempo change stays at zero, so the passage starts at its own speed and is a quarter faster by the end.
