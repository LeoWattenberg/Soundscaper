...
4. Choose **View → Show markers**. The marker lane is where the chapter starts are shown and named.
...
9. Choose **File → Export audio**, set **Format** to **MP3**, set **Output** to **Chapters (split by markers)**, and press **Export**. The file downloads as soon as the render finishes, and its link stays in the dialogue. Each chapter becomes an MP3 inside the archive, numbered in order and named after its marker.
