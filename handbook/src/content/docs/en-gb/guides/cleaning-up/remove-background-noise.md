...
4. Choose **Effect → Noise removal and repair → Noise Reduction** and press **Get noise profile**. The status line reports that the profile is ready. Press **Close** to leave the dialogue for now.
5. Choose **Select → Select all**. The profile is kept; now the effect needs to know what to clean.
6. Choose **Effect → Noise removal and repair → Noise Reduction**. In the **Noise Reduction** dialogue, set **Noise reduction** to `12`, then press **Apply to selection**. Around 12 dB is a good first try. Higher values remove more noise but start to make voices sound hollow.
