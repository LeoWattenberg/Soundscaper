...
2. Choose **File → Import** and pick `guide-music-loop.wav` — a two-second stereo loop. It lands as a clip on its own track.
3. Choose **File → Import** and pick `guide-second-loop.wav` — a second two-second loop to layer on top. It lands as a clip on its own track.
