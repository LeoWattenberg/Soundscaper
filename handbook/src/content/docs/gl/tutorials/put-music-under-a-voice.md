---
title: "Pon música baixo unha voz"
description: "Sobrepoña dúas pistas, atenúe unha baixo a outra automaticamente, mézclalas e expórtalas."
editUrl: false
sidebar:
  order: 3
head:
...
2. Escolle **Ficheiro → Importar** e selecciona `guide-music-loop.wav` — un bucle estéreo de dous segundos. Colócase como un clip no seu propio raiu.
3. Escolle **Ficheiro → Importar** e selecciona `guide-second-loop.wav` — un segundo bucle de dous segundos para superpoñelo. Colócase como un clip no seu propio raiu.
4. Preme **Reproducir** para escoitar e despois **Deter**.
   *Deberías ver:* Ambos os bucles reprodúcense xuntos, cada un no seu propio raiu.
5. Fai clic na barra de nome de `guide-music-loop.wav` para seleccionala. O clip que selecciones é o que se atenua — a música, non a voz.
6. Escolle **Efecto → Volume e compresión → Auto Duck**. No diálogo **Auto Duck**, escolhe **guide-second-loop** para **Raiu de control** e establece **Cantidade de atenuación** en `-12`, e despois preme **Aplicar á selección**. Onde o raiu de control sexa máis alto que o limiar, o raiu seleccionado redúcese pola cantidade de atenuación, con desvanecementos nos bordos.
   *Deberías ver:* A forma de onda da música é máis pequena onde a voz está a reproducirse.
7. Preme **Reproducir** para escoitar e despois **Deter**.
   *Deberías ver:* A música está baixo a voz e recupérase onde a voz está baixa.
8. Fai clic na barra de nome de `guide-music-loop.wav`, e despois mantén premida Shift e fai clic na barra de nome de `guide-second-loop.wav`, para que ambos estean seleccionados. Cada raiu cun clip seleccionado entra na mestura.
...
10. O proxecto agora mostra un clip chamado **Mestura**.
   *Deberías ver:* Un raiu contén un clip chamado Mestura; os dous raius orixinais desapareceron. Editar → Desfacer trairíaos de volta.
...
- Manter as partes separadas para que outra persoa as mesture: [Exportar cada raiu como o seu propio ficheiro](/guides/projects/export-each-track-as-its-own-file/).

## Outros tutoriais

[O teu primeiro proxecto en Soundscaper](/tutorials/your-first-project/) — Importa unha gravación, escoita, divídoo, desvanéceo, exporta un ficheiro e garda o proxecto.
