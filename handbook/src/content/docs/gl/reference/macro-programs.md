...
**Programas**, os programas que gardaches. Prema **+ (Novo programa)** na
cabeceira de Programas para crear un. A mesma barra de accións ofrece **Importar programa**,
**Exportar programa** e **Eliminar programa** para o programa seleccionado. O
painel de detalles mostra o seu **Nome do programa**, o texto do **Programa** e un botón **Executar
...
[Compartir programas](#sharing-programs) para saber o que implica iso.
...
editor, e cada chamada a el devolve unha promesa.
...
Tab insere dous espazos no campo do programa. Prema Escape e despois Tab para saír
do campo.

### O que pode usar un programa

Está presente a biblioteca estándar habitual de JavaScript: `Object`, `Array`, `Map`,
...
### O que non pode usar un programa

Un programa execútase nun traballador (worker) ao que se lle retiraron as súas capacidades antes de que
...
`setTimeout` e `setInterval`. Ler calquera delas devolve `undefined`.

Un programa non pode `import` un módulo; un `import` estático é un erro de sintaxe na
liña que o contén. Todo o que o programa necesite ten que estar dentro do programa.

A fronteira de seguridade non son os globais que faltan, senón o propio editor: só
responde ás chamadas listadas nesta páxina e rexeita todo o máis polo seu nome,
sexa calquera o que o programa logre enviarlle.

## Execución dun programa

Preme en **Executar programa**. Toda a execución é unha entrada no historial do proxecto, polo que
un **Deshacer** revirte todo o que o programa fixo, por moitas modificacións que faga.
Se o programa lanza unha excepción, ou se cancela, ou se executa máis alá do seu prazo límite, o proxecto
...
Despois da execución, o panel mostra o rexistro do programa, seguido de *Programa aplicado.*
cando a execución se completou. Unha execución fallida mostra *O programa fallou na liña N:* e
...
pista enfocada, que é a pista cuxa cabeceira premeu por última vez ou cuxo clip
seleccionou por última vez. Cando non hai selección de tempo pero hai un clip seleccionado, o
efecto cobre ese clip. As chamadas de selección dun programa cambian o intervalo de tempo e
o conxunto de pistas seleccionadas, pero non cal pista ten o foco, polo que unha execución procesa
unha pista. Se nada está enfocado ou a selección está baleira, a execución falla coa
mesma mensaxe que dá o menú Efecto.

## A API `sound`

Todos os métodos seguintes devolven unha promesa a menos que se indique o contrario. Agarda cada chamada
antes de facer a seguinte; un programa que comeza máis de oito chamadas sen
agudarlles ten a novena rexeitada.
...
| `seed` | A semente da que proceden os números aleatorios da execución. Nova en cada execución. |
| `startedAt` | O tempo real no que comezou a execución, como unha cadea ISO 8601. |
...
como JSON. Estes métodos non devolven nada e non precisan ser agardados.

Un rexistro contén como máximo 1.000 liñas ou 256 KiB, calquera que chegue primeiro, e cada liña
córtase nos 4.096 caracteres. As liñas que excedan ese límite descártanse e contanse; a contaxe
...
taxa de mestrea do proxecto en hertz, que é en que se mide cada contaxe de fotogramas nesta páxina.

`sound.project.tracks()` devolve un array de pistas en orde cronolóxica:
...
`sound.select.time(start, end, options)` establece o intervalo de tempo en segundos. É
o comando `SelectTime` de Audacity, e `options.relativeTo` elixe desde onde se mide cada
...
O final do proxecto é o último fotograma ao que chega calquera clip. As pistas seleccionadas quedan
como estaban.

`sound.select.frames(startFrame, endFrame, options)` establece o intervalo de tempo en
fotogramas á taxa de mestura do proxecto. `options.trackIds` nomea as pistas a
seleccionar; cando se omite, as pistas que xa están seleccionadas permanecen seleccionadas.
O intervalo acóustase á liña temporal e os bordes intercámbianse se están invertidos.
...
retirar esas pistas dela. O intervalo de tempo queda como estaba.

`sound.select.frequencies(options)` é o comando `SelectFrequencies` de Audacity.
Establece a selección espectral a `options.low` e `options.high` en hertzios;
...
e `params` é un obxecto dos parámetros dese efecto. Os parámetros que se omiten toman
...
Aplica unha cadea de efectos sobre a selección actual nunha pasada, exactamente como o faría unha macro de lista de pasos con eses pasos. Cada paso é `{ type, params }`, e a cadea necesita polo menos un paso. Resólvese en `null`.
...
[Comandos que un programa pode executar](#commands-a-program-can-run). Os catro comandos de selección aceptan os parámetros descritos alí; os demais non aceptan ningún. Resólvese á selección despois.
...
comeza en 0 e avanza un para cada chamada respondida ao editor, e por
`ms` para cada `sound.wait(ms)`. `sound.wait` resólvese inmediatamente; non hai
...
indicas ningunha. Como un erro lanzado remata a execución e revirte todo o que houbo antes, unha
...
`null`, booleanos, números finitos, cadenas e matrices e obxectos simples dos
mesmos. `NaN`, `Infinity`, funcións, instancias de clase, matrices tipadas e `Date`
os obxectos son rexeitados cun erro, igual que calquera valor maior de 1 MiB, anidado máis
de 12 niveis de profundidade ou que conteña máis de 4.096 entradas nunha matriz ou obxecto.
As propiedades `undefined` descártanse.

## Límites

| Límite | Valor |
| --- | --- |
| Longitude do programa | 256 KiB |
...
clip, polo que pode cubrir 128 clips antes de que se esgote o orzamento.
...
un parámetro fóra do rango. O erro tamén leva un `code`, que é
...
Ese programa complétase, e o seu rexistro di *refused: Unsupported macro command:
ExportWav.*

Un erro que o programa non captura remata a execución, revira o proxecto e móstrase no panel coa liña da que procede. Un programa que non se pode compilar infórmase da mesma maneira antes de que se execute nada.

## Efectos que un programa pode aplicar {#effects-a-program-can-apply}

Estas son as IDs de efectos `sound.effect` e `sound.effects` aceptan, cos parámetros que cada unha toma e os seus valores predeterminados. Os rangos e as unidades están na
[referencia de efectos de audio](/reference/generated/audio-effects/). Os complementos Nyquist non se poden aplicar desde un programa.
...
| Cambiar tonalidade | `audacity-change-pitch` | `semitones: 0`, `preserveFormants: true` |
| Cambiar velocidade e tonalidade | `audacity-change-speed-pitch` | `speedPercent: 0` |
...
| Compresor | `audacity-compressor` | `thresholdDb: -10`, `makeupGainDb: 0`, `kneeWidthDb: 5`, `ratio: 10`, `lookaheadMs: 1`, `attackMs: 30`, `releaseMs: 150` |
...
| Inverter | `audacity-invert` | ningún |
...
| Inverter | `audacity-reverse` | ningún |
...
Dous efectos necesitan algo que un programa non poida proporcionar. A Redución de Ruxe necesita un perfil de ruxe capturado no diálogo propio do efecto, e o Auto Duck necesita unha pista de control por debaixo da pista enfocada.

## Comandos que un programa pode executar {#commands-a-program-can-run}

`sound.command` acepta os nomes de comandos de macros de Audacity que se listan a continuación. Son os mesmos nomes que pode conter unha macro de lista de pasos, polo que un programa e unha lista de pasos teñen exactamente o mesmo alcance. Cada comando executa a acción do editor que a
...
Un parámetro que se deixe sen especificar deixa esa parte da selección intacta, que é como Audacity os interpreta tamén.
...
`Undo` e `Redo` están ausentes porque unha execución xa é unha entrada do historial e un paso que percorrese o historial chegaría máis alá da execución ata as túas propias edicións. Os comandos de transporte e gravación están ausentes porque un programa non ten nada que agardar e non se pode desfacer dunha gravación. Abrir, gardar, pechar, importar, exportar e preferencias están ausentes porque o alcance dun programa é o único proxecto que estaba aberto cando comezou. Os comandos que só abren un diálogo ou cambian a vista están ausentes porque non cambian nada no proxecto.

## Compartir programas {#sharing-programs}

**Exportar programa** escribe o programa seleccionado como un ficheiro `.soundscapemacro`, e
**Importar programa** le un. O ficheiro é JSON e non un ficheiro `.js` simple, polo
que nada no ordenador receptor o confundirá con algo para executar fóra
do editor:
...
programa**; no seu lugar, o panel amosa o programa, o ficheiro do que procede,
unha nota sobre o que un programa pode facer co proxecto aberto e unha caixa de selección que di *Leín este programa e quero executalo.* Marcala para activar **Activar este
programa**, e só entón se pode executar o programa.

Esa permisión é para o texto exacto que leches. Se o programa cambia
depois, sexa porque o editas ou porque importas unha copia máis nova sobre el, a revisión
aparece de novo ata que actives o novo texto. Os programas que escribas ti mesmo no xestor
non necesitan revisión.

## Exemplos

Desvanecer cada clip na primeira pista que os teña. Fais clic no cabeceiro deseña pista
antes de executar, para que o efecto caia na pista que está lendo o programa:
...
Informe do proxecto sen modificalo:
...
Executa unha macro de lista de pasos gardada só cando a selección sexa suficientemente longa:
...
Cada programa desta páxina, desde os fragmentos dunha liña ata os exemplos desenvolvidos,
execútase contra cada compilación de Soundscaper mediante a suite do navegador
