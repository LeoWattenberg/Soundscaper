...
/**
 * The vocabulary a handbook guide is written in.
 *
...
/** Import one of the guide example files, named by its fixture id. */
...
/** Mix selected stereo guide tracks with the Mix & Render dialog defaults. */
...
/** Open the guide's example Audacity project through File → Audacity projects. */
...
/** Throw unless a guide has every field the renderer and the browser suite rely on. */
export function validateGuide(guide, fixtures) {
	if (!guide || typeof guide !== 'object') throw new TypeError('A guide must be an object.');
	for (const key of ['id', 'title', 'description', 'audacity', 'intro']) {
		if (typeof guide[key] !== 'string' || guide[key].length === 0) throw new TypeError(`Guide ${String(guide.id)} needs a ${key}.`);
	}
	if (!/^[a-z0-9]+(?:-[a-z0-9]+)*$/u.test(guide.id)) throw new RangeError(`Guide id ${guide.id} must be a kebab-case slug.`);
	if (!Array.isArray(guide.steps) || guide.steps.length === 0) throw new TypeError(`Guide ${guide.id} needs steps.`);
	if (guide.steps[0].kind !== 'open') throw new RangeError(`Guide ${guide.id} must start by opening the editor.`);
	if (!Array.isArray(guide.tips) || guide.tips.some((tip) => typeof tip !== 'string')) throw new TypeError(`Guide ${guide.id} tips must be strings.`);
	for (const [index, entry] of guide.steps.entries()) {
		if (!isStepKind(entry?.kind)) throw new RangeError(`Guide ${guide.id} step ${String(index + 1)} has unknown kind ${String(entry?.kind)}.`);
		if (entry.kind === 'import' && !(entry.fixture in fixtures)) {
			throw new RangeError(`Guide ${guide.id} imports unknown example ${entry.fixture}.`);
		}
		if (entry.kind === 'select-clips' && entry.fixtures.some((id) => !(id in fixtures))) {
			throw new RangeError(`Guide ${guide.id} selects a clip from an unknown example.`);
		}
	}
	return guide;
}
...
export function describeStep(entry, { fixtureFile }) {
	switch (entry.kind) {
		...
		case 'import':
			return `Choose ${menuPath(['File', 'Import audio'])} and pick your recording. This guide uses \`${fixtureFile(entry.fixture)}\`. The file lands as a clip on its own track.`;
		case 'menu':
		...
		case 'open-audacity-project':
			return `Choose ${menuPath(['File', 'Audacity projects', 'Open Audacity project (.aup3, .aup4)'])} and pick the project. This guide uses a small example project. Its tracks and clips appear as they were in Audacity.`;
		case 'export-project':
