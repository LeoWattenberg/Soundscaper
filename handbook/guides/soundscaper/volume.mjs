...
export const VOLUME_GUIDES = Object.freeze([
	{
		...
		steps: [
			open(),
			importAudio('quiet-take', { what: 'the quiet recording' }),
			selectAll(),
			effect({
			...
			play({ see: 'The recording is noticeably louder and the waveform is taller.' }),
		],
		tips: [
	...
	},
	{
		...
		steps: [
			...
			play({ see: 'The recording plays at a comfortable, consistent level.' }),
		],
		tips: [
	...
	},
	{
		...
		steps: [
			open(),
			importAudio('music-loop', { what: 'the recording with the uneven levels' }),
			selectAll(),
			effect({
				group: 'Volume and compression',
	...
	},
	{
		...
		steps: [
			open(),
			importAudio('music-loop', { what: 'the recording that starts or stops too hard' }),
			selectRange(0, 0.25, { where: 'as much of the start as you want the fade to last' }),
			effect({ group: 'Fading', name: 'Fade In', direct: true }),
			selectRange(0.75, 1, { where: 'the same amount at the end' }),
			effect({ group: 'Fading', name: 'Fade Out', direct: true }),
			play({ see: 'The recording swells in and dies away instead of starting and stopping hard.' }),
		],
		tips: [
	...
	},
	{
		...
		steps: [
			open(),
			importAudio('music-loop', { what: 'the recording with the stray peaks' }),
			selectAll(),
			effect({
				group: 'Volume and compression',
	...
	},
	{
		...
		steps: [
			open(),
			importAudio('music-loop', { what: 'the music' }),
			importAudio('second-loop', { what: 'the voice' }),
