...
4. Audacity's reviewed strings for the locale, committed under `src/common/i18n/audacity/` by the weekly `sync-audacity-translations.yml` run and lazy-loaded the same way, which override any key they carry.
...
The tool is optional authoring tooling: it is not used by documentation builds, CI, or deterministic reference generation, and it never contacts anything but the configured Ollama server; the glossary is read from the committed Audacity layer. Generated files are ordinary working-tree changes: review them and use Git to keep or revert them.

## Routes

A locale is reachable once it has a route: `COMMITTED_LOCALE_TAGS` in `src/common/i18n/locales.js` lists every locale the static build publishes, the sidebar and preferences offer, and the desktop packages. A machine catalog earns its locale that route, so adding a language means generating its catalog and listing its tag there. Audacity's strings are not a condition for a route: they are laid over whatever the locale has, however many of them there are.
