...
const outputPath = outputArgumentIndex >= 0
	...
	: join(lameDirectory, manifest.wasm.path);
...
const environment = {
	...process.env,
	// LAME 4.0 requires a pkg-config command even when both consumers are disabled.
	// `true` supplies no flags; the closed build links neither sndfile nor mpg123.
...
try {
	...
	const sourceRoot = join(temporaryDirectory, 'lame-4.0');
	const buildRoot = join(temporaryDirectory, 'build-lame');
	mkdirSync(buildRoot);
	const commonFlags = [
		'-O3', '-flto', '-mno-simd128', '-fno-fast-math', '-fno-finite-math-only',
		'-fvisibility=hidden', '-DNDEBUG=1',
		`-ffile-prefix-map=${sourceRoot}=lame-4.0`, `-fdebug-prefix-map=${sourceRoot}=lame-4.0`,
	...
	run(emmake, ['make', '-j1'], { cwd: buildRoot });
	const library = join(buildRoot, 'libmp3lame/.libs/libmp3lame.a');
	const members = archiveEvidence(library);
	verifyArchiveEvidence(members);
	const exportedFunctions = manifest.wasm.requiredExports
		.filter((name) => name.startsWith('sclm_'))
		.map((name) => `_${name}`);
	const temporaryOutput = join(temporaryDirectory, 'lame.wasm');
	run(emcc, [
		join(lameDirectory, 'native/soundscaper_lame.c'), library,
		`-I${join(sourceRoot, 'include')}`, '-std=c11', ...commonFlags,
	...
	if (bytes.byteLength > manifest.wasm.maximumBytes) {
		throw new Error(`LAME WASM is ${String(bytes.byteLength)} bytes; limit is ${String(manifest.wasm.maximumBytes)}.`);
	}
	if (manifest.wasm.sha256 && hash !== manifest.wasm.sha256) {
		throw new Error(`LAME WASM hash mismatch: expected ${manifest.wasm.sha256}, got ${hash}.`);
	}
	...
	process.stdout.write(
		...
		+ `SHA-256 ${hash}\n`
		+ `libmp3lame archive ${String(members.count)} members, SHA-256 ${members.sha256}\n`,
	);
...
function admittedSourceForgeUrl(value) {
	...
	return url.protocol === 'https:'
		...
		&& url.pathname.endsWith('/project/lame/lame/4.0/lame-4.0.tar.gz');
}
...
function verifyArchiveEvidence(actual) {
	...
	if (expected.membersSha256 && (actual.count !== expected.memberCount
		...
		throw new Error('The libmp3lame compiled archive membership changed.');
	}
}
...
function verifyManifest() {
	if (manifest.schemaVersion !== 1 || manifest.lame.release !== '4.0'
		...
		throw new Error('LAME source admission must remain pinned to the official 4.0 archive.');
	}
	if (manifest.toolchain.emscriptenVersion !== '3.1.64'
		...
		throw new Error('LAME build toolchain or linear-memory bounds changed.');
	}
	...
	])) throw new Error('LAME configure admission changed.');
}

function verifyLocalFiles() {
	for (const file of manifest.localFiles) {
		const actual = sha256(readFileSync(join(lameDirectory, file.path)));
		if (actual !== file.sha256) throw new Error(`Pinned LAME local file mismatch for ${file.path}: ${actual}.`);
	}
}
...
function verifyCompiler() {
	...
	if (!`${result.stdout}\n${result.stderr}`.includes(manifest.toolchain.emscriptenVersion)
		&& process.env.LAME_ALLOW_TOOLCHAIN_MISMATCH !== '1') {
