...
/** Build target support, not a claim that a particular machine completed inference. */
...
function runtimeNotice(availability, published = true) {
	...
	if (blockedBy === 'kokoro-offline-g2p-unprovisioned') {
		return `Desktop builds can supply the ${familyId} ${version} engine, but the authenticated offline Kokoro G2P helper is not yet available for any supported target. The production worker accepts only a verified helper, so installing model weights cannot enable speech generation yet. The required real-model test fails closed until the helper and its data are published and verified.`;
	}
	if (availablePlatforms.length === 0) {
		return `**The required native engine is not yet available.** The desktop build has no ${familyId} ${version} target for this model. Downloading weights does not enable processing. The required real-model test reports a failure until the model and verified native runtime are supplied.`;
	}
	if (offlineG2pRuntime === 'package-generated') {
		return `Desktop builds can download and authenticate the offline Kokoro G2P helper alongside the ${familyId} ${version} engine for ${availablePlatforms.map((platform) => PLATFORM_NAMES[platform]).join(', ')}. ${published ? 'Install the model and its required runtimes through Model Manager, then run speech generation locally.' : 'Model installation still requires its digest-pinned catalog publication.'} Consult the nightly report for real text-to-WAV results from each package and machine.`
			+ (missingPlatforms.length ? ` The complete speech runtime is not available for ${missingPlatforms.map((platform) => PLATFORM_NAMES[platform]).join(', ')}.` : '');
	}
	return `Desktop builds can download and authenticate the required ${familyId} ${version} engine for ${availablePlatforms.map((platform) => PLATFORM_NAMES[platform]).join(', ')}. ${published ? 'Install this model and its required runtime through Model Manager, then run its task locally.' : 'Model installation still requires its digest-pinned catalog publication.'} These are supported build targets; consult the nightly test report for results on a particular package and machine.`
		+ (missingPlatforms.length ? ` The desktop build has no downloadable engine for ${missingPlatforms.map((platform) => PLATFORM_NAMES[platform]).join(', ')} even though model weights are listed for those platforms.` : '');
}
...
function modelPage(entry, cases, runtimeSources) {
	...
	const body = [
		...
		`Approximate ${published ? 'model artifact download' : 'candidate artifact'} size: **${downloadSize(downloadBytes)}**. The required runtime adds to the first download if it is not already installed. Minimum system memory: **${formatBinaryBytes(entry.minimumMemoryBytes)}**. This ${published ? 'catalog' : 'candidate'} requirement refers to total memory, not currently free memory. Inference also needs sufficient free memory, and installation needs working space; memory is not a speed guarantee.`, '',
		`${published ? 'Published' : 'Planned'} platforms: ${entry.platforms.map((platform) => PLATFORM_NAMES[platform] ?? platform).join(', ')}. A matching downloadable runtime ${published ? 'is also' : 'and catalog publication are'} required.`, '',
...
function indexPage(catalog, cases, runtimeSources) {
	...
	const rows = catalog.entries.map((entry) => [
		...
		entry.pendingPublication ? 'Catalog publication pending' : availability.get(entry.modelId).availablePlatforms.length === 0
				...
				? 'Offline G2P helper pending' : 'Native engine pending' : 'Downloadable; see supported platforms',
	...
	return page('Local model guides and real execution tests',
		...
		'These guides describe the published catalog and the additional models required by the nightly tests. Desktop installers omit the native inference engines; Model Manager downloads and verifies the required runtime and weights when you install a model. A prepared runtime and a required test do not grant model publication authority.', '',
		...(pending.length ? [`**Local runtime publication is incomplete: ${String(pending.length)} required ${pending.length === 1 ? 'model' : 'models'} in ${String(pendingCases.length)} ${pendingCases.length === 1 ? 'case' : 'cases'} ${pending.length === 1 ? 'lacks' : 'lack'} a required downloadable component.** Their tests fail until the selected target has the complete runtime.`, '']
			: ['All published models have a downloadable native engine on supported desktop targets. Install the model and its runtime, then use the task menus or Tools → Advanced Local Processing. This build capability does not claim that every platform has passed the real-model tests.', '']),
		...
		table(['Model guide', 'Purpose', 'Runtime availability', 'Desktop execution case'], rows), '',
		WINDOWS_REQUIREMENT, '',
		...
		'The ordinary browser tests check the interface with a simulated desktop backend. The nightly-with-tests Electron package downloads verified model and runtime files before requesting real inference. Missing required runtime components are reported as failures on catalog-supported platforms. Expensive model runs are excluded from the normal browser suite. Read the report from a particular package run for its actual results; these pages describe the required checks, not a claim that every build or machine has passed.', '',
