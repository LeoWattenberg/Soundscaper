...
const editorOptionalArchiveModule = String.raw`(?:archive-media-reader|aup-legacy(?:-block-budget|-conversion|-xml)?|aup4-(?:client|opaque-merge|opaque-persistence|profile-values|profile|sanitization|time-signature|track-nodes)|audacity-(?:annotation-interchange|tempo-import)|dawproject-(?:archive|export(?:-clip-omissions|-context|-lanes)?|format|import(?:-maps|-project|-structure|-timeline)?|xml)|sesx-import(?:-project)?|scape-(?:archive-(?:copy|layout(?:-witness)?|manifest|reader)|export-(?:destination|plan)|import-capacity|import-transaction|project-admission|project-source-remap)|scape-project(?:-canonical-inspection|-timing-assets)?)`;
 ...
 */
const editorEagerAssistanceModule = String.raw`(?:assistance-asset-command-v1|assistance-asset-reference-v1|operation|shots|transcript|transcript-scape-asset-extension-v1|tts-script-(?:body-publication|scape-asset-extension)-v1)`;
export const editorOptionalSurfaceModule = String.raw`(?:project-attribution-(?:csv|report)\.ts|ui[\\/](?:AttributionTab\.tsx|AudacityEffectLayout\.jsx|MetadataEditorTabs\.tsx|ParametricEqEditor\.jsx|PrivacyPolicyRoute\.tsx|ProjectLockToast\.tsx|SoundActivationPreferences\.tsx|VideoDeliveryFields\.jsx|audio-editor-design-system[\\/]06a-panels-(?:attribution|freesound)\.css|desktop-export-codec-model\.ts|export-(?:dialog-audio-codec-options\.ts|dialog-model\.js|preset-(?:actions\.js|model\.ts))|framescaper-(?:caption-file-interchange|finishing-dialog-model|native-services-dialog-model|visual-inspector-model)\.ts|local-assistance-(?!lazy-)(?!menu\.ts$)(?!review-authority\.ts$)[a-z\d-]+\.ts|local-model-manager-(?:bridge|store)\.ts|soundscaper-(?:production-dialog-model|routing-editor-model)\.ts|video-keyframe-(?:curve-transfer|dialog-model)\.ts|workspace[\\/](?:(?:FramescaperCaptureSources|Freesound(?:OriginalFallbackDialog|Panel(?:Container)?|PublishDialog|UploadArea)|ProjectAttributionTab|ProjectMetadataPanel|RecordingSetupPanel|WebVcrPanel|WebVcrPreview|SoundscaperRoutingGraph(?:Inspector|View))\.tsx|freesound-(?:auth-upload-client|panel-session|upload-file-preparation|upload-queue)\.ts|linked-video-choice-handoff\.ts|project-attribution-presentation\.ts|soundscaper-routing-(?:folder-authority|graph-(?:candidates|gesture|layout))\.ts)))`;
...
/** Audio/video delivery and Freesound clip materialization isolated from the eager controller graph. */
 ...
 *
 * The rack's realtime-effect shortcut handler and lazy parameter-editor wrapper
 * belong here too. Left to their own chunks they land with the overlay's lazy
 * facade, which re-exports the overlay, so this chunk imports the facade while
...
const framescaperProjectFoundationModules = Object.freeze([
	...
	// Runtime retime and nested sequence playback call back into these foundations.
	// Keep them in one emitted chunk so the lazy desktop edge cannot split a cycle.
	'editor-project-retime-structural-admission',
	'editor-project-retime-validation',
	'editor-project-retime-runtime',
	'editor-project-retime',
	'editor-project-runtime-profile',
	'editor-project-sequence-multicam',
	'editor-project-sequence-multicam-playback',
	'editor-project-sequence-nested-playback',
	'editor-project-sequence-nested-retime',
	'editor-project-sequence-nested-sequence',
	'editor-project-sequence-runtime',
...
export const EDITOR_DOMAIN_CHUNK_TEST = new RegExp(
	`${editorPath}(?!${editorOptionalArchiveModule}\\.(?:[cm]?[jt]s)$)(?!${editorOptionalExecutionModule}\\.(?:[cm]?[jt]s)$)(?!${editorOptionalCaptureFlatModule}\\.ts$)(?:[^\\\\/]+|assistance[\\\\/](?:internal[\\\\/]radix-two-fft-v1|[^\\\\/]+)|(?:design-system-adapters|platform)[\\\\/][^\\\\/]+)\\.(?:[cm]?[jt]s)$`,
);
