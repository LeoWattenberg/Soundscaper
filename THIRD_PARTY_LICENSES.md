...
The converted strings are committed source: one JSON catalog per locale at
`src/common/i18n/translations/<locale>.json`, generated by
`scripts/audacity-qt-translations.mjs` (`scripts/lib/audacity-committed-layer.mjs`)
and lazy-loaded by the runtime like the machine catalogs. Each catalog embeds
its own `provenance` object recording the exact upstream repository, head
commit, GitHub workflow run and artifact ID, source archive name, byte length
and SHA-256 digest, the `GPL-3.0-only` SPDX identifier, the upstream project
and commit-specific license URLs, the reviewed key-mapping version and digest,
and a stable modification notice. The same commit, artifact, and mapping
digest are also recorded in
[`src/common/i18n/translations/NOTICE.md`](src/common/i18n/translations/NOTICE.md),
alongside the licence text in
[`src/common/i18n/translations/LICENSE.txt`](src/common/i18n/translations/LICENSE.txt).
The GPL-3.0-only strings are combined with this AGPL-3.0-only application
under section 13 of both licences; the corresponding source for the
translated material is this repository itself, with the upstream artifact and
commit named in each catalog's provenance.
...
- Audacity design system (vendored in-tree at `vendor/audacity-design-system/`) — `@audacity-ui/components` 0.10.3, `@audacity-ui/core` 0.1.0, `@audacity-ui/tokens` 0.1.0 — declared MIT in each package manifest (the upstream repository ships no LICENSE file); vendored from branch `master`, commit `f5a0e9ee9e9c1ea59a3a13adb516ef0cc3f89447`, which released 0.10.3 and moved the components package from the `@dilsonspickles` scope without pushing a `components-v0.10.3` tag; source: <https://github.com/DilsonsPickles/audacity-design-system/tree/f5a0e9ee9e9c1ea59a3a13adb516ef0cc3f89447/packages>; local modifications are recorded in `vendor/audacity-design-system/README.md` and the pinned upstream revision in `vendor/audacity-design-system/UPSTREAM` (verified by `check:notices`)
...
- `@fontsource-variable/inter` 5.3.0 — SIL Open Font License 1.1; self-hosted variable Inter, Copyright 2016 The Inter Project Authors; upstream: <https://github.com/rsms/inter>; full license: `LICENSES/inter-variable.txt`
...
- `@fontsource/nunito-sans` 5.3.0 — SIL Open Font License 1.1; self-hosted editor skin fonts, Copyright 2016 The Nunito Sans Project Authors; upstream: <https://github.com/Fonthausen/NunitoSans>; full license: `LICENSES/nunito-sans.txt`
- `@fontsource/jetbrains-mono` 5.3.0 — SIL Open Font License 1.1; self-hosted editor skin fonts, Copyright 2020 The JetBrains Mono Project Authors; upstream: <https://github.com/JetBrains/JetBrainsMono>; full license: `LICENSES/jetbrains-mono.txt`
...
- `gettext-parser` 9.1.1 — MIT; Copyright (c) 2014-2015 Andris Reinman; source: <https://github.com/smhg/gettext-parser>; development-only PO parsing for reviewed community translation imports, excluded from the browser application
- `@types/gettext-parser` 9.0.0 — MIT; Copyright (c) Microsoft Corporation; source: <https://github.com/DefinitelyTyped/DefinitelyTyped/tree/master/types/gettext-parser>; development-only TypeScript declarations for the PO importer
...
#### Kokoro v1.0 speech synthesis model

- Kokoro-82M v1.0 — Apache-2.0 model weights; original model by hexgrad and
  ONNX export by ONNX Community. The exact fp32 ONNX graph, tokenizer, and 54
  unchanged voice-style files come from [ONNX Community revision
  `1939ad2a8e416c0acfeecc08a694d14ef25f2231`](https://huggingface.co/onnx-community/Kokoro-82M-v1.0-ONNX/tree/1939ad2a8e416c0acfeecc08a694d14ef25f2231).
  The [originating model card at
  `f3ff3571791e39611d31c381e3a41a3af07b4987`](https://huggingface.co/hexgrad/Kokoro-82M/blob/f3ff3571791e39611d31c381e3a41a3af07b4987/README.md)
  and the [ONNX model card](https://huggingface.co/onnx-community/Kokoro-82M-v1.0-ONNX/blob/1939ad2a8e416c0acfeecc08a694d14ef25f2231/README.md)
  are retained without edits in [`LICENSES/local-models/`](LICENSES/local-models/README.md).
  The full Apache-2.0 text is retained there as well. The original card states
  that the model used a few hundred hours of permissive or non-copyrighted
  audio, including public-domain, permissively licensed, and some synthetic
  audio. It does not provide a complete training dataset inventory.

- Attribution from the originating model card: [Koniwa `tnc`](https://github.com/koniwa/koniwa)
  audio (under one hour) is identified as [CC BY 3.0](https://creativecommons.org/licenses/by/3.0/deed.ja),
  and [SIWIS](https://datashare.ed.ac.uk/handle/10283/2353) audio (under eleven
  hours) is identified as [CC BY 4.0](https://datashare.ed.ac.uk/bitstream/handle/10283/2353/license_text).
  Those attributions describe training inputs; the model files themselves are
...
byte-range, CORS, and full SHA-256 readback are retained in
[`evidence/local-model-publication/kokoro-82m-v1.0.json`](evidence/local-model-publication/kokoro-82m-v1.0.json).
The source paths and pinned URLs are in
[`config/assistance-kokoro-model-source-candidate.json`](config/assistance-kokoro-model-source-candidate.json).

- `af_alloy.bin` — 522,240 bytes; SHA-256 `c4a6b876047fd7fb472edf4ebd63cfac7c3b958a7cae7c106e8f038ca6308c45`.
- `af_aoede.bin` — 522,240 bytes; SHA-256 `4a004c33430762e2461eedb2013fad808ef4ab3121f5300f554476caf58d8361`.
- `af_bella.bin` — 522,240 bytes; SHA-256 `f69d836209b78eb8c66e75e3cda491e26ea838a3674257e9d4e5703cbaf55c8b`.
- `af_heart.bin` — 522,240 bytes; SHA-256 `d583ccff3cdca2f7fae535cb998ac07e9fcb90f09737b9a41fa2734ec44a8f0b`.
- `af_jessica.bin` — 522,240 bytes; SHA-256 `a240a5e3c15b43563d6e923bdca8ef5613a23471d9b77653694012435df23bd8`.
- `af_kore.bin` — 522,240 bytes; SHA-256 `9be5221b6a941c04b561959b8ff0b06e809444dcc4ab7e75a7b23606f691819e`.
- `af_nicole.bin` — 522,240 bytes; SHA-256 `cd2191ab31b914ed7b318416b0e4440fdf392ddad9106a060819aa600a64f59a`.
- `af_nova.bin` — 522,240 bytes; SHA-256 `18778272caa0d0eebaea251c35fd635f038434f9eee5e691d02a174bd328414f`.
- `af_river.bin` — 522,240 bytes; SHA-256 `00a2bcf82b1d86e8f19902ede58c65ccf6c0e43b44b7d74fad54e5d8933c9c30`.
- `af_sarah.bin` — 522,240 bytes; SHA-256 `4409fbc125afabacc615d94db5398d847006a737b0247d6892b7a9a0007a2f0a`.
- `af_sky.bin` — 522,240 bytes; SHA-256 `4435255c9744f3f31659e0d714ab7689bf65d9e77ec1cce060f083912614f0b9`.
- `am_adam.bin` — 522,240 bytes; SHA-256 `162b035ed91cfc48b6046982184c645f72edcdd1b82843347f605d7bf7b15716`.
- `am_echo.bin` — 522,240 bytes; SHA-256 `3968b92c3c4cd1c4416dbded36c13eaa388a90d5788d02a13e4d781f5f8cf3c3`.
- `am_eric.bin` — 522,240 bytes; SHA-256 `e8b5be17edd1e3636901ce7598baafe2dc8dd8ff707a0c23bf9e461add7e2832`.
- `am_fenrir.bin` — 522,240 bytes; SHA-256 `c27989f741f7ee34d273a39d8a595cc0837d35f5ced9a29b7cc162614616df43`.
- `am_liam.bin` — 522,240 bytes; SHA-256 `52403be32fd047c6a44517cb0bcd6b134f2a18baa73e70ef41651e0eab921ade`.
- `am_michael.bin` — 522,240 bytes; SHA-256 `1d1f21dd8da39c30705cd4c75d039d265e9bc4a2a93ed09bc9e1b1225eb95ba1`.
- `am_onyx.bin` — 522,240 bytes; SHA-256 `da5d135b424164916d75a68ffb4c2abce3d7d5ccc82dd1ee6cf447ce286145e6`.
- `am_puck.bin` — 522,240 bytes; SHA-256 `fcf73c989033e9233e0b98713eca600c8c74dcc1614b37009d5450ff4a2274a0`.
- `am_santa.bin` — 522,240 bytes; SHA-256 `61150cf726ab6c5ed7a99f90a304f91f5a72c00c592e89ec94e5df11c319227a`.
- `bf_alice.bin` — 522,240 bytes; SHA-256 `08afa6ba24da61ea5e8efa139e5aadc938d83f0a6da5a900adaf763ac1da5573`.
- `bf_emma.bin` — 522,240 bytes; SHA-256 `669fe0647f9dd04fcab92f1439a40eeb4c8b4ab1f82e4996fe3d918ce4a63b73`.
- `bf_isabella.bin` — 522,240 bytes; SHA-256 `3754352c4aaa46d17f27654ab7518d65b62ad6163a0f55a5f4330c2da2c4e94f`.
- `bf_lily.bin` — 522,240 bytes; SHA-256 `5e0ee32ebe64a467124976b14e69590746f1c4ce41a12b587a50c862edfea335`.
- `bm_daniel.bin` — 522,240 bytes; SHA-256 `6b3194bbceffb746733cbc22c8f593dd44e401a71d53895a2dca891bc595a1e8`.
- `bm_fable.bin` — 522,240 bytes; SHA-256 `f889083196807b4adb15e9204252165f503b8d33d3982e681c52443c49d798f1`.
- `bm_george.bin` — 522,240 bytes; SHA-256 `c4b235a4c1f2cd3b939fed08b899ce9385638b763f7b73a59616c4fc9bd6c9bc`.
- `bm_lewis.bin` — 522,240 bytes; SHA-256 `b8f671cef828c30e66fdf0b0756a76bba58f6bb3398cbbf27058642acbcedb97`.
- `ef_dora.bin` — 522,240 bytes; SHA-256 `f66ec66bd295acb18372e37008533a9a3228483ccd294e7538d5d9294ac9a532`.
- `em_alex.bin` — 522,240 bytes; SHA-256 `27809e9eafdcbcfff90a3016c697568676531de2a2c39cee29c96c7bd6b83e95`.
- `em_santa.bin` — 522,240 bytes; SHA-256 `ad43b774e1ca24d05c6161297d8aeb770ac3d29bb95daf516727af5f7d543683`.
- `ff_siwis.bin` — 522,240 bytes; SHA-256 `a35f5675ad08948e326ae75fd0ea16ba5d0042e4f76b5f3d1df77d0a48c54861`.
- `hf_alpha.bin` — 522,240 bytes; SHA-256 `040be6a4425411cc01fda5fd06693c76bfa78572632852bc8cda9c99232ffb56`.
- `hf_beta.bin` — 522,240 bytes; SHA-256 `cd83ae0bb9b2e4e4fb92b4973bd8d1822ca0036d3c498bf4fc89aa8e33917cc7`.
- `hm_omega.bin` — 522,240 bytes; SHA-256 `b02d9222d9ed00ce26b302173a862c2c93f96cc40b5c422b8d14910b9ff34137`.
- `hm_psi.bin` — 522,240 bytes; SHA-256 `644daf88ba8aeb7bd08950bbdcd4453bb280864e49dc4df93fabc6be32e03f37`.
- `if_sara.bin` — 522,240 bytes; SHA-256 `409b69248798fcdc2542330c76953d230710f19b057e59cb82fdc3c4cf71265c`.
- `im_nicola.bin` — 522,240 bytes; SHA-256 `bc578e510d52a96d6940d46f12e96d7b3df00905dbea075113226d100e6e1ab0`.
- `jf_alpha.bin` — 522,240 bytes; SHA-256 `56b479360aad9f367aeb8cef908f9201cf48b4555e488c5f4590c9dfcd978bb6`.
- `jf_gongitsune.bin` — 522,240 bytes; SHA-256 `0f1181f3772d27b7c12aaf4bcd71e31b186c4146e330d074a3dc64ee392af396`.
- `jf_nezumi.bin` — 522,240 bytes; SHA-256 `13cb71eebb0b48739d444558322aa35a8c9a489b80e1e631f14d2e6aea93026b`.
- `jf_tebukuro.bin` — 522,240 bytes; SHA-256 `29c6c0561b4288d59639677bebe7533c919743d5ea68d0d2ae992644beea6696`.
- `jm_kumo.bin` — 522,240 bytes; SHA-256 `09e959d239724c734d65661f06f14cdabcddfd476bfaaad905a937099ae9e64f`.
- `model.onnx` — 325,532,232 bytes; SHA-256 `8fbea51ea711f2af382e88c833d9e288c6dc82ce5e98421ea61c058ce21a34cb`.
- `pf_dora.bin` — 522,240 bytes; SHA-256 `3da7b5b2d91847ebf5646f57631af6ececae3c29a89cd300f06edf9aa6cfe9ee`.
- `pm_alex.bin` — 522,240 bytes; SHA-256 `0175c753f59c54e7fd5a995bedef0c5ff2fb67e0043dd3dcb2ae74ec2acbeb2a`.
- `pm_santa.bin` — 522,240 bytes; SHA-256 `8b012db3185778afe2e45a62cbad69db73021774fe68dda634bcc748a982eede`.
- `tokenizer.json` — 3,497 bytes; SHA-256 `77a02c8e164413299b4b4c403b14f8e0e1c1b727db4d46a09d6327b861060a34`.
- `zf_xiaobei.bin` — 522,240 bytes; SHA-256 `5dde6e1c9c4f12c8b327bc29c0cee361a23b52b952c04636858ba637ec66e640`.
- `zf_xiaoni.bin` — 522,240 bytes; SHA-256 `08892b62a39af0a615cd0581238db7e19e44c578e8fa0bfd0e586e93327d9cba`.
- `zf_xiaoxiao.bin` — 522,240 bytes; SHA-256 `03adb5d5e3ddd88b047954e974e651cb0a4b524c985057e5d872e962c7be1169`.
- `zf_xiaoyi.bin` — 522,240 bytes; SHA-256 `bc1555c5c486099196ac254bae5e0bb543c121952a3092f50b7d8724f1bc36b3`.
- `zm_yunjian.bin` — 522,240 bytes; SHA-256 `de48a00bdbf3649f07162269a2b6e0513604389bfac8a2e6c75cb34b323ad6fa`.
- `zm_yunxi.bin` — 522,240 bytes; SHA-256 `7243892fb4e560d47014090ddf010f8b8b790f3c6b029ff82b2ac06aa4e27c8b`.
- `zm_yunxia.bin` — 522,240 bytes; SHA-256 `6b2b8fc15b3df19a368daebe5c581c7fabf433ee5b8a17ffd6b3d723cff8936d`.
- `zm_yunyang.bin` — 522,240 bytes; SHA-256 `261e2c89470534dbbcb8fd98b8fdc495ec94063d9bb6c8277f7be43cccba3f42`.
