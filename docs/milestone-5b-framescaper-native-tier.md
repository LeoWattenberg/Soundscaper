...
| --- | --- |
| V20 selection | Both App branches authenticate and select V20, enable `videoKeyframes`, preserve V18/V19-owned behavior, and use the distinct desktop V17 boundary with immutable V12 copy-forward. The route is provisional, not qualified. |
...
- **Native payloads and executable feature depth.** Both 5B payload manifests
  ...
  self-test remains not ready until native static-geometry execution/adapter,
