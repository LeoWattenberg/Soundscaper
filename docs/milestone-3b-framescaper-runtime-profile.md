...
Framescaper singleton, resolve its authenticated children, and bind every V18
owner in one reviewed change. An equal runtime profile created from other
authentic child tokens does not gain product selection authority.
...
module imports only the two generic child-profile modules. It has no product,
schema, storage implementation, archive, compatibility, controller, desktop,
UI, or callback dependency.
...
5. authenticate the prerequisite token through its private owner;
6. authenticate the capability token through its private owner; and
7. require the two authenticated snapshots to have the same exact `owner`.
...
The frozen definition snapshot is a fresh record retaining the exact two child
token identities. It does not expose either child definition, flatten literals,
or retain the caller's container. The child profiles remain independently
authenticated by their existing private owners.
...
may admit only the exact new product module and focused test as their narrow
...
- the exact singleton retains both exact child-token identities and, through
  their authenticated owners, the exact Framescaper owner and reviewed c1a/c1b
  facts without flattening them;
...
- child authentication order, forged/wrapped child refusal with zero child
  traps, same-owner acceptance, and cross-owner refusal;
...
The first reachable composition remains the full atomic c-c transition. Stop
if the token becomes selectable, if child authority is flattened or structural,
or if any existing global V17/Soundscaper owner changes.
