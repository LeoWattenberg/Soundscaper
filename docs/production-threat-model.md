...
The Framescaper composition authenticates its exact runtime profile before project traversal and selects product-owned schema-18 create, load, clone, validation, command, history, storage, archive, compatibility, and playback owners; exact V17 refuses with typed re-import and V19 remains opaque read-only, while Soundscaper continues to own exact V17. V18 validation requires dense closed nested-sequence and multicamera collections and reconciles their nonempty state to reserved owned requirements that are available only in Framescaper. Nested graphs reject cycles, excessive depth, inexact mapping and lossy clipboard projection. Multicamera groups reject foreign project, sequence, output-clip, track, or source ownership, duplicate outputs, retimed outputs, stale command fences, and invalid sample offsets. Explicit copy-only preservation to Soundscaper keeps activation and editing forbidden. This control does not authorize retime execution or proxy use.
<!-- /policy-narrative:framescaper-v18-editorial-document-admission -->
