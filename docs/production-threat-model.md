...
This boundary is enforced for the RC, but it does not close stable 1.0
qualification. Signing, accessibility, platform, hardware, native-payload,
licensing, and remaining Milestone 9 evidence stay release-blocking.
...
Soundscaper and Framescaper desktop libraries independently authenticate their exact family-v1 handshakes, application IDs, user_version 1 databases, fresh v1 roots, and product-qualified IPC namespaces. Both provide current catalog, publication, body, duplicate, delete, lease, recovery, and plug-in-state operations without any pre-release importer or copy-forward chain. Same project IDs remain separate by schema family.
<!-- /policy-narrative:framescaper-v20-desktop-v17-isolation -->
...
The Soundscaper and Framescaper family-v1 desktop libraries each hold one renewable process-lifetime main-owned writer lease in a separate fresh root and user_version 1 database with a persistently monotonic fencing token. Startup recovers journals before renderer admission; renewal loss fences new work; shutdown drains admitted operations and releases only the exact owned lease. The closed matrix exercises same-project contention, cross-product isolation, lease transfer and takeover, conflicting commits, renderer loss, orderly restart, and crash recovery for both products. A reviewed no-retry cohort from GitHub Actions run 33261286048 at revision e7c76c7013f94f14834c13a5fa25c261cd754627 accepted all five maintained targets, each with fourteen product-specific cases plus paired scope isolation and no losing managed-media publication. The unrelated Soundscaper Linux x64 packaging failure is outside the accepted scope; Milestone 9 human stable-release qualification remains separate.
<!-- /policy-narrative:packaged-cross-platform-electron-lease-matrix -->
...
The current Milestone 5 implementation supersedes the two historical grounding
paragraphs retained immediately below. Soundscaper S30/V11 now reaches its
exact S29-founded persistent native audio plus reviewed `native-plugin` real-time and offline
...
`native-plugin-hosting` is partial and conditionally admitted: professional
format surfaces are enabled for testing and defer execution to exact machine
payload, platform, containment, consent, and quarantine checks. Human trust,
licensing, packaged-fixture, and qualification review belongs to milestone 9
stable 1.0 admission and is not runtime authority.

The shared source audit authenticates 0/10 required external
archive/extracted-tree inputs. All professional payload rows remain
`pending-external`, both Framescaper payload manifests are empty, and the
qualification audit records zero accepted cohorts. Launcher source/contracts/
tests implement Linux namespaces and Landlock/seccomp, macOS Seatbelt, and
Windows AppContainer, but no authenticated built target launcher/payload exists
in the checked-in manifests. Current execution is therefore machine payload
unavailable. The historical absent-caller and absent-launcher statements below
are audit history, not current behavior.
...
On macOS, the launcher retains exact descriptors, uses `POSIX_SPAWN_SETEXEC` with `POSIX_SPAWN_START_SUSPENDED`, verifies the stopped peer's executable vnode identity, and resumes only that peer. The peer then invokes its first-party Seatbelt bootstrap before containment probes or framed protocol work and attests that transition. The pinned `com.apple.security.cs.disable-library-validation` exception is applied only to the Soundscaper professional plug-in peer so it can load the selected plug-in. Candidate signing strictly verifies every artifact and binds hashed successful `codesign --test-requirement` evidence: a positive entitlement requirement for the peer and a negative requirement for every other signed candidate artifact.
...
The gaps are equally real. Native plug-ins execute arbitrary code and therefore require the exact per-OS launcher and its namespace/Landlock/seccomp, Seatbelt, or AppContainer policy rather than falling back to a same-UID utility process. The macOS stopped-vnode transition prevents the selected plug-in from running before Seatbelt, but it is not a defense against an already-running same-UID process that can race a newly valid same-vnode code signature or send `SIGCONT`; that actor already has ambient account execution and remains outside this control. No authenticated professional target payload and launcher closure is checked in today, making real third-party execution machine-unavailable. The source and fixture tests do not establish packaged hostile-code containment, broad compatibility, resource behavior, vendor provenance, or publisher trust; candidate signature and positive/negative entitlement-requirement evidence establish candidate integrity and the narrow exception scope, not those broader claims. The helper's host job and product callers implement real-time and offline hosting, PDC handoff, continuity, and vendor-window lifecycle, but those paths still lack a packaged third-party target fixture and long-session evidence. Publisher, licensing, packaging, and manual review remains mandatory for milestone-9 stable 1.0 admission, while exact payload identity, OS containment, consent, and quarantine remain mandatory for every test execution. Neither category may substitute for the other.
...
and original/relink relationships after retime. Guided-local sign-off, resource
qualification, and external qualification remain open. Neither the capture-only
route nor the V27 general route has pre-encode end-to-end working-set, decoder,
browser-heap, process-RSS, GC-headroom, or product-wide qualification.
...
and derivatives.
<!-- policy-narrative:milestone-2-offline-cache-qualification-threat-model -->
...
still starts, and reimports the same project identity. Milestone-2 scope
revision 2 historically deferred WebKit release qualification; that human
checkpoint now belongs to milestone 9. Automated tests run the pinned
Playwright WebKit build and skip an individual workflow only when a concrete
...
authenticity controls, independent protected approval attestation, recorded
publication/rollback drills, and a deliberate reviewed change to the bundle
audit. The manifest's self-declared review marker preserves internal consistency
but is not production authorization.
...
qualification and consent controls, and owns admitted desktop audio plus exact
...
audit and requires protected independent approval plus a reviewed architecture,
licensing, security, and rollback qualification change.
