...
The model is grounded on 2026-08-28. It must be updated when a trust boundary, supported input, renderer bridge, worker ABI, native executable, plug-in surface, release channel, or long-job lifecycle changes.
...
The gaps are equally real. Native plug-ins execute arbitrary code with the user account's authority unless a specific operating-system sandbox demonstrably removes that authority, so execution requires the exact per-OS launcher and its namespace/Landlock/seccomp, Seatbelt, or AppContainer policy rather than falling back to a same-UID utility process. No authenticated professional target payload and launcher closure is checked in today, making real third-party execution machine-unavailable. The source and fixture tests do not establish packaged hostile-code containment, broad compatibility, resource behavior, or vendor provenance. The helper's host job and product callers implement real-time and offline hosting, PDC handoff, continuity, and vendor-window lifecycle, but those paths still lack a packaged third-party target fixture and long-session evidence. Signature, publisher, licensing, packaging, and manual review remains mandatory for milestone-9 stable 1.0 admission, while exact payload identity, OS containment, consent, and quarantine remain mandatory for every test execution. Neither category may substitute for the other.
