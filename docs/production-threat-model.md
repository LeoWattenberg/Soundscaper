...
capability-evaluated service applies only the audio projection to final mix
delivery. It supports normalized mix mode only; stems, BW64, and ADM reject
before integrity verification or other export work. The canonical project,
history, persistence, and save state remain unchanged.

Delivery does not reuse activation-time byte admission. Under the owned
export-task signal, an exact selector binds the requirement ID, feature ID,
audio kind, source ID, and SHA-256 and verifies only that selected PCM body;
unrelated fallback storage is not read. It scans the full canonical
...
The composed fresh managed handoff test corrupts recipient-local fallback PCM
after activation and proves delivery refuses before render or download. It then
restores the exact PCM, observes the expected fallback samples in a successful
WAV, and proves the canonical project remains unchanged. This is not a durable
storage-record or byte lease and provides no cross-process immutability. It
makes no generic or third-party, simultaneous, authored fallback or proxy,
linked-only, unmanaged, stems, BW64, ADM, UI, packaged-runtime,
reference-scale, whole-handoff-atomicity, or earlier-Soundscaper-schema claim.
