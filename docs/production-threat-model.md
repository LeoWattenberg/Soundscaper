...
The maintained exact direct MP4 route binds canonical `mp4`, `.mp4`, and `video/mp4`, and the exact direct WebM route binds canonical `webm`, `.webm`, and `video/webm`. Both use target purpose `video`, require a safe matching output name, and admit only a canonical version-6 descriptor-bound video plan whose complete serialized fingerprint stays unchanged through pre-commit admission. Admission requires positive finite duration and frame rate, positive safe output frames and range, positive even canvas dimensions, `yuv420p`, at least one uniquely identified video source, at most one final staged-audio input, and exact descriptor agreement for format, container, extension, MIME, video encoder, optional audio encoder, and audio strategy. MP4 `libx264`, optional `aac`, `yuv420p`, and `+faststart`, plus WebM `libvpx-vp9`, optional `libopus`, and `yuv420p`, are deterministic FFmpeg command facts, not codec execution or conformance claims. First-party rendered-fallback verification and its delivery projection complete before the plan and target selection; the verified fallback `Blob` may feed this route, but source authentication and projection remain owned by the separate rendered-fallback control. In a browser, the prepared target is selected after planning but before storage preflight, ordinary source loading, optional audio rendering, or FFmpeg, while its exact writer stays unopened until after finalization and one safe nonnegative FFmpeg stat. On desktop, target preparation is deliberately deferred inside sink `open` after that stat because a selected main-owned save target has a 900,000-millisecond TTL; this is a design reason for late selection, not desktop or native-picker platform qualification. Source video `Blob` inputs and an optional staged WAV `Blob` are mounted through WORKERFS. The complete finalized output remains in worker MEMFS. After finalization the patched runtime performs one stat, reads exact monotonic ranges of at most one MiB, permits at most one range read and one awaited destination write at a time under backpressure, and checks currentness around external operations. Exact stat, emitted, destination-written, and committed-result counts agree around sink close/seal and one explicit commit. No output `readFile`, final renderer video `Blob`, Object URL, or download publication occurs on the direct path. Prepared Blob mode declines direct publication and preserves the legacy whole-read, final-Blob, Object-URL or download path. Focused Node evidence covers both canonical formats, early browser and late desktop preparation, rendered-fallback ordering, a provider-injected four-byte output, the legacy Blob path, and a 2,097,169-byte body transferred in three exact ranges of 1,048,576, 1,048,576, and 17 bytes with one stat and zero output `readFile` calls. Worker MEMFS, source-video and staged-audio Blob residency, codec execution, codec conformance, native or WASM codec memory, renderer or browser heap, GC, process RSS, CPU, elapsed time, actual browser or operating-system behavior, native picker, packaged application behavior, reference scale, quota, filesystem durability, crash, and power loss remain unqualified.
<!-- /policy-narrative:exact-direct-mp4-webm-video-save -->
