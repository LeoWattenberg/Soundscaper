...
`scape-archive-expansion` remains **release-blocked** for production qualification. Before reading manifest bytes, the shared envelope validates entry count, encryption state, and safe compressed/uncompressed metadata, and rejects more than 64 GiB of cumulative declared uncompressed data. Manifest JSON is capped at 32 MiB and project JSON at 256 MiB by both metadata preflight and byte-counted text sinks. Descriptor sizes must match entry metadata before project JSON or asset extraction. For the tested native `.scape` open/save path, one task `AbortSignal` now reaches incremental ZIP enumeration and extraction, source reads and writes, archive output, and file publication. Cancellation closes the archive reader, returns source iterators, aborts unpublished output and audio writers, deletes provisional media, and restores the previous project together with its retained revision history.

The remaining promotion gate covers local-header consistency and overlapping entries, compression-ratio or STORE policy, cumulative **actual** extracted bytes, bounded streaming media extraction, and safe PCM frame arithmetic with a bounded pending buffer. Video extraction still materializes a complete `Blob`. Tests must prove those failures stop before their working-set limits and leave project/source inventory unchanged.
