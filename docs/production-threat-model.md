...
Exact schema 12 now normalizes feature-requirements manifest schema 2 with the
...
Final video rendered-fallback delivery is another narrow exact schema 12
...
Descriptor validation alone does not hash or authenticate the referenced media bytes; the separate exact-schema-12 controller admission described above verifies referenced local bytes at its narrower boundary.
...
After admission, the current exact schema 12 project and at most 64 retained
...
product-local current document is exact schema 12 at the catalog revision and no
