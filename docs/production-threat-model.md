...
The model is grounded on 2026-08-14. It must be updated when a trust boundary, supported input, renderer bridge, worker ABI, native executable, plug-in surface, release channel, or long-job lifecycle changes.
...
The Framescaper composition authenticates its exact runtime profile before project traversal. Browser routes without a desktop bridge select product-owned schema-19 create, load, clone, validation, command, history, storage, compatibility, playback, Scape, and video-geometry owners; exact V18 requires re-import there and V20 or newer remains opaque read-only. Packaged desktop routes deliberately retain the qualified schema-18 environment and desktop V10 proxy/timing archive, cleanup, retention, and renderer authority until a complete V19/V11 boundary exists, so authored geometry is unavailable there rather than silently downgraded. The V18 foundation continues to validate nested and multicamera state with exact owned requirement reconciliation, and copy-only preservation remains explicit in Soundscaper. Soundscaper continues to own exact V17 and explicit cross-product preservation never grants activation or editing. V19 geometry validation requires a closed bounded composition on every video clip, exact reserved requirement ownership for authored state, matching blend and compositing order across transition peers, and detached renderer descriptions consumed by preview and export. This control does not qualify the pending reference-GPU parity cohort or authorize proxy consumption.
<!-- /policy-narrative:framescaper-v18-editorial-document-admission -->

<!-- policy-narrative:framescaper-v20-keyed-project-admission -->
Exact Framescaper V20 model admission authenticates one process-local model authority before project traversal. It accepts exact schema 20 only after the shared whole-document structural budget and then requires closed per-occurrence own-data video-keyframe collections on every timeline and Project Bin video occurrence, canonical target order, bounded curve and anchor counts, exact clip-duration bounds, and the reserved `framescaper.video-keyframes` requirement without publisher substitution. Earlier schemas reject and future schemas receive a descriptor-snapshotted opaque read-only preservation result without keyframe semantic traversal. The model and capability profiles remain dormant and unselected: videoKeyframes is unavailable, and this control does not activate any capability, browser route, or desktop route.
<!-- /policy-narrative:framescaper-v20-keyed-project-admission -->

<!-- policy-narrative:exact-v20-keyed-export-authority -->
The dormant keyed export strategy authenticates canonical V20, derives one detached deep-frozen exact V17 playback projection, binds that projection and each version-7 plan to the exact canonical object, and rederives and compares the projection before planning so canonical or projected drift refuses. It requires fallback-free delivery, resolves one exact range, and inventories only active visible video clips and their uniquely bound canonical sources. The detached plan fixes source storage keys, MIME types and lowercase SHA-256 identities, exact rational duration, range, canvas, reduced frame rate, output frame count, format, codecs, and optional audio geometry under version-7 authority. Product integration snapshots a dense unique active-source ID list before timing or media work; only those source timing views are acquired, source Blobs are SHA-256 verified against project identity before browser decode, and exact timing leases remain held through encode and publication cleanup. Generation, currentness, and AbortSignal checks fence timing acquisition, media authentication, rendering, encoder execution, and output publication external operations. Encoder cleanup is always attempted and preserves cleanup failures; the timing lease is always released in `finally`. This model remains dormant and unselected, reuses the existing video publication authority, and neither activates V20 nor changes a product capability or route.
<!-- /policy-narrative:exact-v20-keyed-export-authority -->
