...
Schema 15 also requires one own enumerable boolean `locked` field on every
...
Exact schema 15 now normalizes feature-requirements manifest schema 2 with the
...
Final video rendered-fallback delivery is another narrow exact schema 15
...
Descriptor validation alone does not hash or authenticate the referenced media bytes; the separate exact-schema-15 controller admission described above verifies referenced local bytes at its narrower boundary.
...
After admission, the current exact schema 15 project and at most 64 retained
...
product-local current document is exact schema 15 at the catalog revision and no
