...
`desktop-write-path-capabilities` is **partial**. Save targets are high-entropy, expiring, single-use tokens. Exact-size output and project-only maximum-bounded output use acknowledged sequential one-MiB chunks, a private same-directory temporary file, file sync, atomic rename, and abort cleanup. Abort waits for an active chunk acknowledgement before closing staging, and active sessions are removed on main-frame navigation, renderer loss, or window close. Application shutdown synchronously closes both target and session admission, drains operations admitted before that fence, lets an admitted finish settle through its sync-and-rename commit boundary, aborts remaining or late-opened staging, and only then completes save disposal. An unacknowledged handle close or staging unlink rejects that barrier so the failure-aware process lifecycle cannot report a clean exit; fault-injection tests stall open, sync, and rename independently and force both cleanup failures. Unconsumed targets still lack a webContents owner, aggregate and practical disk limits remain open, an active chunk cannot be preempted inside its filesystem write, and parent-directory/per-platform durability still needs fault qualification.
