...
| `application-to-native-extension-surface` | Application requests | Supervised helper processes and future native plug-in processes | The probe helper speaks contract v1 behind main-owned spawn, verified binaries, and pathless renderer channels; every further helper or plug-in surface needs its own enactment. |
...
Soundscaper desktop-library V9 hosts open concurrently as lease-free observers and acquire one short-lived fenced writer lease only around a project or managed-media mutation. Its coordinator waits at most the production lease TTL, renews while admitted work drains, recovers pending journals and tracked stages before mutation and after cooperative failure, aborts and permanently fences its host on renewal loss, invokes lease-loss notification once, and releases only the exact lease. Startup recovery is opportunistic and retries after an unexpired crashed holder's recorded expiry. The pathless commit bridge carries an expected authoritative revision: null is create-only; updates compare the current V9 revision while permitting a higher new document revision; identical already-published bytes are idempotent after acknowledgement loss; and stale, lower, equal-divergent, or losing input returns a conflict before staging or catalog and managed-media advertisement. Each renderer owner carries one abort signal through reads, commits, deletes, bundle and source reads, and upload finishing through catalog publication. Revocation and global disposal fence admission and abort before draining. Unexpected render-process-gone cleanup drains the old owner and reloads only the trusted editor URL with a new owner, while cleanup or reload failure exits nonzero. Focused tests cover observer concurrency, Soundscaper mutation serialization, orderly transfer, stale takeover, monotonic fencing, renewal loss, compare-and-swap conflict and idempotence, synchronous renderer cancellation, loss during finishing publication, disposal, reload, stage cleanup, and real child termination at prepared and committed journal checkpoints. The current closed Soundscaper V9 smoke plan executes seven workflows: `same-project-simultaneous-open`, `writer-lease-transfer`, `stale-lease-takeover`, `conflicting-canonical-commit`, `renderer-loss-during-operation`, `orderly-process-restart`, and `crash-restart-recovery`. The `cross-product-simultaneous-open` workflow is historical and is not run against current Framescaper packages; the frozen historical eight-workflow V9/V17 set remains evidence only and does not authorize Framescaper V17. Soundscaper uses its separate V9 storage scope and database, so evidence beside Framescaper V10 proves cross-product isolation rather than shared mutation. Windows x64 and Linux x64 are pending-external, and no accepted packaged Soundscaper V9 result is checked in. This control and its gate remain partial; implementation and CI configuration alone do not promote them.
<!-- /policy-narrative:packaged-cross-platform-electron-lease-matrix -->
