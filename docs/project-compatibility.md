...
- A format-1 archive carrying a project schema greater than 16 opens read-only
...
- Current-format exact schema 16 `.scape` round trips promise JSON-semantic
...
current-format exact schema 16 path. Under the 512 MiB linked-original ceiling,
...
Non-empty schema 16 timelineAnnotations state reconciles one reserved
...
video rendered fallback for it rejects. Exact-V16 clone, validation, runtime
...
Schema 16 persists closed top-level `trackFolders` metadata and authoritative
...
fresh desktop V8 library preserve tested nonempty state exactly. Folder-aware
...
Every schema 16 video source carries a characteristics record stating what an
...
Schema 16 is the only writable and readable raw-project schema before the first
shipped release. Inputs from schemas 1 through 15, including V15 breakpoint-map
retime state, fail at the core boundary with the typed `REIMPORT_REQUIRED` error. They are never partially loaded, silently
...
V16 replaces V15's video breakpoint map with one closed JSON-safe V2 curve wire
on timeline and Project Bin video clips. `null` is the writable default. A
non-null map has 1 through 4,096 segments and one more point, uses exact canonical
number rationals and safe integers, spans outer frame zero through the owning
`sequenceFrameCount`, stays inside the owning source bounds, and delegates all
direction, freeze, ramp, endpoint, crossing, denominator, and bounded BigInt work
to the exact V2 algebra adapter. Validation returns a deeply frozen snapshot and
rejects the old V15 wire without guessing or migration.
...
fresh desktop v8 library preserve both clip stores without shared mutable curve
...
tagged-binary formats remain 1, and historical desktop v7/metadata-7/exact-V15/
user-version-9 data remains untouched. This is preservation and refusal only: it
...
Schema 16 requires one own boolean `locked` field on every audio, video, and
...
envelope. A fresh filesystem library scope `v8`
(`kw.media/scape-project-library/v8`) uses SQLite `user_version` 10 and ignores
rather than migrates the prior shared `v7` scope, preserving its metadata schema
7, exact schema 15 catalog, and SQLite `user_version` 9 in place. The older
`v6`/schema-14/user-version-8 and earlier scopes remain historical and untouched.
A copied `v7` database with `user_version` 9 placed at the `v8` path is rejected
without mutation instead of being migrated, adopted, or backfilled. Metadata
schema 8 binds a separate opaque library entry ID to the project identity,
exact schema 16, project revision,
...
exact-V16 maintained-persistence-domain validator to the decoded document before
...
`JSON.parse`; each exact-V16 decoded codec traversal and maintained-domain
...
The renderer repository repeats the same maintained-persistence-domain exact-V16
...
video. After flushing the current exact-schema-16 project, the
...
Before local shadow save or activation, a latest exact-schema-16 source-bearing
...
recipient-local exact-schema-16 snapshot of the same project to bind its logical
...
current document is exact schema 16 at the catalog revision and at most 64 exact
...
activation, the reachability repository validates the current exact-schema-16
project plus every retained exact-schema-16 revision. Each revision must have its
...
the bounded product-local exact-schema-16 current and retained graphs whose
...
schema 16 role-defined audio whole-mix fallback for the unknown canonical
...
an exact schema 16 editable retained original alongside a manifest-only
...
schema 16 role-defined unavailable-or-unknown audio whole-mix fallback—and
retained original video. The
qualified video slice also covers the maintained exact schema 16 manifest-only
...
preload IPC, exact-SHA-256 verifies its expected canonical source-free schema 16
...
origin profile. Its fixed exact schema 16 project contains one canonical PCM
...
exact-schema-16 role witnesses from the same shared library. The
...
Pre-release schemas 1 through 15 require source-media re-import and have no raw-
project migration path. Migration from the prior shared `v7` scope, the older
`v6`, `v5`, `v4`, `v3`, `v2`, and `v1` scopes, or product-private Soundscaper libraries also remains unsupported by
this contract. AUP,
legacy XML AUP, and AUP4 remain separate maintained interchange boundaries.

## Project feature requirements

Schema 16 establishes the raw-project declaration and evaluation foundation. Its
...
Schemas 1 through 15 are rejected at the raw-project boundary before feature-
requirement reconciliation. Maintained exact-schema-16 create, load, clone, and
...
is unknown. It evaluates exact schema 16 from the actual project history that
...
report before activation side effects. Projection requires exact schema 16 and a
...
The maintained exact schema 16 audio whole-mix fallback is defined by the closed
...
The maintained exact schema 16 video rendered-fallback projection has two closed
...
derives a transient activation projection only for exact schema 16 when the
...
This exact-schema-16 bypass slice does not attempt compatibility with earlier
...
name none. It runs only for exact schema 16 with a `soundscaper-project` report
...
ID, because schema 16 enforces ID uniqueness only within a collection and a track
...
step for exact schema 16. The maintained role-defined audio and closed
...
substitute fallback media at runtime; the separate exact-schema-16 role-defined
...
exact-schema-16 report into an explicit choice: **Open read-only** or **Cancel**.
...
This archive evidence is deliberately limited to schema 16 and `.scape` format
...
bytes referenced by the authoritative exact-schema-16 activation project. That
...
- Before `JSON.parse`, every project schema receives an iterative raw-JSON
  ...
  allowance needed to keep the bounded exact-schema-16 binary representation
  round-trip closed; tests may only lower the underlying limits.
- Exact schema 16, format 1 `.scape` export preserves `Uint8Array` values,
...
exact-schema-16 mono/stereo role-defined audio whole-mix, role-defined video
