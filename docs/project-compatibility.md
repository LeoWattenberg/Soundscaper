...
- A format-1 archive carrying a project schema greater than 15 opens read-only
...
- Current-format exact schema 15 `.scape` round trips promise JSON-semantic
...
current-format exact schema 15 path. Under the 512 MiB linked-original ceiling,
...
Non-empty schema 15 timelineAnnotations state reconciles one reserved
...
video rendered fallback for it rejects. Exact-V15 clone, validation, runtime
...
Schema 15 persists closed top-level `trackFolders` metadata and authoritative
...
fresh desktop V7 library preserve tested nonempty state exactly. Folder-aware
...
Every schema 15 video source carries a characteristics record stating what an
...
Schema 15 is the only writable and readable raw-project schema before the first
shipped release. Inputs from schemas 1 through 14 fail at the core boundary with
...
Schema 15 requires one own boolean `locked` field on every audio, video, and
...
envelope. A fresh filesystem library scope `v7`
(`kw.media/scape-project-library/v7`) uses SQLite `user_version` 9 and ignores
rather than migrates the prior shared `v6` scope, preserving its metadata schema
6 and exact schema 14 catalog in place. A copied `v6` database with
`user_version` 8 placed at the `v7` path is rejected without mutation instead of
being migrated, adopted, or backfilled. Metadata schema 7 binds a separate
opaque library entry ID to the project identity,
exact schema 15, project revision,
...
exact-V15 maintained-persistence-domain validator to the decoded document before
...
`JSON.parse`; each exact-V15 decoded codec traversal and maintained-domain
...
The renderer repository repeats the same maintained-persistence-domain exact-V15
...
video. After flushing the current exact-schema-15 project, the
...
Before local shadow save or activation, a latest exact-schema-15 source-bearing
...
recipient-local exact-schema-15 snapshot of the same project to bind its logical
...
current document is exact schema 15 at the catalog revision and at most 64 exact
...
activation, the reachability repository validates the current exact-schema-15
project plus every retained exact-schema-15 revision. Each revision must have its
...
the bounded product-local exact-schema-15 current and retained graphs whose
...
schema 15 role-defined audio whole-mix fallback for the unknown canonical
...
an exact schema 15 editable retained original alongside a manifest-only
...
schema 15 role-defined unavailable-or-unknown audio whole-mix fallback—and
retained original video. The
qualified video slice also covers the maintained exact schema 15 manifest-only
...
preload IPC, exact-SHA-256 verifies its expected canonical source-free schema 15
...
origin profile. Its fixed exact schema 15 project contains one canonical PCM
...
exact-schema-15 role witnesses from the same shared library. The
...
Pre-release schemas 1 through 14 require source-media re-import and have no raw-
project migration path. Migration from the prior shared `v6` scope, the older
`v5`, `v4`, `v3`, `v2`, and `v1` scopes, or product-private Soundscaper libraries also remains unsupported by
this contract. AUP,
legacy XML AUP, and AUP4 remain separate maintained interchange boundaries.

## Project feature requirements

Schema 15 establishes the raw-project declaration and evaluation foundation. Its
...
Schemas 1 through 14 are rejected at the raw-project boundary before feature-
requirement reconciliation. Maintained exact-schema-15 create, load, clone, and
...
is unknown. It evaluates exact schema 15 from the actual project history that
...
report before activation side effects. Projection requires exact schema 15 and a
...
The maintained exact schema 15 audio whole-mix fallback is defined by the closed
...
The maintained exact schema 15 video rendered-fallback projection has two closed
...
derives a transient activation projection only for exact schema 15 when the
...
This exact-schema-15 bypass slice does not attempt compatibility with earlier
...
name none. It runs only for exact schema 15 with a `soundscaper-project` report
...
ID, because schema 15 enforces ID uniqueness only within a collection and a track
...
step for exact schema 15. The maintained role-defined audio and closed
...
substitute fallback media at runtime; the separate exact-schema-15 role-defined
...
exact-schema-15 report into an explicit choice: **Open read-only** or **Cancel**.
...
This archive evidence is deliberately limited to schema 15 and `.scape` format
...
bytes referenced by the authoritative exact-schema-15 activation project. That
...
- Before `JSON.parse`, every project schema receives an iterative raw-JSON
  ...
  allowance needed to keep the bounded exact-schema-15 binary representation
  round-trip closed; tests may only lower the underlying limits.
- Exact schema 15, format 1 `.scape` export preserves `Uint8Array` values,
...
exact-schema-15 mono/stereo role-defined audio whole-mix, role-defined video
