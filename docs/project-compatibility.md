...
- A format-1 archive carrying a project schema greater than 12 opens read-only
...
- Current-format exact schema 12 `.scape` round trips promise JSON-semantic
...
current-format exact schema 12 path. Under the 512 MiB linked-original ceiling,
...
Non-empty schema 12 timelineAnnotations state reconciles one reserved
...
video rendered fallback for it rejects. Exact-V12 clone, validation, runtime
...
Schema 12 persists closed top-level `trackFolders` metadata and authoritative
...
fresh desktop V4 library preserve tested nonempty state exactly. Folder-aware
commands, clipboard behavior, and native UI are outside this slice.

## Pre-release schema breaks

Schema 12 is the only writable and readable raw-project schema before the first
shipped release. Inputs from schemas 1 through 11 fail at the core boundary with
...
envelope. A fresh filesystem library scope `v4`
(`kw.media/scape-project-library/v4`) uses SQLite `user_version` 6 and ignores
rather than migrates the prior shared `v3` scope, preserving its metadata schema
3 and exact schema 11 catalog in place. A copied `v3` database with
`user_version` 5 placed at the `v4` path is rejected without mutation instead of
being migrated, adopted, or backfilled. Metadata schema 4 binds a separate
opaque library entry ID to the project identity,
exact schema 12, project revision,
...
exact-V12 maintained-persistence-domain validator to the decoded document before
...
`JSON.parse`; each exact-V12 decoded codec traversal and maintained-domain
...
The renderer repository repeats the same maintained-persistence-domain exact-V12
...
video. After flushing the current exact-schema-12 project, the
...
Before local shadow save or activation, a latest exact-schema-12 source-bearing
...
recipient-local exact-schema-12 snapshot of the same project to bind its logical
...
current document is exact schema 12 at the catalog revision and at most 64 exact
...
activation, the reachability repository validates the current exact-schema-12
project plus every retained exact-schema-12 revision. Each revision must have its
...
the bounded product-local exact-schema-12 current and retained graphs whose
...
schema 12 role-defined audio whole-mix fallback for the unknown canonical
...
an exact schema 12 editable retained original alongside a manifest-only
...
schema 12 role-defined unavailable-or-unknown audio whole-mix fallback—and
retained original video. The
qualified video slice also covers the maintained exact schema 12 manifest-only
...
preload IPC, exact-SHA-256 verifies its expected canonical source-free schema 12
...
origin profile. Its fixed exact schema 12 project contains one canonical PCM
...
exact-schema-12 role witnesses from the same shared library. The
...
Pre-release schemas 1 through 11 require source-media re-import and have no raw-
project migration path. Migration from the prior shared `v3` scope, the older
`v2` and `v1` scopes, or product-private Soundscaper libraries also remains unsupported by
this contract. AUP,
legacy XML AUP, and AUP4 remain separate maintained interchange boundaries.

## Project feature requirements

Schema 12 establishes the raw-project declaration and evaluation foundation. Its
...
Schemas 1 through 11 are rejected at the raw-project boundary before feature-
requirement reconciliation. Maintained exact-schema-12 create, load, clone, and
...
is unknown. It evaluates exact schema 12 from the actual project history that
...
report before activation side effects. Projection requires exact schema 12 and a
...
The maintained exact schema 12 audio whole-mix fallback is defined by the closed
...
The maintained exact schema 12 video rendered-fallback projection has two closed
...
derives a transient activation projection only for exact schema 12 when the
...
This exact-schema-12 bypass slice does not attempt compatibility with earlier
...
name none. It runs only for exact schema 12 with a `soundscaper-project` report
...
ID, because schema 12 enforces ID uniqueness only within a collection and a track
...
step for exact schema 12. The maintained role-defined audio and closed
...
substitute fallback media at runtime; the separate exact-schema-12 role-defined
...
exact-schema-12 report into an explicit choice: **Open read-only** or **Cancel**.
...
This archive evidence is deliberately limited to schema 12 and `.scape` format
...
bytes referenced by the authoritative exact-schema-12 activation project. That
...
- Before `JSON.parse`, every project schema receives an iterative raw-JSON
  ...
  allowance needed to keep the bounded exact-schema-12 binary representation
  round-trip closed; tests may only lower the underlying limits.
- Exact schema 12, format 1 `.scape` export preserves `Uint8Array` values,
...
exact-schema-12 mono/stereo role-defined audio whole-mix, role-defined video
