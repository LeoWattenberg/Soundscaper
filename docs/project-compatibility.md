...
- A format-1 archive carrying a project schema greater than 14 opens read-only
...
- Current-format exact schema 14 `.scape` round trips promise JSON-semantic
...
current-format exact schema 14 path. Under the 512 MiB linked-original ceiling,
...
Non-empty schema 14 timelineAnnotations state reconciles one reserved
...
video rendered fallback for it rejects. Exact-V14 clone, validation, runtime
...
Schema 14 persists closed top-level `trackFolders` metadata and authoritative
...
fresh desktop V6 library preserve tested nonempty state exactly. Folder-aware
...
Every schema 14 video source carries a characteristics record stating what an
...
Schema 14 is the only writable and readable raw-project schema before the first
shipped release. Inputs from schemas 1 through 13 fail at the core boundary with
...
envelope. A fresh filesystem library scope `v6`
(`kw.media/scape-project-library/v6`) uses SQLite `user_version` 8 and ignores
rather than migrates the prior shared `v5` scope, preserving its metadata schema
5 and exact schema 13 catalog in place. A copied `v5` database with
`user_version` 7 placed at the `v6` path is rejected without mutation instead of
being migrated, adopted, or backfilled. Metadata schema 6 binds a separate
opaque library entry ID to the project identity,
exact schema 14, project revision,
...
exact-V14 maintained-persistence-domain validator to the decoded document before
...
`JSON.parse`; each exact-V14 decoded codec traversal and maintained-domain
...
The renderer repository repeats the same maintained-persistence-domain exact-V14
...
video. After flushing the current exact-schema-14 project, the
...
Before local shadow save or activation, a latest exact-schema-14 source-bearing
...
recipient-local exact-schema-14 snapshot of the same project to bind its logical
...
current document is exact schema 14 at the catalog revision and at most 64 exact
...
activation, the reachability repository validates the current exact-schema-14
project plus every retained exact-schema-14 revision. Each revision must have its
...
the bounded product-local exact-schema-14 current and retained graphs whose
...
schema 14 role-defined audio whole-mix fallback for the unknown canonical
...
an exact schema 14 editable retained original alongside a manifest-only
...
schema 14 role-defined unavailable-or-unknown audio whole-mix fallback—and
retained original video. The
qualified video slice also covers the maintained exact schema 14 manifest-only
...
preload IPC, exact-SHA-256 verifies its expected canonical source-free schema 14
...
origin profile. Its fixed exact schema 14 project contains one canonical PCM
...
exact-schema-14 role witnesses from the same shared library. The
...
Pre-release schemas 1 through 13 require source-media re-import and have no raw-
project migration path. Migration from the prior shared `v5` scope, the older
`v4`, `v3`, `v2`, and `v1` scopes, or product-private Soundscaper libraries also remains unsupported by
this contract. AUP,
legacy XML AUP, and AUP4 remain separate maintained interchange boundaries.

## Project feature requirements

Schema 14 establishes the raw-project declaration and evaluation foundation. Its
...
Schemas 1 through 13 are rejected at the raw-project boundary before feature-
requirement reconciliation. Maintained exact-schema-14 create, load, clone, and
...
is unknown. It evaluates exact schema 14 from the actual project history that
...
report before activation side effects. Projection requires exact schema 14 and a
...
The maintained exact schema 14 audio whole-mix fallback is defined by the closed
...
The maintained exact schema 14 video rendered-fallback projection has two closed
...
derives a transient activation projection only for exact schema 14 when the
...
This exact-schema-14 bypass slice does not attempt compatibility with earlier
...
name none. It runs only for exact schema 14 with a `soundscaper-project` report
...
ID, because schema 14 enforces ID uniqueness only within a collection and a track
...
step for exact schema 14. The maintained role-defined audio and closed
...
substitute fallback media at runtime; the separate exact-schema-14 role-defined
...
exact-schema-14 report into an explicit choice: **Open read-only** or **Cancel**.
...
This archive evidence is deliberately limited to schema 14 and `.scape` format
...
bytes referenced by the authoritative exact-schema-14 activation project. That
...
- Before `JSON.parse`, every project schema receives an iterative raw-JSON
  ...
  allowance needed to keep the bounded exact-schema-14 binary representation
  round-trip closed; tests may only lower the underlying limits.
- Exact schema 14, format 1 `.scape` export preserves `Uint8Array` values,
...
exact-schema-14 mono/stereo role-defined audio whole-mix, role-defined video
