...
- Current-format exact schema 13 `.scape` round trips promise JSON-semantic
...
current-format exact schema 13 path. Under the 512 MiB linked-original ceiling,
...
Non-empty schema 13 timelineAnnotations state reconciles one reserved
...
video rendered fallback for it rejects. Exact-V13 clone, validation, runtime
...
shipped release. Inputs from schemas 1 through 12 fail at the core boundary with
...
envelope. A fresh filesystem library scope `v5`
(`kw.media/scape-project-library/v5`) uses SQLite `user_version` 7 and ignores
rather than migrates the prior shared `v4` scope, preserving its metadata schema
4 and exact schema 12 catalog in place. A copied `v4` database with
`user_version` 6 placed at the `v5` path is rejected without mutation instead of
being migrated, adopted, or backfilled. Metadata schema 5 binds a separate
opaque library entry ID to the project identity,
exact schema 13, project revision,
...
exact-V13 maintained-persistence-domain validator to the decoded document before
...
`JSON.parse`; each exact-V13 decoded codec traversal and maintained-domain
...
The renderer repository repeats the same maintained-persistence-domain exact-V13
...
video. After flushing the current exact-schema-13 project, the
...
Before local shadow save or activation, a latest exact-schema-13 source-bearing
...
recipient-local exact-schema-13 snapshot of the same project to bind its logical
...
current document is exact schema 13 at the catalog revision and at most 64 exact
...
activation, the reachability repository validates the current exact-schema-13
project plus every retained exact-schema-13 revision. Each revision must have its
...
the bounded product-local exact-schema-13 current and retained graphs whose
...
schema 13 role-defined audio whole-mix fallback for the unknown canonical
...
an exact schema 13 editable retained original alongside a manifest-only
...
schema 13 role-defined unavailable-or-unknown audio whole-mix fallback—and
retained original video. The
qualified video slice also covers the maintained exact schema 13 manifest-only
...
preload IPC, exact-SHA-256 verifies its expected canonical source-free schema 13
...
origin profile. Its fixed exact schema 13 project contains one canonical PCM
...
exact-schema-13 role witnesses from the same shared library. The
...
Pre-release schemas 1 through 12 require source-media re-import and have no raw-
...
Schemas 1 through 12 are rejected at the raw-project boundary before feature-
requirement reconciliation. Maintained exact-schema-13 create, load, clone, and
...
is unknown. It evaluates exact schema 13 from the actual project history that
...
report before activation side effects. Projection requires exact schema 13 and a
...
The maintained exact schema 13 audio whole-mix fallback is defined by the closed
...
The maintained exact schema 13 video rendered-fallback projection has two closed
...
derives a transient activation projection only for exact schema 13 when the
...
This exact-schema-13 bypass slice does not attempt compatibility with earlier
...
name none. It runs only for exact schema 13 with a `soundscaper-project` report
...
ID, because schema 13 enforces ID uniqueness only within a collection and a track
...
step for exact schema 13. The maintained role-defined audio and closed
...
substitute fallback media at runtime; the separate exact-schema-13 role-defined
...
exact-schema-13 report into an explicit choice: **Open read-only** or **Cancel**.
...
This archive evidence is deliberately limited to schema 13 and `.scape` format
...
bytes referenced by the authoritative exact-schema-13 activation project. That
...
- Before `JSON.parse`, every project schema receives an iterative raw-JSON
  ...
  allowance needed to keep the bounded exact-schema-13 binary representation
  round-trip closed; tests may only lower the underlying limits.
- Exact schema 13, format 1 `.scape` export preserves `Uint8Array` values,
...
exact-schema-13 mono/stereo role-defined audio whole-mix, role-defined video
