...
} from '../src/framescaper/editor-native-image-sequence-selection-v25.ts';
...
test('the candidate picker turns opaque desktop ranges into fresh pathless streams', async () => {
	...
	let releases = 0;
	const selected = await selectFramescaperDesktopImageSequenceV25({
		bridge: {
...
test('the candidate picker fails closed unless the complete pathless bridge is mounted', async () => {
	await assert.rejects(() => selectFramescaperDesktopImageSequenceV25({
