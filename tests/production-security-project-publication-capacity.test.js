...
test('project publication evidence keeps canonical admission distinct from backend capacity', async () => {
	...
	assert.match(documentation, /project-publication-capacity-accounting.*not an exact IndexedDB byte count.*structured-clone.*snapshot clone.*serializer string.*heap.*RSS.*maintained facade saves.*point-in-time.*unreserved.*memory fallback.*unknown estimates.*desktop shared projects.*browser-side estimate.*local IndexedDB shadow.*exact-absent managed-media body publication.*prospective catalog geometry.*same-store point-in-time managed-root admission.*appData project-document staging admits the exact serialized document size.*point-in-time fail-closed `statfs`.*before document directory or stage work.*Neither control covers.*renderer\/main IPC.*SQLite catalog or WAL allocation.*filesystem allocation overhead.*cross-store or cross-process coordination.*whole-handoff reservation.*later external allocation.*write-time success.*shared-project load.*outside the save facade.*Scape rollback restoration.*aggregated rollback failure.*directly constructed repository/isu);
});
