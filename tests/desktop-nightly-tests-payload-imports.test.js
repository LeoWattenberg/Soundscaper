...
test('the nightly payload runs the Framescaper baseline lifecycle against the shipped route', async () => {
	const lifecyclePath = relative(REPOSITORY_ROOT, FRAMESCAPER_LIFECYCLE_SPEC);
	assert.ok(
	...
	const lifecycleSource = await readFile(FRAMESCAPER_LIFECYCLE_SPEC, 'utf8');
