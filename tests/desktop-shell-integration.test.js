...
test('sandbox preload exposes only the versioned narrow bridge', async () => {
	...
	const baseFields = [
		...
		'collectAssistanceModelGarbage', 'runDesktopAudioCodecOperation', 'runWindowAction', 'setLocale', 'setNativeAudioHelperEnabled', 'setNativePluginBypassed', 'setNativePluginConsent', 'setNativePluginInstallationAllowed', 'signalReady', 'statDesktopVideoCodecOutput', 'writeChunk', 'writeDesktopVideoCodecInput',
		].sort();
