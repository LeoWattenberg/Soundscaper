...
test('linked retained-video policy pins exact local lifecycle behavior', async () => {
	...
	assert.match(
		linkedVideoOriginal.currentBehavior,
		/latest exact-schema-10 document-only shared load.*every reachable linked-video alias.*without reading its body.*complete groups.*identical physical-body identity.*exact managed source geometry.*bound byte length.*4,094-source.*aggregate 64 GiB.*65,536-PCM-chunk preflight.*only after.*preflight succeeds.*first body request.*opaque locator.*expected revision.*exact size and SHA-256.*4-MiB digest windows.*recheck every binding token.*malformed, incomplete, conflicting, replaced, stale, wrong-size, or wrong-digest.*before shadow publication/iu,
	);
	assert.match(
		linkedVideoOriginal.currentBehavior,
