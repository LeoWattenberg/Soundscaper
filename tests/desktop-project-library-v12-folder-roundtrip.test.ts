...
test('fresh desktop library V9 saves and reopens current folder hierarchy byte-exactly', async (context) => {
	...
	assert.equal(DESKTOP_LIBRARY_SCHEMA_VERSION, 9);
	assert.equal(DESKTOP_LIBRARY_PROJECT_SCHEMA_VERSION, 17);
	...
	assert.equal(Number(database.prepare('PRAGMA user_version').get()?.user_version), 11);
	database.close();
