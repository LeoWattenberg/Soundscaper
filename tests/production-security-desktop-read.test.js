...
test('desktop read capability behavior remains verified for its current surface', async () => {
	...
	assert.match(
		...
		/opaque main-owned.*committed main-frame document.*immutable profile.*all four profiles.*128 pending or live capability slots per owner.*before the first file-open await.*`materialized-v1`.*512 MiB.*per owner.*`scape-range-v1`.*four capabilities.*65 GiB.*globally and per owner.*linked audio and video.*`linked-audio-range-v1`.*`linked-video-range-v1`.*128 capabilities.*64 GiB.*globally and per owner.*512 MiB per linked-original audio or video file.*1,000,000,000-byte ceiling for generic selected audio imports.*16 active range requests.*count reserves before open.*bytes charge after stat.*before publication/iu,
	);
	assert.match(
		rendererOwnedRead.summary,
