...
test('track ripple transforms only addressed track lanes through the exact interval map', () => {
	...
	const result = applySoundscaperProjectCommandV21(project, rippleCommand(project, ['voice']));
...
test('visibly all-track ripple transforms master authority as one common interval map', () => {
	const project = fixture();
	const result = applySoundscaperProjectCommandV21(project, rippleCommand(project, ['voice', 'music']));
...
test('insert paste opens only target-track automation and carries no strip lanes from clipboard', () => {
	...
	const command = preparePasteCommand({
		...
		project: soundscaperProjectForCommandConsumersV21(project),
...
test('effect removal atomically removes its lane while reorder preserves it byte-for-byte', () => {
	...
	const reordered = applySoundscaperProjectCommandV21(project, {
	...
	const removed = applySoundscaperProjectCommandV21(reordered, {
	...
	const removedSidechain = applySoundscaperProjectCommandV21(removed, {
...
function rippleCommand(
	project: ReturnType<typeof createSoundscaperProjectV21>,
	trackIds: readonly string[],
): AudioEditorCommand {
	return prepareRangeDeleteCommand(soundscaperProjectForCommandConsumersV21(project), {
