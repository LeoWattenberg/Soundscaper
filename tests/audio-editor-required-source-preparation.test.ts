...
function createFixture(options: Readonly<{ long?: boolean; cacheFits?: boolean; cacheThrows?: boolean }> = {}) {
	...
	const runtime = {
		...
		copy: {},
		createStoredChunkProviderCandidate: () => options.long === true ? preparedProvider : null,
		engine: {
		...
		sourceAudioBufferBytes: () => preparedBuffer.length * preparedBuffer.numberOfChannels * 4,
		sourceBuffers,
