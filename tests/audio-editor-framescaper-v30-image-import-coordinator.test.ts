...
import {
	...
	type FramescaperTimelineImagePublicationPortV30,
...
test('V30 image import publishes successful files independently and sequentially', async () => {
	const project = createFramescaperProjectV30(
		FRAMESCAPER_V30_PROJECT_RUNTIME_PROFILE, framescaperV20Options(),
	);
	const commands: unknown[] = [];
	const bodies: Blob[] = [];
	const publisher: FramescaperTimelineImagePublicationPortV30 = {
		async publish(request) {
			commands.push(request.command); bodies.push(request.body);
			return applyFramescaperProjectCommandV30(
				FRAMESCAPER_V30_PROJECT_RUNTIME_PROFILE, request.project, request.command,
				{ now: '2026-08-25T13:00:00.000Z' },
			);
		},
	};
	let id = 0;
	const result = await importFramescaperTimelineImagesV30({
		project,
...
test('cancellation stops future files without reverting earlier imports', async () => {
	...
	const result = await importFramescaperTimelineImagesV30({
		...
		publisher: {
			async publish(request) {
				const updated = applyFramescaperProjectCommandV30(
					FRAMESCAPER_V30_PROJECT_RUNTIME_PROFILE, request.project, request.command,
				);
