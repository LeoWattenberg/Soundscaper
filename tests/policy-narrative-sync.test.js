...
async function createFixture(context, { summary, block }) {
	...
	const binding = POLICY_NARRATIVE_BINDINGS[0];
	const register = {
		risks: [{
			id: binding.riskId,
			currentControls: POLICY_NARRATIVE_BINDINGS.map(({ controlId }) => ({ id: controlId, summary })),
		}],
	};
	const documentText = POLICY_NARRATIVE_BINDINGS.map(({ marker }) =>
		`Preamble.\n\n<!-- policy-narrative:${marker} -->\n${block}\n<!-- /policy-narrative:${marker} -->\n`).join('\n');
	for (const [path, text] of [
		[binding.register, `${JSON.stringify(register, null, '\t')}\n`],
		[binding.document, documentText],
	]) {
		await mkdir(dirname(join(root, path)), { recursive: true });
		await writeFile(join(root, path), text);
	}
	return { root, documentPath: join(root, binding.document) };
}
...
test('sync reports and rewrites stale narrative blocks, then converges', async (context) => {
	...
	const { root, documentPath } = await createFixture(context, { summary, block: 'stale text' });
	...
	assert.match(updated, /A separate maintained Linux x64 CI job runs Soundscaper → Framescaper covering `project-audio-mix-v1`\./u);
...
test('sync rejects documents with missing narrative markers', async (context) => {
	...
	const { root, documentPath } = await createFixture(context, { summary, block: 'body' });
	await writeFile(documentPath, 'no markers here\n');
