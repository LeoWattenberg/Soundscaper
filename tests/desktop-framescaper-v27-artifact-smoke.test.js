...
test('Framescaper artifact renderer witnesses the exact ready UI, V18 preload, handshake, and V27 bundle', async () => {
	...
	const injectedSmoke = vm.runInNewContext(`(${runFramescaperV27ArtifactRendererSmoke.toString()})`);
...
test('Framescaper artifact probe waits for renderer readiness and joins sanitized V27 main evidence', async () => {
	const renderer = rendererFixture().expected;
	const main = validMainEvidence(renderer.framescaperV27.project);
	...
	assert.equal(fixture.window.webContents.executions.length, 0, 'load alone must not inspect an unready V27 UI');
	...
	assert.deepEqual(fixture.evidenceCalls, [renderer.framescaperV27.project.projectId]);
	...
	assert.deepEqual(payload, {
		...
		framescaperV27: { ...renderer.framescaperV27, main },
	});
	assert.doesNotMatch(JSON.stringify(payload), /metadataFile|instanceId|processId|libraryRoot|"document"/iu);
});

test('Framescaper artifact probe rejects renderer and main V27 readback disagreement', async () => {
	const renderer = rendererFixture().expected;
	const main = validMainEvidence(renderer.framescaperV27.project);
	...
	assert.match(fixture.errors[0], /SOUNDSCAPER_DESKTOP_SMOKE failed:.*V27.*match|readback/iu);
});
