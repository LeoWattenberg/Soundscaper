...
test('linked-original compatibility policy qualifies bounded save and activation source reachability', async () => {
	...
	assert.match(
		rule.requiredOutcome,
		/exact-schema-13 current project.*every retained revision.*timeline.*Project Bin.*feature fallback.*durable roots.*first- or third-party provenance/iu,
	);
	assert.match(
		rule.requiredOutcome,
	...
	assert.match(
		rule.currentBehavior,
		/current exact-schema-13 document.*every retained exact-schema-13 revision.*timeline clips, Project Bin clips, and every feature-requirement fallback.*without provenance gating.*at most 64.*100,000.*128.*fails? closed.*no binding deletion/iu,
	);
	assert.match(
		rule.currentBehavior,
	...
	assert.match(
		documentation,
		/current exact-schema-13 project.*every retained exact-schema-13 revision.*timeline clips, Project Bin clips, and every feature-requirement fallback.*first-party or third-party provenance.*64 retained revisions.*100,000.*128/iu,
	);
	assert.match(
		documentation,
