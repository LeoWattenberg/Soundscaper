...
} from '../src/framescaper/editor-selected-v32-image-frame-source.ts';
import {
...
test('V32 image preview authenticates once and maps sample boundaries to exact packed frames', async () => {
...
test('V32 image preview coalesces inherited entries and exact ledgers on one track', async () => {
	const fixture = imageProject('timeline');
	...
	assert.equal(session.renderExact, undefined,
		'the inherited flattened exact graph cannot silently omit or misorder a V32 image layer');
	session.dispose();
	assert.equal(inheritedDisposed, true);
});

test('V32 Project Bin thumbnails and timeline filmstrips read the addressed authenticated frames', async () => {
	const bin = imageProject('project-bin', '1000000');
	const thumbnail = await createFramescaperSelectedProjectBinThumbnailV32({
		profile: PROFILE,
		project: bin.project,
	...
	const filmstrip = await createFramescaperSelectedTimelineFilmstripV32({
		profile: PROFILE,
...
test('every V32 image preview consumer fails closed on changed frame-pack bytes', async () => {
	const fixture = imageProject('timeline');
...
test('V32 stored image readers snapshot structural bodies before authenticating ranges', async () => {
	...
	const store: FramescaperStoredImageAssetStoreV32 = {
		loadMediaAsset: () => Promise.resolve(body),
	};
	const source = authentic.project.sources.find(({ kind }) => kind === 'image');
	assert.ok(source);
	const reader = await openFramescaperStoredImageFramePackV32(store, source);
...
function imageProject(
...
): Readonly<{ project: FramescaperProjectV32; bytes: Uint8Array }> {
	...
	videoTrack.clipIds = [];
	const base = createFramescaperProjectV32(PROFILE, baseOptions);
