...
function nodeIds(project: AudioEditorProjectCurrent): readonly string[] {
...
test('promote lifts children, retires the bus when audio leaves, and delete-contents cleans dependents', () => {
	...
	assert.equal(validateCurrentAudioEditorProject(promoted), true);
	...
	assert.equal(validateCurrentAudioEditorProject(deleted), true);
...
test('moves keep buses, routes, and preorder consistent in one undoable step', () => {
	...
	assert.deepEqual(nodeIds(moved.present as AudioEditorProjectCurrent), ['drums', 'kick', 'band', 'bass', 'vocals']);
	const mixer = mixerOf(moved.present as AudioEditorProjectCurrent);
	...
	assert.equal(validateCurrentAudioEditorProject(moved.present), true);

	const undone = undoEditorCommand(moved);
	assert.deepEqual(nodeIds(undone.present as AudioEditorProjectCurrent), ['band', 'drums', 'kick', 'bass', 'vocals']);
	assert.deepEqual(mixerOf(undone.present as AudioEditorProjectCurrent).groups.map(({ id }) => id), ['band']);
	assert.equal(validateCurrentAudioEditorProject(undone.present), true);

	const redone = redoEditorCommand(undone);
	assert.deepEqual(nodeIds(redone.present as AudioEditorProjectCurrent), ['drums', 'kick', 'band', 'bass', 'vocals']);
...
test('direct mixer edits cannot break a folder bus mirror or ownership', () => {
	...
	assert.equal(validateCurrentAudioEditorProject(regained), true);
...
test('legacy structural commands compose with folder-aware ones on a foldered document', () => {
	...
	assert.equal(validateCurrentAudioEditorProject(combined), true);
});
