...
test('compatibility policy qualifies role-defined audio whole-mix fallback playback and delivery', async () => {
	...
	assert.match(
		...
		/authoritative.*exact schema 15.*exactly one.*canonical namespaced feature ID.*unavailable or unknown.*declared and effective rendered-fallback.*closed project-audio-mix-v1.*canonical manifest/iu,
	);
	assert.match(
		rule.currentBehavior,
	...
	assert.match(normalizedDocumentation, /exact schema 15.*canonical namespaced feature ID.*unavailable or unknown.*closed `project-audio-mix-v1` role.*whole-mix.*frame zero.*editor playback/isu);
