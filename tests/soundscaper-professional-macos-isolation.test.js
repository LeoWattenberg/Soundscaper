...
test('macOS reports bounded pre-enforcement launcher and bootstrap stages', () => {
	...
	assert.match(BOOTSTRAP_SOURCE,
		/exactWrite\(enforcementDescriptor[\s\S]*bootstrapFailure\("enforcement-write"/u);
	assert.doesNotMatch(`${LAUNCHER_SOURCE}\n${BOOTSTRAP_SOURCE}`, /attest/iu);
});
...
test('macOS maps its exact peer descriptor closure before SETEXEC', () => {
	assert.match(LAUNCHER_SOURCE,
		/dup2\(enforcementSource, bootstrap::enforcementDescriptor\)[\s\S]*dup2\(policySource, bootstrap::policyDescriptor\)/u);
	assert.match(LAUNCHER_SOURCE,
