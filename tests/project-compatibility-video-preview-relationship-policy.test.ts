...
test('disposable video previews remain reproducible local relationships, not durable project media', async () => {
	...
	assert.match(
		relationship.currentBehavior,
		/maintained exact-schema role-defined audio whole-mix fallback.*separately qualified.*fresh-recipient managed acquisition.*activation.*role-defined whole-project video render.*one closed videoEffects-only clip-target render relationship.*separately qualified.*controller activation.*bounded video delivery.*independent route-specific relationship.*source.*digest admissions.*Framescaper V18 proxy attachment preservation.*isolated re-attestation.*qualified.*selected Framescaper V28 retains.*generation.*Original\/Proxy\/Auto.*offline editing.*relink\/regenerate.*resource- and externally unqualified.*grant no delivery or native-runtime authority.*browser export.*V14 carrier producer.*delivery.*original-authoritative.*refuse.*original.*unavailable/iu,
	);
