...
test('disposable video previews remain reproducible local relationships, not durable project media', async () => {
	...
	assert.match(
		relationship.currentBehavior,
		/maintained exact-schema first-party audio whole-mix fallback.*separately qualified.*fresh-recipient managed acquisition.*activation.*first-party video-effects whole-project render.*one closed clip-target render relationship.*separately qualified.*controller activation.*bounded video delivery.*independent route-specific relationship.*source.*digest admissions.*generic video fallback and proxy relationships.*unqualified/iu,
	);
