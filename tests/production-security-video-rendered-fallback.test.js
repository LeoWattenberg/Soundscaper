...
test('role-defined whole-project and first-party clip-local video fallback playback stay narrow', async () => {
	...
	assert.match(
		control.summary,
		/exact schema 10.*project-video-render-v1.*canonical namespaced feature ID.*unavailable or unknown.*declared and effective rendered-fallback.*closed role.*feature ID.*opaque.*does not discover, load, or execute.*feature code.*canonical manifest/iu,
	);
	assert.match(
		control.summary,
