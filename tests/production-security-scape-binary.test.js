...
test('security policy separates all-schema JSON admission from exact-V16 binary decoding', async () => {
	...
	assert.match(
		control.summary,
		/exact-schema-16.*current-format export.*copies.*Uint8Array.*offset-view.*ArrayBuffer.*independent lower-only ceilings.*256 payloads.*4 MiB.*8 MiB.*100,000 logical traversed nodes.*depth 128/iu,
	);
	assert.match(
		control.summary,
	...
	assert.match(
		threatModel,
		/exact schema 16.*opaque `Uint8Array`.*offset-view.*`ArrayBuffer`.*reserved.*versioned JSON tag.*export.*independently.*256 payloads.*4 MiB.*8 MiB.*100,000 logical traversed nodes.*depth 128.*complete decoded budget/isu,
	);
	assert.match(
		threatModel,
