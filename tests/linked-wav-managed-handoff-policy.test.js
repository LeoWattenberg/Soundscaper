...
test('linked PCM desktop security controls preserve the maintained AIFF profiles', async () => {
	...
	assert.match(
		portability.summary,
		/current-format exact-schema-11 `.scape`.*no owned sender PCM.*canonical Float32 chunks.*`audio-f32le-chunks-v1`.*external container bytes.*locator identity.*excluded.*fresh portless import.*owned PCM.*durable reopen/iu,
	);
