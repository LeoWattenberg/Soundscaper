...
for (const action of ['play', 'play-selection', 'play-at-speed'] as const) {
	void test(`${action} refuses preparation without an open project`, async () => {
		const fixture = createTransportFixture();
		fixture.setProjectAvailable(false);
		await assert.rejects(action === 'play-at-speed'
			? fixture.service.handlePlayAtSpeed()
			: fixture.service.handleTransport(action), /requires an open project/u);
		assert.deepEqual(fixture.calls.begins, []);
		assert.equal(fixture.calls.plays, 0);
		assert.equal(fixture.state.playAtSpeedAbort, null);
	});
}
