...
test('stopping while scheduled recording playback resumes cancels playAt', async () => {
	const context = new MockAudioContext();
	let signalResumeRequested;
	const resumeRequested = new Promise((resolve) => { signalResumeRequested = resolve; });
	let allowResume;
	...
	const pending = engine.playAt(0, 0);
	await resumeRequested;
	engine.stop();
	allowResume();
...
test('stopping while variable-speed playback resumes cancels its pending start', async () => {
	const context = new MockAudioContext();
	let signalResumeRequested;
	const resumeRequested = new Promise((resolve) => { signalResumeRequested = resolve; });
	let allowResume;
	...
	const pending = engine.playAtSpeed(2);
	await resumeRequested;
	engine.stop();
	allowResume();
	await assert.rejects(pending, { name: 'AbortError' });
