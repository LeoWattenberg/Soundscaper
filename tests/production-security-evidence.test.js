...
test('threat-model documentation defines the limits of enforced controls', async () => {
	...
	assert.match(
		documentation,
		/shared-desktop-project-library-integrity.*partial.*product-neutral appData library.*fresh filesystem library scope `?v3`?.*SQLite `?user_version`? 5.*ignores rather than migrates.*prior shared `?v2`? scope.*preserv.*metadata schema 2.*schema 10.*in place.*copied `?v2`?.*`?user_version`? 4.*`?v3`? path.*reject.*without mutation.*migrat.*adopt.*backfill.*metadata schema 3.*separate opaque library entry ID.*exact schema 11.*bounded byte length.*SHA-256.*immutable revision-and-digest path.*canonical tagged-binary codec.*opaque binary state.*non-raiseable 256 MiB.*lower-only test seam.*persistence root.*reserves.*canonical path.*unique random attempt.*lease.*fencing-token.*authoritative project and stage inventories.*same immediate transaction.*before exclusive stage creation.*exact-lease cleanup.*acknowledged.*exclusive-open failure.*registration.*without unlinking.*error after exclusive creation.*registered random stage.*lost-lease or failed cleanup.*registration.*takeover.*successful materialization.*exact metadata and stage paths.*lease ID.*fencing token.*renames and syncs.*marks the canonical row materialized.*removes the stage row.*every catalog reference.*before an exact plus-one journaled catalog commit.*before staging.*before publication.*transactionally at catalog commit.*old or new complete file-and-catalog pair.*stale fencing token cannot publish.*serializes commits.*continues lease renewal while close fences new work and drains admitted work/isu,
	);
	assert.match(
		documentation,
	...
	assert.match(
		documentation,
		/renderer repository.*repeats maintained-persistence-domain exact-schema-11 validation and canonical reserialization.*before local mutation.*product-local shadow.*shared latest document and summary list.*authoritative.*fails closed.*incomplete desktop bridge.*source-free editor fixture.*Soundscaper.*same identity and revision.*fresh Framescaper-local store.*next revision.*higher fencing token.*shared media catalog.*empty/isu,
	);
	assert.match(
		documentation,
		/dedicated Linux x64 CI.*two separate unpacked packages.*Soundscaper.*Framescaper.*Soundscaper.*isolated appData.*separate product profiles.*reuses.*Soundscaper profile.*renderer[- ]ready.*pathless preload IPC.*exact[- ]SHA-256.*source-free.*schema 11.*revisions 1, 2, and 3.*summary.*main-only catalog row.*clean recovery.*no stale takeover.*higher fencing token.*increasing catalog revisions?.*preferred product.*process exit.*lease release.*combined with.*composed editor.*closes only.*generic packaged source-free preload\/IPC\/multi-process\/executable lifecycle gap/isu,
	);
	assert.match(
		documentation,
...
test('project feature requirements are bounded and fail closed at activation and pre-open inspection', async () => {
	...
	assert.match(control.summary, /schema 11.*create.*load.*clone.*commit.*reserved `soundscaper\.audio-effects`.*track.*group.*send.*master.*disabled.*inactive.*publisher-authored.*take precedence.*missing.*foreign.*do not trigger.*reserved-ID conflicts.*reject/iu);
	...
	assert.match(control.summary, /schema 11.*actual project history.*before activation.*intrinsically read-only.*deep-frozen.*session metadata clones.*snapshot.*future schemas.*null.*not traversed/iu);
	assert.match(control.summary, /same-ID tab.*stored read-only declaration.*ignored incoming.*flags/iu);
	assert.match(control.summary, /current-format \.scape inspection.*provider-owned.*caller.*override.*exact schema 11.*before.*collision lookup.*deep-frozen.*future schemas.*null.*not traversed/iu);
	...
	assert.match(control.summary, /exact schema 11.*registered `audioEffects`.*unavailable.*declared `bypass`.*effective `bypassed`.*bounded.*non-persisted.*engine projection.*before activation side effects.*canonical project.*history.*persistence.*unchanged/iu);
	...
	assert.match(control.summary, /current-format.*exact schema 11.*fallback.*claim.*canonical asset descriptor.*before.*collision.*storage/iu);
	...
	assert.match(control.summary, /separate maintained-controller admission.*exact-schema-11 raw and stored-project fallback bytes.*direct store loads.*runtime fallback substitution.*third-party/iu);
	...
	assert.match(
		...
		/exact schema 11.*canonical namespaced feature ID.*unavailable or unknown.*declared and effective rendered-fallback.*closed project-audio-mix-v1 role.*canonical manifest.*mono or stereo.*whole-mix.*frame zero.*removes canonical audio.*neutral.*mixer and master.*retains video and label/iu,
	);
	assert.match(
		fallbackPlayback.summary,
	...
	assert.match(documentation, /schema 11.*create.*load.*clone.*commit.*`soundscaper\.audio-effects`.*track.*group.*send.*master.*disabled.*inactive.*publisher-authored.*take precedence.*missing.*foreign.*do not trigger.*reserved-ID conflicts.*reject/iu);
	...
	assert.match(documentation, /schema 11.*actual project history.*before activation.*intrinsically read-only.*deep-frozen.*session metadata clones.*snapshot.*future schemas.*`null`.*not traversed/iu);
	...
	assert.match(documentation, /exact schema 11.*registered `audioEffects`.*unavailable.*declared `bypass`.*effective `bypassed`.*bounded.*non-persisted.*engine projection.*before activation side effects.*canonical project.*history.*persistence.*unchanged/iu);
	...
	assert.match(documentation, /exact schema 11.*registered `videoEffects`.*unavailable.*declared `bypass`.*effective `bypassed`.*bounded.*non-persisted.*preview-playback projection.*before activation side effects.*canonical project.*history.*source loading.*persistence.*save paths.*video export.*unchanged/iu);
	...
	assert.match(documentation, /current-format `\.scape` inspection.*provider-owned.*caller.*override.*schema 11.*before.*collision lookup.*deep-frozen.*future schemas.*`null`.*not traversed/iu);
	...
	assert.match(documentation, /current-format.*exact schema 11.*fallback.*claim.*asset descriptor.*before.*collision.*storage/iu);
	...
	assert.match(documentation, /inspection.*does not hash.*asset bodies.*maintained exact-schema-11 controller activation.*referenced local audio and video fallback bytes/iu);
	...
	assert.match(documentation, /role-defined audio rendered-fallback playback.*exact schema 11.*canonical namespaced feature ID.*unavailable or unknown.*declared and effective `rendered-fallback`.*closed `project-audio-mix-v1` role.*opaque identity.*does not discover, load, or execute.*feature code.*whole-mix.*frame zero.*canonical project.*unchanged/isu);
...
test('legacy AUP evidence pins structural and block-materialization budgets', async () => {
	...
	assert.match(
		sharedBudget.exposure,
		/raw-JSON structural preflight.*every schema.*before `JSON\.parse`.*101,536 JSON values.*depth 130.*exact schema 11.*decoded.*semantic validator.*independent ceilings.*100,000 logical nodes.*depth 128/iu,
	);
	assert.match(
		sharedBudget.exposure,
	...
	assert.match(
		documentation,
		/shared-project-parse-budget.*remains open.*101,536-value\/depth-130 raw preflight.*per-phase 100,000-node\/depth-128 exact-V11 decode and validator admissions.*do not combine.*end-to-end work budget.*CPU or elapsed time.*cancellation.*allocation.*total main-process RSS/isu,
	);
