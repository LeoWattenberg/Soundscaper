...
test('project feature requirements are bounded and fail closed at activation and pre-open inspection', async () => {
	...
	assert.match(control.summary, /schema-17 take groups.*mandatory.*closed canonical records.*sequence and audio-track ownership.*audio-source bounds.*canonical lane, take, region, and group ordering.*4,096.*160-character.*globally unique.*identities.*nonoverlapping.*`soundscaper\.take-comp`.*`Take lanes and comps`.*`bypass`.*no fallback.*both products.*known but unavailable.*nonempty state.*read-only.*excluded from audio and video rendered-fallback eligibility/iu);
	...
	assert.match(documentation, /schema 17.*create.*load.*clone.*commit.*`soundscaper\.audio-effects`.*track.*group.*send.*master.*disabled.*inactive.*publisher-authored.*take precedence.*missing.*foreign.*do not trigger.*reserved-ID conflicts.*reject/iu);
	...
	assert.match(documentation.replace(/\s+/gu, ' '), /schema 17.*take groups.*mandatory.*closed canonical records.*sequence and audio-track ownership.*audio-source bounds.*canonical lane, take, region, and group ordering.*4,096.*160-character.*globally unique.*identities.*overlaps.*soundscaper\.take-comp.*Take lanes and comps.*bypass.*no fallback.*both products.*known but unavailable.*nonempty state.*read-only.*excluded from audio and video rendered-fallback eligibility/isu);
	assert.match(documentation, /schema 17.*actual project history.*before activation.*intrinsically read-only.*deep-frozen.*session metadata clones.*snapshot.*future schemas.*`null`.*not traversed/iu);
	...
	assert.match(documentation, /exact schema 17.*registered `audioEffects`.*unavailable.*declared `bypass`.*effective `bypassed`.*bounded.*non-persisted.*engine projection.*before activation side effects.*canonical project.*history.*persistence.*unchanged/iu);
	...
	assert.match(documentation, /exact schema 17.*registered `videoEffects`.*unavailable.*declared `bypass`.*effective `bypassed`.*bounded.*non-persisted.*preview-playback projection.*before activation side effects.*canonical project.*history.*source loading.*persistence.*save paths.*video export.*unchanged/iu);
	...
	assert.match(documentation, /current-format `\.scape` inspection.*provider-owned.*caller.*override.*schema 17.*before.*collision lookup.*deep-frozen.*future schemas.*`null`.*not traversed/iu);
	...
	assert.match(documentation, /current-format.*exact schema 17.*fallback.*claim.*asset descriptor.*before.*collision.*storage/iu);
	...
	assert.match(documentation, /inspection.*does not hash.*asset bodies.*maintained exact-schema-17 controller activation.*referenced local audio and video fallback bytes/iu);
	...
	assert.match(documentation, /role-defined audio rendered-fallback playback.*exact schema 17.*canonical namespaced feature ID.*unavailable or unknown.*declared and effective `rendered-fallback`.*closed `project-audio-mix-v1` role.*opaque identity.*does not discover, load, or execute.*feature code.*whole-mix.*frame zero.*canonical project.*unchanged/isu);
