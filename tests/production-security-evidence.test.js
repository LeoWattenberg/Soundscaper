...
test('threat-model documentation defines the limits of enforced controls', async () => {
	...
	assert.match(
		documentation,
		/selection and import adapter.*whole-Blob materializer.*closed exact locator ID-and-revision CAS release.*missing, malformed, or accessor revision.*never authorizes cleanup.*desktop visual activation.*ranged playback lease.*exact binding revision.*complete pinned handle.*4 MiB ranges.*binding and CAS fence.*returns only the media URL and one-shot release.*does not construct another original-video Blob/isu,
	);
	assert.match(
		documentation,
