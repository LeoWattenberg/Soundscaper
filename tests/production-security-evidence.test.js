...
test('threat-model documentation defines the limits of enforced controls', async () => {
	...
	for (const claim of [
		...
		/verified reference evidence.*opt-in.*`npm run test:reference:scape-8gib`.*routine Node.*coverage.*fast-skip.*measured all-files coverage.*passed.*525 seconds.*does not demote.*collision-cancel.*corrupted-CRC negative rollback.*routine coverage.*sparse-file support.*Node protocol shim.*not packaged UI/isu,
...
test('project feature requirements are bounded and fail closed at activation and pre-open inspection', async () => {
	...
	assert.match(roadmap, /first-party audio whole-mix editor playback through the short decoded-source.*persistent active-fallback indicator.*browser-qualified.*stream-provider readiness.*unit coverage only.*does not prefetch or revalidate chunks after\s+point-in-time admission.*exit.*remains open/isu);
