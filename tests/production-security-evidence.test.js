...
test('threat-model documentation defines the limits of enforced controls', async () => {
	...
	assert.match(
		documentation,
		/desktop-read-path-capabilities.*enforced for the current versioned materialized, Scape range, and linked-video playback range profiles.*`?materialized-v1`?.*`?scape-range-v1`?.*`?linked-video-range-v1`?.*all three profiles.*128 pending\/live.*`?materialized-v1`?.*512 MiB.*per-owner.*`?scape-range-v1`?.*at most four capabilities.*65 GiB.*globally and per owner.*count.*before open.*bytes.*after stat.*before descriptor publication.*cleanup failure.*retains the range charge.*fences.*expiry for the expiring profiles.*OS-open dispatch.*four Scape descriptors.*fifth.*unopened.*acknowledged release.*redispatches.*renderer-send failure.*releases.*descriptor/isu,
	);
	assert.match(
		documentation,
	...
	assert.match(
		documentation,
		/separately admitted `?linked-video-range-v1`? playback profile.*exact current locator revision.*pathname stat.*opens a handle.*same identity.*pathname-replacement race.*512 MiB.*128 capabilities.*64 GiB.*16 active range requests.*no wall-clock expiry.*protocol admits `HEAD`.*start-based closed or open-ended `GET`.*4 MiB.*rejects a full-body `GET`.*successful response or ordinary seek cancellation.*preserves the pinned capability.*inner stream failure retires.*resolver verifies the exact byte length and MIME.*hashes the entire admitted handle sequentially.*at-most-4-MiB `206` responses.*binding and CAS fence.*aggregate error/isu,
	);
	assert.match(
		documentation,
		/`?materialized-v1`? tier.*whole `?Blob`?.*512 MiB.*excludes Scape and linked-video range descriptors.*declared `?Content-Length`?.*emitted-byte.*final `?Blob`?-size.*16 MiB.*caller.*`?AbortSignal`?.*never calls `?response\.blob\(\)`?.*not decoder amplification or whole-process RSS/isu,
	);
	for (const claim of [
	...
	assert.match(
		documentation,
		/shared-desktop-project-library-integrity.*partial.*product-neutral appData library.*fresh filesystem library scope `?v2`?.*ignores rather than migrates.*prior shared `?v1`? scope.*database schema 3(?:.*schema 1.*schema 2.*`?v2`? path.*reject|.*`?v2`? path.*rejects schemas 1 and 2).*instead of implicitly migrat(?:ed|ing).*metadata schema 2.*separate opaque library entry ID.*exact schema 9.*bounded byte length.*SHA-256.*immutable revision-and-digest path.*canonical tagged-binary codec.*opaque binary state.*non-raiseable 256 MiB.*lower-only test seam.*persistence root.*reserves.*canonical path.*unique random attempt.*lease.*fencing-token.*authoritative project and stage inventories.*same immediate transaction.*before exclusive stage creation.*exact-lease cleanup.*acknowledged.*exclusive-open failure.*registration.*without unlinking.*error after exclusive creation.*registered random stage.*lost-lease or failed cleanup.*registration.*takeover.*successful materialization.*exact metadata and stage paths.*lease ID.*fencing token.*renames and syncs.*marks the canonical row materialized.*removes the stage row.*every catalog reference.*before an exact plus-one journaled catalog commit.*before staging.*before publication.*transactionally at catalog commit.*old or new complete file-and-catalog pair.*stale fencing token cannot publish.*serializes commits.*continues lease renewal while close fences new work and drains admitted work/isu,
	);
	assert.match(
		documentation,
	...
	assert.match(
		documentation,
		/linked retained-video slice.*schema-1 closed product-local binding.*exact project and source.*pathless opaque locator ID.*opaque locator-revision fence.*independent repository-owned CAS binding token.*storage key.*video MIME.*exact source geometry.*byte length.*lowercase SHA-256.*no filesystem path, URL, handle, or linked body.*project ID, source ID, storage key, MIME type.*every geometry field.*before privileged platform I\/O.*expected locator revision.*exact length.*complete SHA-256.*4 MiB windows.*rereads.*binding.*CAS fence.*fresh per-operation alias session.*module-private WeakMap.*forged.*rejected.*complete reachable video alias group.*before any linked body read.*metadata.*aggregate budget preflight.*before lazy.*body resolution.*storage key alone.*never authorizes.*binding, descriptor-free shared admission, and visual activation.*no durable product-owned copy.*explicit managed handoff.*project deletion and whole-store clear.*only local binding records.*external target.*untouched/isu,
	);
	assert.match(
		documentation,
	...
	assert.match(
		documentation,
		/selection and import adapter.*whole-Blob materializer.*ID-scoped locator release.*desktop visual activation.*ranged playback lease.*exact binding revision.*complete pinned handle.*4 MiB ranges.*binding and CAS fence.*returns only the media URL and one-shot release.*does not construct another original-video Blob/isu,
	);
	assert.match(
		documentation,
	...
	assert.match(
		documentation,
		/same-inode external mutation during or after sequential digest verification.*not fenced.*not an immutable, durable, or cross-process byte snapshot.*selection, binding, whole-Blob resolution, availability, and handoff.*complete body.*512 MiB.*visual activation.*at-most-4-MiB ranges.*without constructing another original-video Blob.*decoder or codec amplification.*RSS.*browser caching.*garbage-collection headroom.*no reference-scale evidence/isu,
	);
	assert.match(
		documentation,
		/privileged service.*compromised renderer.*over-budget or maintained-domain-invalid exact-schema-9 input.*before.*host.*stage a project.*shared-project-parse-budget.*remains open.*unmanaged recipient admission.*not an atomic snapshot.*same-metadata replacement.*can go undetected.*managed handoff now closes.*Soundscaper-to-Framescaper edit\/save\/return.*canonical PCM and retained original video.*manifest-only exact-schema first-party audio whole-mix and video-effects fallbacks.*startup-bounded tracked managed-media reclamation.*continuous runtime cleanup.*linked retained video.*product-local chooser.*validated main\/preload boundary.*whole-Blob selection\/import adapter.*exact-binding import.*owner-scoped exact-revision ranged visual playback.*private main-owned registry.*pathless.*not an operating-system bookmark.*pathname replacement after playback admission cannot retarget.*same-inode external mutation.*not fenced.*not an immutable, durable, or cross-process byte snapshot.*selection, binding, whole-Blob resolution, availability, and handoff.*complete body.*visual activation.*at-most-4-MiB ranges.*without constructing another original-video Blob.*decoder or codec amplification.*RSS.*browser caching.*reference-scale evidence.*bounded cooperative startup pass.*point-in-time authoritative project-ID catalog.*10,000.*readwrite binding-store transaction.*100,000.*128.*absent-project bindings.*live alias.*rolls back before IPC.*source-level reachability.*surviving project.*remains open.*catalog snapshot.*binding transaction.*main registry write.*not one cross-boundary atomic operation.*hostile renderer.*inventory completeness.*separate store, profile, or process.*not serialized.*continuous runtime cleanup.*remains open.*packaged executable.*operating-system file-dialog and identity behavior.*linked audio.*authored-proxy relationships.*generic or third-party rendered-fallback.*browser `<video>` codec behavior.*packaged Electron UI source-bearing workflows.*hard-link.*power-loss.*Linux x64 source-free packaged lifecycle.*remaining platform.*compatibility beyond.*prior shared `v1` scope.*deferred and unsupported.*Audacity.*separate boundary/isu,
	);
	assert.match(
		documentation,
		/bounded cooperative startup reconciliation pass.*main\/renderer durability boundary.*durable IndexedDB opens.*before.*project loading.*point-in-time authoritative.*catalog.*10,000 exact canonical project IDs.*exact linked-binding identity contract.*before opening the binding.*transaction.*invalid or duplicate project ID.*rejects bootstrap.*Memory fallback returns before catalog listing, binding mutation,.*or IPC.*durable platform port without reconciliation.*catalog.*no binding mutation or IPC.*one IndexedDB readwrite binding-store transaction.*100,000.*closed rows.*128 unique exact locator\/revision pairs.*live and.*absent-project bindings.*Malformed rows.*conflicting revisions.*bounds.*binding deletion failure.*abort and roll back.*before IPC.*complete scan.*delete bindings whose project ID is absent.*live alias.*keeps.*locator live.*frozen sorted.*positive inventory.*existing.*preload\/IPC.*limited to project IDs.*does not parse.*sources.*surviving project.*source-level binding.*reachability.*catalog snapshot.*binding transaction.*main registry write.*not one cross-boundary atomic operation.*binding deletion commits before.*separate main operation.*later main.*rejection.*retry.*Main serializes.*startup-loaded metadata.*runtime-created records.*failed.*pass can retry.*successful pass.*store\/process.*Unknown or stale references.*before mutation.*registry.*write.*in-memory inventory.*Owner revocation.*persisted restore.*on-disk outcome.*indeterminate.*never stats or.*deletes external video bytes.*successful full bootstrap.*binding publication.*binding row.*durably deleted.*Current-process abandoned records.*later main-process restart.*cannot authenticate inventory.*completeness.*compromised renderer.*omit.*live references.*startup metadata.*Separate store, profile, or process.*not serialized.*cooperative.*first-party lifecycle.*housekeeping.*not a renderer-compromise integrity control.*orderly close, dispose, and reopen.*abrupt process death.*fsync.*power loss.*not qualified.*Continuous runtime cleanup.*hostile IndexedDB row.*remain open/isu,
	);
