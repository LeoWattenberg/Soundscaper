...
test('threat-model documentation defines the limits of enforced controls', async () => {
	...
	assert.match(
		documentation,
		/privileged service.*compromised renderer.*over-budget or maintained-domain-invalid exact-schema-9 input.*before.*host.*stage a project.*maintained-domain-invalid or structurally over-budget loaded commit results and stored documents.*before a renderer response.*renderer repository.*repeats validation.*loaded-result refusal.*host commit may already be published.*closes.*privileged-domain-validation residual.*qualifies per-phase project-shape node and depth ceilings.*shared-project-parse-budget.*remains open.*256 MiB.*101,536-value\/depth-130 raw preflight.*per-phase 100,000-node\/depth-128.*do not combine.*end-to-end work budget.*CPU or elapsed time.*cancellation.*allocation.*main-process RSS.*unmanaged recipient admission.*bounded sequential readability check.*not an atomic snapshot.*selected metadata.*not transactionally bound.*same-metadata replacement.*can go undetected.*replacement or deletion afterward.*not fenced.*separate repository instances or processes.*not serialized.*managed handoff now closes.*headless composed Soundscaper-to-Framescaper edit\/save\/return path.*canonical PCM and retained original video.*Separate one-way headless fixtures.*manifest-only exact-schema first-party audio whole-mix and video-effects fallbacks.*editable originals.*transfer authenticates.*descriptors and body digests.*controller independently verifies the manifest declaration.*exact-absent managed publication.*same-store point-in-time prospective catalog.*managed-root admission.*before body or optional hard-link work.*startup-bounded tracked managed-media reclamation.*logical catalog-row retirement.*continuous runtime cleanup.*later startup.*unregistered or legacy.*empty director.*SQLite\/WAL.*linked retained video.*maintained product-local chooser.*validated main\/preload boundary.*whole-Blob renderer adapter.*exact-binding import.*binding-scoped disposable previews.*source and component tests.*raw paths.*private main-owned registry.*renderer and project state.*pathless.*point-in-time.*not an operating-system bookmark.*replacement or deletion after.*Blob snapshot.*not fenced.*no durable byte lease.*range protocol.*reference-scale.*512 MiB.*4 MiB.*digest.*not.*provider allocation.*bounded cooperative startup pass.*startup-loaded metadata.*point-in-time inventory.*continuous runtime cleanup.*canonical-project reachability reconciliation.*remain open.*not qualified in a packaged executable.*linked audio.*authored-proxy relationships.*generic or third-party rendered-fallback authoring, acquisition, and handoff.*packaged first-party video-effects fallback workflows.*relink and watch behavior.*general consolidation.*exact allocation.*whole-handoff.*renderer-session.*cross-store.*cross-process.*write-time capacity behavior.*browser.*video.*codec behavior.*packaged Electron UI source-bearing workflows remain open.*hard-link availability.*crash or power-loss behavior.*unqualified.*bounded upload fallback.*recognized unsupported-link failures.*Linux x64 source-free packaged lifecycle.*qualified.*remaining platform.*fault.*power-loss.*parent- and database-path identity.*Windows directory-sync and deny-delete behavior.*junction.*time-of-check\/time-of-use.*interrupted foreign collisions.*registered random stage paths.*unregistered or legacy pre-inventory stage-looking files.*foreign.*not adopted or deleted.*compatibility beyond those retained raw-document migration paths.*prior shared `v1` scope or product-private Soundscaper libraries.*deferred and unsupported.*Audacity.*separate boundary/isu,
	);
	assert.match(
		documentation,
