...
test('threat-model documentation defines the limits of enforced controls', async () => {
	...
	assert.match(
		documentation,
		/identity service.*frozen preload.*owner-scoped IPC.*bounded, pathless list, read, bundle, commit, delete, and managed-source transfer.*256 MiB.*4 KiB.*10,000-summary.*64 GiB source-body.*4,094-bundle-descriptor.*4 MiB chunk.*renderer transfer code.*4,094 reachable logical sources.*before media I\/O.*four active source uploads.*four active source reads.*across the bridge service.*capacity remains charged.*publication or abort settlement.*disposal waits.*finishing publications.*catalog summaries.*entry IDs.*main-owned catalog\/filesystem paths.*digests.*product preferences.*raw `updatedAtMs` fields.*leases.*fencing tokens.*owner revocation.*fences new work.*aborts owned upload sessions.*drains admitted operations/isu,
	);
	assert.match(
		documentation,
		/ordinary shared-project saves remain document-only.*explicit project-handoff action.*before any source read or bridge call.*4,094 reachable logical sources.*refuses video and mixed-media sets.*deduplicates compatible physical audio bindings.*aggregate 64 GiB canonical-byte.*65,536-chunk budget.*two full digesting source reads.*binding is absent.*second read.*uploads bounded sequential chunks.*changed second digest/isu,
	);
	assert.match(
		documentation,
		/main revalidates the exact current project revision.*requested reachable audio identity and geometry.*derives the catalog document SHA-256 rather than accepting it from the renderer.*serialized host repeats exact revision-and-document-digest validation.*immutable binding includes project identity, exact revision, exact document digest, and storage-key\/media geometry.*prior-revision row or same-revision document variant.*neither advertised nor accepted as present.*exact-present reuse.*declared length and SHA-256.*reverifies the body/isu,
	);
	assert.match(
		documentation,
		/private regular stage.*syncs it.*atomically renames it inside the fixed managed-media root.*syncs the directory.*only then publishes the catalog row.*matching retry.*consume and validate its offered stream.*reusing that body.*managed-media reclamation and capacity reservation remain unqualified/isu,
	);
	assert.match(
		documentation,
	...
	assert.match(
		documentation,
		/latest authoritative exact-schema-9 source-bearing load.*4,094 reachable timeline, Project Bin, and fallback sources.*before recipient-local or shared media I\/O.*deduplicates compatible physical audio bindings.*aggregate 64 GiB canonical-byte.*65,536-chunk ceilings.*fresh recipient first acquires.*managed canonical-PCM descriptors.*bounded reads.*staged product-local source writers.*identity and geometry.*canonical byte length.*SHA-256.*atomic if-absent local source-record publication.*loses the absence race.*only its own staging.*preserves the winner.*partial transfers.*pre-shadow failures.*exact acquisition-owned records.*source-token or path payloads.*concurrent replacement.*preserved.*not acquired.*pre-existing latest recipient-local exact-schema-9 snapshot.*same project.*logical identity, kind, storage key, MIME type.*media geometry.*ordered Float32Array PCM.*exact chunk, channel, and frame geometry.*trusted recipient-local SHA-256.*before.*video body read.*genuine.*Blob.*SHA-256.*4 MiB.*must match.*legacy PCM-on-read migration and media-digest backfill.*disabled.*source integrity, availability, binding, geometry, budget, body, and digest failures.*before shadow publication.*preserve.*prior local shadow.*prevent activation.*cancellation.*after the exact shadow is durable.*retains the exact shadow and acquired PCM.*rendered-fallback-declaration digest check.*follows repository shadowing.*source-free.*zero source or media I\/O.*Bootstrap.*lifetime signal.*load, save, and delete.*serialized per project.*explicitly publishes canonical PCM from Soundscaper.*fresh Framescaper-local store.*before activation.*next shared project revision/isu,
	);
	assert.match(
		documentation,
		/privileged service.*compromised renderer.*over-budget or maintained-domain-invalid exact-schema-9 input.*before.*host.*stage a project.*maintained-domain-invalid or structurally over-budget loaded commit results and stored documents.*before a renderer response.*renderer repository.*repeats validation.*loaded-result refusal.*host commit may already be published.*closes.*privileged-domain-validation residual.*qualifies per-phase project-shape node and depth ceilings.*shared-project-parse-budget.*remains open.*256 MiB.*101,536-value\/depth-130 raw preflight.*per-phase 100,000-node\/depth-128.*do not combine.*end-to-end work budget.*CPU or elapsed time.*cancellation.*allocation.*main-process RSS.*unmanaged recipient admission.*bounded sequential readability check.*not an atomic snapshot.*selected metadata.*not transactionally bound.*same-metadata replacement.*can go undetected.*replacement or deletion afterward.*not fenced.*separate repository instances or processes.*not serialized.*managed handoff closes exact-document-bound fresh-recipient acquisition only for canonical PCM.*linked and video originals.*proxies and rendered fallbacks.*relink and watch behavior.*general copy\/consolidate.*stable byte lease through playback.*return handoff.*cleanup and capacity reservation.*packaged two-product source-bearing lifecycle remain open.*Linux x64 source-free packaged lifecycle.*qualified.*remaining platform.*fault.*power-loss.*parent- and database-path identity.*Windows directory-sync and deny-delete behavior.*junction.*time-of-check\/time-of-use.*interrupted foreign collisions.*registered random stage paths.*unregistered or legacy pre-inventory stage-looking files.*foreign.*not adopted or deleted.*compatibility beyond those retained raw-document migration paths.*prior shared `v1` scope or product-private Soundscaper libraries.*deferred and unsupported.*Audacity.*separate boundary/isu,
	);
