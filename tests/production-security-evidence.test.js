...
test('threat-model documentation defines the limits of enforced controls', async () => {
	...
	assert.match(
		documentation,
		/separately admitted `?linked-audio-range-v1`? and `?linked-video-range-v1`? profiles.*exact current locator revision.*pathname stat.*opens an owner-scoped handle.*same identity.*pathname-replacement race.*128-capability.*64 GiB.*16-active-request.*512 MiB per file.*protocol admits `HEAD`.*start-based closed or open-ended `GET`.*4 MiB.*rejects a full-body `GET`.*successful response or ordinary cancellation.*preserves the pinned capability.*inner stream failure retires.*exact WAV\/RF64 MIME\/name pair.*hashes the entire admitted handle sequentially.*at-most-4-MiB `206` responses.*binding and CAS fence.*linked-WAV path.*without constructing a second whole-original Blob.*release.*once.*aggregate error.*initial binding.*materialized the complete external WAV.*not a content-frozen, durable, or cross-process lease.*same-inode mutation.*unfenced.*Float32 arrays.*WAV metadata.*process RSS.*no packaged-executable.*operating-system.*reference-scale qualification/isu,
	);
	assert.match(
		documentation,
