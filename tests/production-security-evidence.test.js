...
test('threat-model documentation defines the limits of enforced controls', async () => {
	...
	assert.match(
		documentation,
		/inspection collision-cancel witness.*exact 8 GiB sparse Zip64 current-schema `?\.scape`? fixture.*real read-capability store.*protocol handler.*renderer adapter.*structural inspection.*collision lookup.*before cancellation.*less than 8 MiB.*65,557-byte suffix.*does not hash.*authentic exact 8 GiB fixture.*8,589,932,094-byte.*SHA-256.*7feeb1e9eacb6561f3c5afb4ebf3896c8237660a9b4ed8917d3275c79bed38be.*CRC-32.*2,909,126,900.*`?checkSignature: true`?.*negative rollback.*full-import witness.*real read-capability store.*Node protocol handler shim.*renderer adapter.*file service.*project service.*full import.*independent counting-SHA-256.*zero payload retention.*no Blob materialization.*capability release.*exactly once.*pinned handle close.*exactly once.*verified reference evidence.*opt-in.*`npm run test:reference:scape-8gib`.*routine Node.*coverage.*fast-skip.*measured all-files coverage.*passed.*525 seconds.*does not demote.*collision-cancel.*corrupted-CRC negative rollback.*routine coverage.*sparse-file support.*Node protocol shim.*not packaged UI.*OPFS.*IndexedDB.*durable.*quota.*preflight.*browser heap.*process RSS.*whole-storage atomicity.*publisher authentication/isu,
	);
	assert.match(
		documentation,
