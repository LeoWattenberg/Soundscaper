...
test('threat-model documentation defines the limits of enforced controls', async () => {
	...
	assert.match(
		documentation,
		/separately admitted `?linked-audio-range-v1`? and `?linked-video-range-v1`? profiles.*exact current locator revision.*pathname stat.*opens an owner-scoped handle.*same identity.*pathname-replacement race.*128-capability.*64 GiB.*16-active-request.*512 MiB per file.*protocol admits `HEAD`.*start-based closed or open-ended `GET`.*4 MiB.*rejects a full-body `GET`.*successful response or ordinary cancellation.*preserves the pinned capability.*inner stream failure retires.*maintained WAV\/RF64 MIME\/name contract.*exact `?\.aif`? or `?\.aiff`? name.*`?audio\/aiff`?.*hashes the entire admitted handle sequentially.*at-most-4-MiB `206` responses.*binding and CAS fence.*linked-PCM path.*without constructing a second whole-original Blob.*FORM\/AIFF.*COMM.*SSND.*signed big-endian integer PCM.*8.*16.*24.*32.*AIFC and compressed variants reject.*release.*once.*aggregate error.*provider-owned stable PCM\s+read session.*one full-container digest,? and one parsed descriptor.*serialized random or sequential chunk reads.*complete alias group.*exact binding.*before and after.*per-read.*cancellation.*local.*provider retirement.*terminal.*exact-once release.*backing cleanup.*aggregate.*linked-PCM range control.*initial binding.*materialized the complete external WAV or AIFF.*512 MiB.*not a content-frozen, durable,.*cross-process lease.*same-inode mutation.*unfenced.*Float32 arrays.*source-container metadata.*does not preserve WAV or AIFF ancillary metadata.*no packaged-executable.*operating-system.*reference-scale qualification/isu,
	);
	assert.match(
		documentation,
	...
	assert.match(documentation, /maintained linked-PCM slice.*shared owner-scoped range pool.*exact-revision.*digest-verified.*WAV\/RF64 or structurally validated classic AIFF inspection.*canonical PCM chunk reads.*without a second whole-original Blob.*same-inode external mutation during or after sequential digest verification.*not fenced.*not an immutable, durable, or cross-process byte snapshot.*selection and initial binding, whole-Blob resolution, availability, and handoff.*complete body.*512 MiB.*visual activation and linked-PCM canonical reads.*at-most-4-MiB responses.*without constructing another original Blob.*Float32 arrays.*metadata.*decoder or codec amplification.*RSS.*browser caching.*garbage-collection headroom.*no packaged, operating-system, or reference-scale evidence/isu);
