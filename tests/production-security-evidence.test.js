...
test('threat-model documentation defines the limits of enforced controls', async () => {
	...
	assert.match(
		documentation,
		/selection and import adapter.*whole-Blob materializer.*closed exact locator ID-and-revision CAS release.*missing, malformed, or accessor revision.*never authorizes cleanup.*explicit exact-content relink.*currently missing.*already-bound.*writable Project Bin video source.*pathless selected Blob.*opaque locator ID and revision.*exact old binding token.*selected Blob.*old byte length and SHA-256.*candidate.*exact selected revision.*same selected length and digest.*synchronous `assertCanPublish`.*same compensated memory batch or IndexedDB readwrite transaction.*immediately before.*binding and provisional-root pair.*task cancellation.*writable writer.*project generation.*missing-source status.*stops Project Bin preview.*revokes.*before CAS.*Activation happens after CAS.*clears missing state.*publishes.*canonical project.*history identities remain unchanged.*prepublication.*preserves the old binding.*alias-aware.*distinct candidate locator.*postpublication activation failure.*retains.*missing state.*displaced old locator.*not immediately released.*bounded later startup reconciliation.*desktop visual activation.*ranged playback lease.*exact binding revision.*complete pinned handle.*4 MiB ranges.*binding and CAS fence.*returns only the media URL and one-shot release.*does not construct another original-video Blob/isu,
	);
	assert.match(
		documentation,
