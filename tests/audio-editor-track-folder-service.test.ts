...
test('folder creation targets the session-selected folder and publishes state', () => {
	...
	assert.equal(validateAudioEditorProjectV13(fixture.project), true);
});
...
test('the snapshot never traverses older, future, folder-free, or hostile documents', () => {
	...
	assert.deepEqual(createDocumentTrackFolderSnapshot({ schemaVersion: 14 }).sequences, []);
	assert.deepEqual(createDocumentTrackFolderSnapshot({
		schemaVersion: 14,
		get trackFolders(): never { throw new Error('trackFolders was traversed'); },
	}).sequences, []);
	const folderFree = createAudioEditorProjectV13({
