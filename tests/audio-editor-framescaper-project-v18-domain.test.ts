...
test('V18 runtime and command projections preserve attachment authority and reconcile all-null drafts', () => {
	...
	assert.equal(command.schemaVersion, 17);
	const runtimeSources = runtime.sources as readonly Record<string, unknown>[];
	assert.equal(Object.hasOwn(runtimeSources[0] ?? {}, 'proxyAttachment'), false);
	...
	assert.deepEqual(flattened, [
		{
			...
			leafStartFrame: { numerator: 5n, denominator: 1n },
			leafEndFrame: { numerator: 25n, denominator: 1n },
