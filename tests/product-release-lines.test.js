...
test('product release lines select versions without an admission state machine', () => {
	...
	assert.equal(resolveProductApplicationVersion('framescaper', RELEASE_LINES), '1.0.0-rc.3');
	...
	assert.deepEqual(resolveProductReleaseTag('framescaper-v1.0.0-rc.3', RELEASE_LINES), {
...
test('the resolver supports divergent product versions without package metadata', () => {
	...
	assert.equal(resolveProductApplicationVersion('framescaper', validated), '1.0.0-rc.3');
