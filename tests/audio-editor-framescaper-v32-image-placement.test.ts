...
import { createFramescaperImageBatchPlacementV32 } from '../src/framescaper/editor-image-placement-v32.ts';
...
test('V32 image batches prefer the selected unlocked collision-free video lane', () => {
	const project = selectedTrackProject();
	const plan = createFramescaperImageBatchPlacementV32(project, {
...
test('V32 image batches create one Images lane when the complete range collides', () => {
	const project = selectedTrackProject();
	const plan = createFramescaperImageBatchPlacementV32(project, {
...
test('V32 image batches use another suitable lane before creating one', () => {
	...
	const project = createFramescaperProjectV32(FRAMESCAPER_V32_PROJECT_RUNTIME_PROFILE, options);
	const plan = createFramescaperImageBatchPlacementV32(project, {
...
test('V32 image placement rejects empty, overflowing, and non-positive batches', () => {
	const project = selectedTrackProject();
	for (const sequenceFrameCounts of [[], [0], [-1], [Number.MAX_SAFE_INTEGER]] as const) {
		assert.throws(() => createFramescaperImageBatchPlacementV32(project, {
