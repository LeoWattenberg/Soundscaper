...
test('successful Nyquist generators and destructive effects close their dialog', async () => {
	const dom = installReactTestDom();
	const actGlobal = globalThis as typeof globalThis & { IS_REACT_ACT_ENVIRONMENT?: boolean };
	const priorAct = actGlobal.IS_REACT_ACT_ENVIRONMENT;
	const priorReact = Object.getOwnPropertyDescriptor(globalThis, 'React');
	...
	const controller = {
		actions: {
			nyquist: {
				evaluate: () => Promise.resolve({ type: 'audio', channelCount: 1, frameCount: 48_000 }),
	...
	const snapshot = {
		nyquist: { processing: false }, effects: { previewing: false }, readOnly: false,
		project: { sampleRate: 48_000 },
	};
	const { createRoot } = await import('react-dom/client');
	const root = createRoot(dom.container as unknown as Element);
	try {
		for (const pluginId of ['nyquist:pluck', 'nyquist:tremolo']) {
			await act(async () => root.render(<NyquistDialog
				controller={controller}
				snapshot={snapshot}
				copy={ENGLISH_COPY}
				target={{ prompt: false, pluginId }}
				run={(operation: () => unknown) => operation()}
				onClose={(request?: unknown) => { closeRequests.push(request); }}
			/>));
			await act(async () => {
				await reactProps(buttonWithText(dom.container, ENGLISH_COPY.nyquistApply)).onClick({});
			});
		}
