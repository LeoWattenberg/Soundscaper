...
test('exact direct MP4 and WebM publication has narrow capability and rollback controls', async () => {
	...
	assert.match(
		fallback.summary,
		/exact schema 12.*relationship role.*target clip ID.*canonical native Blob.*sole video input.*project-video-render-v1.*selected target input.*video-clip-render-v1.*direct MP4\/WebM route.*legacy final-Blob route.*does not add codec qualification/isu,
	);
});
