...
import {
	addFramescaperV32BoundaryImage,
	framescaperV32BoundaryImage,
} from './helpers/framescaper-v32-boundary-fixture.ts';

const PROFILE = FRAMESCAPER_V32_PROJECT_RUNTIME_PROFILE;

test('generic V32 repository saves preserve but never introduce or replace image-body authority', async () => {
...
test('authenticated store authority publishes one verified image body and exact project revision together', async () => {
	...
	await store.ready();
	const base = createFramescaperProjectV32(PROFILE, {
	...
	const authority = framescaperProjectStoreAuthorityV32(PROFILE, store);
...
test('first image publication may atomically create its one fresh Images track', async () => {
	...
	assert.ok(await store.createProjectIfAbsent(base));
	const fixture = framescaperV32BoundaryImage(base, projectId);
...
test('stale image publication removes its owned staged body and leaves the current project untouched', async () => {
	...
	const fixture = addFramescaperV32BoundaryImage(base, `${projectId}-stale`);
	const result = await framescaperProjectStoreAuthorityV32(PROFILE, store)
...
test('invalid or tampered image publication fails before leaving a body or project reference', async () => {
	...
	tampered[0] ^= 0xff;
	await assert.rejects(
		framescaperProjectStoreAuthorityV32(PROFILE, store).timelineImages.publishIfCurrent({
	...
	const detached = framescaperV32BoundaryImage(base, `${projectId}-detached`);
	await assert.rejects(
		framescaperProjectStoreAuthorityV32(PROFILE, store).timelineImages.publishIfCurrent({
...
function uniqueId(prefix: string): string {
	return `v32-image-${prefix}-${Date.now().toString(36)}-${Math.random().toString(36).slice(2)}`;
}
