...
} from '../src/framescaper/editor-project-v30-commands.ts';
import {
...
test('V30 adds one authenticated image source and timeline placement atomically', () => {
	...
	const command = snapshotFramescaperProjectCommandV30({
		type: 'batch',
		commands: [{
	...
	const updated = applyFramescaperProjectCommandV30(profile, project, command, {
	...
	assert.throws(() => applyFramescaperProjectCommandV30(profile, updated, command), /stale/iu);
});

test('V30 project-bin image placement stays off timeline tracks and survives inherited edits', () => {
	const profile = FRAMESCAPER_V30_PROJECT_RUNTIME_PROFILE;
	const project = createFramescaperProjectV30(profile, framescaperV20Options());
	const source = sourceFixture();
	const clip = clipFixture(String(project.primarySequenceId));
	const imported = applyFramescaperProjectCommandV30(profile, project, {
		type: 'batch', commands: [{
			type: 'image-source/set', sourceId: source.id,
			expectedSource: null, source,
		}, {
	...
	const renamed = applyFramescaperProjectCommandV30(profile, imported, {
...
test('V30 refuses mismatched image references and locked timeline placement', () => {
	...
	const source = sourceFixture();
	const clip = clipFixture(String(project.primarySequenceId));
	assert.throws(() => applyFramescaperProjectCommandV30(profile, project, {
		type: 'image-clip/set', clipId: clip.id,
		expectedClip: null, expectedPlacement: null,
		clip, placement: { scope: 'project-bin' },
	}), /source/iu);
	const locked = project.tracks.find(({ type, locked }) => type === 'video' && locked);
	if (locked) assert.throws(() => applyFramescaperProjectCommandV30(profile, project, {
		type: 'batch', commands: [{
...
test('V30 history restores image source, placement, and conditional requirement', () => {
	const profile = FRAMESCAPER_V30_PROJECT_RUNTIME_PROFILE;
	const project = createFramescaperProjectV30(profile, framescaperV20Options());
	const source = sourceFixture();
	const clip = clipFixture(String(project.primarySequenceId));
	...
	const imported = executeFramescaperProjectCommandV30(
		profile, createFramescaperProjectHistoryV30(profile, project), command,
	);
	assert.equal(imported.undoStack.length, 1);
	assert.equal(imported.present.sources.some(({ id }) => id === source.id), true);
	const undone = undoFramescaperProjectCommandV30(profile, imported);
	...
	const redone = redoFramescaperProjectCommandV30(profile, undone);
