...
test('render-input authority delegates unified V9-V12 without inspecting or settling V7/V8 inputs', async (context) => {
	for (const version of [9, 10, 11, 12] as const) {
		await context.test(`V${String(version)}`, async () => {
			...
			const calls: string[] = [];
			const authority = renderInputAuthority({
				...
				renderInputs: {
					...
					inspect: async () => {
						...
						throw new Error('Unified plans have no derived render-input stage.');
					},
...
test('render-input authority preserves V7 carrier materialization, byte accounting, and one-shot cleanup', async () => {
...
test('render-input authority materializes V8 audio while silent V8 delegates without a stage', async () => {
	const calls: string[] = [];
	const prepared = preparedJob(async (outcome) => { calls.push(`project.cleanup:${outcome}`); });
	const authority = renderInputAuthority({
...
test('render-input V7/V8 failures clean the delegated scratch job without settling its resumable stage', async () => {
	const calls: string[] = [];
	const prepared = preparedJob(async (outcome) => { calls.push(`project.cleanup:${outcome}`); });
	const authority = renderInputAuthority({
		prepare: async () => prepared,
...
function renderInputAuthority(options: Readonly<{
...
}>): FramescaperNativeRenderInputProjectAuthority {
	return new FramescaperNativeRenderInputProjectAuthority({
		project: {
