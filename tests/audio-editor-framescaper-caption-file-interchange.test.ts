...
import {
	captionSidecarFormatForFileName,
	openFramescaperCaptionSidecarFile,
	saveFramescaperCaptionSidecarFile,
} from '../src/common/editor/ui/framescaper-caption-file-interchange.ts';

test('caption file interchange recognizes every selected V27 sidecar extension', () => {
	assert.equal(captionSidecarFormatForFileName('captions.srt'), 'srt');
	assert.equal(captionSidecarFormatForFileName('captions.VTT'), 'webvtt');
	assert.equal(captionSidecarFormatForFileName('captions.ttml'), 'imsc1.1');
	assert.equal(captionSidecarFormatForFileName('captions.imsc'), 'imsc1.1');
...
test('browser and desktop caption picks stay pathless and decode strict UTF-8', async () => {
	...
	const calls: unknown[] = [];
	const desktop = await openFramescaperCaptionSidecarFile({
		fileService: {
			isDesktop: true,
	...
	assert.equal(await openFramescaperCaptionSidecarFile({
...
test('caption export uses the common save convention with exact format names and MIME types', async () => {
	...
	const result = await saveFramescaperCaptionSidecarFile({
		fileService: {
