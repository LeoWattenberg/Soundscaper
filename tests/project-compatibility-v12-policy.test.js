...
test('the compatibility register binds exact V12 editing to current implementation evidence', async () => {
	...
	assert.deepEqual(policy.projectSchema, {
		currentVersion: 12,
		minimumReadableVersion: 12,
		retainedMigrationSources: [],
	});
	assert.equal(policy.schemaRetirement.currentMinimumVersion, 12);
	assert.match(legacySchema.currentBehavior, /Schema 12.*Schemas 1 through 11.*REIMPORT_REQUIRED.*AUP4.*schema 12/iu);
	assert.ok(legacySchema.evidence.includes('tests/audio-editor-project-schema-policy.test.ts'));
	assert.match(currentSchema.currentBehavior, /Schema 12.*maintained writable raw-project schema.*cloned/iu);
