...
function hierarchyOf(project: AudioEditorProjectCurrent): readonly Readonly<{ id: string; parentFolderId: string | null }>[] {
...
test('a mixdown-shaped batch retires the emptied folder bus and appends the render at root', () => {
	...
	assert.equal(validateCurrentAudioEditorProject(mixed), true);
});
...
test('legacy removal of one lane member removes the pair and their nodes together', () => {
	...
	assert.deepEqual(hierarchyOf(removed as AudioEditorProjectCurrent), [
		{ id: 'picture', parentFolderId: null },
		{ id: 'vocals', parentFolderId: null },
	]);
	assert.equal(validateCurrentAudioEditorProject(removed), true);
});
...
test('legacy reorder moves across folder boundaries by adopting the destination parent', () => {
	...
	assert.equal(validateCurrentAudioEditorProject(adopted), true);
	...
	assert.equal(validateCurrentAudioEditorProject(escaped), true);
});
...
test('explicit folder placement on track/add is capability-gated and lands under the parent', () => {
	...
	assert.equal(validateCurrentAudioEditorProject(placed), true);
});
