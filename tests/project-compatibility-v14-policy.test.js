...
test('the compatibility register binds exact V15 editing to current implementation evidence', async () => {
	...
	assert.deepEqual(policy.projectSchema, {
		currentVersion: 15,
		minimumReadableVersion: 15,
		retainedMigrationSources: [],
	});
	assert.equal(policy.schemaRetirement.currentMinimumVersion, 15);
	assert.match(legacySchema.currentBehavior, /Schema 15.*Schemas 1 through 14.*REIMPORT_REQUIRED.*AUP4.*schema 15/iu);
	assert.ok(legacySchema.evidence.includes('tests/audio-editor-project-schema-policy.test.ts'));
	assert.match(currentSchema.currentBehavior, /Schema 15.*maintained writable raw-project schema.*cloned/iu);
