...
test('project switching cancels signal-ignoring playback source readiness before teardown', async () => {
	...
	const sourceLifecycle = createSourceLifecycleService({
		...
		allProjectClips: (candidate) => candidate.clips,
		audioBufferChannels: () => [],
		...
		copy: {},
		createStoredChunkProviderCandidate: () => null,
		engine,
		findClip: () => null,
		findSource: () => null,
		...
		sourceAudioBufferBytes: () => decodedBuffer.length * decodedBuffer.numberOfChannels * 4,
		sourceBuffers,
