...
test('the security matrix qualifies maintained save- and activation-triggered linked-original cleanup narrowly', async () => {
	...
	assert.match(control.summary, /exact-schema-16 current project and at most 64 retained revisions.*timeline clips, Project Bin clips, and all feature-fallback declarations.*future, invalid, missing-current-revision, duplicate, or over-bound.*suppress(?:es)? cleanup/isu);
...
test('the threat model records the same maintained save boundary and residuals', async () => {
	...
	assert.match(documentation, /current exact schema 16 project.*64 retained revisions.*timeline.*Project Bin.*all feature-fallback declarations/isu);
