...
test.describe('Framescaper v1 exact visual preview', () => {
	...
	test('menu-authored generator, preset, presentation, and mask change pixels, reopen, and reach video export', async ({
		...
		await assertNoSeriousAxeViolations(page, '[data-framescaper-visual-inspector]');
		...
		await expect(authoring.locator('[data-framescaper-authoring-preset-name]')).toBeFocused(VISUAL_COMMAND_OPTIONS);
		await assertNoSeriousAxeViolations(page, '[data-framescaper-selected-authoring]');
		...
		await expect(authoring).toBeVisible(VISUAL_COMMAND_OPTIONS);
		await authoring.locator('[data-framescaper-authoring-mask-shape]').selectOption('ellipse');
