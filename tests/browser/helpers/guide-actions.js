...
/**
 * The guide steps that reach beyond the menu bar: track effect racks, project
 * files in both directions, clip properties and clip dragging. Each function
 * drives one step kind for `guide-runner.js`, which owns the shared state.
 */
...
export const AUDACITY_EXAMPLE = Object.freeze({ file: 'guide-audacity-project.aup3', track: 'Fixture track', clip: 'Audio 1' });
...
export async function runDragClip(page, state, entry, clip, fixture) {
	...
	const pixelsPerSecond = clipBox.width / GUIDE_FIXTURES[fixture].seconds;
