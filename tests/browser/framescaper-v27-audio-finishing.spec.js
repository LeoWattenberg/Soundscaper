...
test.describe('selected Framescaper V27 audio finishing', () => {
	...
	test('authors, restores, executes, and delivers the shared V21 audio workflows from menus', async ({ page }) => {
		...
		await expect(page.locator('[data-framescaper-v27-finishing-dialog]')).toHaveCount(0);
		...
		await importFiles(editor, [createDeterministicAvFixture('framescaper-v27-audio-finishing.webm')]);
		...
		await expect(dialog.getByRole('textbox', { name: 'Canonical finishing document', exact: true }))
			.toBeFocused();
		await assertNoSeriousAxeViolations(page, '[data-framescaper-v27-finishing-dialog="automation"]');
