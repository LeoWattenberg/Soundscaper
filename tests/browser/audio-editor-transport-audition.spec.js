...
test.describe('transport audition shortcuts', () => {
	...
	test('P pauses and resumes recording, and the Record pause menu displays the same shortcut', async ({ page }) => {
		...
		let menu = page.getByRole('menu', { name: 'Record options', exact: true });
		await expect(menuItem(menu, 'Resume recording')).toBeVisible();
		await expect(menuItem(menu, 'Resume recording').locator('.context-menu-item-shortcut')).toHaveText('P');
		await page.keyboard.press('Escape');
		await page.keyboard.press('p');
		await editor.getByRole('button', { name: 'Record options', exact: true }).click();
		menu = page.getByRole('menu', { name: 'Record options', exact: true });
		await expect(menuItem(menu, 'Pause recording')).toBeVisible();
		await expect(menuItem(menu, 'Pause recording').locator('.context-menu-item-shortcut')).toHaveText('P');
		await page.keyboard.press('Escape');
