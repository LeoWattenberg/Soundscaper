...
test.describe('Soundscaper product surface', () => {
	...
	test('the File menu launches an exact editable-copy intent without changing its source', async ({ page }) => {
		...
		const soundIntent = handoffIntent(page.url());
		expect(soundIntent).toMatchObject({
			kind: 'cross-product-editable-copy',
			version: 1,
			source: {
				schemaFamily: 'soundscaper', schemaVersion: 1, projectId: soundscaperProjectId,
			},
			destination: { schemaFamily: 'framescaper', schemaVersion: 1 },
		});
