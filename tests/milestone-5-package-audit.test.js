...
test('a genuine package audit binds one exact target manifest and every packaged byte', {
	...
	assert.equal(isAuditedMilestone5Package(audit), true);
	assert.equal(isAuditedMilestone5Package(structuredClone(audit)), false);
	assert.equal(isAuditedMilestone5Package({ ...audit }), false);
...
test('package audit binds the selected product release line, not the root package version', {
	...
	const audit = await auditMilestone5Package({
		repositoryRoot: fixture.root,
		packageRoot: fixture.packageRoot,
		productId: 'soundscaper',
		targetId: 'linux-x64',
...
test('package audit rejects missing, unexpected, wrong-version, source-revision, and wrong-target names', {
	...
	for (const failure of [
		...
		try {
			await assert.rejects(auditMilestone5Package({
				repositoryRoot: fixture.root,
				packageRoot: fixture.packageRoot,
				productId: 'soundscaper',
				targetId,
