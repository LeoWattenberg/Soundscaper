...
test('snapshots closed name descriptors exactly once without ordinary gets', () => {
	...
	target.databaseName = 'changed-after-capture';
	assert.deepEqual(editorProjectStorageProfileNames(profile), FRAME_NAMES);

	let getters = 0;
	const accessor = names() as Record<string, unknown>;
...
test('refuses open, inherited, exotic, and malformed name records', () => {
	...
	const missing = names();
	delete (missing as Partial<EditorProjectStorageProfileNames>).databaseName;
	const extra = { ...names(), extra: 'value' };
	const symbol = names() as Record<PropertyKey, unknown>;
	symbol[Symbol('extra')] = true;
	const nonEnumerable = names();
	Object.defineProperty(nonEnumerable, 'databaseName', { value: FRAME_NAMES.databaseName });
	for (const value of [
...
test('navigator and fallback locks use only the resolved profile prefix', async () => {
	...
	const queued = await acquireProjectLock('queued', {
		projectStorageProfile: FRAMESCAPER_V18_PROJECT_STORAGE_PROFILE,
		navigator: { locks }, BroadcastChannel: null, navigatorLockHandoffMs: 0,
	}) as FinishedProjectLock;
	queued.release();
	await queued.finished;
	const forced = await acquireProjectLock('forced', {
		projectStorageProfile: FRAMESCAPER_V18_PROJECT_STORAGE_PROFILE,
		navigator: { locks }, BroadcastChannel: null, force: true,
	}) as FinishedProjectLock;
	forced.release();
...
test('the exact Framescaper selector remains statically dormant and product-owned', async () => {
	const files = await sourceFiles(['src', 'desktop', 'scripts']);
	...
	assert.deepEqual(references, [PRODUCT_MODULE]);
	assert.deepEqual([...literalOwners], [PRODUCT_MODULE]);
