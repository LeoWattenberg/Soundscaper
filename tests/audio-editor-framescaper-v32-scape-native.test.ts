...
import { rebindFramescaperSourceIdentitiesV32 } from '../src/framescaper/editor-project-v32-source-rebind.ts';
...
import { createFramescaperScapeNativeRuntimeV32 } from '../src/framescaper/editor-scape-native-v32.ts';
...
const RUNTIME = createFramescaperScapeNativeRuntimeV32(PROFILE);
type Store = ReturnType<typeof createProjectStore>;

test('selected V32 Scape round-trips the exact semantic image body', async (context) => {
	...
	assert.equal(inspected.schemaVersion, 32);
	...
	assert.equal(validateFramescaperProjectV32(PROFILE, imported.project), true);
...
test('selected V32 Scape collision import rekeys image source, clip, storage, and body together', async (context) => {
	const fixture = imageOnlyFixture();
	...
	const imported = await RUNTIME.importScapeProject(exported.blob, recipient);
	const project = imported.project as FramescaperProjectV32;
...
test('V32 source rebind is idempotent across pre-rebound and canonical image state', () => {
	const fixture = imageOnlyFixture();
	...
	rebindFramescaperSourceIdentitiesV32(project, mapping);
...
test('choosing a V28 Scape archive is the explicit V32 reimport boundary', async (context) => {
	...
	assert.equal(inspected.readOnly, false);
	const imported = await RUNTIME.importScapeProject(exported.blob, recipient);
	assert.equal(imported.project.schemaVersion, 32);
	assert.equal(validateFramescaperProjectV32(PROFILE, imported.project), true);
});

function imageOnlyFixture() {
	const image = createFramescaperV32ImageFixture();
	const base = createFramescaperProjectV32(PROFILE, sourceFreeOptions());
	...
	const project = applyFramescaperProjectCommandV32(PROFILE, base, {
		type: 'batch',
		commands: [{
...
function memoryStore(context: TestContext, label: string): Store {
	const store = createProjectStore({
		indexedDB: null, preferOpfs: false, databaseName: `framescaper-v32-native-${label}`,
	});
