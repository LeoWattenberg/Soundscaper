...
test('document annotation snapshot projects internal V17 and Soundscaper v1 by runtime timing', () => {
...
test('document annotation snapshot does not traverse obsolete, Framescaper, future, or absent schema state', () => {
	...
	assert.deepEqual(createDocumentTimelineAnnotationSnapshot(hostile(19)), []);
	assert.deepEqual(createDocumentTimelineAnnotationSnapshot(hostile(PROJECT_SCHEMA_VERSION + 1)), []);
