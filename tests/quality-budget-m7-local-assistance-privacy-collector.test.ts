...
import {
	assessM7AssistancePrivacyQualification,
...
function makeMeasurement(): MutableMeasurement {
	return {
		...
		qualificationEnvironmentId: 'owner-qualified-windows-x64-rtx3090-01',
		observedEnvironmentId: 'local-development-linux-x64',
		evidenceClass: 'local-development',
		budgetSha256,
		sourceRevision,
		observedEnvironment: {
...
test('complete authenticated evidence deterministically derives all five metrics', () => {
...
test('fixture, identity, package, artifact, media, observation, and run shapes fail closed', () => {
	...
	const relabelled = makeMeasurement();
	relabelled.observedEnvironmentId = 'owner-qualified-windows-x64-rtx3090-01';
	assert.throws(() => validateM7AssistancePrivacyMeasurement(relabelled, expectation()), /local development.*owner lab/iu);
...
test('local results stay pending and name every absent owner-lab authority', () => {
	const qualification = assessM7AssistancePrivacyQualification(config);
	assert.equal(qualification.provisioned, false);
	assert.match(qualification.blockers.join('\n'), /unprovisioned/iu);
	assert.match(qualification.blockers.join('\n'), /does not list m7-local-assistance-privacy/iu);
	assert.match(qualification.blockers.join('\n'), /status is optional/iu);
	...
	assert.match(result.evaluation.failures.join('\n'), /hardware renderer is required/iu);
	assert.match(result.canonicalMeasurementSha256, /^[a-f\d]{64}$/u);
});

test('a provisioned owner profile is refused until an accepted writer exists', () => {
	...
	environment.eligibleWorkloadIds = [
		...(environment.eligibleWorkloadIds as string[]),
		'm7-local-assistance-privacy',
	];
	for (const field of ['gpuDriverVersion', 'powerMode', 'displayMode']) {
		(environment.fingerprint as MutableRecord)[field] = `recorded-${field}`;
	}
	provisioned.workloads.find(({ id }) => id === 'm7-local-assistance-privacy')!.status = 'qualified';
	provisioned.fixtures.find(({ id }) => id === 'm7-local-assistance-privacy-v1')!.status = 'qualified';
	provisioned.qualification.qualifiedWorkloadIds.push('m7-local-assistance-privacy');
	assert.equal(assessM7AssistancePrivacyQualification(provisioned).provisioned, true);
	assert.throws(
		() => createM7AssistancePrivacyResult(makeMeasurement(), provisioned),
		/accepted-evidence writer must land with the owner lab/u,
	);
});

test('collector retains raw evidence but cannot publish qualification', async () => {
	const result = createM7AssistancePrivacyResult(makeMeasurement(), config);
	...
	assert.throws(
		...
		/qualification is unavailable/u,
	);
	...
	assert.equal((written as { value: { status: string } }).value.status, 'pending-external');
});
...
test('the local writer retains one raw record and one digest-bound aggregate', async (context) => {
	...
	assert.match(written.rawPath, /m7-local-assistance-privacy\.linux-x64\.pending-external\.raw\.json$/u);
	assert.match(written.resultPath, /m7-local-assistance-privacy\.linux-x64\.pending-external\.json$/u);
