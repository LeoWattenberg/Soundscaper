...
const EXPECTED = Object.freeze([
	...
	registration('audioAutomation', 'org.soundscaper.capability.audio-automation', false),
	...
	registration('audioMixerGraph', 'org.soundscaper.capability.audio-mixer-graph', false),
...
test('multicamera is globally known, native only in Framescaper, and never rendered fallback', () => {
	assert.equal(PROJECT_FEATURE_CAPABILITY_IDS.multicamera, MULTICAMERA_ID);
	assert.equal(FRAMESCAPER_PROFILE.capabilities.multicamera, true);
	assert.equal(SOUNDSCAPER_PROFILE.capabilities.multicamera, false);
	for (const product of [FRAMESCAPER_PROFILE, SOUNDSCAPER_PROFILE]) {
		const snapshot = snapshotProjectFeatureCapabilities(product.capabilities);
		assert.equal(snapshot.knownFeatureIds.includes(MULTICAMERA_ID), true, product.id);
		assert.equal(
			snapshot.availableFeatureIds.includes(MULTICAMERA_ID),
			product.id === 'framescaper',
			product.id,
		);
	}
	assert.equal(isProjectFeatureCapabilityId(MULTICAMERA_ID), true);
	assert.equal(isProjectFeatureAudioCapabilityId(MULTICAMERA_ID), false);
	assert.equal(isProjectFeatureVideoCapabilityId(MULTICAMERA_ID), false);
});
...
test('keeps private capability ownership within the closed cumulative domain set', async () => {
	...
	assert.deepEqual(genericPathReferences, [
		...
		'src/framescaper/editor-project-feature-capability-profile-v27.ts', 'src/framescaper/editor-project-feature-capability-profile-v28.ts',
		'src/framescaper/editor-project-feature-capability-profile-v30.ts',
		...
		'src/framescaper/editor-project-feature-requirements-v27.ts', 'src/framescaper/editor-project-feature-requirements-v28.ts',
		'src/framescaper/editor-project-feature-requirements-v30.ts',
		'src/framescaper/editor-project-feature-requirements-v31.ts',
		FINAL_PRODUCT_MODULE,
		...
		'tests/audio-editor-framescaper-v27-core.test.ts', 'tests/audio-editor-framescaper-v28-capability-profile.test.ts',
		'tests/audio-editor-framescaper-v30-project.test.ts',
		'tests/audio-editor-mastering-sequence-capability.test.ts',
