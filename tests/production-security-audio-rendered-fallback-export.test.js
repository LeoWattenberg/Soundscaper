...
test('role-defined audio rendered-fallback export stays private and narrowly qualified', async () => {
	...
	assert.match(
		control.summary,
		/exact schema 14.*canonical namespaced feature ID.*unavailable or unknown.*declared and effective rendered-fallback.*closed project-audio-mix-v1 role.*canonical manifest.*only.*standalone final audio mix delivery/iu,
	);
	...
	const sectionEnd = threatModel.indexOf('\nWhen exact schema 14 reports registered `videoEffects`', sectionStart);
	assert.ok(sectionStart >= 0 && sectionEnd > sectionStart);
	const documentation = threatModel.slice(sectionStart, sectionEnd).replace(/\s+/gu, ' ');
	assert.match(
		documentation,
		/final audio rendered-fallback delivery.*exact schema 14.*canonical namespaced feature ID.*unavailable or unknown.*declared and effective `rendered-fallback`.*closed `project-audio-mix-v1` role.*only.*standalone final mix/iu,
	);
