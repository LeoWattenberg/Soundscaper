...
} from '../src/framescaper/editor-timeline-image-current-project-publication-v32.ts';
import {
...
test('published image CAS is installed as one saved undoable active history revision', async () => {
	...
	let active = base;
	let history = createFramescaperProjectHistoryV32(PROFILE, base);
	let token: Readonly<{ generation: number }> = Object.freeze({ generation: 1 });
	let publishedBytes: Uint8Array | null = null;
	let opened = 0;
	const signal = new AbortController().signal;
	const port = createFramescaperTimelineImageCurrentProjectPublicationV32({
		controller: {
		...
		executeCommand: (value, command, options) => (
			executeFramescaperProjectCommandV32(PROFILE, value, command, options)
		),
	...
	function session() {
		return {
			...
			updateProjectHistory: (_projectId: string, value: FramescaperProjectHistoryV32) => {
	...
	const port = createFramescaperTimelineImageCurrentProjectPublicationV32({
		...
		session: {
			captureProjectHistory: () => ({
				history: createFramescaperProjectHistoryV32(PROFILE, base), token: null,
			}),
			assertProjectHistoryToken: () => undefined,
			updateProjectHistory: () => undefined,
			markProjectSaved: () => undefined,
			getProjectHistory: () => createFramescaperProjectHistoryV32(PROFILE, base),
		},
