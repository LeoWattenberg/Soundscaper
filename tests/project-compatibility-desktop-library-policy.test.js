...
test('shared desktop project policy pins the current editor handoff boundary', async () => {
	...
	assert.match(
		...
		/fresh filesystem library scope v5.*SQLite user_version 7.*ignores rather than migrates.*prior shared v4 scope.*preserv.*metadata schema 4.*schema 12.*in place.*copied v4.*user_version 6.*v5 path.*reject.*without mutation.*migrat.*adopt.*backfill.*metadata schema 5.*separate opaque library entry ID.*project identity.*exact schema 13.*project revision.*byte length.*SHA-256.*immutable revision-and-digest path/iu,
	);
	assert.match(
		rule.currentBehavior,
	...
	assert.match(
		rule.currentBehavior,
		/each exact-V13 decoded codec traversal and maintained-domain validation phase.*independently capped.*100,000 nodes.*depth 128/iu,
	);
	assert.match(
		rule.currentBehavior,
	...
	assert.match(
		rule.currentBehavior,
		/renderer repository.*repeats.*maintained-persistence-domain exact-V13 validation.*defense in depth.*before local mutation.*product-local shadow.*shared latest document.*authoritative.*fails closed.*incomplete desktop bridge/iu,
	);
	assert.match(
		rule.currentBehavior,
	...
	assert.match(
		documentation,
		/fresh filesystem library scope `v5`/iu,
	);
	assert.match(
		documentation,
		/fresh filesystem library scope `v5`.*SQLite `user_version` 7.*ignores\s+rather\s+than\s+migrates.*prior\s+shared `v4` scope.*preserv.*metadata\s+schema\s+4.*schema\s+12.*in place.*copied `v4`.*`user_version` 6.*`v5` path.*rejected without mutation.*migrat.*adopt.*backfill/isu,
	);
	assert.match(
		documentation,
		/metadata\s+schema\s+5.*separate\s+opaque\s+library\s+entry\s+ID.*exact\s+schema\s+13.*project\s+revision.*byte\s+length.*SHA-256.*immutable\s+revision-and-digest\s+path/isu,
	);
	assert.match(
		documentation,
		/main process.*bounded tagged-binary Scape codec.*256 MiB.*low-level store.*root schema, identity,\s+title, and revision.*main-owned identity service.*strict\s+exact-V13 maintained-persistence-domain validator.*before\s+permitting host staging.*catalog publication.*renderer commit.*validates the loaded commit result.*stored project again.*before\s+returning.*canonical document/isu,
	);
	assert.match(
		documentation,
	...
	assert.match(
		documentation,
		/each exact-V13 decoded codec traversal and maintained-domain\s+validation phase.*independently capped.*100,000 nodes.*depth 128/isu,
	);
	assert.match(
		documentation,
	...
	assert.match(
		documentation,
		/renderer repository.*repeats.*maintained-persistence-domain exact-V13\s+validation.*defense in depth.*before\s+local mutation.*shared catalog is authoritative.*product-local IndexedDB.*remote commit failure.*retryable local shadow.*incomplete shared-project bridge.*fails closed/isu,
	);
	assert.match(
		documentation,
	...
	assert.match(
		documentation,
		/pre-release schemas 1 through 12.*source-media re-import.*no raw-\s*project migration path.*prior shared\s+`?v3`?.*older\s+`?v2`?.*`?v1`?.*product-\s*private Soundscaper libraries.*unsupported.*AUP.*legacy XML AUP.*AUP4.*separate maintained interchange/isu,
	);
