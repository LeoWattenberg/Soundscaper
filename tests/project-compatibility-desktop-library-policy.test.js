...
test('shared desktop project policy pins the current editor handoff boundary', async () => {
	...
	assert.match(
		...
		/fresh filesystem library scope v7.*SQLite user_version 9.*ignores rather than migrates.*prior shared v6 scope.*preserv.*metadata schema 6.*schema 14.*in place.*copied v6.*user_version 8.*v7 path.*reject.*without mutation.*migrat.*adopt.*backfill.*metadata schema 7.*separate opaque library entry ID.*project identity.*exact schema 15.*project revision.*byte length.*SHA-256.*immutable revision-and-digest path/iu,
	);
	assert.match(
		rule.currentBehavior,
	...
	assert.match(
		rule.currentBehavior,
		/each exact-V15 decoded codec traversal and maintained-domain validation phase.*independently capped.*100,000 nodes.*depth 128/iu,
	);
	assert.match(
		rule.currentBehavior,
	...
	assert.match(
		rule.currentBehavior,
		/renderer repository.*repeats.*maintained-persistence-domain exact-V15 validation.*defense in depth.*before local mutation.*product-local shadow.*shared latest document.*authoritative.*fails closed.*incomplete desktop bridge/iu,
	);
	assert.match(
		rule.currentBehavior,
	...
	assert.match(
		documentation,
		/fresh filesystem library scope `v7`/iu,
	);
	assert.match(
		documentation,
		/fresh filesystem library scope `v7`.*SQLite `user_version` 9.*ignores\s+rather\s+than\s+migrates.*prior\s+shared `v6` scope.*preserv.*metadata\s+schema\s+6.*schema\s+14.*in place.*copied `v6`.*`user_version` 8.*`v7` path.*rejected without mutation.*migrat.*adopt.*backfill/isu,
	);
	assert.match(
		documentation,
		/metadata\s+schema\s+7.*separate\s+opaque\s+library\s+entry\s+ID.*exact\s+schema\s+15.*project\s+revision.*byte\s+length.*SHA-256.*immutable\s+revision-and-digest\s+path/isu,
	);
	assert.match(
		documentation,
		/main process.*bounded tagged-binary Scape codec.*256 MiB.*low-level store.*root schema, identity,\s+title, and revision.*main-owned identity service.*strict\s+exact-V15 maintained-persistence-domain validator.*before\s+permitting host staging.*catalog publication.*renderer commit.*validates the loaded commit result.*stored project again.*before\s+returning.*canonical document/isu,
	);
	assert.match(
		documentation,
	...
	assert.match(
		documentation,
		/each exact-V15 decoded codec traversal and maintained-domain\s+validation phase.*independently capped.*100,000 nodes.*depth 128/isu,
	);
	assert.match(
		documentation,
	...
	assert.match(
		documentation,
		/renderer repository.*repeats.*maintained-persistence-domain exact-V15\s+validation.*defense in depth.*before\s+local mutation.*shared catalog is authoritative.*product-local IndexedDB.*remote commit failure.*retryable local shadow.*incomplete shared-project bridge.*fails closed/isu,
	);
	assert.match(
		documentation,
	...
	assert.match(
		documentation,
		/pre-release schemas 1 through 14.*source-media re-import.*no raw-\s*project migration path.*prior shared\s+`?v6`?.*older\s+`?v5`?.*`?v4`?.*`?v3`?.*`?v2`?.*`?v1`?.*product-\s*private Soundscaper libraries.*unsupported.*AUP.*legacy XML AUP.*AUP4.*separate maintained interchange/isu,
	);
