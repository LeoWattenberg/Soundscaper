...
test('shared desktop project policy pins the current editor handoff boundary', async () => {
	...
	assert.match(
		documentation,
		/startup-loaded chooser metadata.*durable binding.*binding rows.*durably removed.*project-absent.*project-catalog snapshot, local binding transaction, and main reconciliation.*separate, not atomic.*catalog mutation.*observed\s+only later.*binding deletion commits before IPC or main rejects.*remains committed.*later startup retry.*prune.*Project presence alone.*source or\s+storage key.*still-present project remains bound.*Current-process records.*one-shot pass.*later main-process restart.*cannot authenticate inventory completeness.*compromised renderer.*omit live references.*delete startup locator metadata.*cooperative\s+availability maintenance.*not a compromised-renderer integrity control.*Continuous runtime cleanup.*cross-store or cross-process mutation serialization.*source-level reachability.*hostile IndexedDB row.*not implemented/isu,
	);
	assert.match(
		documentation,
