...
test('shared desktop project policy pins the current editor handoff boundary', async () => {
	...
	assert.deepEqual(linkedVideoOriginal.evidence, [
		...
		'src/common/editor/storage/linked-video-original-binding.ts', 'src/common/editor/storage/linked-video-original-lifecycle-coordinator.ts',
		...
		'tests/audio-editor-project-bootstrap-service.test.ts', 'tests/audio-editor-linked-video-locator-lifecycle.test.ts',
	...
	assert.match(
		linkedVideoOriginal.currentBehavior,
		/durable IndexedDB opens.*before project loading.*point-in-time authoritative project-ID snapshot.*active project repository.*shared catalog.*stale local shadow.*platform reconciliation port.*validates every catalog project ID.*at-most-10,000-ID bound.*before opening one readwrite transaction.*walks at most 100,000 closed binding rows.*validates every authoritative binding key and identity.*at most 128 unique exact locator\/revision pairs.*full inventory.*conflict.*exceeded inventory bound.*project-absent rows.*aborts and rolls back before IPC.*complete scan.*deletes only bindings whose project ID is absent.*retains every binding for a present project.*locator live.*present-project alias.*deletion-write failure.*rolls back every binding removal.*Memory fallback.*before requesting the catalog.*mutating bindings.*invoking reconciliation.*durable load-only injected port.*may request the catalog snapshot.*no binding mutation or reconciliation IPC.*surviving exact references.*closed preload\/IPC boundary.*catalog snapshot, binding transaction, and main reconciliation.*separate rather than atomic.*catalog mutation after the snapshot.*observed only later.*binding deletion commits before IPC or main rejects.*deletion remains committed.*later startup retry.*prune.*Startup reconciliation.*project presence alone.*root.*retains every binding for a present project.*maintained save\/activation source-reachability passes.*separate.*Main's serialized pass.*startup-loaded metadata.*submitted positive inventory.*runtime-created records.*retry after failure.*at most once per store\/process.*failed first registry write.*restores the in-memory inventory.*owner revocation after publication.*second persisted restore.*surfaces either failure.*never stats or deletes the external files.*project-absent.*one live AudioEditorProjectStore.*one renderer process.*lifecycle coordinator.*serializes binding publication, exact unlink, release-unused, startup reconciliation, project deletion, and whole-store clear.*complete before\/after binding inventory.*100,000 closed rows.*128 unique exact locator\/revision pairs.*pending cleanup set.*128.*after local project and binding deletion commits.*rescan.*no surviving same-store alias.*Clear.*local-commit signal.*memory or IndexedDB binding store.*before fallible OPFS cleanup.*precommit failure preserves bindings.*Thrown platform-release failures.*reported as committed cleanup errors.*later serialized retry.*rescans same-store aliases.*fulfilled true or false settles.*do not roll back local deletion.*never stat or delete the external file.*outside.*maintained binding, save, successful writable activation, delete, and clear paths.*later main-process restart.*cannot authenticate inventory completeness.*compromised renderer.*cooperative availability maintenance.*not a compromised-renderer integrity control.*cleanup beyond one live store.*maintained save\/successful-writable-activation\/delete\/clear lifecycle.*cross-store, cross-profile, or cross-process mutation serialization.*abrupt-crash or power-loss durability.*hostile IndexedDB row.*not implemented.*packaged executable\/UI.*operating-system.*unqualified/isu,
	);
	...
	assert.match(
		documentation,
		/pathname move or replacement.*open\s+handle.*same-inode\s+in-place mutation.*during or after verification.*not fenced.*content-frozen.*point-in-time identity.*operating-system bookmark.*watch\/relink.*cross-restart playback identity.*whole-`Blob`.*import.*shared-load.*handoff.*packaged.*operating-system.*browser codec.*unqualified/isu,
	);
	assert.match(
		documentation,
	...
	assert.match(
		documentation,
		/durable IndexedDB opens.*before project loading.*point-in-time authoritative project-ID snapshot.*active\s+project repository.*shared catalog.*stale product-local shadow.*reconciliation-capable port.*validates every catalog project ID.*10,000-ID\s+maximum.*before opening a binding transaction.*Memory fallback.*before requesting the catalog.*mutating a\s+binding.*invoking main-process\s+reconciliation.*durable load-only injected port.*may request the catalog\s+snapshot.*no binding mutation or reconciliation IPC/isu,
	);
	assert.match(
		documentation,
		/One readwrite transaction.*100,000 closed binding rows.*authoritative binding key and identity.*128\s+unique exact locator\/revision pairs.*full inventory.*conflicting\s+revision.*exceeded bound.*rejects reconciliation.*rolls back the transaction.*project absent.*complete scan.*delete\s+bindings whose project ID is absent.*retains every binding for a present\s+project.*present-project alias.*shared locator live.*failed\s+binding deletion.*rolls every removal back before IPC/isu,
	);
	assert.match(
		documentation,
	...
	assert.match(
		documentation,
		/startup-loaded chooser metadata.*durable binding.*binding rows.*durably removed.*project-absent.*one live.*AudioEditorProjectStore.*one renderer process.*serializes binding publication, exact unlink, unused\s+locator release, startup reconciliation, project deletion, and whole-store\s+clear.*complete before\/after binding inventory.*100,000\s+closed rows.*128 unique exact locator\/revision pairs.*pending cleanup set.*128.*committed.*local\s+project and binding removal.*rescan.*no surviving same-store alias.*local-commit signal.*memory or IndexedDB.*before\s+fallible OPFS cleanup.*failure before.*preserves the bindings.*physical-cleanup failure.*does not undo.*Thrown platform-release failures.*reported as committed cleanup errors.*later serialized retry.*rescans current same-store aliases.*fulfilled.*true.*or.*false.*settles.*never stats, writes, or deletes the external video file.*project-catalog snapshot, local binding transaction, and main reconciliation.*separate, not atomic.*catalog mutation.*observed\s+only later.*binding deletion commits before IPC or main rejects.*remains committed.*later startup retry.*prune.*Project presence alone remains the startup reconciliation root.*maintained save and successful writable activation paths.*source-level reachability.*outside\s+the maintained binding, save, successful writable activation,\s+delete, and clear lifecycle.*later main-process\s+restart.*cannot authenticate\s+inventory\s+completeness.*compromised renderer.*omit live references.*delete\s+startup locator\s+metadata.*cooperative\s+availability maintenance.*not a\s+compromised-renderer integrity control.*Cleanup beyond one live store's\s+maintained save\/successful-writable-activation\/delete\/clear lifecycle.*cross-store, cross-profile, or\s+cross-process mutation serialization.*abrupt-crash or power-loss durability.*hostile IndexedDB row.*not\s+implemented.*Packaged executable\/UI\s+and operating-system behavior remain\s+unqualified/isu,
	);
	assert.match(
		documentation,
