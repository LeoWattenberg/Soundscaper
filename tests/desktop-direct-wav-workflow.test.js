...
test('desktop package metadata exposes the packaged direct-WAV smoke', async () => {
...
test('desktop CI runs the direct-WAV smoke only for packaged Soundscaper Linux x64', async () => {
	...
	const directWavSmokeIndex = packageJob.indexOf('- name: Smoke packaged direct WAV export');
	...
	const directWavStep = packageJob.slice(directWavSmokeIndex, retainManifestIndex);
	assert.match(
		directWavStep,
		/^- name: Smoke packaged direct WAV export\s+if: matrix\.product == 'soundscaper' && matrix\.target\.platform == 'linux' && matrix\.target\.arch == 'x64'\s+run: npm run desktop:smoke:direct-wav\s+env:\s+SOUNDSCAPER_SMOKE_ARCH: x64\s+SOUNDSCAPER_SMOKE_XVFB: 'true'\s*$/u,
	);
	assert.equal(
