...
test('policy preserves the V16 wire and scopes selected V27 web-core activation', async () => {
	...
	assert.match(rule.currentBehavior, /Soundscaper V17.*unavailable.*read-only-or-cancel.*Framescaper V27.*videoRetime consumer.*set, reset, constant, ramp, reverse, and freeze.*lazy Edit-menu dialog.*one-step/iu);
	...
	assert.match(rule.currentBehavior, /Clipboard V11.*Scape custody.*desktop library V18.*V25\/V26.*opaque read-only/iu);

	const documentation = (await readFile(documentationUrl, 'utf8')).replace(/\s+/gu, ' ');
	assert.match(documentation, /V16 wire preservation and selected V27 web-core retime.*V17 preserves.*closed JSON-safe V2 curve wire introduced by V16.*timeline and Project Bin.*1 through 4,096.*exact V2 algebra/iu);
	assert.match(documentation, /Soundscaper V17.*videoRetime.*unavailable.*read-only-or-cancel.*Framescaper V27.*web-core consumer/iu);
