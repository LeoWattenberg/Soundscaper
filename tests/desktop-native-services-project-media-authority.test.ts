...
test('the baseline reserves an image-sequence export from exact pack and inventory custody', async () => {
...
test('the baseline routes its exact evaluated carrier only through native encode and mux', async () => {
	...
	const authority = new FramescaperNativeProjectMediaAuthority({
		project: {
			...
			readProjectBundle: async () => Object.freeze({
				project: Object.freeze({ ...PROJECT_IDENTITY, projectRevision, sha256: projectSha256 }),
				bodies: Object.freeze([body]),
			}),
	...
	if (!prepared.execute || !prepared.publish) throw new Error('Baseline preparation returned no executable publication.');
...
test('the baseline runs its carrier-free legacy-unmanaged full-frame family on native CPU', async () => {
	const bytes = new Uint8Array([1, 2, 3, 4]);
	const sourceSha256 = digest(bytes);
	const profile = FRAMESCAPER_NATIVE_MEDIA_PROJECT_RUNTIME_PROFILE;
...
test('the baseline attempts the opted-in OS encoder once before identical native CPU', async () => {
	const bytes = new Uint8Array([1, 2, 3, 4]);
	const sourceSha256 = digest(bytes);
	const profile = FRAMESCAPER_NATIVE_MEDIA_PROJECT_RUNTIME_PROFILE;
	const options = silentVideoOptions();
...
test('the baseline prepares one exact original as a native ProRes Proxy MOV job', async () => {
