...
import type { FramescaperImageSourceV1 } from '../src/common/editor/timeline-image-model-v30.ts';
import {
...
} from '../src/framescaper/editor-selected-v30-image-preview-resources.ts';
import {
...
test('V30 preview resource plans count retained, drawable, output, cache, and transient bytes', () => {
	const { source } = createFramescaperV30ImageFixture({ imageOnly: true });
	const context = { source, width: 2, height: 1 };
	const preview = admitFramescaperImageTimelinePreviewResourcesV30([context, context]);
	...
	const filmstrip = admitFramescaperImageTimelineFilmstripResourcesV30([context, context, context]);
	...
	const thumbnail = admitFramescaperImageProjectBinThumbnailResourcesV30(source, 2, 1);
...
test('V30 preview resource plans cap contexts and distinct sources', () => {
	const { source } = createFramescaperV30ImageFixture({ imageOnly: true });
	const context = { source, width: 2, height: 1 };
	assert.throws(() => admitFramescaperImageTimelinePreviewResourcesV30(
		Array.from({ length: FRAMESCAPER_IMAGE_PREVIEW_MAXIMUM_CONTEXTS_V30 + 1 }, () => context),
	), /context count bound/iu);
	assert.throws(() => admitFramescaperImageTimelineFilmstripResourcesV30(
...
test('V30 preview consumers refuse aggregate packs before reads, inherited work, or drawables', async () => {
	...
	await assert.rejects(createFramescaperSelectedProjectBinThumbnailV30({
		profile: PROFILE,
		project: binProject,
		store,
...
test('V30 preview startup waits for both routes and disposes a successful inherited peer', async () => {
	const fixture = createFramescaperV30ImageFixture({ imageOnly: true });
...
test('V30 filmstrip failure zeroes a fulfilled inherited sibling output', async () => {
...
test('V30 preview disposal zeroes eager frames and releases every drawable', async () => {
	const fixture = createFramescaperV30ImageFixture({ imageOnly: true });
...
type Budget = ReturnType<typeof admitFramescaperImageTimelinePreviewResourcesV30>;
...
function withClaimedAssetBytes(projectValue: FramescaperProjectV30, byteLength: number): FramescaperProjectV30 {
...
function moveImageClipToProjectBin(projectValue: FramescaperProjectV30): FramescaperProjectV30 {
	const project = structuredClone(projectValue) as unknown as Record<string, unknown>;
