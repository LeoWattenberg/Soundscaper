...
interface TestProject extends ProjectLifecycleProject {
	readonly clips: AudioEditorProjectCurrent['clips'];
	readonly sources: AudioEditorProjectCurrent['sources'];
	readonly featureRequirements: AudioEditorProjectCurrent['featureRequirements'];
}
...
function createFixture(options: Readonly<{
	...
	const active = createCurrentAudioEditorProject({ id: 'active-project' }) as unknown as TestProject;
	...
	const runtime = {
		...
		loadProject: (project: unknown) => ({ project: project as TestProject, readOnly: false }),
