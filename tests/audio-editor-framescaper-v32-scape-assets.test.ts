...
import {
	createFramescaperScapeProjectAssetExtensionV32,
} from '../src/framescaper/editor-scape-assets-v32.ts';
import {
	FRAMESCAPER_SCAPE_IMAGE_ASSET_KIND_V32,
	collectFramescaperScapeImageAssetReferencesV32,
} from '../src/framescaper/editor-scape-asset-plan-v32.ts';
...
test('V32 Scape extension plans and validates one exact semantic image body', async (context) => {
	...
	const extension = createFramescaperScapeProjectAssetExtensionV32(PROFILE);
	const assets = await extension.planExportAssets({ project: fixture.project, store });
	const image = assets.find(({ kind }) => kind === FRAMESCAPER_SCAPE_IMAGE_ASSET_KIND_V32);
	assert.ok(image);
	assert.equal(image.sourceId, `framescaper-v32:image:${fixture.source.id}`);
	...
	assert.deepEqual(collectFramescaperScapeImageAssetReferencesV32(fixture.project).map(
	...
	assert.equal(extension.assetKinds.includes(FRAMESCAPER_SCAPE_IMAGE_ASSET_KIND_V32), true);
});

test('V32 Scape import stages a rebound image body with exact owned identity', async (context) => {
	const fixture = createFramescaperV32ImageFixture();
...
test('V32 Scape semantic validation rejects internally tampered frames even with a rebound outer digest', async (context) => {
	const fixture = createFramescaperV32ImageFixture();
	...
	const image = (await extension.planExportAssets({ project, store }))
		.find(({ kind }) => kind === FRAMESCAPER_SCAPE_IMAGE_ASSET_KIND_V32)!;
...
type PlannedAsset = Awaited<ReturnType<ReturnType<typeof createFramescaperScapeProjectAssetExtensionV32>['planExportAssets']>>[number];

function memoryStore(context: TestContext, label: string): Store {
	const store = createProjectStore({ indexedDB: null, preferOpfs: false, databaseName: `v32-scape-${label}` });
