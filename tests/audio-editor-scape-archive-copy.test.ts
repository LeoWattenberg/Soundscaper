...
test('a future V13 archive copies byte-for-byte from Blob and byte-source inputs', async (context) => {
	...
	assert.equal(fromBlob.result.schemaVersion, 13);
...
test('current-schema and mismatched or unknown-format archives refuse the unchanged copy', async (context) => {
	...
	const mismatched = await rewriteScapeManifest(
		...
		(manifest: { project: { schemaVersion: number } }) => { manifest.project.schemaVersion = 14; },
	);
...
async function futureArchive(context: TestContext): Promise<Blob> {
	return rewriteScapeProjectDocument(
		...
		(document: { schemaVersion: number }) => { document.schemaVersion = 13; },
	);
}
