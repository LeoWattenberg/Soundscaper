...
test('compatibility policy narrowly qualifies the track-local audio-effects render relationship', async () => {
	...
	assertSemanticClaims(rule.currentBehavior, [
		...
		['packaged boundary', /source-level and portable-open evidence.*source\/component binding.*packaged runtime or UI activation, transport playback, and final-delivery workflows.*unqualified/iu],
...
test('security controls bind the track relationship through playback, integrity, and delivery', async () => {
	...
	assertSemanticClaims(playback.summary, [
		...
		['packaged boundary', /source-level and portable-open evidence.*source\/component binding.*packaged runtime or UI activation, transport playback, and final-delivery workflows.*not qualified/iu],
	]);
...
test('compatibility and security documents state the track slice and its narrow non-goals', async () => {
	...
	assertSemanticClaims(threatModel, [
		['threat model: family handoff boundary', /Managed-media and rendered-fallback witnesses are same-family workflows.*packaged activation, transport playback, and final delivery remain unqualified/iu],
	]);
});
