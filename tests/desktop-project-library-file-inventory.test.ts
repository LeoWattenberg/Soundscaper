...
test('schema 11 reserves and lease-fences immutable project materialization', async (context) => {
	...
	assert.equal(readUserVersion(fixture.paths.databasePath), 11);
});
...
test('catalog publication requires a materialized project inventory row', async (context) => {
	...
	const metadata = {
		...
		projects: [{
			...
			projectSchemaVersion: 17 as const,
			projectRevision: 3,
			byteLength: 23,
			sha256: DIGEST,
		}],
...
test('database schemas 1 through 4 are rejected without an implicit inventory migration', async (context) => {
