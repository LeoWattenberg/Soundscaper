...
test('schema 10 reserves and lease-fences immutable project materialization', async (context) => {
	...
	assert.equal(readUserVersion(fixture.paths.databasePath), 10);
});
...
test('catalog publication requires a materialized project inventory row', async (context) => {
	...
	const metadata = {
		...
		projects: [{
			...
			projectSchemaVersion: 16 as const,
			projectRevision: 3,
			byteLength: 23,
			sha256: DIGEST,
		}],
