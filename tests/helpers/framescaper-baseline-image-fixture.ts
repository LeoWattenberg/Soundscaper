...
export interface FramescaperBaselineImageFixture {
...
export function createFramescaperBaselineImageFixture(
	options: Readonly<{
		sourceId?: string;
		clipId?: string;
		originalText?: string;
...
): FramescaperBaselineImageFixture {
	...
	const baseOptions = framescaperBaselineOptions();
	if (options.imageOnly === true) {
		...
		if (!baseVideoTrack) throw new Error('The Framescaper image fixture requires a video track.');
		baseVideoTrack.clipIds = [];
	}
	const base = createFramescaperProject(
		FRAMESCAPER_PROJECT_RUNTIME_PROFILE,
		baseOptions,
	);
	const videoTrack = (base.tracks as readonly Readonly<Record<string, unknown>>[])
		.find(({ type }) => type === 'video');
	if (!videoTrack) throw new Error('The Framescaper image fixture requires a video track.');
	const clip: FramescaperImageClipV1 = {
		schemaVersion: 1,
		kind: 'image',
		id: clipId,
		sourceId,
		sequenceId: String(base.primarySequenceId),
	...
	const project = applyFramescaperProjectCommand(
		...
		{ type: 'batch', commands: [{
			...
			clip, placement: { scope: 'timeline', trackId: String(videoTrack.id) },
		}] },
	);
	return Object.freeze({
		...
		source,
		clip,
		descriptor: imageDescriptor(sourceId, clipId, String(videoTrack.id), clip.sequenceId),
