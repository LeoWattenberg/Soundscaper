...
test('shared desktop project publication is fenced and remains narrowly partial', async () => {
	...
	assert.match(
		...
		/maintained-domain-validated exact schemaVersion-17 project documents.*revision-and-document-digest-bound.*canonical-PCM and retained-original-video descriptors and bodies/iu,
	);
	assert.match(
		libraryBoundary.data,
		/fresh v9 filesystem scope.*SQLite user_version 11.*metadata schema 9.*preserv.*v8.*V16.*user-version-10.*older history.*copied-v8.*reject.*without mutation.*project-file and stage-attempt inventories.*managed-media canonical and stage-attempt inventories.*persisted bounded reclamation state.*collector-owned quarantine files/iu,
	);
	...
	for (const path of [
		...
		'src/common/editor/project-validation-budget.ts',
		'src/common/editor/project-validation-primitives.ts',
		...
		'src/common/editor/project-audio-warp-validation.ts',
	...
	assert.match(
		control.summary,
		/fresh filesystem library scope v9.*SQLite user_version 11.*ignores rather than migrates.*prior shared v8 scope.*preserv.*metadata schema 8.*exact schema 16.*user_version 10.*in place.*older v7.*schema-15.*user-version-9.*historical.*untouched.*copied v8.*user_version 10.*v9 path.*reject.*without mutation.*migrat.*adopt.*backfill.*metadata schema 9.*separate opaque library entry ID.*exact schema 17.*bounded byte length.*SHA-256.*immutable revision-and-digest path.*canonical tagged-binary codec.*non-raiseable 256 MiB.*lower-only test seam.*persistence root identity.*reserves.*lease.*fencing-token.*authoritative project-file inventory.*before stage creation.*private file.*syncs it.*atomically renames it.*materialized.*every catalog reference.*before an exact plus-one catalog journal publication.*before staging.*before publication.*transactionally at catalog commit.*serializes commits.*renews its lease while close drains admitted work/isu,
	);
	assert.match(
		control.summary,
		/main-owned editor service.*strict exact-schema-17 maintained-persistence-domain validator.*before calling host commit.*before project staging.*loaded commit results.*stored reads.*before returning a renderer response.*core project, document, media, and graph structures.*strictly checked.*all audio effects.*cloneable.*generic identity, enabled, and parameter structure.*type-specific semantic checks.*missing-effect compatibility metadata.*parametric EQ.*other first- and third-party effect payload semantics.*not gated/isu,
	);
	assert.match(
		control.summary,
	...
	assert.match(
		control.summary,
		/before `?JSON\.parse`?.*every schema.*structural scan.*101,536 JSON values.*depth 130.*exact schema 17.*independent decoded-codec.*semantic-validator.*100,000 logical nodes.*depth 128 per phase/iu,
	);
	assert.match(
		control.summary,
	...
	assert.match(
		control.summary,
		/renderer repository.*repeats maintained-persistence-domain exact-schema-17 validation and canonical reserialization.*before local mutation.*shared latest document and summary list.*authoritative.*product-local revision, source, and media shadow.*fails closed.*incomplete desktop bridge/isu,
	);
	assert.match(
		control.summary,
	...
	assert.match(
		...
		/latest authoritative exact-schema-17 shared load.*4,094 reachable timeline, Project Bin, and fallback sources.*before source bodies are read.*deduplicates compatible same-kind physical bindings.*rejects conflicts.*aggregate 64 GiB audio-and-video byte budget.*audio-only 65,536-chunk budget.*fresh recipient.*managed canonical-PCM and retained-original-video descriptors.*4 MiB reads.*staged local audio-source or media-asset writers.*descriptor identity.*kind and storage key.*byte length.*SHA-256.*canonical audio byte geometry.*atomic if-absent.*opaque retained video.*not decoded or probed for media geometry/iu,
	);
	assert.match(
		mediaAdmissionControl.summary,
		/loses the absence race.*only its staging.*pre-shadow failure.*exact acquisition-owned audio records or owned video publications.*source-token, path, or media-chunk payloads.*preserving concurrent replacements.*unacquired sources.*pre-existing recipient-local exact-schema-17 binding.*compatible same-kind aliases.*verified once.*audio.*ordered Float32Array geometry.*video.*trusted local digest.*SHA-256.*4 MiB windows.*without on-access storage maintenance.*pre-shadow failures preserve the prior local shadow.*prevent activation.*after the exact shadow is durable.*retains.*acquired audio and video.*source-free loads.*no media I\/O/iu,
	);
	assert.match(
		mediaAdmissionControl.summary,
