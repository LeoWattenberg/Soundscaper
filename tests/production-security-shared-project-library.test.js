...
test('shared desktop project publication is fenced and remains narrowly partial', async () => {
	...
	assert.match(
		linkedVideoControl?.summary ?? '',
		/binding, descriptor-free shared admission, and visual activation.*no durable product-owned media row or body copy.*explicit managed handoff.*maintained managed sender.*project deletion and whole-store clear.*only local binding records.*external target.*untouched.*platform port.*injected and optional.*no concrete packaged main\/preload filesystem capability.*external replacement or deletion after the verified Blob snapshot.*not fenced.*no durable byte lease.*range-read protocol.*reference-scale.*4 MiB bound.*digest slices.*not.*whole-Blob provider allocation.*relink or watch UI.*packaged UI or browser-codec evidence.*linked audio.*absent/isu,
	);
	assert.match(
	...
	assert.match(
		managedMedia?.exposure ?? '',
		/unmanaged recipient admission.*sequential point-in-time check.*metadata.*not transactionally bound.*same-metadata replacement.*undetected.*later replacement or deletion.*not fenced.*separate repository instances or processes.*not serialized.*non-cooperative work.*continue after rejection.*unmanaged audio.*lacks a publisher digest.*linked retained video.*product-local pathless binding.*operation-scoped verified Blob snapshot.*no packaged platform capability.*external replacement or deletion after that snapshot.*not fenced.*no durable byte lease or range protocol.*linked audio.*authored proxies.*generic and third-party rendered fallbacks.*cross-product or packaged first-party video-effects fallback workflows.*maintained first-party video-effects preview\/playback.*exact admitted local body.*relink.*watch behavior.*general consolidation.*browser video-codec behavior.*packaged Electron UI two-product source-bearing lifecycle.*open.*cross-platform hard-link.*crash or power-loss.*unqualified/isu,
	);
	assert.doesNotMatch(
		managedMedia?.exposure ?? '',
	...
	assert.match(
		threatModel,
		/rendered-fallback limitation.*narrowed by two.*maintained editor exceptions.*exact-schema first-party audio whole-mix PCM.*manifest is the only reference.*explicit\s+managed handoff.*editable original.*fresh recipient.*managed transfer.*descriptor and body digest.*controller separately.*project fallback declaration.*read-only transient\s+playback activation.*exact samples.*exact-schema registered.*first-party `videoEffects` fallback.*exact, locally available.*controller-digest-admitted full render.*transient preview and playback.*manifest is its only project reference.*does not itself.*cross-product or packaged fallback handoff.*authored proxies.*generic\s+or third-party rendered-fallback relationships remain open.*unknown and\s+third-party activation/isu,
	);
});
