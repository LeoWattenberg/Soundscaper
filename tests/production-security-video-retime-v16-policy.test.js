...
test('V16 security evidence limits video retime to preservation and refusal', async () => {
	...
	assert.match(control.summary, /Exact schema 16.*closed V2 video-retime wire.*1 through 4,096.*timeline and Project Bin.*null remains writable.*V15 breakpoint map.*typed re-import/iu);
	...
	assert.match(control.summary, /clone, history, clipboard codec.*format-1 \.scape.*desktop-v8.*direct and arbitrarily nested command boundary.*focused Chromium fixture.*exact curve preservation.*\.scape re-export.*no authoring, evaluator, playback, preview, export, nested sequence, fallback, or native timing claim/iu);

	const threatModel = (await readFile(threatModelUrl, 'utf8')).replace(/\s+/gu, ' ');
	assert.match(threatModel, /v16-video-retime-preservation-admission.*V16 preservation boundary.*1 through 4,096.*timeline and Project Bin.*null remains writable.*V15 breakpoint maps require typed re-import/iu);
