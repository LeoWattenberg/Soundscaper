...
import {
	createFramescaperV30ImageFixture,
	imagePasteCommand,
} from './helpers/framescaper-v30-image-fixture.ts';
...
test('V13 same-body paste reuses immutable authority and places one fresh grouped image clip', () => {
	...
	const pasted = applyFramescaperProjectCommandV30(PROFILE, fixture.project, prepared.command);
...
test('V13 collision paste copies exact bytes to a fresh source identity with owned rollback', async (context) => {
	const fixture = createFramescaperV30ImageFixture();
	const clipboard = createFramescaperSessionClipboardV13(PROFILE, fixture.project, fixture.descriptor);
	const destinationBase = createFramescaperProjectV30(PROFILE, framescaperV20Options());
	const conflicting = createFramescaperV30ImageFixture({ originalText: 'different bytes' }).source;
	const destination = applyFramescaperProjectCommandV30(PROFILE, destinationBase, {
	...
	retained.complete();
	const pasted = applyFramescaperProjectCommandV30(PROFILE, destination, prepared.command);
...
function memoryStore(context: TestContext, label: string): Store {
	const store = createProjectStore({ indexedDB: null, preferOpfs: false, databaseName: `v30-clipboard-${label}` });
