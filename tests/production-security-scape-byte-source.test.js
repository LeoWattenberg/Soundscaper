...
test('production evidence pins bounded random-access .scape admission', async () => {
	...
	]) assertOrderedClaim(control.summary, claim);
	...
	]) assertOrderedClaim(packagedOpenControl.summary, claim);
	for (const path of [
	...
	for (const claim of [
		...
		/exercises only orderly process-restart automatic source-bearing persistence and reopen plus transport entry, playback-clock advancement, master-meter activity, and explicit stop and reset.*known current-schema fixture/iu,
		/does not exercise audible or device output.*`--mute-audio`.*playback fidelity.*dropout.*glitch.*full-duration.*mixer.*routing.*effect correctness.*storage durability.*crash.*power[- ]loss.*fsync.*eviction.*quota.*reservation.*concurrency.*Windows.*macOS.*ARM.*Framescaper.*cross-product transfer.*third-party ZIP or effect.*legacy Soundscaper/iu,
		/Third-party activation gating.*legacy Soundscaper compatibility.*not current priorities.*Audacity project interchange.*separate/iu,
	]) assertOrderedClaim(packagedReopenControl.summary, claim);
	...
	]) assertOrderedClaim(capacityControl.summary, claim);
	for (const path of [
	...
	assert.match(referenceScaleTest, /EXACT_REQUIRED_FREE_BYTES = 9_448_923_946/u);
