...
test('production evidence pins bounded random-access .scape admission', async () => {
	...
	for (const claim of [
		/Soundscaper-only Linux x64 packaged smoke.*production-exports.*exact-schema-12.*one mono source.*one track.*one clip.*16,384 Float32 frames.*48 kHz.*65,536 PCM bytes.*four-byte chunk header.*65,540-byte asset.*larger than.*65,557-byte.*no larger than 96 KiB/iu,
	...
	for (const claim of [
		/maintained Soundscaper-only Linux x64 packaged smoke.*production-exports.*exact-schema-12.*one mono source.*one track.*one clip.*16,384 Float32 frames.*48 kHz.*65,540-byte.*asset.*65,557-byte.*96 KiB/isu,
	...
	for (const claim of [
		/second maintained Soundscaper-only Linux x64 packaged process.*orderly process-restart persistence witness.*70,051-byte.*schema 12.*revision 7.*source-bearing/isu,
