...
test('compatibility rules distinguish enforced guarantees from planned lossless fallbacks', async () => {
	...
	assert.match(
		...
		/exact-schema-10 create, load, clone, and commit.*reserved soundscaper\.audio-effects.*non-label.*non-video.*mixer group.*mixer send.*master rack/iu,
	);
	assert.match(
		featureRequirements.currentBehavior,
	...
	assert.match(
		currentControllerFeatureReport.currentBehavior,
		/exact schema 10.*actual project history.*before activation side effects.*unavailable or unknown.*intrinsically read-only.*deeply frozen.*session metadata clones.*document snapshot/iu,
	);
	...
	assert.match(
		...
		/authoritative actual project history.*exact schema 10.*soundscaper-project.*audioEffects.*unavailable.*declares bypass.*effective bypassed/iu,
	);
	assert.match(
		audioEffectPlaybackBypass.currentBehavior,
	...
	assert.match(
		...
		/authoritative actual project history.*exact schema 10.*soundscaper-project.*videoEffects.*unavailable.*declares bypass.*effective bypassed/iu,
	);
	assert.match(
		videoEffectPlaybackBypass.currentBehavior,
	...
	assert.match(
		...
		/selected product.*provider-owned.*caller.*override.*archive.*source.*validation.*exact schema 10.*before.*collision lookup.*deeply frozen/iu,
	);
	assert.match(
		currentScapePreOpenFeatureReport.currentBehavior,
	...
	assert.match(
		binaryOpaqueState.currentBehavior,
		/exact schema 10.*export.*Uint8Array.*offset-view.*ArrayBuffer.*reserved tagged descriptor.*export and post-parse decode.*independently.*100,000 traversal nodes.*depth 128.*other ArrayBuffer views reject/iu,
	);
	assert.match(
		binaryOpaqueState.currentBehavior,
	...
	assert.match(
		...
		/offline rendering.*narrow exact-schema-10 audio and video playback fallback relationships.*bounded operation-verified video fallback delivery exist.*audio freeze, unfreeze, commit, relink, freshness.*generic cross-platform fallback-authoring semantics.*not implemented/iu,
	);
...
test('schema retirement and forward-read rules fail closed without claiming unsupported losslessness', async () => {
	...
	assert.match(documentation, /exact schema 10.*JSON-semantic.*byte-exact preservation.*supported bounded tagged binary/isu);
	...
	assert.match(
		documentation,
		/exact schema 10.*format 1.*Uint8Array.*offset view.*ArrayBuffer.*\$soundscaperOpaqueBinary.*encoding and post-parse decoding.*independently.*100,000 traversed nodes.*depth 128.*256 payloads.*4 MiB.*8 MiB/isu,
	);
	assert.match(
		documentation,
	...
	assert.match(
		documentation,
		/Schemas 1 through 9.*rejected at the raw-project boundary.*before feature-\s+requirement reconciliation.*exact-schema-10 create, load, clone, and\s+commit.*`soundscaper\.audio-effects`.*non-label.*non-video.*mixer group.*mixer send.*master rack/isu,
	);
	assert.match(
		documentation,
	...
	assert.match(documentation, /exact schema 10[\s\S]*before activation side effects/iu);
	...
	assert.match(
		documentation,
		/first-party audio-effect slice.*transient playback projection.*authoritative activation project.*exact schema 10.*registered audio-effects.*unavailable.*declares bypass.*effective bypassed/isu,
	);
	assert.match(
		documentation,
	...
	assert.match(
		documentation,
		/first-party video-effect slice.*transient activation projection.*exact schema 10.*registered video-effects.*unavailable.*declares bypass.*effective bypassed/isu,
	);
	assert.match(
		documentation,
	...
	assert.match(
		documentation,
		/exact-schema-10 mono\/stereo role-defined audio whole-mix.*role-defined video\s+whole-project.*one first-party video-effects clip-target slice.*narrow forms of step 3.*editor playback.*maintained delivery.*operation-time integrity admission/isu,
	);
	assert.match(
		documentation,
